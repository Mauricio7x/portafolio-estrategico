...
| `ordenar_por=ganancia` | Lo que más deja | Ganancia del contrato (`lib/ganancia`) × opción de ganar. Sin presupuesto oficial, al final: **jamás un cero** |
| `ordenar_por=margen` | Más recorrido de precio | `techo − piso` (Fase 3) SOLO para procesos con un borrador de APU guardado con `costo_directo`; los demás «Sin referencia», al final. Es RECORRIDO DE PRECIO, no la plata que deja. **Nunca se asume margen cero** |
