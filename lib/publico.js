...
const MOTIVO_BAJA_PUBLICO = "Sin credencial el precio no ajusta la probabilidad (la cifra es la misma con o sin este paso). "
  + "Hasta dónde suele bajar esta entidad es inteligencia de precio y requiere credencial.";
...
/* EL GEMELO DE P3, CAZADO EN EL MISMO CENSO (13-sep-2026). P2 también tiene DOS
   ausencias —faltan datos de la EMPRESA, o el proceso no publica cuantía— y las
   dos salían por aquí como «el proceso no publica cuantía», sobre uno que sí la
   publica. P2 no lleva `sin_dato_de`, pero las dos ramas ya son distinguibles
   por los datos que devuelven: la del perfil manda `crp: null` (no se pudo
   calcular la capacidad) y la de la cuantía manda el K calculado con un CRPC en
   0. Se lee de ahí y no se inventa un campo nuevo en lib/puertas; si algún día
   P2 publica su propio `sin_dato_de`, esta rama se cambia por él. */
function mensajeP2Publico(p2) {
  if (p2.sin_dato) {
    return p2.crp == null
      ? "La capacidad de contratación no se puede verificar todavía: faltan datos de la empresa."
      : "El proceso no publica cuantía: la capacidad de contratación no se puede verificar. Confírmela en el pliego.";
  }
...
/* P3 TIENE DOS AUSENCIAS POSIBLES Y NO DICEN LO MISMO (13-sep-2026). Desde que
   `lib/puertas.p3Caja` dejó de convertir el anticipo NO PUBLICADO en un 0 %
   medido, un `sin_dato` de P3 puede ser la cuantía que el proceso no publica o
   el anticipo que SECOP II no trae — y `sin_dato_de` dice cuál. Responder a los
   dos «el proceso no publica cuantía» era una FALSEDAD sobre un proceso con
   cuantía publicada, y el usuario fija un precio con lo que lee. Se ramifica, y
   la rama por defecto no inventa cuál falta: si mañana aparece una tercera
   ausencia dirá que falta un dato, no un dato equivocado. */
function mensajeP3Publico(p3) {
  if (p3.sin_dato) {
    if (p3.sin_dato_de === "anticipo") {
      /* la cifra a financiar es PÚBLICA (cuantía × 0,20) y es la advertencia:
         se conserva, que es lo que hace útil el aviso */
      return `El proceso no publica si tiene anticipo. Si no hubiera ninguno, necesitaría financiar ≈ ${cop(p3.financiacion_requerida)} `
        + "antes del primer cobro. Confirme el anticipo en el pliego antes de decidir.";
    }
    if (p3.sin_dato_de === "cuantia" || !p3.sin_dato_de) return "El proceso no publica cuantía: no se puede estimar cuánto habría que financiar.";
    return "Falta un dato del proceso para estimar cuánto habría que financiar: confírmelo en el pliego.";
  }
  const cuanto = `Necesitaría financiar ≈ ${cop(p3.financiacion_requerida)} antes del primer cobro`;
  return p3.pasa
    ? `${cuanto}, y su patrimonio alcanza.`
    /* aquí el 0 % ya NO es «no se sabe»: con el anticipo sin publicar P3 sale
       por la rama de arriba, así que un 0 que llega hasta aquí lo declaró el
       pliego. Decir «se asume 0 %» sería el mismo aviso viejo, ahora falso. */
    : `${cuanto}, y su patrimonio no alcanza.`
      + (p3.anticipo_pct === 0 ? " El pliego declara que no habrá anticipo: la obra se financia entera hasta el primer cobro." : "");
}
...
function sinFinanzas(resultado) {
  ...
  // la lectura del departamento (M-COMP-01) es la misma inteligencia de precio
  // un nivel arriba: de ella se despeja la mediana de su zona
  if ("baja_departamento" in out) out.baja_departamento = null;
  ...
  /* Y `margen_estimado` es la MISMA familia por partida doble (ago 2026): su
     `piso` es el piso rentable —derivado del costo directo del APU guardado del
     dueño— y de su `techo` se despeja la mediana de baja de la entidad que las
     tres líneas de arriba acaban de tapar (`techo = cuantía × (1 − mediana)`,
     y la cuantía es pública). Anular solo los `baja_*` y dejar este campo sería
     la redacción de mentira de `p2_k.mensaje` otra vez. El listado ya no sirve
     el orden por margen sin credencial; esto es el cinturón. */
  if ("margen_estimado" in out) out.margen_estimado = null;
  /* LA GANANCIA DEL CONTRATO tampoco sale sin credencial, y por DOS motivos
     independientes: sale del costo directo y de la estructura de precio del
     dueño (finanzas del perfil), y además lleva dentro el precio de mercado
     —presupuesto oficial × (1 − baja)—, o sea la misma mediana que los tres
     campos de arriba acaban de tapar. Se anula el objeto ENTERO y no solo el
     escalar: `frase` dice «Si gana este contrato a $950.000.000…» y dejarla
     sería la redacción de mentira de `p2_k.mensaje` por cuarta vez. El listado
     ya no la calcula sin credencial; esto es defensa en profundidad. */
  if ("ganancia" in out) out.ganancia = null;
