...
function hechosDeTexto(texto, { tipo = "otro" } = {}) {
  ...
  return { tipo, paginas: contarPaginas(t), caracteres: t.length, detecciones, requisitos_numericos: numericos, hitos, deducciones, anticipo };
}
...
function resumenLectura(docs) {
  ...
  if (!indice) return { estado: "sin_indice", publicados: null, en_plan: null, leidos: 0, ilegibles: 0, pendientes: [], consultado_el: null, motivo: null };
  ...
  const estado = !archivos.length ? "sin_archivos" : pendientes.length ? "por_leer" : "leido";
  ...
  return { estado, publicados: (indice.resumen && indice.resumen.publicados) != null ? indice.resumen.publicados : archivos.length, en_plan: plan.length,
    leidos: Object.keys(leidos).length, ilegibles: Object.keys(ilegibles).length, pendientes, consultado_el: indice.consultado_el || null, motivo: indice.motivo || null };
}
...
module.exports = { VERSION, TIPOS, MAX_DOCS_PLAN, MAX_PLIEGOS_PLAN, MAX_BYTES_DOC, claveDocs, claveDoc, clasificarArchivo, deduplicar, planDeLectura, hechosDeTexto, resumenLectura, loQueDicen, contarPaginas };
