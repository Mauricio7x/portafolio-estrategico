...
const VERSION = 4;                         // 4: cláusula de participación mínima y fórmula del plural (26-sep-2026) · 3: citas literales por tema (4-sep-2026, noche) · 2: experiencia general y específica como hechos propios
...
/* 3 MB: el PDF vuelve al navegador en base64 (×1,37) dentro de una respuesta de
   Vercel que se corta en 4,5 MB (MEMORIA: «Límites Vercel/Upstash»). Es la MISMA
   constante que aplica el proxy (`op=descargar`), importada de lib/cuerpo.js desde el
   6-sep-2026: hasta entonces el proxy prometía 12 MB y este plan 3, y el hueco entre
   los dos era un fallo mudo. Un documento mayor se lista con su motivo y su enlace:
   no se promete leerlo. `MAX_BYTES_DOC` conserva su nombre exportado. */
const { TOPE_PDF_BASE64: MAX_BYTES_DOC } = require("./cuerpo.js");
const MAX_HITOS = 12;
...
function hechosDeTexto(texto, { tipo = "otro" } = {}) {
  ...
  return { version: hechosVersion(), tipo, paginas: contarPaginas(t), caracteres: t.length, detecciones, requisitos_numericos: numericos, hitos, deducciones, anticipo, citas: citasDeTexto(t), participacion, metodo_plural: metodoPlural };
}
...
function participacionDe(docs) {
  ...
  for (const x of leidos) {
    for (const c of x.hechos.participacion) {
      const clave = `${c.forma}|${c.porcentaje}|${!!c.mayoritaria}`;
  ...
  return { leidos: leidos.length, documentos: leidos.map((x) => etiquetaDoc(x)), clausulas, metodo };
}
...
function loQueDicen(docs, { perfilObj = null } = {}) {
  ...
  if (!leidos.length) return { hechos: [], citas: TEMAS_CITA.map((t) => ({ clave: t.clave, titulo: t.titulo, texto: null, pagina: null, documento: null, id_documento: null })), documentos: [] };
  ...
  return { hechos, citas, documentos: leidos.map((x) => ({ id_documento: x.id_documento, nombre: x.nombre, tipo: x.tipo, tipo_legible: x.tipo_legible || (TIPOS[x.tipo] || TIPOS.otro).legible, paginas: x.paginas, leido_el: x.leido_el || null })) };
}

module.exports = { participacionDe, VERSION, hechosVersion, TIPOS, TEMAS_CITA, MAX_DOCS_PLAN, MAX_PLIEGOS_PLAN, MAX_BYTES_DOC, claveDocs, claveDoc, clasificarArchivo, deduplicar, planDeLectura, hechosDeTexto, citasDeTexto, resumenLectura, loQueDicen, contarPaginas };
