...
const MENSAJES = Object.freeze({
  ...
  /* los tres orígenes de un dictamen (3-sep-2026): la pantalla los enseña tal cual */
  ORIGEN_MODELO: "Dictamen generado por inteligencia artificial.",
  ORIGEN_REGLAS: "Lectura por reglas de la aplicación, sin inteligencia artificial: encuentra lo que el pliego escribe con las palabras habituales y cita la página; no interpreta lo demás.",
  ORIGEN_SESION: "Dictamen escrito en una sesión de Claude Code con su suscripción, verificado cita por cita por la aplicación.",
  SIN_IA_SE_USA_REGLAS: "Este despliegue no tiene clave de inteligencia artificial: se muestra la lectura por reglas. Para un dictamen completo con su suscripción, pídalo desde Claude Code con la orden «/dictamen».",
  SESION_SIN_DICTAMEN: "Falta el campo «dictamen» con el objeto que escribió la sesión.",
  SESION_SIN_DICTAMEN_QUE_HACER: "Envíe en el cuerpo {id_proceso, perfil, motor: \"sesion\", dictamen: {…}} con la forma del expediente (GET &expediente=1).",
  SESION_FORMA: "El dictamen enviado no tiene la forma exigida.",
  SESION_FORMA_QUE_HACER: "Corrija los campos que indica «detalle» y vuelva a enviarlo.",
  /* la lectura completa pedida desde la pantalla (26-sep-2026): el botón despierta la rutina
     del dictamen y una sesión de Claude Code lo escribe con la suscripción del dueño */
  LECTURA_SIN_RUTINA: "La lectura completa con inteligencia artificial no está configurada en este despliegue.",
  LECTURA_SIN_RUTINA_QUE_HACER: "Mientras no esté, pídala desde Claude Code con la orden «/dictamen»; los pasos para dejarla en un botón están en la guía de configuración.",
  LECTURA_PEDIDA: "Se pidió la lectura completa del pliego.",
  LECTURA_PEDIDA_QUE_HACER: "Tarda unos minutos. Mientras tanto se muestra la lectura que ya había; pulse «Ver si ya está lista» para comprobarlo.",
  LECTURA_YA_PEDIDA: "La lectura completa ya se pidió a las {hora}.",
  LECTURA_YA_PEDIDA_QUE_HACER: "Espere unos minutos y pulse «Ver si ya está lista». Si en media hora no aparece, podrá volver a pedirla.",
  LECTURA_INDETERMINADA: "La lectura completa no respondió a tiempo y puede haber arrancado igual.",
  LECTURA_INDETERMINADA_QUE_HACER: "Espere unos minutos y pulse «Ver si ya está lista» antes de volver a pedirla.",
  LECTURA_NO_ARRANCO: "La lectura completa no arrancó: {motivo}.",
  LECTURA_NO_ARRANCO_QUE_HACER: "Vuelva a intentarlo en unos minutos. Si se repite, pídala desde Claude Code con la orden «/dictamen».",
  ADVERTENCIA_REGLAS: "Lectura por reglas del texto guardado del pliego y de los datos de su empresa: sin inteligencia artificial. "
    + "Verifique cada cita en el documento oficial antes de decidir. No fija precios.",
});
...
/* Por qué falta una razón del perfil. En un CONSORCIO no es que «el perfil no
   la tenga»: o falta el balance de un integrante, o el divisor es cero (y el
   pliego tipo la da por cumplida). Mandarle al modelo el motivo genérico era
   darle una premisa falsa (revisión adversaria del 25-sep-2026). */
function motivoRazon(perfil, campo, generico) {
  if (perfil && Array.isArray(perfil.indicadoresIndeterminados) && perfil.indicadoresIndeterminados.includes(campo)) {
    return "indeterminado: el divisor es cero (nadie debe intereses o no hay pasivo corriente); el pliego tipo lo da por cumplido";
  }
  if (perfil && Array.isArray(perfil.indicadoresFaltaBalanceDe) && perfil.indicadoresFaltaBalanceDe.length) {
    return `el consorcio no lo tiene porque falta el balance de ${perfil.indicadoresFaltaBalanceDe.join(" y ")} (el pliego tipo suma los balances de todos)`;
  }
  return generico;
}
...
function armarEntrada({ fila, perfil, perfilId, idProceso = null, texto: textoPliego, version, hoy, cambiosHabilitantes = null }) {
  ...
  const perfilEntrada = {
    ...
    liquidez: num(p.liquidez) ?? faltante("perfil.liquidez", motivoRazon(p, "liquidez", "el perfil no tiene índice de liquidez")),
    endeudamiento: num(p.endeudamiento) ?? faltante("perfil.endeudamiento", motivoRazon(p, "endeudamiento", "el perfil no tiene nivel de endeudamiento")),
    cobertura_intereses: num(p.coberturaIntereses) ?? faltante("perfil.cobertura_intereses", motivoRazon(p, "coberturaIntereses", "el perfil no tiene cobertura de intereses")),
...
/* la lectura completa pedida desde la pantalla: marca y candado a la vez (SET NX), para que
   un segundo clic no despierte otra sesión; se borra al guardar el dictamen de la sesión */
const clavePedido = (id, perfilId) => `dictamen:pedido:${id}:${perfilId}`;
const PEDIDO_TTL_SEG = 30 * 60;
...
module.exports = {
  ...
  hashPrompt, hashConstantes, selloFila, claveCache, claveDictamen, claveCandado, claveCuota, claveUso, clavePedido, PEDIDO_TTL_SEG,
};
