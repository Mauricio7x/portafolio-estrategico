...
async function medirAciertoPaa(redis, opciones = {}) {
  ...
  const cliente = crearClientePaa({ ...opciones, plazoDe: () => PRESUPUESTO_MS - (Date.now() - t0) });
  let sonda;
  try {
    sonda = await cliente.pedir({ "$limit": "5" }, "sonda PAA (medición)");
  } catch (e) {
    return { estado: 502, cuerpo: { ok: false, dataset: DATASET, error: `No se pudo abrir el Plan Anual de Adquisiciones: ${e.message}` } };
  }
