...
function evaluarZona(fila) {
  ...
  if (d.dificil_acceso) {
    ...
    etiqueta = `Tu zona (${base})`;
    mensaje = `La obra es del departamento de tu base (${base}): el costo de llegar es el mínimo posible. `
  ...
  if (d.verificar_orden_publico) {
    ...
    mensaje += " Atención: en partes de este departamento hay alertas de orden público documentadas — "
      + "verificá la zona exacta de la obra antes de presentarte (el municipio puede estar tranquilo).";
  }
