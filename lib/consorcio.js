...
const { leerJSON, escribirJSON } = require("./almacen.js");
...
async function guardarConsorcio(redis, { id, nombre, integrantes }) {
  ...
  todos[id] = { id, nombre, integrantes, guardado: new Date().toISOString() };
...
async function simular(redis, { integrantes, proceso = null, contar, ahora = Date.now() }) {
  ...
  if (typeof contar === "function") {
    const clave = claveSim(hashDe({ integrantes: v.integrantes, proceso: proceso ? proceso.id_del_proceso : null, hora: new Date(ahora).toISOString().slice(0, 13) }));
    let cacheado = null;
...
module.exports = {
  ...
  CLAVE_CONSORCIOS, ID_CONSORCIO_RE, TTL_SIM_SEG, ADVERTENCIA_PARTICIPACION_MINIMA, ADVERTENCIA_K,
};
