...
/* ══════════ EL REPARTO RECOMENDADO PARA ESTE PROCESO (25-sep-2026) ══════════
   ...
   que ya existe». Aquí no se calcula nada nuevo:
     1. las casillas del pliego se pasan UNA vez con `simular` y de ahí salen
        la experiencia y los indicadores exigidos con su documento y su página
        (dato publicado gana a calculado); las cláusulas de participación y la
        fórmula del plural salen de documentos_proceso.participacionDe (C);
...
async function recomendarReparto(redis, { dueno, socio, proceso, documentos = null, conocimiento = {}, ahora = Date.now() }) {
  ...
  // la ESPECÍFICA primero: es la que el pliego tipo reparte 50/5/10; la general solo si no hay otra
  const conCifra = (clave) => (mitad.exigencias || []).find((x) => x.clave === clave && x.tipo_valor === "smmlv" && Number.isFinite(Number(x.exige_valor))) || null;
  const experienciaLeida = conCifra("experiencia_especifica") || conCifra("experiencia_general");
  const presupuesto = presupuestoDe(proceso);
  const carga = presupuesto > 0 ? cargaK(proceso, presupuesto) : null;
  /* lo que los documentos dicen del consorcio (encargo C, 26-sep-2026): las
     cláusulas de participación y la fórmula, de documentos_proceso; y los
     requisitos financieros con cifra, de las MISMAS casillas que acaba de pasar
     `simular` (el sentido de cada uno, de lib/diff.REQUISITOS) */
  const { participacionDe } = require("./documentos_proceso.js");
  const { REQUISITOS, cumpleRequisito } = require("./diff.js");
  const leido = documentos ? participacionDe(documentos) : null;
  const financieros = (mitad.exigencias || []).map((x) => {
    const r = REQUISITOS.find((q) => q.id === x.clave && q.perfil && q.sentido && !/^experiencia/.test(q.id));
    return r && x.exige_valor != null && Number.isFinite(Number(x.exige_valor))
      ? { clave: x.clave, titulo: x.titulo, campo: r.perfil, sentido: r.sentido, exige_valor: Number(x.exige_valor), documento: x.documento || null, pagina: x.pagina == null ? null : x.pagina, cita: x.cita || null } : null;
  }).filter(Boolean);
  // sin lectura con cláusulas (ningún documento leído con la versión 4): `clausulas: null` = no se sabe
  const pliego = { ...(leido && leido.leidos > 0 ? leido : { leidos: 0, documentos: [], clausulas: null, metodo: null }), financieros, cumpleRequisito };
  const f = fronteraReparto({
    dueno: PERFILES[dueno], socio: PERFILES[socio], presupuestoCOP: presupuesto, crpc: carga ? carga.crpc : null, crpcMinimo: carga ? carga.crpc_minimo : null,
    exigidaSMMLV: experienciaLeida ? Number(experienciaLeida.exige_valor) : null, tipoContrato: proceso.tipo_de_contrato || null, pliego,
  });
  ...
  /* la experiencia NO va aquí: la juzga la regla 50/5/10 con hasta siete
     contratos, y la casilla del pliego la mide con el mayor contrato de cada uno
     —dos cifras de «juntos» distintas en la misma tarjeta (revisión adversaria) */
  /* la ficha del consorcio se calcula sumando los balances (el pliego tipo); si el
     pliego DICE otra fórmula y con ella un requisito falla con todo reparto, la
     casilla no puede quedar en verde (revisión adversaria, 26-sep-2026) */
  const rojoLeido = (f.financiero && f.financiero.en_rojo_con_la_formula_del_pliego) || [];
  for (const r of rojoLeido) {
    const x = (resultado.exigencias || []).find((y) => y.clave === r.clave);
    if (x && x.estado !== "no_cumple") Object.assign(x, { estado: "no_cumple", estado_legible: "No cumple", nota: "Con la fórmula que trae el pliego (ponderando por la participación) no se cumple con ningún reparto." });
  }
  const enRojoSiempre = (resultado.exigencias || []).filter((x) => x.estado === "no_cumple" && !/^experiencia_/.test(x.clave))
    .map((x) => ({ clave: x.clave, titulo: x.titulo, exige: x.exige, juntos: rojoLeido.some((r) => r.clave === x.clave) ? null : x.suyo, ...cita(x) }));
  return {
    ...resultado,
    recomendacion: {
      suya: f.suya_maxima, del_socio: f.del_socio, frase: f.frase, deja_en: f.deja_en, avisos: f.avisos,
      capacidad: f.capacidad,
      participacion: f.participacion, financiero: f.financiero,
      provisional: !f.participacion.leida,
