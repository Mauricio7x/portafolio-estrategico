...
const { PERFILES, truncar2, derivarPlural } = require("./perfiles.js");
...
/* La SIMULACIÓN completa: indicadores ponderados, capacidad, unión, contratos y
   ...
   app. */
async function simular(redis, { integrantes, proceso = null, documentos = null, contar, ahora = Date.now() }) {
  ...
  if (proceso) {
    ...
    /* LAS OCHO CASILLAS DEL PLIEGO, PARA EL CONSORCIO (6-sep-2026, M-COMP-02).
       ...
       PONDERADA (lo que lee el evaluador), no la suma. Sin documentos, las ocho
    ...
    Object.assign(resultado, await conPerfilTemporal(perfil, async (id) => {
      ...
      try { guia = guiaDe({ fila: proceso, perfil: id, ctx: { documentos: documentos || null, ahoraMs: ahora } }); }
