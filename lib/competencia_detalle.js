...
async function detalleEntidad(redis, entidadCruda, {
  ...
  if (soloPublicado) {
    ...
    const [publicado, pub] = await Promise.all([leerPublicado(redis, pedida), leerGanadoresPublicados(redis, buscada)]);
    return {
      estado: 200,
      cuerpo: {
        ok: true,
  ...
  if (barridoParcial) {
    const [publicadoParcial, pub] = await Promise.all([
      leerPublicado(redis, nombreOriginal || pedida), leerGanadoresPublicados(redis, buscada),
    ]);
    ...
    const mensaje = publicadoParcial || pub
