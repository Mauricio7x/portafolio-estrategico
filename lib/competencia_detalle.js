...
const claveCache = (buscada) => `${CLAVES.detalleCompetencia}v7:${buscada}`; // v4: + proponentes (hgi6) · v5: + ejecucion (jbjy) · v6: + prorroga (M-DGF-06) · v7: + plazo_adjudicacion/desiertos (M-DGF-08) y baja_media del adjudicatario (M-COMP-01)
...
async function leerPublicado(redis, nombreParaIndice) {
  try {
    ...
    return publicado;
  } catch { return null; } // índice sin construir: se informa igual
}
...
async function detalleEntidad(redis, entidadCruda, {
  ...
  if (barridoParcial) {
    const publicadoParcial = await leerPublicado(redis, nombreOriginal || pedida);
    ...
    return {
      ...
      cuerpo: {
        ...
        entidad: nombreOriginal || pedida,
        entidad_normalizada: buscada,
        indice: publicadoParcial
          ? {
            nivel: ["baja", "media", "alta"].includes(publicadoParcial.nivel) ? publicadoParcial.nivel : "sin_dato",
            promedio_oferentes: publicadoParcial.promedio ?? null,
            mediana_oferentes: publicadoParcial.mediana ?? null,
            min_oferentes: null, max_oferentes: null,
            /* del ÍNDICE, no del recuento a medias: por eso `publicado.procesos` */
            procesos_contados: publicadoParcial.procesos_contados ?? publicadoParcial.procesos ?? null,
            total_procesos_adjudicados: null,
            total_procesos_historico: null,
            min_procesos: MIN_PROCESOS,
            publicado: {
              promedio: publicadoParcial.promedio ?? null,
              procesos: publicadoParcial.procesos ?? publicadoParcial.procesos_contados ?? 0,
              nivel: publicadoParcial.nivel || null,
            },
            encogimiento: null,
            reparto_por_anio: {},
            prorroga: prorrogaPublicada(publicadoParcial),
            ...hechosDeRegistro(publicadoParcial),
          }
          : null,
        adjudicatarios: null, proponentes: null, ejecucion: null,
        ...
        mensaje: publicadoParcial
          ? "El histórico es demasiado grande para revisarlo entero en el tiempo disponible, así que la lista de procesos de esta entidad no se pudo armar. Las cifras de arriba son las que el índice de competencia ya tenía calculadas sobre todo el histórico: son ciertas. Vuelva a intentarlo en unos segundos."
          : "El histórico es demasiado grande para revisarlo entero en el tiempo disponible y el índice de competencia todavía no tiene clasificada a esta entidad, así que no hay nada que enseñar sin inventarlo. Vuelva a intentarlo en unos segundos.",
  ...
  const publicado = await leerPublicado(redis, nombreParaIndice);
...
async function detalleAdjudicatario(redis, claveCruda, { usarCache = true, log = () => {} } = {}) {
  ...
  /* la baja por fila, con la regla del índice; los descartes se cuentan con la
     MISMA clave que la meta del índice para que se lean igual */
  const baja = { n: 0, suma: 0, hist: {} };
  const bajaDescartes = {};
  for (const lic of registros) {
    ...
    if (fb.descarte) bajaDescartes[fb.descarte] = (bajaDescartes[fb.descarte] || 0) + 1;
    else { baja.n++; baja.suma += fb.baja; const c = Math.round(fb.baja); baja.hist[c] = (baja.hist[c] || 0) + 1; }
    for (const c of CAMPOS_ADJUDICATARIO) {
      const v = String(lic[c] == null ? "" : lic[c]).trim();
  ...
  /* BAJA MEDIA CON LA QUE GANA (M-COMP-01): el registro publicable de la regla
     del índice (mínimo, anulación bajo él, nivel) y, aparte, la mediana
     ENCOGIDA hacia la referencia global con la meta del índice —solo para el
     factor de precio, como en la tarjeta: lo que se ENSEÑA es la medida—. Sin
     meta (índice de baja sin construir) la encogida es la medida con
     `peso_datos: 1`, que es lo que `encogerBaja` ya hace. Un ranking de
     adjudicatarios por baja no existe: cada cifra viaja con su n. */
  let metaBaja = null;
  try { metaBaja = await leerIndiceBajaMeta(redis); } catch { metaBaja = null; }
  const regBaja = subRegistro(baja, MIN_PROCESOS_BAJA);
  ...
  const bajaMedia = {
    ...
    motivo: regBaja.baja_mediana != null ? null
      : (Number(bajaDescartes.adjudicatario_no_definido) || 0) > 0
        ? `SECOP no publica el NIT del ganador en ${bajaDescartes.adjudicatario_no_definido === 1 ? "1 de los procesos que ganó" : `${bajaDescartes.adjudicatario_no_definido} de los procesos que ganó`}, y esta medida solo cuenta los que sí lo traen`
