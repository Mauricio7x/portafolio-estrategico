...
/* `v2`: la respuesta ganó el bloque `adjudicatarios` (ago 2026). Sin el
   sufijo, una caché escrita por la versión anterior serviría hasta 1 h de
   respuestas SIN el bloque después de desplegar — y desplegar nunca debe
   exigir reconstruir ni esperar (R11). Las claves viejas caducan solas. */
const claveCache = (buscada) => `${CLAVES.detalleCompetencia}v2:${buscada}`;
...
async function detalleEntidad(redis, entidadCruda, {
  ...
  for (const lic of registros) {
    ...
    if (!quien.clave) {
      ...
      if (!g) {
        g = {
          nombres: new Map(),
          nit: quien.clave.startsWith("nit:") ? quien.clave.slice(4) : null,
  ...
  const top = [...ganadores.values()]
    .map((g) => {
      ...
      return {
        nombre: nombre || (g.nit ? `NIT ${g.nit}` : "(sin nombre)"),
        nit: g.nit,
        ganados: g.ganados,
