...
const { manifestacionDeFila, esUrgente: manifUrgente, sigueValiendoLaPena: manifVigente, yaRecibeOfertas: manifRecibeOfertas, enSorteo: manifEnSorteo, faseEnDuda: manifFaseEnDuda, porAbrirInminente: manifPorAbrirInminente, PLAZO_MANIFESTACION_HABILES } = require("./manifestacion.js");
...
/* EL RECORRIDO GANA LA ESPERA DEL SORTEO (encargo del dueño, 22-sep-2026). En la
   selección abreviada de menor cuantía, entre «me interesa» y «preparando la
   oferta» hay un trámite que la app ya sabía explicar pero no sabía SEGUIR:
   avisar que le interesa y esperar a que la entidad publique quiénes siguen
   (sorteo si avisaron más de diez; la lista de interesados si no). Sin esa
   etapa, el usuario que ya avisó seguía recibiendo «avise HOY», y el que no
   salió en el sorteo solo podía marcar «Perdido» (falso: no llegó a ofertar y
   contaminaría el registro con el que se valida la predicción) o «Descartado»
   (falso: no lo decidió él). Dos etapas nuevas, en su sitio del recorrido:
     · `manifestado`  «Avisé que me interesa · en espera del sorteo» — entre
                      interesa y preparando; vale también cuando no hay sorteo
                      (con diez avisos o menos la entidad publica la lista).
     · `no_sorteado`  «No salí en el sorteo» — salida sin desenlace: no es
                      derrota (`desenlaceDe` sigue en null) ni decisión propia.
   Un guardado anterior conserva su etapa: la lista solo crece. */
const ESTADOS = Object.freeze(["interesa", "manifestado", "preparando", "presentado", "ganado", "perdido", "no_sorteado", "descartado"]);
const ESTADO_ETIQUETA = Object.freeze({ interesa: "Me interesa", manifestado: "Avisé que me interesa · en espera del sorteo", preparando: "Preparando la oferta", presentado: "Me presenté", ganado: "Ganado", perdido: "Perdido", no_sorteado: "No salí en el sorteo", descartado: "Descartado" });
/* Las etapas que son HISTORIA, no agenda: no avisan, no viajan en el .ics de
   todo el perfil. UNA lista, porque las dos copias que había (alertas y
   agenda) habrían divergido al entrar la cuarta. Un valor desconocido no es
   historia: cae a «interesa» por `normalizarEstado`, como en todo el módulo. */
const ETAPAS_HISTORIA = Object.freeze(["ganado", "perdido", "descartado", "no_sorteado"]);
/* Las etapas en las que el usuario DECLARÓ que ya avisó que le interesa: a ellas
   no se les pide avisar. SOLO «manifestado»: «preparando» no lo implica —el
   usuario puede estar armando papeles antes de que abra el plazo— y aquí el
   falso caro es el NEGATIVO (perder la manifestación cuesta el proceso; un
   aviso de más a quien ya avisó cuesta un vistazo). Lo terminal no avisa de
   nada por `esHistoria`. La suite fija que «preparando» sigue recibiendo el
   aviso de la ventana corriendo. */
const ETAPAS_YA_AVISO = Object.freeze(["manifestado"]);
/* Las etapas en las que HUBO OFERTA (SECOP II no la recibe sin aviso ni sin
   quedar habilitado): la guía puede dar por hechos el aviso y el sorteo. La
   comparte lib/guia_proceso, que antes tenía su propia lista y divergía. */
const ETAPAS_CON_OFERTA = Object.freeze(["presentado", "ganado", "perdido"]);
...
const esHistoria = (estado) => ETAPAS_HISTORIA.includes(normalizarEstado(estado));
const yaAviso = (estado) => ETAPAS_YA_AVISO.includes(normalizarEstado(estado));
const conOferta = (estado) => ETAPAS_CON_OFERTA.includes(normalizarEstado(estado));
...
/* F0-7 · QUÉ ESTADO ES UN DESENLACE, y por qué solo dos lo son.
   ...
   «no_sorteado» (22-sep-2026) tampoco: la predicción es P(ganar | presentarse)
   y quien no salió en el sorteo nunca presentó oferta.
...
function fotoDe(l) {
  ...
  return {
    ...
    /* LO ÚLTIMO QUE SECOP II PUBLICÓ DE LA FASE (revisión del 22-sep-2026): cuando la fila
       sale del corpus activo (la full mensual o la compactación en cuanto se da por
       cerrada), la manifestación se recalculaba desde la ventana a secas y un proceso
       «por abrir» volvía a «puede estar cerrando o haber cerrado ya», en rojo. La foto
       lleva la fase, el estado y el día en que Socrata vio la fila, para que `senalSecop`
       siga diciendo lo último publicado, con su fecha. `cambiosDe` no los vigila. */
    fase: String(l.fase || "").trim().slice(0, 80) || null,
    estado_secop: String(l.estado_del_procedimiento || l.estado_secop || "").trim().slice(0, 80) || null,
    secop_visto: String(l[":updated_at"] || l.secop_visto || "").slice(0, 24) || null,
    /* y la apertura PUBLICADA de la manifestación (`fecha_de_publicacion`, 22-sep-2026): sin ella, la foto
       sin fila viva volvía a contar el plazo desde la publicación del proceso (la del borrador) */
    manifestacion_publicada: String(l.fecha_de_publicacion || l.manifestacion_publicada || "").slice(0, 10) || null,
  };
}
...
/* LA FOTO QUE SE GUARDA LA PRIMERA VEZ (22-sep-2026, revisión): la del cliente —la fila que
   tenía en pantalla— o, si no trae nombre, la viva; y de la VIVA, siempre, lo último
   publicado de la fase y la apertura publicada de la manifestación, para que «cambió» y
   el plazo se midan contra SECOP II y no contra lo que el cliente había pintado. Vivía
   como una lista de campos en el handler, sin prueba: aquí es una sola regla. */
function fotoAlGuardar(fotoCliente, viva) {
  const fv = viva ? fotoViva(viva) : null;
  const base = fotoCliente && fotoCliente.nombre ? fotoCliente : fv;
  if (!base) return null;
  if (!fv) return base;
  return { ...base, estado_secop: fv.estado_secop, fase: fv.fase, secop_visto: fv.secop_visto, manifestacion_publicada: fv.manifestacion_publicada };
}
...
function hitosDe(l, hoy, opciones) {
  ...
  /* SIN HITO CALCULADO CUANDO LA FASE PUBLICADA LO DESMIENTE (revisión del
     22-sep): a un plazo que todavía no abre, o que SECOP II ya dio por cerrado,
     el calendario no le pone «puede cerrar este día». La fecha del pliego, si la
     hay, sí viaja: es un dato publicado. */
  const calculadoDesmentido = !!m && (m.estado === "por_abrir" || (m.estado === "vencida" && m.origen_vencimiento === "fase_secop"));
  if (m && (m.fecha_limite || (m.puede_cerrar_desde && !calculadoDesmentido)) && !base.some((h) => h.id === "manifestacion")) {
...
function enriquecer(guardado, filaViva, ahoraMs = Date.now(), { fechaManifestacionCronograma = null, carpetas = null } = {}) {
  ...
  /* sin fila viva manda la foto más reciente (el «visto» de «Enterado» o la del guardado), con lo
     último que SECOP II publicó de la fase: un publicado con fecha gana a la ventana calculada */
  const ultimaFoto = [guardado.visto, foto].filter((f) => f && typeof f === "object")
    .sort((a, b) => String(b.secop_visto || "").localeCompare(String(a.secop_visto || "")))[0] || foto;
  const filaHitos = l ? { ...l, fecha_cierre: (fila && fila.fecha_cierre) || l.fecha_cierre || null }
    : { fecha_de_publicacion_del: foto.fecha_publicacion, fecha_cierre: foto.fecha_cierre, fecha_de_apertura_de_respuesta: foto.fecha_apertura, modalidad_de_contratacion: foto.modalidad,
      fase: ultimaFoto.fase || null, estado_del_procedimiento: ultimaFoto.estado_secop || null, ":updated_at": ultimaFoto.secop_visto || null,
      fecha_de_publicacion: ultimaFoto.manifestacion_publicada || null };
  ...
  const completo = {
    ...
    /* la etapa ya dice si avisó: viaja resuelta para que ninguna pantalla copie la lista (22-sep-2026) */
    ya_aviso: yaAviso(guardado.estado), es_historia: esHistoria(guardado.estado),
    ...
    /* CON QUIÉN CONVIENE IR, congelado el día en que lo guardó (11-sep-2026).
       Se PUBLICA aquí de forma explícita porque este objeto se construye campo
       a campo: lo que no se nombre en esta función no llega al navegador aunque
       esté en Redis — la lección literal de F0-7, que guardaba la predicción y
       no la enseñaba. Un guardado anterior a esta versión viaja en null: no se
       inventa el consejo de un día pasado. */
    socio: guardado.socio || null,
...
function aLigero(p) {
  ...
  /* `socio` viaja con los pesados: el veredicto completo son ~1,4 KiB por
     proceso y la lista no lo pinta —se lee en el expediente, que es donde el
     navegador entra al guardar—. Con 200 guardados serían ~280 KiB de más en
     una respuesta cuyo tope de Vercel es 4,5 MB y que ya se recortó una vez por
     esto mismo. La lista conserva `tiene_socio` para poder señalarlo sin
     cargarlo. */
  const { tareas: _t, documentos: _d, notas: _n, avisos: _a, guia: _g, socio: _s, ...resto } = p;
  return { ...resto, tiene_notas: !!p.notas, tiene_guia: !!p.guia, tiene_socio: !!(p.socio && p.socio.recomendacion) };
}
...
function alertasDe(procesos, { dias = 7, hoy = null } = {}) {
  ...
  for (const p of procesos || []) {
    if (esHistoria(p.estado)) continue;
    ...
    /* A QUIEN YA AVISÓ NO SE LE PIDE AVISAR (22-sep-2026). En la etapa «avisé ·
       en espera del sorteo» lo que pide atención es el HECHO PUBLICADO de la
       fase: si SECOP II ya recibe ofertas, el sorteo o la lista ya salió y hay
       que mirar si quedó; si cerró los avisos y sigue en esa fase, el sorteo
       está por publicarse. Sin señal, silencio: la ausencia no es un plazo. */
    if (p.estado === "manifestado" && m && m.aplica) {
      if (manifRecibeOfertas(m)) {
        out.push({ tipo: "sorteo", id: p.id, proceso: nombre, fecha: m.secop_fecha || null, urgencia: "alta",
          mensaje: `SECOP II ya recibe ofertas en este proceso${m.secop_fase ? ` (fase «${m.secop_fase}»${m.secop_fecha_legible ? `, vista el ${m.secop_fecha_legible}` : ""})` : ""}: mire si quedó habilitado tras el sorteo o en la lista de interesados. Si quedó, pase el proceso a «Preparando la oferta»; si no, márquelo «No salí en el sorteo».` });
      } else if (manifEnSorteo(m)) {
        /* «ya no recibe avisos» es lo que consta; que el sorteo esté pendiente no
           (revisión del 22-sep): se dice qué mirar, no cuándo saldrá */
        out.push({ tipo: "sorteo", id: p.id, proceso: nombre, fecha: m.secop_fecha || null, urgencia: "media",
          mensaje: `SECOP II ya no recibe avisos de interés en este proceso${m.secop_fecha_legible ? ` (visto el ${m.secop_fecha_legible})` : ""}: mire en el proceso si ya salió la lista de interesados o el sorteo y si usted quedó.` });
      } else if (manifFaseEnDuda(m)) {
        /* la posición es «cerrada» pero el plazo calculado sigue vivo (republicación con
           la fase vieja, la cicatriz de la UPN): no se afirma el sorteo; se manda a mirar */
        out.push({ tipo: "sorteo", id: p.id, proceso: nombre, fecha: m.secop_fecha || null, urgencia: "media",
          mensaje: `SECOP II tiene este proceso en la fase «${m.secop_fase || "posterior"}»${m.secop_fecha_legible ? ` (vista el ${m.secop_fecha_legible})` : ""}, pero se publicó hace poco y esa fase puede venir de una publicación anterior: entre HOY al proceso, mire si su aviso sigue en pie y en qué va, y no dé el sorteo por hecho.` });
      }
    }
    if (!yaAviso(p.estado) && manifUrgente(m)) {
    ...
    /* LAS OBSERVACIONES YA CERRARON Y EL PLAZO PUEDE ABRIR EN CUALQUIER MOMENTO
       (22-sep-2026, medición del dueño: 265 menores cuantías en ese punto). Media,
       no alta: nadie puede avisar todavía; lo que toca es mirar el cronograma hoy. */
    if (!yaAviso(p.estado) && manifPorAbrirInminente(m)) {
      out.push({ tipo: "manifestacion", id: p.id, proceso: nombre, fecha: m.secop_fecha || null, urgencia: "media",
        mensaje: `Las observaciones al pliego ya cerraron según SECOP II${m.secop_fecha_legible ? ` (visto el ${m.secop_fecha_legible})` : ""}: el plazo para avisar que le interesa puede abrir en cualquier momento y durar solo unas horas. Mire hoy el cronograma del proceso en SECOP II y avise el mismo día que abra.` });
    }
    ...
    for (const a of p.avisos || []) {
      ...
      if (a.hito === "manifestacion" && yaAviso(p.estado)) continue; // ya avisó: no hay nada que recordar
...
module.exports = { ESTADOS, ESTADO_ETIQUETA, ETAPAS_HISTORIA, ETAPAS_YA_AVISO, ETAPAS_CON_OFERTA, esHistoria, yaAviso, conOferta, CAMPOS_VIGILADOS, MAX_GUARDADOS, MAX_NOTAS, normalizarEstado, desenlaceDe, fotoDe, fotoViva, fotoAlGuardar, hitosDe, cambiosDe, enriquecer, alertasDe, fichaCompetidor, icsDe, nitONull,
