...
const { manifestacionDeFila, esUrgente: manifUrgente, sigueValiendoLaPena: manifVigente } = require("./manifestacion.js");
    ...
    if (manifUrgente(m)) {
      ...
      out.push({ tipo: "manifestacion", id: p.id, proceso: nombre, fecha: m.fecha_limite || m.puede_cerrar_desde, urgencia: "alta",
        mensaje: m.confirmada
          ...
          : `El plazo para avisar que le interesa puede estar cerrando o haber cerrado ya: la ley da un MÁXIMO de ${3} días hábiles desde la apertura (${m.apertura}) y la entidad pudo poner menos. Entre HOY a SECOP II, verifique el cronograma y manifieste. Sin la manifestación no podrá ofertar.` });
    }
