...
const { manifestacionDeFila } = require("./manifestacion.js");
...
/* Hitos = los del dataset (lib/cronograma) + la apertura de ofertas si viene
   ...
   cuantía): apertura + 3 días hábiles, origen «calculado» y su nota — nunca
   se presenta como fecha publicada. */
function hitosDe(l, hoy) {
  ...
  const m = manifestacionDeFila(l || {}, hoy || hoyColombia());
  if (m && m.vence && !base.some((h) => h.id === "manifestacion")) {
    base.push({ id: "manifestacion", etiqueta: "Manifestar interés en SECOP II, último día (fecha calculada)", fecha: m.vence, origen: "calculado", evidencia: m.nota });
  }
...
function enriquecer(guardado, filaViva, ahoraMs = Date.now()) {
  ...
  const hitos = hitosDe(filaHitos, hoy);
  const avisos = avisosDe(hitos, hoy);
  const manifestacion = manifestacionDeFila(filaHitos, hoy);
  ...
  return {
    ...
    /* manifestación de interés (menor cuantía): fecha límite calculada, días
       hábiles que quedan y si venció; null si la modalidad no la exige */
    manifestacion,
...
function alertasDe(procesos, { dias = 7 } = {}) {
  ...
  for (const p of procesos || []) {
    ...
    if (m && m.vencida === false && m.quedan_habiles != null && m.quedan_habiles <= 2) {
      out.push({ tipo: "manifestacion", id: p.id, proceso: nombre, fecha: m.vence, urgencia: "alta",
        mensaje: m.dias_calendario === 0 ? `Manifestar interés vence HOY (${m.vence_legible}); sin la manifestación en SECOP II no podrá ofertar. Fecha calculada: confírmela en el cronograma.`
          : m.dias_calendario === 1 ? `Manifestar interés vence mañana (${m.vence_legible}): hágalo hoy en SECOP II. Fecha calculada: confírmela en el cronograma.`
            : `Quedan ${m.quedan_habiles} días hábiles para manifestar interés (hasta ${m.vence_legible}). Fecha calculada: confírmela en el cronograma.` });
    }
    ...
    for (const a of p.avisos || []) {
      ...
      if (a.hito === "manifestacion" && m && m.quedan_habiles != null && m.quedan_habiles <= 2) continue; // ya está arriba
