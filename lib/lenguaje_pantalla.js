...
/* FRONTERA DE PALABRA PROPIA (remate H-05, 6-sep-2026): `\b` en JavaScript es ASCII,
   así que «verificá » —cualquier imperativo del voseo terminado en vocal acentuada—
   seguido de espacio o puntuación JAMÁS casaba con `verificá\b`: la cerca era ciega a
   «pensá», «verificá», «revisá», «hacé», «poné» y «andá» (medido: «Verificá a tu
   socio» vivió en README.md con la suite en verde). La frontera mira letras con tilde
   y ñ a los dos lados, y la bandera `i` ve el imperativo que abre una frase. */
const FRONTERA_INI = "(?<![\\wáéíóúñÁÉÍÓÚÑ])";
const FRONTERA_FIN = "(?![\\wáéíóúñÁÉÍÓÚÑ])";
const VOSEO_RE = new RegExp(`${FRONTERA_INI}(Pod[ée]s|Cumpl[íi]s|Ten[ée]s|Quer[ée]s|Deb[ée]s|Sab[ée]s|presentarte|pensá|verificá|revisá|hacé|poné|fijate|and[aá]|dale|vas|ejecutaste|tendr[aá]s|Eliminar[aá]s|puedes|tienes|quieres|debes|hazlo)${FRONTERA_FIN}`, "i");
...
/* EXCEPCIONES DECLARADAS, con su motivo (censo, no lista: quitar una palabra de
   ...
   · sustantivos y adverbios que acaban igual: además, contraste, demás,
     después, detrás, estrés, interés, jamás, más, país, revés — y, desde la
     tanda de frases del 13-sep-2026, «arnés» (el de seguridad en altura) y
     «atrás», de la misma familia que «interés» y «detrás»; con la tanda del
     criterio del dueño entran «cortés», «francés» (el filtro de drenaje) y
     «veintitrés» (la edad del residente), que son la misma terminación
   · presente de indicativo de usted/él en -iste: existe, insiste, persiste —
     con «consiste» (consistir), que es exactamente el mismo caso
   · sustantivos en -aste que no son pretérito de tú: contraste, desgaste — y
     el subjuntivo de usted/él «gaste» (que la nómina se gaste aquí), que
     tampoco es el pretérito «gastaste»
   · nombres propios de Colombia: Andrés, Vaupés
   · adjetivos en -ilo/-ila que no vienen de ningún verbo: tranquilo, tranquila
     — y sus PLURALES «tranquilos»/«tranquilas» (13-sep-2026), que la excepción
     en singular dejaba fuera: el enclítico caza `-ilos?/-ilas?`, así que
     declarar solo el singular deja vivo al hermano de al lado. Salió de una
     frase de portada («Los acuerdos que se escriben tranquilos se cumplen
     tranquilos») y valía igual para cualquier pantalla
   · verbos de usted/él cuya RAÍZ acaba en -il-, que el enclítico confunde con
     un imperativo voseante («alquila» es «se alquila», no «alquilá»): alquila
     — y «perfila» (13-sep-2026), exactamente el mismo caso: «la cuneta que se
     perfila» es tercera persona de perfilar, no el imperativo «perfilá». Salió
     de una frase de portada y se buscó por CENSO en las 4.000 de la tanda: era
     el único candidato sin declarar de esa familia
   · el nombre del evento del navegador «paste» (addEventListener("paste", …)):
     inglés, no pretérito de tú; vive en el guion, jamás en un texto visible
     (13-sep-2026, la respuesta del chat se verifica al pegar) */
const EXCEPCIONES_TUTEO = Object.freeze([
  "además", "arnés", "atrás", "contraste", "cortés", "demás", "después", "detrás", "estrés", "francés", "interés", "jamás", "más", "país", "revés", "veintitrés",
  "consiste", "desgaste", "existe", "gaste", "insiste", "persiste",
  "andrés", "vaupés",
