/* ============================================================================
   ...
   LOS TRES AJUSTES, y por qué cada uno:
     ...
     · colisión de cierres MEDIDA  si la misma entidad cierra varios procesos el
                                 mismo día, los rivales —firmas de 1 a 20
                                 personas con el mismo cuello de botella de
                                 ingeniería— se reparten entre ellos. Desde el
                                 16-ago-2026 (A7) el factor SALE DEL HISTÓRICO:
                                 `indice:competencia:meta.colision` publica el
                                 multiplicador implícito medido por entidad
                                 (§9.3 del doc) y `factorColisionDe` lo convierte
                                 en el factor (umbral de muestra y tope). Medido
                                 en producción: 1,06 sobre 1 465 entidades — el
                                 ×1,15 que había era un supuesto y no estaba
                                 respaldado. FACTOR_COLISION_CIERRES (1,15) queda
                                 SOLO como respaldo declarado cuando no hay
                                 medición (índice sin reconstruir), y el desglose
                                 dice cuál de los dos aplicó (`origen`).

   El factor de prórroga es un SUPUESTO declarado, no un coeficiente ajustado:
   no hay etiqueta (no se sabe qué se ganó y qué se perdió) contra la que
   calibrarlo, y el flag no se persiste al histórico (B3). Por eso es una
   constante con nombre, vive aquí y viaja en el desglose de la respuesta.
...
/* Respaldo del factor de colisión cuando la meta del índice no trae medición.
   Desde A7 el que se APLICA con índice reconstruido es el medido (ver
   `factorColisionDe`); este 1,15 es el supuesto original, conservado solo para
   el hash viejo y declarado como tal en el desglose. */
const FACTOR_COLISION_CIERRES = 1.15;
/* Guardas de la medición: mínimo de entidades con los dos grupos para creerse
   la cifra, y tope por si el histórico trajera un artefacto. */
const COLISION_MIN_ENTIDADES = 30;
const COLISION_FACTOR_MIN = 0.8;
const COLISION_FACTOR_MAX = 1.5;
...
/* LA AUSENCIA SE DESCARTA ANTES DE CONVERTIR (22-sep-2026): `Number(null)` es 0,
   y con `promedio_departamento: null` —lo que el listado pasa cuando la fila
   no trae departamento— la cadena creía tener CERO rivales medidos en el
   departamento: p = 0,95 y la tarjeta decía «1 de 2 se gana» al lado de «sin
   histórico». Es la regla dura de CLAUDE.md aplicada a la única conversión de
   este módulo; un promedio de departamento de 0 rivales no existe (solo se
   agregan entidades con base). */
const numero = (v) => {
...
function rivalesEncogidos(competencia) {
  ...
  const pr = c.prior == null || c.prior === "" ? null : numero(c.prior);
  return {
    rivales: r,
    peso_datos: w != null && w >= 0 && w <= 1 ? w : null,
    desv: sd != null && sd >= 0 ? sd : null,
    // B7: el prior hacia el que se encogió (departamento o global) y su origen
    prior: pr != null && pr >= 0 ? pr : null,
    prior_origen: typeof c.prior_origen === "string" ? c.prior_origen : null,
  };
}
...
/* ─────────────── el factor de PRECIO (A4) ───────────────
   ...
   aquí. */
function factorPrecio(baja, bajaMaxima, origenBmax = null) {
  const b = baja || null;
  ...
  // `apu` (calculada del presupuesto guardado del proceso) o `declarada` (`?baja_max=`)
  const origen = bmax == null ? "neutra" : (origenBmax === "apu" ? "apu" : "declarada");
  if (bmax == null || bmax >= mediana) {
    /* Los campos de dispersión viajan TAMBIÉN en esta rama: un consumidor no
       puede recibir `undefined` según por dónde salga la función. Aquí no hay
       curva que evaluar, así que ninguno de los dos tiene valor — y `null` es
       «no aplica», no una dispersión de cero. */
    return { factor: 1, mediana, b_max: bmax, origen_b_max: origen, ofertable: bmax == null ? mediana : bmax,
      dispersion_pp: null, dispersion_origen: null };
  }
  ...
  return {
    ...
    /* MEDIDA (del IQR de la celda) o SUPUESTA (el defecto del motor, cuando el
       índice no publica un IQR utilizable). Sin esto, un 4 supuesto y un 4
       medido son el mismo número en pantalla — y son dos cosas distintas. */
    dispersion_origen: c ? c.origen_sigma : null,
  };
}

/* Factor de colisión a APLICAR, a partir de la medición publicada por el índice
   (`indice:competencia:meta.colision`, lib/indice_competencia.medirColision).
     medición con ≥ COLISION_MIN_ENTIDADES entidades y multiplicador finito
       → { factor: multiplicador implícito acotado, origen: "medido", … }
     si no → { factor: FACTOR_COLISION_CIERRES, origen: "supuesto" }
   Se REDONDEA a 4 decimales, como todo lo que se publica: `base × Π factores`
   tiene que reproducir `p`. `Number(null)` es 0: la ausencia se descarta antes. */
function factorColisionDe(medicion) {
  const m = medicion || null;
  const crudo = m ? m.multiplicador_implicito : null;
  const mult = crudo == null || crudo === "" ? null : numero(crudo);
  const ents = m ? numero(m.entidades_con_ambos_grupos) : null;
  if (mult == null || mult <= 0 || ents == null || ents < COLISION_MIN_ENTIDADES) {
    return { factor: FACTOR_COLISION_CIERRES, origen: "supuesto", entidades: ents, procesos: null };
  }
  const acotado = Math.min(COLISION_FACTOR_MAX, Math.max(COLISION_FACTOR_MIN, mult));
  return {
    factor: redondear4(acotado), origen: "medido",
    entidades: ents, procesos: numero(m.procesos_colision),
    acotado: acotado !== mult,
  };
}

/* B3: factor de PRÓRROGA a aplicar, a partir de la medición del índice
   (`meta.prorroga`, misma forma que la colisión). Con ≥ COLISION_MIN_ENTIDADES
   entidades medidas, el multiplicador implícito acotado; si no, el 1,20
   declarado. Hoy la señal apenas empieza a acumularse (el delta la estampa
   desde el 16-ago-2026 y el backfill no la trae), así que en producción es
   «supuesto» hasta que haya base — y ese día cambia solo, con su origen. */
function factorProrrogaDe(medicion) {
  const m = medicion || null;
  const crudo = m ? m.multiplicador_implicito : null;
  const mult = crudo == null || crudo === "" ? null : numero(crudo);
  const ents = m ? numero(m.entidades_con_ambos_grupos) : null;
  if (mult == null || mult <= 0 || ents == null || ents < COLISION_MIN_ENTIDADES) {
    return { factor: FACTOR_CIERRE_PRORROGADO, origen: "supuesto", entidades: ents };
  }
  const acotado = Math.min(COLISION_FACTOR_MAX, Math.max(COLISION_FACTOR_MIN, mult));
  return { factor: redondear4(acotado), origen: "medido", entidades: ents, acotado: acotado !== mult };
}
...
/* ─────────────────────── el estimador ───────────────────────
   `contexto`:
     ...
     baja_maxima_origen     "apu" (del presupuesto guardado del proceso) o
                            "declarada" (`?baja_max=`); viaja en el ajuste
     baja_para_precio       la misma baja ENCOGIDA hacia la referencia de su
                            modalidad (lib/indice_baja.encogerBaja); opcional
     colision_cierres       nº de procesos de la MISMA entidad que cierran el
                            MISMO DÍA (incluido este); >1 activa el ajuste.
     colision_medida        `meta.colision` del índice de competencia (la
                            medición A7); sin ella el factor es el respaldo 1,15.
     prorroga_medida        `meta.prorroga` (B3); sin base, el respaldo 1,20.
...
function trazaP(lic, contexto = {}) {
  const l = lic || {};
  let rivales = null, fuente = null, pesoDatos = null, desv = null, encogido = false, prior = null, priorOrigen = null;
  ...
  if (enc) {
    ...
    prior = enc.prior; priorOrigen = enc.prior_origen;
  } else {
  ...
  if (l._cierre_prorrogado) {
    const fpr = factorProrrogaDe(contexto.prorroga_medida);
    aplicar("cierre_prorrogado", fpr.factor,
      fpr.origen === "medido"
        ? `el cierre se movió por adenda: suele indicar que no llegaron ofertas suficientes (efecto MEDIDO en el histórico sobre ${fpr.entidades} entidades)`
        : "el cierre se movió por adenda: suele indicar que no llegaron ofertas suficientes (supuesto declarado: la señal aún no tiene base medida)",
      { origen: fpr.origen, entidades_medicion: fpr.entidades });
  }
  ...
  /* La baja PARA EL PRECIO puede venir encogida hacia la referencia de su
     modalidad (`baja_para_precio`, lib/indice_baja.encogerBaja); si no viene,
     la baja tal cual. La mediana medida de la celda sigue viajando aparte. */
  const bajaPrecio = contexto.baja_para_precio || contexto.baja;
  const fp = factorPrecio(bajaPrecio, contexto.baja_maxima_pct, contexto.baja_maxima_origen);
  if (fp) {
    const celda = bajaPrecio && bajaPrecio.baja_mediana_celda != null ? numero(bajaPrecio.baja_mediana_celda) : null;
    // si la mediana se encogió, se dice de dónde viene la cifra que se usa
    const m = celda != null && celda !== fp.mediana
      ? `${fp.mediana} % (su celda: ${celda} %, ajustada hacia el mercado de la modalidad)`
      : `${fp.mediana} %`;
    const segun = fp.origen_b_max === "apu" ? " (según su presupuesto guardado)" : "";
    const motivo = fp.origen_b_max === "neutra"
      ...
      : fp.factor >= 1
        ? `puede bajar hasta ${fp.b_max} % sin perder plata${segun} y aquí el que gana descuenta ~${m}: puede jugar en el centro del mercado`
        : `solo puede bajar hasta ${fp.b_max} % sin perder plata${segun} y aquí el que gana descuenta ~${m}: ofertar por encima del centro cuesta probabilidad`;
    aplicar("precio", fp.factor, motivo, {
      baja_mediana_pct: fp.mediana, baja_maxima_pct: fp.b_max, origen_b_max: fp.origen_b_max,
      baja_ofertable_pct: fp.ofertable, dispersion_pp: fp.dispersion_pp,
      // encogimiento de la celda (null si no se encogió)
      baja_mediana_celda: bajaPrecio && bajaPrecio.baja_mediana_celda != null ? bajaPrecio.baja_mediana_celda : null,
      peso_datos_baja: bajaPrecio && bajaPrecio.peso_datos != null ? bajaPrecio.peso_datos : null,
      referencia_baja: bajaPrecio && bajaPrecio.referencia ? bajaPrecio.referencia : null,
    });
  }

  const colision = Number(contexto.colision_cierres) || 0;
  if (colision > 1) {
    const fc = factorColisionDe(contexto.colision_medida);
    aplicar("colision_cierres", fc.factor,
      fc.origen === "medido"
        ? `la entidad cierra ${colision} procesos el mismo día: los rivales se reparten (efecto MEDIDO en el histórico sobre ${fc.entidades} entidades)`
        : `la entidad cierra ${colision} procesos el mismo día: los rivales se reparten (supuesto declarado: el índice no trae medición)`,
      { origen: fc.origen, entidades_medicion: fc.entidades });
  }
  ...
  return {
    ...
    encogido, peso_datos: pesoDatos, rivales_desv: desv, prior, prior_origen: priorOrigen,
...
function estimarPDetalle(lic, contexto = {}) {
  ...
  return {
    ...
    prior: t.prior, prior_origen: t.prior_origen,
...
const MAX_MEMO_CANONICA = 20000;
const _memoCanonica = new Map();
function canonicaMemo(nombre) {
  const k = String(nombre);
  const v = _memoCanonica.get(k);
  if (v !== undefined) return v;
  const c = require("./indice_competencia.js").claveCanonica(k);
  if (_memoCanonica.size < MAX_MEMO_CANONICA) _memoCanonica.set(k, c);
  return c;
}
...
function claveColision(l) {
  ...
  /* LA MISMA IDENTIDAD DE ENTIDAD QUE LA MEDICIÓN (ago 2026). El disparador
     agrupaba por NIT y el factor medido (`medirColision`) estratifica por
     `claveCanonica(entidad)`: dos regionales que COMPARTEN NIT —el dataset lo
     hace, y la propia CCE lo advierte— cerrando el mismo día recibían el
     multiplicador de colisión sin colisión real. Es la lección de identidad ya
     pagada («clave canónica → legado → alias»), que no se había aplicado aquí.
     El NIT queda de respaldo para las filas sin nombre de entidad.
     MEMOIZADA POR NOMBRE (ago 2026): `claveCanonica` normaliza y aplica dos
     reemplazos, y esta función se llama DOS veces por fila del corpus (el
     índice de colisión y luego la fila). Medido sobre 20 000 filas, pasar del
     NIT a la clave canónica costaba 15 ms → 269 ms, y a 120 000 filas más de
     un segundo. Los nombres de entidad se repiten masivamente (unas 3 500
     entidades para todo el corpus), así que un Map lo devuelve a ~0. El tope
     existe porque la instancia serverless vive entre peticiones: sin él, un
     corpus con nombres basura haría crecer el Map sin final. */
  const canonica = canonicaMemo(l.entidad || "");
  const ent = canonica || String(l.nit_entidad || "").trim().toUpperCase();
  if (!ent) return null;
  return `${ent}|${d}`;
}
...
module.exports = {
  ...
  factorPrecio, rivalesEncogidos, factorColisionDe, factorProrrogaDe,
  PROMEDIO_CONSERVADOR, COLISION_MIN_ENTIDADES, COLISION_FACTOR_MIN, COLISION_FACTOR_MAX,
