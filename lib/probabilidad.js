/* ============================================================================
   ...
   que ahora sin escalones. Por eso son constantes con nombre, viven todas aquí y
   ...
   2. LOS DOS ESCALONES DE LA BAJA. Costaban −9,1 % de probabilidad al cruzar el
      2 % y −15,0 % al cruzar el 5 %. Ahora es una rampa CONTINUA entre los mismos
      dos extremos.

      PERO OJO CON CÓMO SE CUENTA ESTO, porque es fácil prometer de más:
      · La rampa suaviza la FUNCIÓN; el DATO sigue cuantizado. `lib/indice_baja`
        publica la mediana como una cubeta ENTERA del histograma (`Math.round`),
        así que en producción la mediana solo puede valer …, 2, 3, 4, 5, … y lo
        que se ve NO es una curva sino una escalera de cuatro peldaños
        {≤2 → 1,10 · 3 → 1,0167 · 4 → 0,9333 · ≥5 → 0,85}. Lo que mejora, medido,
        es la ALTURA del peldaño más alto: del 15,0 % al 8,9 %. Decir «ya no hay
        saltos» sería falso mientras la mediana sea entera.
      · LAS COMPARACIONES PASARON DE ESTRICTAS A INCLUSIVAS, y eso mueve DOS
        valores que sí son frecuentes. Antes `> 5` y `< 2` dejaban las medianas
        de exactamente 2 y exactamente 5 en tierra de nadie (×1,00); ahora
        2 → ×1,10 (+10 %) y 5 → ×0,85 (−15 %). Los que de verdad no se mueven ni
        un dígito son 0, 1, 6, 7, 8… — o sea el interior de las mesetas, no sus
        bordes. La ALCALDÍA DE PURIFICACIÓN del corpus de prueba tiene mediana
        exactamente 5, y su `p` cae de 0,325 a 0,2125 por las DOS causas a la vez
        (el tertil que se va y el borde que cambia de lado).
...
/* Resolución ÚNICA de todo lo que se publica desde aquí: base, factores y `p`.
   Que sea la misma en los tres es lo que permite rehacer la cuenta a mano. */
...
/* ─────────────── la rampa de la baja de mercado ───────────────
   ...
   Propiedades que hay que conservar y que la suite vigila:
     ...
     · MESETAS IDÉNTICAS a las de antes, pero NO sus bordes. Para 0, 1, 6, 7, 8…
       el factor es exactamente el de siempre, y ahí es donde está la masa: la
       mediana de baja del mercado colombiano es 0 % (lib/indice_baja), o sea
       ×1,10 en los dos regímenes. En cambio 2 y 5 CAMBIAN, porque la
       comparación pasó de estricta (`> 5`, `< 2`) a inclusiva: antes caían en
       la zona neutra ×1,00 y ahora reciben ×1,10 y ×0,85. Son valores
       frecuentes —la mediana siempre es entera—, así que esto no es una
       sutileza de frontera: es el cambio más grande que produce esta rampa.
...
function estimarPDetalle(lic, contexto = {}) {
  ...
  /* La base se REDONDEA aquí, no al publicarla, y esa es la diferencia entre que
     el desglose cuadre y que casi cuadre. Publicando `round(base)` pero
     multiplicando por la base sin redondear, `base × Π factores` se desviaba de
     `p` en hasta 1,2e-4 —más de una unidad del último decimal que se publica— en
     el 95 % de los casos: la tarjeta enseñaba una cuenta que no daba su propio
     resultado. Redondeando una sola vez, al principio, la única diferencia que
     queda es el redondeo final de `p`, o sea media unidad del último decimal. */
  ...
  /* OJO: aquí NO se llama a `numero()`. Llamarlo aquí dejaba la guarda de
     ...
     se purga nunca y un hash escrito por una versión anterior puede traer esa
     combinación. `factorBaja` descarta la ausencia por su cuenta. */
  const medianaBaja = baja && baja.nivel && baja.nivel !== "sin_dato" ? baja.baja_mediana : null;
  const fBaja = factorBaja(medianaBaja);
  if (fBaja != null) {
    p *= fBaja;
    // `fBaja != null` ya garantiza que `factorBaja` aceptó la mediana, así que
    // aquí `numero()` no puede devolver null: se usa solo para que el mensaje
    // enseñe un número y no la cadena cruda que venga del hash.
  ...
  return {
    p: redondear4(p),
    base,                                  // ya viene redondeada: ver arriba
