...
/* ─────────────────────── el estimador ───────────────────────
   ...
   vista redondeada (el contrato que consume la app) y `estimarP` el atajo que
...
function trazaP(lic, contexto = {}) {
  ...
  if (nivel === "baja") {
    aplicar("competencia_baja", FACTOR_COMPETENCIA_BAJA, "la entidad recibe pocas ofertas en su histórico");
  } else if (nivel === "alta") {
    aplicar("competencia_alta", FACTOR_COMPETENCIA_ALTA, "la entidad recibe muchas ofertas en su histórico");
  }
  ...
  if (medianaBaja != null) {
    if (medianaBaja > BAJA_ALTA_DESDE) {
      aplicar("baja_alta", FACTOR_BAJA_ALTA,
        `la entidad adjudica ~${medianaBaja} % por debajo del presupuesto: ganar exige descontar`);
    } else if (medianaBaja < BAJA_BAJA_HASTA) {
      aplicar("baja_baja", FACTOR_BAJA_BAJA,
        `la entidad adjudica cerca del presupuesto oficial (~${medianaBaja} % de baja)`);
    }
  }
...
function estimarPDetalle(lic, contexto = {}) {
  ...
  return {
    p: Math.round(t.p * 1e4) / 1e4,
    base: Math.round(t.base * 1e4) / 1e4,
...
module.exports = {
  estimarP, estimarPDetalle, trazaP, valorEsperado,
