...
function validarContratos(datos) {
  ...
  lista.forEach((c, i) => {
    ...
    /* `unspsc` OPCIONAL (ago 2026, formato CSV del onboarding): el código del
       contrato ejecutado, si el dueño lo conoce. Si viene, tiene que ser un
       código de 8 dígitos; si no viene, la clave NO se escribe — así los
       contratos cargados con el esquema anterior conservan exactamente su
       forma y nada que compare objetos guardados puede romperse. */
    let unspsc = null;
    if (c.unspsc != null && String(c.unspsc).trim() !== "") {
      unspsc = String(c.unspsc).trim();
      if (!/^\d{8}$/.test(unspsc)) {
        acc.error(`${base}.unspsc`, "si viene, debe ser un código UNSPSC de exactamente 8 dígitos (o dejarse vacío)", c.unspsc);
        unspsc = null;
      }
    }
    limpios.push({
...
/* ⚠️ LA EXPERIENCIA TIENE DUEÑO, Y NO LO GUARDABA (28-ago-2026).
   `config:experiencia` es UNA sola clave global y lo cargado en producción son
   los 106 contratos de `experiencia_genesis_106.json` — de Génesis—. Sin dueño
   escrito, toda pantalla que la leyera se la atribuía a quien estuviera mirando:
   con Helder seleccionado, «Obra que ya ejecutó» le contaba 106 contratos que no
   son suyos, y la auditoría de cobertura le proponía inscribir códigos deducidos
   del trabajo de otro. Lo reportó el dueño: «la información que está dando es en
   general, de Génesis, no aplica a Helder».
   El dueño se GUARDA ahora, y su ausencia es `null` —«no consta de quién es»—,
   jamás «de todos»: es la misma regla de «sin dato» ≠ «cero». Una experiencia
   cargada antes de esta fecha no tiene `perfil`, y por eso NO se le atribuye a
   nadie hasta que se vuelva a cargar diciendo de quién es. */
async function guardarExperiencia(redis, contratos, perfil = null) {
  const vocabulario = construirVocabulario(contratos);
  const version = sello();
  const cargado = new Date().toISOString();
  const dueno = perfil && String(perfil).trim() ? String(perfil).trim().toLowerCase() : null;
  await escribirJSONComprimido(redis, CLAVES.configExperiencia, {
    contratos, _meta: { version, cargado, total: contratos.length, perfil: dueno },
  });
  await escribirJSONComprimido(redis, CLAVES.configExperienciaTerminos, { ...vocabulario, version, cargado, perfil: dueno });
  // el sello va AL FINAL, igual que en la carga de RUP: mientras no cambie,
  // nadie puede leer un estado a medias
  await escribirJSON(redis, CLAVES.configExperienciaVersion, { version, cargado, contratos: contratos.length, perfil: dueno });
  return { version, cargado, perfil: dueno, vocabulario };
}
...
async function leerTerminos(redis) {
  ...
  return {
    ...
    // de quién es esta experiencia; null = no consta (cargada antes del 28-ago-2026)
    perfil: v.perfil || null,
  };
}

/* ¿La experiencia de `dueno` describe lo que `perfilId` sabe hacer?
   · el MISMO perfil, sí;
   · el consorcio, sí para la de CUALQUIERA de sus integrantes: el plural se
     presenta con la experiencia de sus miembros (por eso `expSMMLV` del plural
     es la suma de las de Helder y Génesis en lib/perfiles);
   · sin dueño escrito (`null`), NO: no consta que sea de nadie, y atribuirla es
     exactamente el defecto que esta función existe para cerrar.
   Los integrantes se leen del perfil VIGENTE, no de una lista de ids escrita
   aquí: un consorcio cargado a mano con otros miembros seguiría siendo correcto,
   y dos listas de «quiénes van juntos» divergen a la primera carga. */
function experienciaAplica(dueno, perfilId, perfilVivo) {
  const d = dueno ? String(dueno).toLowerCase() : null;
  const p = perfilId ? String(perfilId).toLowerCase() : null;
  if (!d || !p) return false;
  if (d === p) return true;
  const integrantes = (perfilVivo && Array.isArray(perfilVivo.integrantes)) ? perfilVivo.integrantes : [];
  return integrantes.some((i) => i && i.perfil && String(i.perfil.id || "").toLowerCase() === d);
}
...
module.exports = {
  ...
  guardarExperiencia, leerExperiencia, leerTerminos, selloExperiencia, experienciaAplica,
};
