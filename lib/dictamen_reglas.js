...
const REGLAS_VERSION = "reglas-2026-09-03.3"; // .2: la cláusula gana al índice; .3: mención (índice, título, fórmula, plantilla) ≠ «hay anticipo»
const MOTOR = "reglas";
...
const DETECTORES = [
  ...
  /* `excluye`: la frase de PLANTILLA del Documento Base de Colombia Compra («Dentro de estas
     condiciones se incluye la forma de pago, anticipo y/o pago anticipado, obligaciones…»)
     está en todos los pliegos y no afirma nada: citarla era decir «hay anticipo» siempre */
  { tipo: "anticipo_o_pago_anticipado", re: /\b(?:anticipo|pago anticipado)\b/, excluye: /se incluyen? la forma de pago, anticipo/,
...
/* el futuro y «lugar a» entraron el 3-sep-2026: «No se entregará anticipo» (pliego real)
   ...
   seguido opcionalmente de «ningún», «el pago de» o «pago de» y «anticipo» o «pago anticipado».
   El INFINITIVO («la entidad decide no entregar anticipo», pliego real de Cali, 3-sep-2026)
   entra con la `r`: sin él, la cláusula que niega salía como «hay anticipo». Y entre el verbo y
   «anticipo» caben hasta cinco palabras («no entregará al contratista a título de anticipo»,
   pliego real de Siete de Agosto, 3-sep-2026). */
const SIN_ANTICIPO_RE = /\bno (?:se )?(?:ha previsto|contempla|preve|habra|hay|otorga|aplica|pacta|paga|entrega|reconoce|da)(?:r|ra|ran|n)?(?: \S+){0,5}? (?:anticipo|pago anticipado)\b|\bsin anticipo\b|\banticipo[:\s]+(?:0\b|cero|no aplica)/;
...
/* LA CLÁUSULA GANA AL ÍNDICE (3-sep-2026). El pliego real de Cali citaba «8.3. ANTICIPO
   Y/O PAGO ANTICIPADO 78» (la línea del índice, pág. 6) y decía «hay anticipo», cuando la
   cláusula de la pág. 78 dice «la entidad decide no entregar anticipo». La primera línea
   que casa NO es la mejor: cada línea recibe un peso —0 índice (numerada o con puntos de
   guía y termina en número de página), 1 título o fórmula (sin minúsculas o con menos de
   seis palabras), 2 frase— y por tipo se quedan las de más peso, en su orden. El índice
   sigue valiendo como último recurso: si el pliego solo lo menciona ahí, se cita ahí. */
const MAX_CANDIDATAS_POR_TIPO = 12;
const INDICE_RE = /^(?:\d+(?:\.\d+)*\.?\s+.*|.*(?:\.{3,}|…).*)\s\d{1,3}\s*$/;
const FORMULA_RE = /[=−]|\bx\s*\d+\s*%|\([A-Z]{2,5}\b/; // «POE − Anticipo…», «CTd = (POE - Anticipo) x 33%»
function pesoDeLinea(crudo) {
  const s = String(crudo || "").replace(/\s+/g, " ").trim();
  if (INDICE_RE.test(s)) return 0;
  if (!/[a-záéíóúñ]/.test(s) || s.split(" ").length < 6 || FORMULA_RE.test(s)) return 1;
  return 2;
}
function detectar(texto) {
  const lineas = lineasConPagina(String(texto || ""));
  const candidatas = new Map(); // tipo → [{linea, pagina, peso, orden}]
  let orden = 0;
  for (const { linea, pagina } of lineas) {
    ...
    for (const d of DETECTORES) {
      ...
      const lista = candidatas.get(d.tipo) || [];
      if (lista.length >= MAX_CANDIDATAS_POR_TIPO) continue;
      const cita = citaDe(linea);
      if (!cita) continue;
      /* la NEGACIÓN del anticipo pesa más que una mención: «se incluye la forma de pago,
         anticipo…» va antes en el pliego que «decide no entregar anticipo», y la que
         decide es la segunda */
      const preferida = d.tipo === "anticipo_o_pago_anticipado" && SIN_ANTICIPO_RE.test(p) ? 1 : 0;
      lista.push({ linea: cita, pagina: Number.isInteger(pagina) ? pagina : null, peso: pesoDeLinea(linea) + preferida, orden: orden++ });
      candidatas.set(d.tipo, lista);
    }
  }
  const hallazgos = new Map(); // tipo → [{linea, pagina}]
  for (const [tipo, lista] of candidatas) {
    /* `peso` viaja con el hallazgo: quien lo use sabe si citó una frase (2) o solo un
       título, una fórmula o el índice (≤ 1) — una MENCIÓN no es una afirmación */
    hallazgos.set(tipo, lista.sort((a, b) => b.peso - a.peso || a.orden - b.orden).slice(0, MAX_POR_TIPO).map(({ linea, pagina, peso }) => ({ linea, pagina, peso })));
  }
  return hallazgos;
}
...
function generarDictamenPorReglas({ entrada, texto }) {
  ...
  if (!ant) {
    ...
    /* solo el índice, un título o una fórmula (la de capacidad residual resta el anticipo
       aunque no lo haya): no se afirma; se pregunta */
    preguntas.push("¿Hay anticipo? El pliego solo lo menciona en un título, en el índice o en una fórmula: lea la cláusula de anticipo antes de fijar el precio.");
  } else {
...
module.exports = { generarDictamenPorReglas, detectar, pesoDeLinea, DETECTORES, REGLAS_VERSION, MOTOR, DATO_COMPARADO, SIN_ANTICIPO_RE };
