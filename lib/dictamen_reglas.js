...
const REGLAS_VERSION = "reglas-2026-09-03.2"; // .2: la cláusula gana al índice; negación en infinitivo
const MOTOR = "reglas";
...
/* el futuro y «lugar a» entraron el 3-sep-2026: «No se entregará anticipo» (pliego real)
   ...
   entra con la `r`: sin él, la cláusula que niega salía como «hay anticipo». */
const SIN_ANTICIPO_RE = /\bno (?:se )?(?:ha previsto|contempla|preve|habra|hay|otorga|aplica|pacta|paga|entrega|reconoce|da)(?:r|ra|ran|n)?(?: lugar a)? (?:ningun |el pago de |pago de )?(?:anticipo|pago anticipado)\b|\bsin anticipo\b|\banticipo[:\s]+(?:0\b|cero|no aplica)/;
...
function detectar(texto) {
  ...
  for (const [tipo, lista] of candidatas) {
    hallazgos.set(tipo, lista.sort((a, b) => b.peso - a.peso || a.orden - b.orden).slice(0, MAX_POR_TIPO).map(({ linea, pagina }) => ({ linea, pagina })));
  }
  return hallazgos;
}
