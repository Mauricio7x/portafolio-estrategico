/* ============================================================================
   ...
   exige servicio externo o proceso local» (docs/APU_INFORME_COMPLETO.md §1.G.3).
   ...
   `public/pliego.js` rasteriza cada página con pdf.js a JPEG y las manda de a poco:
...
const { marcador } = require("./paginas.js");
...
/* NINGÚN TEXTO REMOTO SE REENVÍA SIN TACHAR LA CLAVE. El cuerpo de error de
   OCR.space se le muestra al usuario porque es el único diagnóstico útil cuando
   algo falla, pero ese cuerpo lo escribe un tercero a partir de una petición que
   LLEVA la `OCRSPACE_API_KEY`, y hay servicios que la repiten en el mensaje
   («Bad request for apikey=…»). Reenviarlo tal cual publicaba el secreto en la
   respuesta del endpoint. */
/* CENSO de los secretos del entorno que un texto de terceros podría repetir:
   la misma función sirve al error de la sincronización que op=salud publica
   SIN token (6-sep-2026, M-INF-04), así que tacha todos, no solo la del OCR.
   La suite exige que TODO nombre *_TOKEN | *_KEY | *_SECRET que lib/ o api/
   lean del entorno esté aquí (remate V-B3a-02: CRON_SECRET se leía en
   lib/auth desde esa misma mañana y no estaba). */
const SECRETOS_DEL_ENTORNO = [
  "OCRSPACE_API_KEY", "SOCRATA_APP_TOKEN", "UPSTASH_REDIS_REST_TOKEN", "KV_REST_API_TOKEN",
  "HISTORICO_TOKEN", "ANTHROPIC_API_KEY", "VERCEL_AUTOMATION_BYPASS_SECRET", "CRON_SECRET",
  "CORREO_API_KEY", "RUTINA_PRECIOS_TOKEN", "RUTINA_DICTAMEN_TOKEN",
];
function tacharClave(texto) {
  let salida = String(texto == null ? "" : texto);
  for (const nombre of SECRETOS_DEL_ENTORNO) {
    const clave = process.env[nombre] || "";
    if (clave && clave.length >= 4) salida = salida.split(clave).join("«clave tachada»");
  }
  // y por si llega con otro formato: cualquier «apikey=…» o «apikey: …»
  return salida.replace(/apikey\s*[:=]\s*\S+/gi, "apikey=«clave tachada»");
}
...
/**
 * OCR de UNA página.
 * @param {object} pagina { base64, mime } — una URL remota NO se acepta (ver abajo)
...
async function ocrPagina(pagina, opciones = {}) {
  ...
  /* AQUÍ NO HAY RAMA `url`, y quitarla fue un arreglo, no una simplificación.
     Aceptar `{url}` y pasarlo a OCR.space convertía el endpoint en un SSRF POR
     DELEGACIÓN —el tercero descargaba lo que se le dijera, incluidos los
     metadatos de la nube— y además esa rama se saltaba el control de tamaño, que
     solo cubre el base64. El navegador manda IMÁGENES que él mismo rasterizó:
     nunca una URL. */
  if (pagina && pagina.base64) {
    ...
    return { ok: false, status: 400, error: "Se esperaba «base64» para la página (una URL remota no se acepta)." };
  }
  ...
  for (let intento = 0; intento <= REINTENTOS; intento++) {
    ...
    if (!r.ok) {
      ...
      return {
        ...
        error: r.status === 401 || r.status === 403
          ...
          : `OCR.space respondió ${r.status}: ${tacharClave(texto).slice(0, 200)}`,
      };
    }
    ...
    if (json.IsErroredOnProcessing || salida === 3 || salida === 4) {
      ...
      return {
        ok: false, status: 422,
        error: `OCR.space no pudo procesar la página${detalle ? `: ${tacharClave(detalle).slice(0, 300)}` : "."}`,
...
async function ocrPaginas(paginas, opciones = {}) {
  ...
  for (let i = 0; i < tope; i++) {
    ...
    /* Cada página leída va PRECEDIDA de su marcador `\f<i+1>` (índice dentro
       del LOTE: el servidor no sabe en qué página del PDF empieza la tanda ni
       cuáles descartó el cliente por no poder rasterizarlas; el navegador
       renumera con `renumerarMarcadores`, que recibe la LISTA de números reales
       de las páginas efectivamente enviadas). Así una página que falla no corre
       la numeración de las siguientes. */
    if (r.ok) textos.push(`${marcador(i + 1)}\n${r.texto}`);
...
module.exports = {
  ...
  hayClaveOcr, bytesDeBase64, conPrefijoData, tacharClave, SECRETOS_DEL_ENTORNO, ocrPagina, ocrPaginas,
};
