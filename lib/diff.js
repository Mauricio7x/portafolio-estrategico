...
function extraerHabilitantes(texto) {
  ...
  for (const req of REQUISITOS) {
    for (const linea of lineas) {
      ...
      if (!salida[req.id]) salida[req.id] = { id: req.id, etiqueta: req.etiqueta, valor, tipo: req.tipo, evidencia: linea.slice(0, 200) };
      break;
    }
  }
  return salida;
...
function compararHabilitantes(antes, despues, perfilId) {
  ...
  for (const req of REQUISITOS) {
    ...
    const propio = perfil && req.perfil != null ? Number(perfil[req.perfil]) : null;
    if (perfil && req.perfil && Number.isFinite(propio)) {
      const cumple = (v) => (v == null ? true : (req.sentido === "min" ? propio >= v : propio <= v));
...
async function registrarVersion(redis, { idProceso, texto, origen = "lector", perfilId = null, ahora = Date.now() }) {
  ...
  const guardable = recortado ? normalizado.slice(0, MAX_TEXTO) : normalizado;
  const hash = hashDe(guardable);
...
module.exports = {
  normalizarTexto, hashDe, parrafosDe, diffParrafos, extraerHabilitantes, compararHabilitantes,
