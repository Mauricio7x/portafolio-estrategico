/* ============================================================================
   ...
   Patrón de claves:
     ...
     indice:competencia:ganadores                  HASH entidad → «quién gana aquí» publicado (+ :nuevo, swap)
     indice:adjudicatario                          HASH clave de adjudicatario → perfil publicado (+ :nuevo, swap)
     ...
     lock:seguimiento:{perfil}                     candado CORTO de Mis procesos: leer → modificar → escribir (TTL 5 s, conCandado)
     lock:consorcios                               candado CORTO de config:consorcios, que es global (TTL 5 s, conCandado)
     ...
     dictamen:pedido:{proceso}:{perfil}            JSON de la lectura completa pedida desde la pantalla: marca y candado (SET NX, TTL 30 min)
   ...
   El tope que acota ese tamaño NO es Upstash —admite 10 MB por petición y
   100 MB por registro (consultoría del 4-sep-2026 sobre su documentación
   primaria; no releída desde aquí el 6-sep: proxy 403); el «1 MB por valor»
   que decía aquí era un límite que ya no existe— sino la RESPUESTA de una
   función de Vercel, que se corta en 4,5 MB: 500 KB por chunk deja margen para
   servir varios en una sola respuesta. La lectura tolera chunks duplicados o a
   medio compactar: se deduplica por `_k` quedándose con el `:updated_at` más
   reciente.
   ========================================================================== */
"use strict";
...
const CLAVES = {
  ...
  /* Qué DERIVADOS ya terminó esta cadena de invocaciones (7-sep-2026). Sin esta
     memoria, cada invocación reconstruía desde cero lo que la anterior había
     terminado: en una máquina lenta el índice y el de baja se pisaban sin fin y
     la cadena no convergía (GitHub Actions, «la extracción histórica no
     converge»). Se borra sola en cuanto una invocación BAJA datos nuevos: lo
     construido sobre el corpus viejo deja de valer en ese mismo instante. */
  derivadosHechos: "sync:historico:derivados",
  ...
  /* «Quién gana aquí» por entidad y el perfil del competidor por adjudicatario
     (23-sep-2026), construidos en la misma pasada del índice de competencia y
     publicados con el mismo cambio de clave atómico. Son hashes APARTE de
     `indice` porque ese lo lee ENTERO el listado de oportunidades: estos llevan
     NIT y valores y se leen de a UN campo (HGET) desde el modal. */
  indiceGanadores: "indice:competencia:ganadores",
  indiceGanadoresNuevo: "indice:competencia:ganadores:nuevo",
  indiceAdjudicatario: "indice:adjudicatario",
  indiceAdjudicatarioNuevo: "indice:adjudicatario:nuevo",
  ...
  /* ---------- perfiles DINÁMICOS (onboarding: RUP subido en PDF) ----------
     ...
     las whitelists de los perfiles fijos del dueño. Llevan TTL: no son
  ...
  /* el candado del DISPARO de la rutina (13-sep-2026): SET NX con la ventana de
     «repetida», para que tres «Buscar» a la vez no abran tres sesiones */
  apuIaDisparo: (perfil, id) => `apu:ia:disparo:${perfil}:${id}`,
...
/* EL LOTE DE MGET SE MIDE, NO SE SUPONE (14-sep-2026, el 504 del modal de
   competencia). Ocho claves por MGET era el lote de un chunk LLENO: se parten
   a ≤ 500 KB comprimidos (≈ 667 KB en base64 dentro del request REST), así que
   8 × 667 KB ≈ 5,3 MB cabían con margen bajo los 10 MB por petición de Upstash.
   Pero el delta es append-only y escribe chunks PEQUEÑOS todos los días: en un
   corpus fragmentado el chunk medio no pesa 667 KB sino decenas de KB, y esas
   ocho claves llevaban 200 KB donde cabían cinco megas. El precio lo paga el
   número de ROUND-TRIPS, que es lo único que importa contra una Redis por REST
   sin pipeline: medido sobre 150 000 procesos en chunks de 40 filas, 469 MGET
   y 26,8 s solo de red a 50 ms por comando — suficiente para que Vercel corte
   la función con un 504 antes de que el modal pinte nada.

   Ahora el tamaño del lote SALE de lo que se acaba de leer: el primer lote es
   el de siempre (8: sin medición todavía, se respeta el caso del chunk lleno) y
   cada lote siguiente se dimensiona con el chunk MÁS GRANDE visto hasta ahora,
   contra el mismo presupuesto de bytes de antes. Dos topes lo encierran: nunca
   menos de 8 claves (no empeora ningún caso) y nunca más de MAX_CLAVES_MGET
   (un comando con miles de claves es otro problema). No cambia QUÉ se lee, ni
   en qué orden, ni el resultado: solo cuántas veces se cruza la red. */
const PRESUPUESTO_MGET_BYTES = 5 * 1024 * 1024; // el mismo margen que daban 8 chunks llenos
const LOTE_MGET_MIN = 8;
/* EL TOPE DURO SE DERIVA DEL PEOR CASO, NO DE LO OBSERVADO. Medir el chunk más
   grande que ya se leyó dimensiona bien un corpus homogéneo, pero no promete
   nada sobre el siguiente lote: un histórico que empieza con los chunks
   minúsculos de un mes flojo y sigue con los llenos de un mes cargado haría que
   una medición de kilobytes autorizara un MGET de cientos de claves que pesan
   667 KB cada una. Así que el número de claves se acota con el tamaño MÁXIMO
   QUE UN CHUNK PUEDE TENER —que este módulo conoce, porque es él quien los
   parte— y no con el que resultó tener. Sale de la misma constante que usa el
   empaquetador: si algún día los chunks se parten más finos, el tope sube solo.
   El base64 crece un tercio sobre el binario comprimido. */
const TAM_MAX_CHUNK_B64 = Math.ceil(CHUNK_MAX_COMPRIMIDO * 4 / 3);
const TOPE_PETICION_BYTES = 9 * 1024 * 1024;   // Upstash admite 10 MB por petición; se deja margen
const MAX_CLAVES_MGET = Math.max(LOTE_MGET_MIN, Math.floor(TOPE_PETICION_BYTES / TAM_MAX_CHUNK_B64));

/* Se dimensiona con el chunk MÁS GRANDE visto, no con el promedio: el promedio
   es la media de lo que YA se leyó y no promete nada sobre lo que viene. Un
   corpus con los primeros chunks minúsculos y los siguientes llenos haría que
   la media (digamos 1 KB) autorizara doscientas claves que luego pesan 667 KB
   cada una — una petición de más de cien megas contra un servicio que admite
   diez. Con el máximo el cálculo es pesimista y no puede pasarse: si aparece un
   chunk lleno, el lote vuelve solo al de siempre. */
function loteSiguiente(bytesMax, clavesLeidas, lotePedido) {
  // sin medición (primer lote) o claves vacías: el lote de siempre
  if (!clavesLeidas || bytesMax <= 0) return lotePedido;
  const cabe = Math.floor(PRESUPUESTO_MGET_BYTES / bytesMax);
  return Math.max(lotePedido, Math.min(cabe, MAX_CLAVES_MGET));
}
...
/* Lee un conjunto de claves de chunk y deduplica por `_k` (gana el
   ...
   `hasta` (opcional, un instante de `Date.now()`) pone TECHO a la lectura: al
   alcanzarlo se deja de pedir y se avisa por `onIncompleto({leidas, totales})`.
   Sin él —el caso de todos los llamadores de siempre— se lee el corpus entero
   como hasta hoy. Existe porque el coste de este barrido lo fija el corpus, que
   solo crece: quien sirve una PETICIÓN DE USUARIO necesita poder prometer una
   respuesta, y la única forma de prometerla es saber cuándo parar. Lo que NO
   hace este tope es dejar que el llamador presente como completo lo que llegó a
   medias: por eso avisa en vez de callar, y por eso quien lo usa tiene que
   decidir explícitamente qué enseña con una lectura parcial. */
async function leerChunksDedup(redis, claves, { lote = LOTE_MGET_MIN, onCorrupto = null, senales = false, hasta = null, onIncompleto = null } = {}) {
  const mapa = new Map();
  ...
  let bytesMax = 0, clavesLeidas = 0;
  for (let i = 0; i < claves.length;) {
    /* El techo se mira ANTES de pedir, no después: cortar a la vuelta de un
       MGET ya habría gastado el viaje que se quería ahorrar. La primera tanda
       entra siempre —un presupuesto que devuelve cero filas no es una lectura
       parcial, es no haber leído— y así el llamador nunca recibe un «incompleto»
       sin nada dentro. */
    if (hasta != null && i > 0 && Date.now() >= hasta) {
      if (onIncompleto) onIncompleto({ leidas: i, totales: claves.length });
      break;
    }
    const tam = loteSiguiente(bytesMax, clavesLeidas, lote);
    const tanda = claves.slice(i, i + tam);
    const paquetes = await redis.mget(tanda);
    i += tanda.length;
    clavesLeidas += tanda.length;
    for (let j = 0; j < paquetes.length; j++) {
      const b = paquetes[j];
      if (b != null) bytesMax = Math.max(bytesMax, String(b).length);
      ...
      /* `tanda[j]`, NO `claves[i + j]`: `i` ya avanzó al final de la tanda, así
         que con el índice global el aviso de chunk corrupto nombraría una clave
         de la tanda SIGUIENTE — un diagnóstico creíble y falso. */
      if (filas == null && b != null && onCorrupto) onCorrupto(tanda[j]);
...
/* Un valor guardado que no parsea es «sin dato» (null), no un error: es la
   regla de leerJSON de siempre, compartida con leerVariosJSON para que las dos
   lecturas no diverjan. */
function parsearGuardado(v) {
  if (v == null) return null;
  try { return JSON.parse(v); } catch { return null; }
}
async function leerJSON(redis, k) {
  return parsearGuardado(await redis.get(k));
}
/* Varias claves JSON en UN comando (MGET): op=salud lee meta y la meta del
   histórico así para quedarse en dos comandos por petición del monitor. */
async function leerVariosJSON(redis, ks) {
  const vs = await redis.mget(ks);
  return ks.map((_, i) => parsearGuardado(Array.isArray(vs) ? vs[i] : null));
}
...
/* JSON comprimido para valores grandes (acumulador del índice, caché del
   detalle de competencia): mismo formato que un chunk, así queda muy por
   debajo de los 100 MB por registro de Upstash y cabe en la respuesta de
   4,5 MB de Vercel (ver la cabecera). `ttl` en segundos cuando el valor caduca. */
...
/* ═══ CANDADO CORTO: leer → modificar → escribir sin pisarse (6-sep-2026) ═══
   `seguimiento:{perfil}` y `config:consorcios` son UN JSON que se lee y se
   reescribe entero. Medido con dos POST en paralelo contra los handlers reales:
   Mis procesos respondía 200 y 200 y sobrevivía UN proceso; consorcios
   respondía 500 y 200 (el primero releía su registro ya pisado por el segundo)
   y sobrevivía uno. «Guardado» sin escritura es la peor respuesta posible.

   Es el patrón de la casa (lock:sync, lock:dictamen): SET NX EX con testigo,
   TTL SIEMPRE (una función muerta jamás deja el candado atascado) y liberación
   en `finally` solo si el testigo sigue siendo el propio. Lo que añade es el
   REINTENTO con espera breve —la sección crítica son dos comandos de Redis y la
   ventana es de milisegundos, así que esperar unas decenas de ms la gana casi
   siempre— y, si aun así no se obtiene, un error TIPADO (`ocupado`, `status`
   409, `que_hacer`) para que el handler responda que NO guardó y qué hacer.
   Los candados de sync, histórico, dictamen e índice de baja NO se rehacen
   aquí: cada uno tiene su propia política (reencadenamiento, TTL atado al
   reloj del modelo, un GET que lo inspecciona); esto es para la sección
   crítica corta de un JSON compartido. */
const CANDADO_CORTO_TTL_SEG = 5;
const CANDADO_OCUPADO_QUE_HACER = "Espere unos segundos y vuelva a intentarlo.";
function errorCandadoOcupado(clave, accion) {
  const e = new Error(`No se pudo ${accion} porque otra acción estaba en curso sobre los mismos datos.`);
  e.ocupado = true; e.status = 409; e.que_hacer = CANDADO_OCUPADO_QUE_HACER; e.clave = clave;
  return e;
}
const esCandadoOcupado = (e) => !!(e && e.ocupado === true && e.status === 409);
/* La respuesta JSON de un 409 por candado, UNA sola redacción para todos los handlers. */
const cuerpoCandadoOcupado = (e) => ({ ok: false, error: e.message, que_hacer: e.que_hacer });
async function conCandado(redis, clave, ttlSeg, fn, { reintentos = 4, esperaMs = 40, accion = "guardar" } = {}) {
  const testigo = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
  let tomado = false;
  for (let intento = 0; intento <= reintentos; intento++) {
    if ((await redis.set(clave, testigo, { nx: true, ex: ttlSeg })) === "OK") { tomado = true; break; }
    if (intento < reintentos) await new Promise((r) => setTimeout(r, esperaMs * (intento + 1)));
  }
  if (!tomado) throw errorCandadoOcupado(clave, accion);
  try { return await fn(); }
  finally {
    try { if ((await redis.get(clave)) === testigo) await redis.del(clave); } catch { /* el TTL limpia solo */ }
  }
}
...
module.exports = {
  ...
  leerJSON, leerVariosJSON, escribirJSON, escribirJSONComprimido, leerJSONComprimido,
  CANDADO_CORTO_TTL_SEG, conCandado, esCandadoOcupado, cuerpoCandadoOcupado,
};
