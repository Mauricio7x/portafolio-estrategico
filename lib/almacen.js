...
const CLAVES = {
  ...
  /* ---------- presupuestos del editor de APU (ago 2026) ----------
     Fuera de `licitaciones:*` como todo lo que no es corpus: ninguna purga los
     toca. Llevan TTL de 30 días —son borradores de trabajo, no un archivo
     histórico— y el listado se resuelve con SCAN + MGET sobre estas mismas
     claves, SIN un índice aparte: un índice con TTL se desincroniza en cuanto
     un presupuesto caduca y empezaría a listar borradores que ya no existen.
     La clave es la única fuente de verdad de qué presupuestos hay. */
  apuPresupuesto: (perfil, id) => `apu:presupuesto:${perfil}:${id}`,
  patronApuPerfil: (perfil) => `apu:presupuesto:${perfil}:*`,
  patronApu: "apu:presupuesto:*",
...
async function leerChunksDedup(redis, claves, { lote = 8, onCorrupto = null, senales = false } = {}) {
  ...
  for (let i = 0; i < claves.length; i += lote) {
    ...
    for (let j = 0; j < paquetes.length; j++) {
      ...
      for (const r of (filas || [])) {
        if (traza) {
          const t = traza.get(r._k) || { n: 0, cierres: new Set() };
          t.n++;
  ...
  if (traza) {
    for (const r of filas) {
      ...
      const vigente = r.fecha_cierre ? Date.parse(r.fecha_cierre) : NaN;
      r._cierre_prorrogado = Number.isFinite(vigente)
        && [...t.cierres].some((c) => { const v = Date.parse(c); return Number.isFinite(v) && v < vigente; });
    }
  }
  return filas;
}
...
module.exports = {
  PREFIJO, CLAVES, LOCK_TTL_SEG, LOCK_HISTORICO_TTL_SEG, RESUMEN_TTL_SEG, APU_TTL_SEG, CHUNK_MAX_COMPRIMIDO,
