/* ============================================================================
   ...
   dónde vive la caché:
     ...
     - AlmacenKV      → Vercel KV / Upstash Redis vía REST (producción).
                        Reusa el patrón de api/cron.js: POST del comando como
                        array JSON. Sin dependencias npm.
...
class AlmacenKV {
  /* Vercel KV / Upstash REST. Mismo protocolo que api/cron.js. */
  ...
  async get(k) { return this._cmd(["GET", k]); }
  async set(k, v) { return this._cmd(["SET", k, String(v)]); }
  async del(k) { return this._cmd(["DEL", k]); }
  async mget(ks) { return ks.length ? this._cmd(["MGET", ...ks]) : []; }
  async setNX(k, v, ttlSeg) {
    const r = await this._cmd(["SET", k, String(v), "NX", "EX", String(ttlSeg || 60)]);
    return r === "OK";
  }
}
...
/* Elige el almacén según el entorno: KV si hay credenciales; archivo local si
   se pide explícitamente; memoria como último recurso (con aviso). */
function crearAlmacen(opts = {}) {
  ...
  if (url && token) return new AlmacenKV(url, token, opts.fetchImpl);
