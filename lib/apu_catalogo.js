...
/* ⚠️ ESTE LÉXICO CONVIVE CON EL DE `data/apu_tipologias.json`, Y NO SON LA
   MISMA TABLA. Los 22 códigos coinciden, pero aquél separa `anclas` (peso 3) de
   `apoyo` (peso 1) para el clasificador del OBJETO del proceso, y éste es una
   lista PLANA para reconocer la tipología desde el TEXTO de un pliego. Medido:
   las 22 divergen —aquél tiene el «apoyo» que aquí no está, y aquí hay
   variantes propias («box coulvert», los singulares de VIA-MANT)—, así que
   fundirlas cambiaría el peso de todo el lector y no se hace a la ligera.
   Lo que SÍ tiene que estar en las dos es el NOMBRE canónico de la tipología:
   «pavimento flexible» faltaba en las dos a la vez y `tipologiasProbables`
   devolvía [] para «CONSTRUCCION DE PAVIMENTO FLEXIBLE», que es literalmente el
   objeto del caso 10 de tests/apu_bench.js. Hay prueba que lo ata. */
const TIPOLOGIAS = {
  ...
  "VIA-FLEX": { nombre: "Pavimento flexible", anclas: ["pavimento flexible", "pavimentacion", "asfalto", "asfaltico", "asfaltica", "mezcla densa", "mdc", "imprimacion"], familias: ["7214", "7215", "9512"], unidad_dominante: "m2" },
