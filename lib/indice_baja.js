...
function mensajeDe(granularidad, mediana, procesos, m, modalidad) {
  ...
  if (granularidad === "entidad_familia") {
    return `Descuento típico del ${pct}${enMod} en esta entidad para este tipo de obra (${procesos} procesos).`;
  }
  if (granularidad === "entidad") {
    return modalidad
      ? `Descuento típico del ${pct} en esta entidad para ${modalidad} (${procesos} procesos).`
      : `Descuento típico del ${pct} en esta entidad, todos los tipos de obra (${procesos} procesos).`;
  }
  const donde = (m && m.departamento) || "el departamento";
  return `Sin base propia de la entidad: descuento típico del ${pct}${enMod} en ${donde} para este tipo de obra (${procesos} procesos).`;
}
