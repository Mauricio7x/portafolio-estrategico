...
function medirDispersion(gruposEntidad, minProcesos = MIN_PROCESOS) {
  ...
  const meta = {
    ...
    /* Explica un `por_granularidad.entidad_familia` vacío: si esto iguala a
       `procesos_analizados`, la causa no es el mercado sino que el corpus
       histórico no trae el código UNSPSC (no se purga nunca, así que
       conserva lo que escribieron versiones anteriores de la proyección).
       Se arregla re-extrayendo el rango, no reconstruyendo el índice. */
...
function dondeSeMidio(granularidad) {
  ...
  if (g === "entidad" || g === "entidad_familia") return "esta entidad";
  if (g.startsWith("departamento")) return "su departamento, en obras así";
  return "esta zona";
}
...
function mensajeDepartamento(mediana, procesos, depto, conFamilia, modalidad) {
  const enMod = modalidad ? ` en ${modalidad}` : "";
  const base = `${procesos} ${procesos === 1 ? "contrato ya adjudicado" : "contratos ya adjudicados"}`;
  const alcance = conFamilia ? " en este tipo de obra" : " en todos los tipos de obra";
  if (mediana <= 0) {
