...
function medirDispersion(gruposEntidad, minProcesos = MIN_PROCESOS) {
  ...
  const meta = {
    ...
    /* Explica un `por_granularidad.entidad_familia` vacío: si esto iguala a
       ...
       analizados, entonces sí falta la columna y hay que re-extraer. */
...
/* ══════════ EL MISMO DATO, DICHO COMO INSTRUCCIÓN DE PRECIO ═════════════════
   ...
   OFERTAR. */
/* EL ALCANCE DE UN NIVEL SIN FAMILIA, dicho una sola vez (23-sep-2026). La
   entidad entera y el departamento entero cuentan TODO lo que se adjudicó ahí
   con un código que pasó la ingesta —obra, consultoría, interventoría, y
   también servicios como los de salud (8512 pasa el filtro de ingesta,
   medido)—, así que «todos los tipos de obra» afirmaba de más. Una constante
   para las tres frases que lo dicen (mensajeDe con y sin baja, y la del
   departamento): tres redacciones divergirían a la primera corrección. */
...
function mensajeDe(granularidad, mediana, procesos, m, modalidad) {
  ...
  if (mediana <= 0) {
    ...
    /* …y con la ENTIDAD ENTERA se dice su alcance, igual que en la rama de
       ...
       como si fueran obras como esta. */
    const alcance = granularidad === "entidad" ? ` en esta entidad, ${ALCANCE_TODO},` : "";
    return `Aquí se gana sin bajar el precio: los que ganaron${enMod}${alcance} ofertaron prácticamente por el `
      + `presupuesto oficial (${base}).`
      + (propia ? "" : ` Esta entidad no tiene historial propio suficiente: sale de lo adjudicado en ${dondeAjeno} para este tipo de obra.`);
  }
  ...
  if (granularidad === "entidad") {
    return modalidad
      ? `${instruccion}: es lo que descontaron los que ganaron en esta entidad en ${modalidad}, ${ALCANCE_TODO} (${base}).`
      : `${instruccion}: es lo que descontaron los que ganaron en esta entidad, ${ALCANCE_TODO} (${base}).`;
  }
  return `${instruccion}. Esta entidad no tiene historial propio suficiente, así que sale de lo que `
    + `descontaron los que ganaron${enMod} en ${dondeAjeno} para este tipo de obra (${base}).`;
}
...
/* DÓNDE SE MIDIÓ LA BAJA, en palabras de pantalla (22-sep-2026). Una sola
   ...
   (`bajaDeMercado`), pero la frase existe para quien la lea. */
const DONDE_SE_MIDIO = Object.freeze({
  entidad_familia: "esta entidad, en contratos como este",
  entidad: "esta entidad, en todo lo que contrata",
  departamento_familia: "su departamento, en contratos como este",
  departamento: "su departamento, en todo lo que se contrata",
});
...
function mensajeDepartamento(mediana, procesos, depto, conFamilia, modalidad) {
  const enMod = modalidad ? ` en ${modalidad}` : "";
  const base = `${procesos} ${procesos === 1 ? "contrato ya adjudicado" : "contratos ya adjudicados"}`;
  const alcance = conFamilia ? " en este tipo de obra" : ` ${ALCANCE_TODO}`;
  if (mediana <= 0) {
    return `En ${depto} se gana sin bajar el precio: los que ganaron${enMod} ofertaron prácticamente por el `
      + `presupuesto oficial (${base}${alcance}).`;
  }
  return `En ${depto} los que ganaron${enMod} descontaron cerca de ${mediana}% del presupuesto oficial `
    + `(${base}${alcance}).`;
}
...
function bajaDepartamentoDe(indice, lic, { modalidad } = {}) {
  ...
  if (hallazgo) {
    ...
    return {
      ...
      granularidad_utilizada: g,
      modalidad_utilizada: usoMod,
      mensaje: mensajeDepartamento(ok.mediana, ok.procesos, depto, g === "departamento_familia", etiqueta),
    };
  }
  return {
    departamento: depto,
