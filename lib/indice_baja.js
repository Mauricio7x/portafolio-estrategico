/* ============================================================================
   ...
   La cuarta, `departamento`, NO entra en esa cascada: `bajaDepartamentoDe` la
   lee (con `departamento_familia` delante) solo para ENSEÑAR cómo se adjudica
   en el departamento del proceso, junto a la cifra que decide (M-COMP-01).
...
const {
  ...
  // el percentil sobre histograma vive en el módulo de abajo desde el 6-sep-2026
  // (el plazo de adjudicación usa la misma forma); aquí se importa y se re-exporta
  percentilHistograma,
} = require("./indice_competencia.js");
const { extraerCodigos } = require("./unspsc.js");
const { norm } = require("./semantica.js");

/* El defecto de dispersión del motor de precio se IMPORTA de su dueño
   (`lib/apu/rentabilidad`), con `require` DIFERIDO: copiar el 4.0 aquí sería
   una segunda fuente de verdad de una cifra que decide probabilidades, y este
   módulo no puede depender de `apu/` en tiempo de carga. */
const DISPERSION_DEFECTO_MOTOR = (() => {
  try { return require("./apu/rentabilidad.js").DISPERSION_DEFECTO; } catch (_) { return null; }
})();
const NOTA_DISPERSION = "`iqr_cero` es «no se pudo medir la dispersión», no «dispersión cero»: el histograma "
  + "es de puntos porcentuales enteros y una celda concentrada lo colapsa. Ahí el motor de precio usa su "
  + "defecto declarado. Si `por_debajo_del_defecto` es una fracción pequeña de `medibles`, ese defecto es "
  + "más estrecho que el mercado observado y conviene revisarlo CON esta medición, no a ojo.";
...
/* EL CÓDIGO PRINCIPAL, LEÍDO CON LA REGLA QUE YA RETIRA EL PREFIJO (23-sep-2026).
   SECOP II publica `codigo_principal_de_categoria` con prefijo de versión
   («V1.72141200», «v1_72141200», «V1 72141200»: fila real pegada por el dueño
   el 22-sep-2026, `V1.72102900`). Aquí se leía con `normalizarCodigo`, que
   exige SOLO dígitos: con el prefijo devolvía null, ningún proceso tenía
   familia y las dos granularidades por familia quedaban vacías —el
   `sin_familia_legible = 46 013 de 46 013` medido en producción el 24-ago-2026
   que se atribuyó a un corpus sin código—. La cascada caía entonces SIEMPRE a
   la entidad entera, y la tarjeta decía «7 contratos» de todo lo que contrata
   la entidad como si fueran obras así. `extraerCodigos` (lib/unspsc) es la
   regla que ya retira ese prefijo para el matching del RUP: se LLAMA, no se
   escribe una segunda regex. Solo el campo principal (las categorías
   adicionales no dicen de qué es el contrato) y su primer código válido. */
function codigoPrincipalDe(lic) {
  const crudo = lic && lic.codigo_principal_de_categoria;
  if (crudo == null || String(crudo).trim() === "") return null;
  const { codigos } = extraerCodigos(crudo);
  return codigos.length ? codigos[0] : null;
}
/* EL SELLO DE ESA LECTURA en el progreso del índice (23-sep-2026): un progreso a
   medias escrito con OTRA lectura del código no se reanuda (ver `construir`).
   Cambia cuando cambie `codigoPrincipalDe`: si no, se mezclarían dos reglas. */
const REGLA_FAMILIA = "extraerCodigos";
...
function familiaDe(lic) {
  // `normalizarCodigo` (dentro de `extraerCodigos`) ya devuelve la familia
  // calculada con la jerarquía real (rellena a 8, descarta el segmento «00» y
  // lee el nivel por los pares finales). Recortar aquí los 4 primeros dígitos
  // a mano sería una segunda definición de «familia», y esa es exactamente la
  // clase de duplicación que este proyecto paga cara.
  const c = codigoPrincipalDe(lic);
...
/* LA BAJA DE UNA FILA, con los filtros de higiene del índice en el orden en
   que se cuentan. Es la ÚNICA regla de «cuánto descontó el que ganó»: la usa
   `acumular` (el índice) y la baja media del adjudicatario en
   lib/competencia_detalle (M-COMP-01, 6-sep-2026) — una segunda copia
   divergiría a la primera corrección de un umbral. Devuelve `{baja, descarte}`:
   `descarte` es la clave del contador de la meta (`sin_precio_base`,
   `sin_adjudicado`, `adjudicatario_no_definido`, `bajo_30_pct`,
   `sobre_110_pct`) o null cuando la fila aporta baja. */
function bajaDeFila(lic) {
  const pb = numero(lic.precio_base);
  if (pb == null || pb <= 0) return { baja: null, descarte: "sin_precio_base" };
  const va = numero(primero(lic, CAMPOS_VALOR_ADJUDICADO));
  if (va == null || va <= 0) return { baja: null, descarte: "sin_adjudicado" };
  if (!adjudicatarioReal(lic)) return { baja: null, descarte: "adjudicatario_no_definido" };
  if (va < pb * RATIO_MIN) return { baja: null, descarte: "bajo_30_pct" };
  const baja = (1 - va / pb) * 100;
  if (baja < BAJA_MIN) return { baja: null, descarte: "sobre_110_pct" };
  return { baja, descarte: null };
}

function acumular(acc, stats, lic) {
  stats.filas++;

  const fila = bajaDeFila(lic);
  if (fila.descarte) { stats[fila.descarte]++; return; }
  ...
  /* UN 0 EN `por_granularidad` TIENE DOS CAUSAS Y HAY QUE DISTINGUIRLAS. Los
     niveles por familia solo nacen `if (fam)`, así que «0 grupos» puede querer
     decir «ningún grupo alcanzó el mínimo» o «ningún proceso trae código UNSPSC
     legible» — que es un problema del CORPUS, no del mercado. Medido en
     producción (24-ago-2026): `entidad_familia` y `departamento_familia` con
     CERO grupos sobre 46 015 procesos analizados, mientras la suite los puebla
     con el mismo código; el corpus histórico no se purga nunca, así que sigue
     vivo lo que escribieron versiones anteriores de la proyección. Sin este
     contador, ese 0 es indistinguible de un mercado sin datos. */
  if (!fam) stats.sin_familia_legible++;
  if (!seg) stats.sin_segmento_legible++;
...
/* ---------- encogimiento de la MEDIANA hacia la referencia de su modalidad ----------
   (docs/PROBABILIDAD_MEJORADA.md §3.3, ago 2026.) Para el factor de PRECIO de
   P(ganar) la mediana de una celda de 5 procesos no vale lo mismo que la de una
   de 200: se encoge hacia el centro de SU modalidad (o el global) con
   w = n/(n+m_b), y m_b sale del propio índice por método de los momentos sobre
   los grupos por ENTIDAD con base (n ≥ MIN_PROCESOS), con la media y la varianza
   EXACTAS del histograma de cada grupo:
     τ̂²   = Var(media_i) − mean(s²_i / n_i)      entre entidades, menos el ruido
     σ̂²   = Σ(n_i−1)·s²_i / Σ(n_i−1)              dentro de entidad, pooled
     m_b  = σ̂² / τ̂²                                («procesos que vale el prior»)
   OJO: esto NO toca lo que la tarjeta enseña como «Suelen bajar N %» (la mediana
   medida de la celda) ni el panel Piso/Techo: solo lo que consume `factorPrecio`.
   Sin `encogimiento` en la meta (hash anterior) `encogerBaja` devuelve la baja
   tal cual, con `peso_datos: 1` — desplegar no exige reconstruir. */
const MIN_ENTIDADES_ESTIMACION_BAJA = 3;
function estimarEncogimientoBaja(gruposEntidad) {
  const conBase = [];
  for (const g of Object.values(gruposEntidad || {})) {
    if (!g || !g.hist || !(g.n >= MIN_PROCESOS)) continue;
    let s2 = 0; const media = g.suma / g.n;
    for (const [v, c] of Object.entries(g.hist)) s2 += c * (Number(v) - media) ** 2;
    conBase.push({ n: g.n, media, s2: g.n > 1 ? s2 / (g.n - 1) : 0 });
  }
  if (conBase.length < MIN_ENTIDADES_ESTIMACION_BAJA) {
    return { m: null, tau2: null, sigma2_dentro: null, entidades_estimacion: conBase.length, entidad_no_distingue: null,
      motivo: `hacen falta ${MIN_ENTIDADES_ESTIMACION_BAJA} entidades con ≥${MIN_PROCESOS} procesos` };
  }
  const mediaGlobal = conBase.reduce((a, e) => a + e.media, 0) / conBase.length;
  const varEntre = conBase.reduce((a, e) => a + (e.media - mediaGlobal) ** 2, 0) / (conBase.length - 1);
  const ruido = conBase.reduce((a, e) => a + e.s2 / e.n, 0) / conBase.length;
  const gl = conBase.reduce((a, e) => a + (e.n - 1), 0);
  const sigma2 = gl > 0 ? conBase.reduce((a, e) => a + (e.n - 1) * e.s2, 0) / gl : 0;
  const tau2 = varEntre - ruido;
  const noDistingue = !(tau2 > 0);
  const red2 = (x) => Math.round(x * 100) / 100;
  return {
    m: noDistingue ? null : red2(sigma2 / tau2),
    tau2: red2(tau2), sigma2_dentro: red2(sigma2),
    var_entre_entidades: red2(varEntre), ruido_muestral: red2(ruido),
    entidades_estimacion: conBase.length, entidad_no_distingue: noDistingue,
    metodo: `método de los momentos sobre los grupos por entidad con ≥${MIN_PROCESOS} procesos (media y varianza exactas del histograma)`,
  };
}

/* La baja de una celda ENCOGIDA hacia la referencia de su modalidad (o la
   global) para el factor de precio. Devuelve un registro con la misma forma que
   `bajaDeMercado` más `peso_datos`, `referencia` y `baja_mediana_celda`. */
function encogerBaja(baja, meta) {
  const b = baja || null;
  if (!b || !b.nivel || b.nivel === "sin_dato" || b.baja_mediana == null) return b;
  const enc = meta && meta.encogimiento ? meta.encogimiento : null;
  if (!enc || (enc.m == null && !enc.entidad_no_distingue)) return { ...b, peso_datos: 1, referencia: null, baja_mediana_celda: b.baja_mediana };
  const mod = b.modalidad_utilizada || null;
  const refMod = mod && meta.por_modalidad && meta.por_modalidad[mod] && meta.por_modalidad[mod].baja_mediana != null ? meta.por_modalidad[mod] : null;
  const ref = refMod
    ? { mediana: refMod.baja_mediana, p25: refMod.baja_p25, p75: refMod.baja_p75, nombre: `modalidad:${mod}` }
    : meta.baja_mediana_global != null
      ? { mediana: meta.baja_mediana_global, p25: meta.baja_p25_global, p75: meta.baja_p75_global, nombre: "global" }
      : null;
  if (!ref) return { ...b, peso_datos: 1, referencia: null, baja_mediana_celda: b.baja_mediana };
  const n = Math.max(0, Number(b.procesos_contados) || 0);
  const w = enc.entidad_no_distingue ? 0 : n / (n + enc.m);
  const mezcla = (x, y) => (x == null || y == null ? (x != null ? x : y) : w * x + (1 - w) * y);
  return {
    ...b,
    baja_mediana_celda: b.baja_mediana,
    baja_mediana: red1(mezcla(numero(b.baja_mediana), numero(ref.mediana))),
    baja_p25: red1(mezcla(numero(b.baja_p25), numero(ref.p25))),
    baja_p75: red1(mezcla(numero(b.baja_p75), numero(ref.p75))),
    peso_datos: Math.round(w * 1000) / 1000,
    referencia: ref.nombre,
    referencia_mediana: ref.mediana,
  };
}
...
async function construir(redis, { presupuestoMs, reiniciar, log, t0 }) {
  ...
  /* UN PROGRESO LEÍDO CON OTRA REGLA DE FAMILIA SE DESCARTA (23-sep-2026). El
     escrito antes de `codigoPrincipalDe` no contó ninguna familia en los meses
     que ya recorrió («V1.» ilegible): reanudarlo publicaba la baja «en contratos
     como este» medida solo con los meses posteriores al despliegue, con su n, y
     `sin_familia_legible` casi igual a los analizados, que es la señal con la que
     se diagnostica que falta la columna. Aquí no se puede normalizar como la
     modalidad (abajo): los meses acumulados no guardan de qué familia era cada
     proceso. Tirarlo cuesta a lo sumo una construcción a medias. */
  if (p && Array.isArray(p.pendientes) && p.regla_familia !== REGLA_FAMILIA) {
    log("índice de baja: el progreso guardado se leyó con otra regla del código UNSPSC: se empieza de cero");
    p = null;
  }
  if (!p || !Array.isArray(p.pendientes)) {
    p = {
      regla_familia: REGLA_FAMILIA,
      ...
      stats: {
        ...
        // por qué pueden estar vacías las granularidades por familia
        sin_familia_legible: 0, sin_segmento_legible: 0,
  ...
  /* Un progreso a medias escrito por la versión ANTERIOR no trae los campos de
     ...
     Desde el sello de arriba (23-sep-2026) ese progreso tampoco trae
     `regla_familia` y ya se descartó: esto queda como guarda, no como camino. */
...
/* ══════ ¿CUÁNTO SE DISPERSA LA BAJA? La pregunta que hoy contesta un supuesto ══════
   `lib/apu/rentabilidad.multiplicadorPrecio` deriva su σ del IQR de la celda
   (`(p75 − p25) / 1.349`), y cuando el IQR es CERO cae a `DISPERSION_DEFECTO`,
   una constante de 4 pp que no tiene fuente escrita. Ese σ decide cuánta
   probabilidad pierde una oferta que no puede bajar hasta el centro del
   mercado: con la mediana en 35 % y una baja máxima del 20 %, σ = 4 hunde el
   factor de precio hasta el suelo de 0,01 y el proceso se va al fondo del
   orden; con un σ del orden del observado (~25 pp) el mismo caso da ≈ 0,56.

   No se toca la constante: en 13 entidades de ejemplo de producción la σ
   medida iba de 2,2 a 43,7 pp con mediana 24,5 —seis veces el defecto—, pero
   eran los EXTREMOS del índice, no una muestra. Calibrar con eso sería la
   «suavización» que este proyecto prohíbe. Lo que se hace es MEDIRLO sobre el
   corpus entero y publicarlo, para que la próxima reconstrucción responda con
   datos: es lo mismo que hace `baja_exactamente_cero` con el cruce de columnas.

   `iqr_cero` NO es «dispersión cero»: es «no se pudo medir la dispersión»
   —el histograma es de puntos porcentuales enteros y una celda concentrada la
   colapsa—, y por eso el motor cae a su defecto en vez de usar σ = 0. Un σ de
   cero sería certeza absoluta justo donde menos información hay, que es la
   lección que `rivales_desv` ya aprendió en el índice de competencia. */
function medirDispersion(gruposEntidad, minProcesos = MIN_PROCESOS) {
  const iqrs = [];
  let celdas = 0, iqrCero = 0;
  /* Misma forma que `estimarEncogimientoBaja`: el acumulador por entidad es un
     objeto indexado, no un Map. Y se leen los percentiles del REGISTRO
     PUBLICADO, no del histograma crudo: bajo el mínimo el registro los anula a
     propósito, y contarlos aquí resucitaría una cifra derivada sin base. */
  for (const g of Object.values(gruposEntidad || {})) {
    const r = registroPublicado(g);
    if (!r || r.procesos == null || r.procesos < minProcesos) continue;
    if (r.baja_p25 == null || r.baja_p75 == null) continue;
    celdas++;
    const iqr = r.baja_p75 - r.baja_p25;
    if (!(iqr > 0)) { iqrCero++; continue; }
    iqrs.push(iqr);
  }
  if (!celdas) return { celdas: 0, iqr_cero: 0, medibles: 0, iqr_p25: null, iqr_mediana: null, iqr_p75: null,
    sigma_mediana: null, por_debajo_del_defecto: null, defecto_del_motor: DISPERSION_DEFECTO_MOTOR, nota: NOTA_DISPERSION };
  iqrs.sort((a, b) => a - b);
  const pct = (q) => (iqrs.length ? iqrs[Math.min(iqrs.length - 1, Math.floor(q * iqrs.length))] : null);
  const sigmaDe = (iqr) => Math.max(0.5, iqr / 1.349);
  const mediana = pct(0.5);
  return {
    celdas, iqr_cero: iqrCero, medibles: iqrs.length,
    iqr_p25: pct(0.25), iqr_mediana: mediana, iqr_p75: pct(0.75),
    sigma_mediana: mediana == null ? null : Math.round(sigmaDe(mediana) * 10) / 10,
    /* Cuántas celdas MEDIBLES tienen una dispersión por debajo del defecto que
       el motor usa cuando no la puede medir. Si son pocas, el defecto es más
       estrecho que el mercado y está afirmando una precisión que no existe. */
    por_debajo_del_defecto: iqrs.filter((i) => sigmaDe(i) < DISPERSION_DEFECTO_MOTOR).length,
    defecto_del_motor: DISPERSION_DEFECTO_MOTOR,
    nota: NOTA_DISPERSION,
  };
}
  ...
  const meta = {
    ...
    // m_b para encoger la mediana de una celda hacia su modalidad (factor de precio)
    encogimiento: estimarEncogimientoBaja(p.acc.entidad),
    // cuánto se dispersa la baja de verdad: hoy, cuando no se puede medir,
    // el motor de precio usa un supuesto sin fuente. Ver `medirDispersion`.
    dispersion: medirDispersion(p.acc.entidad),
    /* Explica un `por_granularidad.entidad_familia` vacío: si esto iguala a
       `procesos_analizados`, la causa no es el mercado sino que el código
       principal no se pudo leer. El 24-ago-2026 se atribuyó a un corpus sin
       código; el 23-sep-2026 el árbol lo desmintió: `familiaDe` rechazaba el
       prefijo «V1.» con el que SECOP II lo publica (ver `codigoPrincipalDe`).
       La proyección guarda la columna tal cual, así que una RECONSTRUCCIÓN
       del índice debería bastar; si tras ella sigue igualando a los
       analizados, entonces sí falta la columna y hay que re-extraer. Vale ya
       en la primera construcción: un progreso de la lectura anterior se
       descarta (`REGLA_FAMILIA`), no se mezcla. */
    sin_familia_legible: p.stats.sin_familia_legible,
    sin_segmento_legible: p.stats.sin_segmento_legible,
    descartados: {
...
const SIN_DATO = Object.freeze({
  ...
  /* el máximo visto es 0 (o no hay índice): lo que falta es la baja, dicho sin
     «históricos», que el dueño leyó como «no hay histórico» junto a una tarjeta
     que sí lo tenía (24-sep-2026) */
  mensaje: "Sin datos de baja de esta entidad.",
});

/* LA BAJA QUE NO SE PUDO LEER NO ES «SIN DATOS HISTÓRICOS» (23-sep-2026). Un
   fallo del HGETALL de `indice:baja:*` o de su meta se tragaba y cada fila
   decía «Sin datos históricos de baja para esta entidad» con 0 contratos, y la
   ganancia caía al presupuesto oficial afirmando «no hay historial suficiente»
   de una entidad con doce adjudicados. Es la regla de la competencia
   (lib/indice_competencia.SIN_LECTURA), con el MISMO centinela: quien no pudo
   leer pasa INDICE_NO_LEIDO y `bajaDeMercado` responde este registro. Conteo
   null —nadie contó nada, jamás 0— y el mensaje dice lo único cierto. */
const { INDICE_NO_LEIDO } = require("./indice_competencia.js");
const SIN_LECTURA_BAJA = Object.freeze({
  ...SIN_DATO,
  procesos_contados: null,
  motivo: "no_se_leyo",
  mensaje: "No se pudo consultar cuánto se suele bajar en esta entidad: falló la consulta, no es falta de datos. Vuelva a cargar la página en unos minutos.",
});
...
/* ══════════ EL MISMO DATO, DICHO COMO INSTRUCCIÓN DE PRECIO ═════════════════
   ...
   OFERTAR. */
/* LA TABLA ÚNICA DE ALCANCES (23-sep-2026): qué cuenta cada nivel de la
   cascada, dicho UNA vez, y la leen la nota de la tarjeta (`DONDE_SE_MIDIO`,
   publicada como `baja_donde`), el panel de Precios (`fraseBaja`, que llama a
   `dondeSeMidio`) y las frases de este módulo (`mensajeDe`,
   `mensajeDepartamento`). La entidad entera y el departamento entero cuentan
   TODO lo que se adjudicó ahí con un código que pasó la ingesta —obra,
   consultoría, interventoría, y también servicios como los de salud (8512 pasa
   el filtro de ingesta, medido)—, así que «todos los tipos de obra» afirmaba de
   más. Y un nivel por familia no es «este tipo de obra»: la familia de un
   contrato de salud o de consultoría tampoco es obra. Antes había cuatro
   redacciones del mismo hecho —«en todo lo que contrata» en la nota, «en todos
   los tipos de contrato» en el título del mismo botón, «para este tipo de obra»
   y «en este tipo de obra» en las frases— y ya decían cosas distintas. */
const ALCANCE_FAMILIA = "en contratos como este";
const ALCANCE_TODO = "en todos los tipos de contrato";
const ALCANCE_DE = Object.freeze({
  entidad_familia: ALCANCE_FAMILIA,
  entidad: ALCANCE_TODO,
  departamento_familia: ALCANCE_FAMILIA,
  departamento: ALCANCE_TODO,
});
/* «, en contratos como este» para pegar detrás del lugar; una granularidad
   desconocida es INERTE: sin alcance, nunca uno de los dos elegido a dedo. */
const conAlcance = (granularidad) =>
  (Object.prototype.hasOwnProperty.call(ALCANCE_DE, granularidad) ? `, ${ALCANCE_DE[granularidad]}` : "");
function mensajeDe(granularidad, mediana, procesos, m, modalidad) {
  ...
  /* El alcance va DETRÁS DEL LUGAR, nunca dentro del paréntesis de la base:
     «(7 contratos ya adjudicados en contratos como este)» repetía la palabra. */
  const alcance = conAlcance(granularidad);
  const propia = granularidad === "entidad" || granularidad === "entidad_familia";
  ...
  /* …y con base ajena a la entidad se dice de dónde sale, igual que abajo
     (22-sep-2026): esta rama se saltaba el reparto por granularidad y la
     tarjeta leía «aquí» como «esta entidad» sobre contratos de todo el
     departamento. */
  const dondeAjeno = (m && m.departamento) || "el departamento";
  if (mediana <= 0) {
    /* …y con la base de la entidad se dice su alcance, igual que en la rama de
       abajo (23-sep-2026): «Aquí se gana sin bajar el precio (7 contratos)»
       sobre todo lo que contrata la entidad —salud, consultoría, obra— se leía
       como si fueran obras como esta; con la familia, lo mismo en su medida. */
    const lugar = propia ? ` en esta entidad${alcance},` : "";
    return `Aquí se gana sin bajar el precio: los que ganaron${enMod}${lugar} ofertaron prácticamente por el `
      + `presupuesto oficial (${base}).`
      + (propia ? "" : ` Esta entidad no tiene historial propio suficiente: sale de lo adjudicado en ${dondeAjeno}${alcance}.`);
  }

  const instruccion = `Para tener opción hay que ofertar cerca de ${pct} por debajo del presupuesto oficial`;
  if (propia) {
    return `${instruccion}: es lo que descontaron los que ganaron${enMod} en esta entidad${alcance} (${base}).`;
  }
  return `${instruccion}. Esta entidad no tiene historial propio suficiente, así que sale de lo que `
    + `descontaron los que ganaron${enMod} en ${dondeAjeno}${alcance} (${base}).`;
}
...
function bajaDeMercado(indice, lic, { granularidad = "entidad_familia", modalidad } = {}) {
  if (indice === INDICE_NO_LEIDO) return SIN_LECTURA_BAJA;
  ...
  const { hallazgo, vistosSinBase: vistos, dondeVisto } = resolverCascada(indice, orden, mod);
  if (hallazgo) return respuesta(hallazgo.g, hallazgo.ok, hallazgo.usoMod, hallazgo.usoMod ? hallazgo.m : hallazgo.ok.m);
  vistosSinBase = vistos;

  /* EL CONTEO DICE DÓNDE SE CONTÓ (24-sep-2026): «hacen falta 5 procesos
     adjudicados y hay 1» salía sin lugar para una entidad sin un solo contrato,
     y el 1 era de su departamento. El lugar sale de la tabla única
     (`dondeSeMidio`), la misma de la nota de la tarjeta y del panel. */
  if (vistosSinBase > 0) {
    return {
      ...SIN_DATO,
      procesos_contados: vistosSinBase,
      mensaje: `Sin base suficiente: hacen falta ${MIN_PROCESOS} procesos adjudicados y hay ${vistosSinBase} en ${dondeSeMidio(dondeVisto)}.`,
    };
  }
  return SIN_DATO;
}

/* EL RECORRIDO DE UNA CASCADA, una sola vez: el primer nivel con base, con el
   refinamiento por modalidad dentro de cada nivel antes de la cifra mezclada.
   `utilizable` se reutiliza tal cual sobre la cubeta —tiene la misma forma que
   el registro— para que el umbral de «tiene base» sea uno solo y no dos que se
   puedan desincronizar. Lo comparten `bajaDeMercado` (la cascada que DECIDE el
   precio) y `bajaDepartamentoDe` (la lectura del departamento, que solo se
   ENSEÑA): dos bucles «equivalentes hoy» divergirían a la primera corrección. */
function resolverCascada(indice, orden, mod) {
  ...
  // …Y DÓNDE se vio (24-sep-2026): «hay 1» sin lugar se leía como de la
  // entidad, que no tenía ninguno; el 1 era de su departamento. En un empate
  // gana el nivel más específico (el primero de la cascada).
  let vistosSinBase = 0;
  let dondeVisto = null;
  for (const { g, clave } of orden) {
    ...
    if (mod && m && !m.ref && m.por_modalidad) {
      ...
      if (okMod) return { hallazgo: { g, ok: okMod, usoMod: mod, m }, vistosSinBase, dondeVisto };
    }
    const ok = utilizable(m);
    if (!ok) {
      const n = m ? Math.max(0, Math.trunc(numero(m.procesos ?? m.procesos_contados) || 0)) : 0;
      if (n > vistosSinBase) { vistosSinBase = n; dondeVisto = g; }
      continue;
    }
    return { hallazgo: { g, ok, usoMod: null, m: ok.m }, vistosSinBase, dondeVisto };
  }
  return { hallazgo: null, vistosSinBase, dondeVisto };
}

/* DÓNDE SE MIDIÓ LA BAJA, en palabras de pantalla (22-sep-2026). Una sola
   derivación para la tarjeta (`lib/ganancia` la publica como `baja_donde`) en
   vez de que cada pantalla decodifique la granularidad por su cuenta; las
   frases enteras del panel (`fraseBaja`) y del chip (`mensajeDe`) siguen
   siendo suyas. Sin granularidad (una respuesta de la versión anterior) se
   dice «esta zona», que no afirma de más.
   CUATRO LUGARES, NO DOS (23-sep-2026): «esta entidad» juntaba la entidad en
   contratos de la misma familia con la entidad EN TODO LO QUE CONTRATA, y los
   «7 contratos» de la tarjeta podían ser de salud o consultoría dichos como
   si fueran obras así. Cada granularidad dice su alcance; una desconocida es
   INERTE («esta zona»), nunca una de las cuatro elegida por prefijo. La
   granularidad «departamento» no entra en la cascada que decide
   (`bajaDeMercado`), pero la frase existe para quien la lea. El alcance sale
   de la tabla única (`ALCANCE_DE`), la misma de las frases. */
const DONDE_SE_MIDIO = Object.freeze({
  entidad_familia: `esta entidad${conAlcance("entidad_familia")}`,
  entidad: `esta entidad${conAlcance("entidad")}`,
  departamento_familia: `su departamento${conAlcance("departamento_familia")}`,
  departamento: `su departamento${conAlcance("departamento")}`,
});
function dondeSeMidio(granularidad) {
  const g = String(granularidad || "");
  return Object.prototype.hasOwnProperty.call(DONDE_SE_MIDIO, g) ? DONDE_SE_MIDIO[g] : "esta zona";
}

/* ============================ bajaDepartamentoDe ============================
   «CÓMO SE ADJUDICA EN SU DEPARTAMENTO» (M-COMP-01, 6-sep-2026): la lectura
   del hash `departamento` (y de `departamento_familia` cuando el proceso trae
   familia legible), que el índice construía desde ago 2026 y ningún consumidor
   leía. Se EXPONE, no se recalcula, y NO entra en la cascada de
   `bajaDeMercado`: el hash de departamento sigue siendo solo para consulta
   (decisión del 24-ago-2026), así que esta lectura acompaña a la cifra que
   decide el precio y no la sustituye. Misma regla de base (`utilizable`, el
   mismo mínimo), mismo refinamiento por modalidad, y «sin dato» con el conteo
   cuando no alcanza — nunca se rebaja el umbral. `null` sin departamento en
   el proceso (sin dato ≠ cero) o sin índice. */
function mensajeDepartamento(mediana, procesos, depto, granularidad, modalidad) {
  const enMod = modalidad ? ` en ${modalidad}` : "";
  const base = `${procesos} ${procesos === 1 ? "contrato ya adjudicado" : "contratos ya adjudicados"}`;
  // el alcance, de la tabla única y detrás del lugar: dentro del paréntesis
  // decía «(5 contratos ya adjudicados en contratos como este)»
  const alcance = conAlcance(granularidad);
  if (mediana <= 0) {
    return `En ${depto}${alcance}, se gana sin bajar el precio: los que ganaron${enMod} ofertaron prácticamente por el `
      + `presupuesto oficial (${base}).`;
  }
  return `En ${depto}${alcance}, los que ganaron${enMod} descontaron cerca de ${mediana}% del presupuesto oficial `
    + `(${base}).`;
}

function bajaDepartamentoDe(indice, lic, { modalidad } = {}) {
  ...
  const fam = familiaDe(lic);
  const orden = [
    { g: "departamento_familia", clave: fam ? `${depto}|${fam}` : null },
    { g: "departamento", clave: depto },
  ];
  const { hallazgo, vistosSinBase } = resolverCascada(indice, orden, mod);
  if (hallazgo) {
    const { g, ok, usoMod, m } = hallazgo;
    const etiqueta = usoMod ? ((ok.m && ok.m.etiqueta) || usoMod) : null;
    return {
      departamento: depto,
      nivel: ok.m.nivel,
      baja_mediana: ok.mediana,
      ...
      granularidad_utilizada: g,
      modalidad_utilizada: usoMod,
      mensaje: mensajeDepartamento(ok.mediana, ok.procesos, depto, g, etiqueta),
    };
  }
  return {
    departamento: depto,
    nivel: "sin_dato",
    baja_mediana: null, baja_promedio: null, baja_p25: null, baja_p75: null,
    procesos_contados: vistosSinBase,
    min_procesos: MIN_PROCESOS,
    granularidad_utilizada: null,
    modalidad_utilizada: null,
    mensaje: vistosSinBase > 0
      ? `Sin dato en ${depto}: hacen falta ${MIN_PROCESOS} contratos adjudicados con presupuesto y valor adjudicado y hay ${vistosSinBase}.`
      : `Sin dato en ${depto}: el histórico no tiene contratos adjudicados con presupuesto y valor adjudicado en este departamento.`,
  };
}
...
module.exports = {
  dondeSeMidio,
  estimarEncogimientoBaja, encogerBaja,
  construirIndiceBaja, leerIndiceBaja, leerIndiceBajaMeta, bajaDeMercado, bajaSegmentoDe,
  // el registro de la baja que no se pudo leer (quien falla pasa INDICE_NO_LEIDO)
  SIN_LECTURA_BAJA,
  // M-COMP-01: la lectura del departamento (se expone, no decide) y la regla
  // de la baja por fila + el registro publicable, para que el perfil del
  // adjudicatario las LLAME en vez de copiarlas
  bajaDepartamentoDe, bajaDeFila, subRegistro, resolverCascada,
  ...
  // la tabla única de alcances: las pruebas afirman contra ella, no contra una segunda copia del texto
  ALCANCE_FAMILIA, ALCANCE_TODO,
};
