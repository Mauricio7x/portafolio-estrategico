...
function bajaDeMercado(indice, lic, { granularidad = "entidad_familia", modalidad } = {}) {
  ...
  const { hallazgo, vistosSinBase: vistos } = resolverCascada(indice, orden, mod);
...
function resolverCascada(indice, orden, mod) {
  ...
  for (const { g, clave } of orden) {
    ...
    if (mod && m && !m.ref && m.por_modalidad) {
      ...
      if (okMod) return { hallazgo: { g, ok: okMod, usoMod: mod, m }, vistosSinBase };
    }
    ...
    return { hallazgo: { g, ok, usoMod: null, m: ok.m }, vistosSinBase };
  }
  return { hallazgo: null, vistosSinBase };
}
