...
const GRANULARIDADES = ["entidad_familia", "entidad", "departamento_familia"];
...
function sumar(grupo, clave, bajaPct, extra) {
  let g = grupo[clave];
  if (!g) { g = grupo[clave] = { n: 0, hist: {}, ...extra }; }
...
function acumular(acc, stats, lic) {
  ...
  const nombre = String(lic.entidad || "").trim() || "Entidad no informada";

  if (ent) {
    sumar(acc.entidad, ent, baja, { nombre });
    if (fam) sumar(acc.entidad_familia, `${ent}|${fam}`, baja, { nombre, familia: fam });
  }
  if (depto && fam) {
    sumar(acc.departamento_familia, `${depto}|${fam}`, baja, { departamento: depto, familia: fam });
  }
}
...
function registroPublicado(g) {
  ...
  if (g.n < MIN_PROCESOS) {
    return { ...comun, baja_mediana: null, baja_p25: null, baja_p75: null, nivel: "sin_dato" };
  }
...
async function construirIndiceBaja(redis, { presupuestoMs = 40000, reiniciar = false, log = () => {} } = {}) {
  ...
  if (!p || !Array.isArray(p.pendientes)) {
    p = {
      ...
      acc: { entidad: {}, entidad_familia: {}, departamento_familia: {} },
      stats: {
...
const SIN_DATO = Object.freeze({
  nivel: "sin_dato", baja_mediana: null, baja_p25: null, baja_p75: null,
...
module.exports = {
  construirIndiceBaja, leerIndiceBaja, leerIndiceBajaMeta, bajaDeMercado,
