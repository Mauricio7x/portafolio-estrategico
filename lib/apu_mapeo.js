/* ============================================================================
   ...
   TRES SEÑALES, y ninguna sola alcanza:

     1. SOLAPAMIENTO DE TÉRMINOS con los sinónimos del ítem (peso 0,60). Es la
     ...
     2. LEVENSHTEIN sobre la descripción canónica (peso 0,25). Existe por el OCR:
...
function tokenizarItem(texto) {
  const salida = [];
  for (const bruto of norm(texto).replace(/[²³]/g, (c) => (c === "²" ? "2" : "3")).split(/[^a-z0-9ñ]+/)) {
    if (!bruto) continue;
...
function mapearItem(fila, opciones = {}) {
  ...
  const firme = mejor.score >= UMBRAL_FIRME && margen >= 0.08;
...
module.exports = {
  ...
  puntuarCandidato, mapearItem, mapearTabla,
};
