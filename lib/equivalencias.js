...
/* El predicado, EXPORTADO (23-sep-2026): el NOMBRE que se enseña del ganador
   (lib/indice_competencia, `nombreAdjudicatarioDe`) aplica la MISMA regla que
   la identidad. Tenía la suya —ninguna— y un NIT válido con el nombre en
   «No Definido» salía en pantalla como «No Definido ganó 7 de 9». Una regla,
   un sitio: quien necesite saber si un nombre es relleno llama a esto. */
const esNombreRelleno = (v) => ADJUDICATARIO_RELLENO.has(norm(v));

function claveAdjudicatario(lic) {
  for (const c of CAMPOS_ADJUDICATARIO_NIT) {
    const v = String(lic[c] == null ? "" : lic[c]).replace(/\D/g, "");
    /* `campo` viaja con la clave: los cuatro candidatos NO son la misma cosa
       (nit real · documento · código interno de SECOP) y quien PUBLIQUE el
       identificador tiene que poder rotularlo por lo que es. En producción
       GPS S.A.S llega con nit «No Definido» y solo `codigoproveedor`: llamarlo
       «NIT» era una cifra con rótulo falso. La CLAVE no cambia de forma — es
       la identidad estable y está por todo el índice. */
    if (v.length >= 5) return { clave: `nit:${v}`, porNombre: false, campo: c };
  }
  for (const c of CAMPOS_ADJUDICATARIO) {
    const v = norm(lic[c]);
    if (v.length >= 4 && !esNombreRelleno(v)) return { clave: `n:${v}`, porNombre: true, campo: c };
  }
  return { clave: null, porNombre: false, campo: null };
}
...
module.exports = {
  ...
  claveAdjudicatario, esNombreRelleno, clasesDe, acumular, calcularPares,
