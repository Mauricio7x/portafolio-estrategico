...
/* Clave de entidad: el nombre normalizado manda (siempre viene) y el NIT entra
...
function claveEntidad(lic) {
  ...
  const nombre = norm(lic.entidad);
  return {
    clave: nombre || (nit ? `nit:${nit}` : ""),
...
async function construirIndice(redis, { presupuestoMs = 40000, reiniciar = false, log = () => {} } = {}) {
  ...
  /* Escribe el hash completo en `destino`, por lotes de CAMPOS_POR_HSET campos:
     ...
     nombre normalizado y, si tiene NIT, un alias que apunta al mismo registro
     (sin duplicar la carga útil). */
  async function publicarEn(destino) {
    let lote = {}, enLote = 0;
    for (const e of entidades) {
      lote[e.clave] = {
        nombre: e.nombre, nit: e.nit, procesos: e.procesos, oferentes_total: e.oferentes_total,
        promedio: e.promedio, mediana: e.mediana, nivel: e.nivel,
      };
      enLote++;
      if (e.nit && `nit:${e.nit}` !== e.clave) { lote[`nit:${e.nit}`] = { ref: e.clave }; enLote++; }
...
function competenciaDe(indice, lic) {
  ...
  const { clave, aliasNit } = claveEntidad(lic);
  let m = (aliasNit && Object.prototype.hasOwnProperty.call(indice, aliasNit) ? indice[aliasNit] : null)
    || (Object.prototype.hasOwnProperty.call(indice, clave) ? indice[clave] : null);
  if (m && m.ref) m = Object.prototype.hasOwnProperty.call(indice, m.ref) ? indice[m.ref] : null;
  if (!m || !m.procesos) return SIN_DATO;
  return {
    nivel: ["baja", "media", "alta"].includes(m.nivel) ? m.nivel : "sin_dato",
    promedio_oferentes: m.promedio ?? null,
    mediana_oferentes: m.mediana ?? null,
    total_procesos: m.procesos || 0,
  };
}
...
module.exports = {
  ...
  oferentesDe, esAdjudicado, claveEntidad,
  cortesTertiles, nivelPorCortes, medianaHistograma,
