...
async function guardarProgreso(redis, p, tope = TOPE_PROGRESO_B64) {
  ...
  if (b64.length > tope && p.quienGana) {
    ...
    delete p.quienGana;
    p.quienGana_motivo = `El resumen de quién gana no cupo en el progreso guardado (${mb} MB comprimidos, tope ${tope / 1048576} MB): `
...
async function construirIndice(redis, { presupuestoMs = 40000, reiniciar = false, log = () => {}, topeProgresoB64 = null } = {}) {
  ...
  if (!p || !Array.isArray(p.pendientes)) {
    p = {
      ...
      // con los meses que CONTÓ (sube junto a `stats.meses`: ver la guarda de abajo)
      quienGana: { ...nuevoQuienGana(), meses: 0 },
    };
  } else if (p.quienGana ? p.quienGana.meses !== p.stats.meses : !p.quienGana_motivo) {
    /* UN PROGRESO QUE OTRA VERSIÓN AVANZÓ no contó quién gana en los meses que
       recorrió ella: el despliegue anterior no trae el acumulador, y si lo
       avanza a mitad de una cadena (una reversión de Vercel, otro despliegue
       con el mismo Redis) lo reescribe INTACTO mientras sube `stats.meses`.
       Seguir desde aquí publicaría ganadores de medio corpus con la fecha de
       hoy —otro líder, otra base: una cifra creíble y falsa—, así que se suelta
       el acumulador, NO se publica y la meta dice cómo arreglarlo. Lo delata el
       contador: el acumulador lleva sus meses y tiene que coincidir con los que
       lleva el progreso (un progreso sin contador, de antes, tampoco se da por
       bueno). El índice de siempre sigue su curso sin cambios. */
    delete p.quienGana;
    p.quienGana_motivo = "Parte de los meses de esta construcción los recorrió otra versión de la aplicación y no consta que contara quién gana en ellos, "
      + "así que el resumen no cubriría todo el histórico y no se publica. Vuelva a armarlo desde cero con /api/sync/historico?reconstruir_indice=true&reiniciar=1";
  }
