...
let esNombreRellenoDiferido = null;
/* El primer nombre no vacío de CAMPOS_ADJUDICATARIO, tal como viene; y si ese
   nombre es un relleno del dataset («No Definido»), NINGUNO (23-sep-2026). Con
   un NIT válido y el nombre en relleno, «No Definido ganó 7 de 9» salía en la
   tabla, en la frase de alerta y en el perfil: la identidad descartaba el
   relleno y el nombre no. La regla es la de la identidad (`esNombreRelleno`,
   lib/equivalencias, con el mismo require DIFERIDO), llamada y no copiada.
   Y NO se pasa al campo siguiente: el último es `nombre_del_adjudicador`, el
   funcionario de la entidad que firmó la adjudicación —una persona natural que
   no se enseña—, y saltar el relleno habría puesto su nombre como ganador. Sin
   nombre, `identidadPublicada` cae en el nombre real más visto de ese NIT o en
   «NIT …». */
function nombreAdjudicatarioDe(lic) {
  for (const c of CAMPOS_ADJUDICATARIO) {
    const v = String(lic[c] == null ? "" : lic[c]).trim();
    if (!v) continue;
    if (!esNombreRellenoDiferido) esNombreRellenoDiferido = require("./equivalencias.js").esNombreRelleno;
    return esNombreRellenoDiferido(v) ? null : v;
  }
  return null;
}
...
async function publicarHashAtomico(redis, vigente, nuevo, pares) {
  ...
  try {
    ...
    // el motivo en lenguaje de pantalla (lo enseña el botón de Mi empresa); el error técnico, aparte
    return { ok: false, n: pares.length, motivo: "no se pudo cambiar el resumen anterior por el nuevo; sigue en pie el anterior", detalle: String((e && e.message) || e).slice(0, 80) };
  }
...
async function publicarQuienGana(redis, q, construido) {
  ...
  const detalle = [rE.detalle, rA.detalle].filter(Boolean).join(" · ") || null;
  return {
...
/* «QUIÉN GANA AQUÍ», EN UNA FRASE DE PANTALLA (24-sep-2026): si la construcción
   lo publicó y, si no, por qué, en llano. Una sola redacción para el botón de
   Mi empresa (que la enseña tal cual) y para `?estado=true`
   (lib/handlers/procesos/historico.js): los dos leen lo mismo, el `ganadores`
   de la respuesta o de la meta. Sin él —la construcción vigente la hizo una
   versión anterior a este resumen— no se sabe, y se dice así: jamás «no se
   publicó», que sería afirmar lo que no consta. */
function fraseQuienGana(g) {
  if (!g || typeof g !== "object") {
    return "No consta si «Quién gana aquí» está publicado: la competencia se calculó por última vez con una versión anterior de la aplicación, que no lo armaba.";
  }
  if (g.publicado === true) {
    const n = g.entidades == null || !Number.isFinite(Number(g.entidades)) ? null : Number(g.entidades);
    return `«Quién gana aquí» quedó publicado${n != null ? ` para ${n.toLocaleString("es-CO")} ${n === 1 ? "entidad" : "entidades"}` : ""}.`;
  }
  const m = String(g.motivo || "la construcción no dijo por qué").trim();
  return `«Quién gana aquí» no se publicó: ${m}${/[.!?]$/.test(m) ? "" : "."}`;
}
...
/* EL PROGRESO REANUDABLE, con un tope para lo que añade «quién gana»
   ...
   El tope llega como ARGUMENTO (23-sep-2026) para que la suite lo rebase con un
   corpus de juguete y lo vea funcionar: sin eso, desactivarlo dejaba la suite en
   verde. Solo lo cambia una opción de código de `construirIndice`, jamás la URL,
   y un valor que no sea un número positivo es inerte: vale el de siempre. */
const TOPE_PROGRESO_B64 = 8 * 1024 * 1024;
async function guardarProgreso(redis, p, tope = TOPE_PROGRESO_B64) {
  let b64 = comprimir(p);
  if (b64.length > tope && p.quienGana) {
    const mb = Math.round((b64.length / 1048576) * 10) / 10;
    delete p.quienGana;
    /* en lenguaje de pantalla: el botón de Mi empresa lo enseña tal cual (24-sep-2026) */
    p.quienGana_motivo = `El resumen de quién gana no cupo en el avance que se guarda entre tandas (${mb} MB comprimidos; el tope es ${tope / 1048576} MB): `
      + "la competencia de cada entidad se recalculó sin él.";
    b64 = comprimir(p);
  }
...
async function construirIndice(redis, { presupuestoMs = 40000, reiniciar = false, log = () => {}, topeProgresoB64 = null } = {}) {
  const t0 = Date.now();
  const tope = Number.isFinite(topeProgresoB64) && topeProgresoB64 > 0 ? topeProgresoB64 : TOPE_PROGRESO_B64;
  ...
  /* UN PROGRESO QUE NO PUEDE TERMINAR PUBLICANDO QUIÉN GANA SE DESCARTA Y SE
     EMPIEZA DE CERO, SOLO (24-sep-2026) — la idea del sello del índice de baja
     (lib/indice_baja, `REGLA_FAMILIA`), llamada aquí con el sello que este
     progreso ya lleva: el acumulador de quién gana cuenta sus meses y tienen que
     cuadrar con los del progreso. No cuadran cuando OTRA VERSIÓN avanzó la
     construcción: el despliegue anterior no trae el acumulador, y si lo avanza a
     mitad de una cadena (una reversión de Vercel, otro despliegue con el mismo
     Redis) lo reescribe INTACTO mientras sube `stats.meses`. Seguir desde ahí
     publicaría ganadores de medio corpus con la fecha de hoy —otro líder, otra
     base: una cifra creíble y falsa—.
     Hasta hoy se seguía SIN publicar y la meta mandaba reconstruir con
     `&reiniciar=1`: el botón de Mi empresa, que no sabe mandarlo, respondía
     «reconstruido» con «Quién gana aquí» sin publicar, y el dueño tenía que
     aprender un parámetro. Tirar el avance cuesta a lo sumo una construcción; lo
     otro costaba un resumen que no llegaba nunca.
     Lo que ESTA versión soltó a propósito porque no cabía (`guardarProgreso`,
     con su motivo escrito) no se descarta: volvería a no caber. */
  let descartado = null;
  if (p && Array.isArray(p.pendientes) && (p.quienGana ? p.quienGana.meses !== p.stats.meses : !p.quienGana_motivo)) {
    descartado = "lo avanzó otra versión de la aplicación sin contar quién gana en todos sus meses: se empezó de cero";
    log(`índice: el progreso guardado ${descartado}`);
    p = null;
  }
  if (!p || !Array.isArray(p.pendientes)) {
    p = {
      ...
      // con los meses que CONTÓ (sube junto a `stats.meses`: ver la guarda de arriba)
      quienGana: { ...nuevoQuienGana(), meses: 0 },
    };
  }
  // la invocación que tiró el progreso lo dice en su respuesta (quien pegó la URL la lee)
  const conDescarte = (r) => (descartado ? { ...r, progreso_descartado: descartado } : r);
  ...
  while (p.pendientes.length) {
    ...
    if (tanda.agotado()) {
      await guardarProgreso(redis, p, tope);
      return conDescarte({ done: false, pendientes: p.pendientes.length, entidades: Object.keys(p.acc).length });
    }
    ...
    p.stats.meses++;
    if (p.quienGana) p.quienGana.meses++;
    p.pendientes.shift();
  ...
  if (!p.quienGana) {
    ...
    try {
      ...
      ganadores = {
        publicado: false,
        motivo: "no se pudo guardar el resumen de quién gana; el anterior, si lo había, sigue en pie. Vuelva a recalcular más tarde.",
        detalle: String((e && e.message) || e).slice(0, 120),
      };
    }
  }
  ...
  return conDescarte({ done: true, ...meta });
}
...
/* UN ÍNDICE QUE NO SE PUDO LEER NO ES «ESTA ENTIDAD NO TIENE DATOS» (23-sep-2026).
   Los lectores tragaban el fallo del HGETALL con un `catch` vacío y seguían
   con el índice en null: todas las filas salían con SIN_DATO —y un
   `total_procesos: 0` que nadie contó— con HTTP 200 y la respuesta afirmando
   que el índice existía. Quien no pudo leer lo DICE pasando este centinela a
   `competenciaDe`, que responde SIN_LECTURA: nivel «sin_dato» (no hay nivel que
   enseñar), conteo null (no se contó nada) y `motivo: "no_se_leyo"`.
   Es `false` A PROPÓSITO, no un objeto: un centinela verdadero en un `if` se
   colaba en la memoria caliente de lib/handlers/procesos/listar.js (que sirve
   cualquier `indice` verdadero hasta que cambia la meta) y en
   lib/handlers/perfil/entrada.js (`if (indice)` lo tomaba por un índice
   legible). Siendo falso, todo lector que solo pregunta «¿hay índice?» sigue
   respondiendo «no» sin cambiar una línea. El motivo NO se llama «sin_indice»:
   ese literal ya es de lib/documentos_proceso.js y significa otra cosa. */
const INDICE_NO_LEIDO = false;
const SIN_LECTURA = Object.freeze({ nivel: "sin_dato", promedio_oferentes: null, mediana_oferentes: null, total_procesos: null, motivo: "no_se_leyo" });
...
/* `contratos_adjudicados` (24-sep-2026): cuántos contratos ADJUDICADOS tiene la entidad en el
   histórico, aunque ninguno traiga el número de ofertas. Es otra magnitud que `total_procesos`
   (procesos CON conteo de oferentes) y por eso otro nombre. El chip sin base invita a ver
   «quién gana aquí» solo si es > 0. Sale del lector ÚNICO de los hechos (`hechosDeRegistro`),
   no de una segunda cuenta; es un conteo, nunca adjudicatarios, NIT ni valores. */
function contratosAdjudicadosDe(m) {
  const d = hechosDeRegistro(m).desiertos;
  return d && d.adjudicados > 0 ? d.adjudicados : null;
}
const sinDatoCon = (procesos, extra = null, adjudicados = null) => (procesos > 0 || extra || adjudicados
  ? { nivel: "sin_dato", promedio_oferentes: null, mediana_oferentes: null, total_procesos: procesos, ...(extra || {}),
    ...(adjudicados ? { contratos_adjudicados: adjudicados } : {}) }
  : SIN_DATO);
...
/* El REGISTRO publicado de la entidad de una licitación, o null. Es la única
   ...
   EL ORDEN VIVE EN `clavesDeBusqueda` (24-sep-2026) porque lo usa también el
   detalle de la entidad (lib/competencia_detalle), que lee el hash campo a
   campo en vez de entero: con el nombre actual distinto del histórico y el
   mismo NIT, el chip encontraba la entidad por el alias y el modal —que solo
   buscaba por nombre— decía «No hay procesos». El detalle trae los campos en
   este orden y RESUELVE con esta misma función: si el chip la encuentra, el
   modal también. */
function clavesDeBusqueda(lic) {
  const { clave, claveLegado, aliasNit } = claveEntidad(lic);
  return [...new Set([clave, claveLegado, aliasNit].filter(Boolean))];
}
function registroDe(indice, lic) {
  ...
  let m = null;
  for (const k of clavesDeBusqueda(lic)) { m = en(k); if (m) break; }
...
function competenciaDe(indice, lic) {
  if (indice === INDICE_NO_LEIDO) return SIN_LECTURA;
  ...
  const adjudicados = contratosAdjudicadosDe(m);
  if (procesos < MIN_PROCESOS) return sinDatoCon(procesos, extra, adjudicados);
  if (!NIVELES_CLASIFICADOS.includes(m.nivel)) return sinDatoCon(procesos, extra, adjudicados);
  const promedio = numero(m.promedio);
  if (promedio == null) return sinDatoCon(procesos, extra, adjudicados); // nivel sin promedio: no hay nada que enseñar
  return {
    nivel: m.nivel,
...
module.exports = {
  ...
  construirIndice, leerIndice, leerIndiceMeta, competenciaDe, registroDe, clavesDeBusqueda,
  // el índice que no se pudo leer (ver SIN_LECTURA): lo pasan los lectores que fallan
  INDICE_NO_LEIDO, SIN_LECTURA,
  ...
  valorAdjudicadoDe, primeraFecha, TOP_GANADORES, tipoIdentificacion, nuevoAcumuladorGanadores, acumularGanador, ganadoresDe, fraseQuienGana,
