...
function medirPeriodos(entidades) {
  ...
  return {
    ...
    ventana_garantias_2026: {
      ...
      lectura: "cociente > 1: durante la ventana se presentaron MÁS oferentes que en el resto del período de la misma entidad; "
        + "el promedio de dos años que usa el estimador mezcla los dos. Es medición, no corrección: el estimador todavía no segmenta.",
    },
...
function encogerEntidad(e, enc) {
  ...
  /* Con τ̂² ≤ 0 todo se encoge a μ y la posterior es degenerada: la desviación
     sería 0, y una banda de ancho CERO se leería como certeza absoluta justo
     donde MENOS información individualizada hay. `null`: no hay banda. */
  if (enc.entidad_no_distingue) return { rivales_estimados: redondear2(mu), peso_datos: 0, rivales_desv: null };
  const m = enc.m;
...
module.exports = {
  ...
  medirColision, diaCierreDe, medirPeriodos, VENTANA_GARANTIAS_2026,
};
