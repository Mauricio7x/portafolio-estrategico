...
function cuentaParaCompetencia(lic) {
  if (esAdjudicado(lic)) return true;
...
function acumular(acc, stats, lic) {
  ...
  /* HECHOS DE CIERRE (M-DGF-08), ANTES del descarte por oferentes: cuánto tardó
     ...
     cuántos se presentaron. Adjudicado va primero (evidencia más dura); un
     cerrado sin ninguno de los dos desenlaces no entra en ninguna base. */
  const desenlace = esAdjudicado(lic) ? "adjudicado" : esDesierto(lic) ? "desierto" : null;
  if (desenlace) {
...
async function construirIndice(redis, { presupuestoMs = 40000, reiniciar = false, log = () => {} } = {}) {
  ...
  const plazoAdjudicacion = {
    ...
    nota: "plazo = días hábiles (lib/habiles) entre el día de cierre y el de adjudicación, solo con las DOS fechas; "
      ...
      + "desiertos: estado o fase con «desierto»; la base es adjudicados + desiertos (cancelados y revocados no entran).",
  };
  const meta = {
...
module.exports = {
  ...
  oferentesDe, esAdjudicado, esDesierto, cuentaParaCompetencia, claveEntidad, claveCanonica,
