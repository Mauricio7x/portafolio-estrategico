...
function repartoDe({ perfilBase, socio, fila, puertas }) {
  ...
  const crpc = presupuesto > 0 ? cargaK(fila, presupuesto).crpc_minimo : null;
  const f = fronteraReparto({ dueno: perfilBase, socio, presupuestoCOP: presupuesto, crpc, tipoContrato: fila.tipo_de_contrato || null });
  return {
    suya: f.suya_maxima,
...
function socioPorProceso({ fila, base = "helder", candidatos = [], ctx = {} } = {}) {
  ...
  for (const socioId of candidatos) {
    ...
    opciones.push({
      ...
      reparto: repartoDe({ perfilBase, socio, fila, puertas }),
...
module.exports = {
  ...
  carenciasDe, avisoMipyme, indicadoresQueEmpeoran, repartoDe, loQueNoSeSabe,
