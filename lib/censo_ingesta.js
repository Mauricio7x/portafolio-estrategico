...
/* Reparto del motivo que más volumen tira, `modalidad_no_competitiva`, POR EL
   LITERAL de la modalidad. Hace falta porque ese motivo mezcla dos cosas que no
   son la misma: la contratación directa —descarte correcto y la inmensa
   mayoría— y el «Régimen Especial» sin ofertas, que es donde publican las
   universidades estatales, las ESE y las empresas de servicios públicos. Con el
   agregado y cinco ejemplos, un falso negativo de mil procesos es
   indistinguible de cero.

   El censo NO decide nada nuevo: cuenta por el literal que ya venía
   descartando. Y la cota es dura porque esto viaja a Redis y se publica en el
   diagnóstico: lo que no cabe se CUENTA en «(otras)», jamás se tira en
   silencio — un literal perdido dejaría las cifras igual de creíbles sobre
   menos procesos. */
const MAX_MODALIDADES = 40;
const SIN_MODALIDAD = "(sin modalidad)";   // una clave vacía nunca es una clave
const OTRAS_MODALIDADES = "(otras)";
...
function crearCenso({ maxEjemplos = MAX_EJEMPLOS, maxModalidades = MAX_MODALIDADES } = {}) {
  ...
  /* El literal va RECORTADO pero SIN normalizar: la pregunta es con qué texto
     exacto publica la entidad, y normalizarlo perdería justo lo que decide
     («Régimen Especial» frente a «Régimen Especial (con ofertas)»). */
  function contarModalidad(fila) {
    const lit = recortar(fila && (fila.modalidad_de_contratacion || fila.tipo_de_proceso), 60) || SIN_MODALIDAD;
    if (porModalidad[lit] === undefined && Object.keys(porModalidad).length >= maxModalidades) {
      porModalidad[OTRAS_MODALIDADES] = (porModalidad[OTRAS_MODALIDADES] || 0) + 1;
      return;
    }
    porModalidad[lit] = (porModalidad[lit] || 0) + 1;
  }
  ...
  function registrar(motivo, fila) {
    ...
    cuenta[motivo]++;
    if (motivo === "modalidad_no_competitiva") contarModalidad(fila);
  ...
  return {
    ...
    resumen() {
      ...
      return {
        ...
        por_motivo: { ...cuenta },
        // INVARIANTE HERMANA: Σ por_modalidad = por_motivo.modalidad_no_competitiva
...
function fusionar(a, b) {
  /* Los atajos NORMALIZAN el campo nuevo: un consumidor que lea
     `por_modalidad` no puede recibir `undefined` según la rama por la que
     salga la fusión, y en producción sigue vivo el censo escrito antes de que
     el campo existiera. Es la regla que ya sigue `gananciaDeProceso` con su
     rama sin cifra. */
  const conReparto = (x) => (x && !x.por_modalidad ? { ...x, por_modalidad: {} } : x);
  if (!a) return b ? conReparto(b) : null;
  if (!b) return conReparto(a);
  ...
  /* El reparto por literal se suma igual. Un censo YA GUARDADO en producción no
     trae el campo: se trata como vacío, nunca como un cero inventado. */
  const por_modalidad = {};
  for (const fuente of [a.por_modalidad, b.por_modalidad]) {
    for (const [lit, n] of Object.entries(fuente || {})) por_modalidad[lit] = (por_modalidad[lit] || 0) + (n || 0);
  }
  ...
  return { leidas, aceptadas, descartadas, cuadra: leidas === aceptadas + descartadas, por_motivo, por_modalidad, ejemplos };
}

module.exports = { MOTIVOS, MAX_EJEMPLOS, MAX_MODALIDADES, SIN_MODALIDAD, OTRAS_MODALIDADES,
  crearCenso, fusionar, ficha, motivoNoAdmisible, motivoNoAbierto };
