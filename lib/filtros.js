/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   Los tres filtros que deciden si un proceso puede interesar SIQUIERA:
     ...
     3. objeto_valido(l, perfil)  — objeto dentro del RUP + capa anti-suministro
...
/* Capa ANTI-SUMINISTRO: los RUP incluyen clases de segmentos de BIENES
   (materiales de construcción, eléctricos, TI, mobiliario…) porque instalarlos
   es obra — pero esas mismas clases dejan pasar compras puras de materiales o
   muebles disfrazadas de proceso afín. Regla: si TODAS las clases declaradas
   son de segmentos de bienes Y el objeto se redacta como compra (verbo de
   adquisición) SIN ningún verbo de obra → descartar. Un solo código de un
   segmento de obra (72, 77, 81, 95…) ancla el proceso y desactiva la capa. */
const SEGMENTOS_SUMINISTRO = new Set(["30", "39", "43", "48", "56"]);
const VERBO_ADQUISICION_RE = /\b(?:suministro|adquisicion|compra|dotacion|entrega)\s+de\b/;
const VERBO_OBRA_RE = /\b(?:construccion|construir|rehabilitacion|mantenimiento|instalacion|montaje|obras?|adecuacion|mejoramiento|reparacion|ampliacion|remodelacion|demolicion|pavimentacion)\b/;
...
function evaluarObjeto(lic, perfil) {
  ...
  if (BLACKLIST_OBJETO.test(texto)) {
    return { ok: false, unspsc_ok: false, fuente_unspsc: null, anti_suministro: false, motivo: "objeto fuera de los RUP (blacklist semántica)" };
  }
  ...
  if (!unspsc_ok) {
    return { ok: false, unspsc_ok, fuente_unspsc, anti_suministro: false, motivo: fuente_unspsc === "codigo" ? "UNSPSC fuera del RUP" : "sin UNSPSC y el objeto no es obra civil" };
  }
  if (clases.length && clases.every((c) => SEGMENTOS_SUMINISTRO.has(c.slice(0, 2)))) {
    const tn = norm(texto);
    if (VERBO_ADQUISICION_RE.test(tn) && !VERBO_OBRA_RE.test(tn)) {
      return { ok: false, unspsc_ok, fuente_unspsc, anti_suministro: true, motivo: "suministro puro (segmentos de bienes + verbo de compra sin verbo de obra)" };
    }
  }
  // (La ruta por texto no necesita la capa: WHITELIST_OBRA ya exige
  // vocabulario de obra, incompatible con una compra pura.)
  return { ok: true, unspsc_ok: true, fuente_unspsc, anti_suministro: false, motivo: null };
}
...
module.exports = {
  ...
  SEGMENTOS_SUMINISTRO, unspscClasesDe, evaluarObjeto, objeto_valido,
};
