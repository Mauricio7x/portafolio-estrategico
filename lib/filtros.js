...
const { MANIFESTACION_INTERES_RE, FASE_ANTES_MANIFESTACION_RE } = require("./semantica.js");   // una sola definición: las usa también lib/manifestacion
...
/* El problema que resuelve (medido sobre el corpus real): la whitelist de los
   ...
   verificar) y NUNCA bloquea por falta de información:
     ...
     3-salud. ¿prestación de servicios
        de salud? (23-sep-2026)        → NO PERTINENTE (rojo); después del 1 y
...
function evaluarPertinencia(textoNorm, { tier, codigos, casados } = {}) {
  ...
  /* 3-salud (23-sep-2026). La PRESTACIÓN de servicios de salud no es obra. Va
     ...
     de salud. Las demás listas conservan su orden: no se tocan. */
  const salud = textoNorm.match(TERMINOS_PRESTACION_SALUD);
  if (salud) {
    return {
      ok: false, nivel: "rojo", tipo: "no_pertinente", etiqueta: "No pertinente",
      termino: salud[0],
...
function evaluarObjeto(lic, perfil, conocimiento = {}, opciones = {}) {
  ...
  const pertinencia = evaluarPertinencia(t, { tier: unspsc.tier, codigos, casados });
  if (!pertinencia.ok) {
