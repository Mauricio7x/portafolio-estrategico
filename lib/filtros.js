/* ============================================================================
   ...
   │ evaluarObjeto(l, perfil, conocimiento)  el JUICIO FINO, por perfil.      │
   │ Cascada: convenio → blacklist → UNSPSC jerárquico → equivalencias →      │
   │          texto → PERTINENCIA → anti-suministro.                          │
   └──────────────────────────────────────────────────────────────────────────┘
...
const {
  ...
  TERMINOS_NO_PERTINENTES, TIPO_CONSULTORIA, TIPO_INFRAESTRUCTURA,
...
const esSegmentoDeBienes = (clase) => String(clase).slice(0, 2) < "70";
...
function evaluarObjeto(lic, perfil, conocimiento = {}) {
  const texto = `${lic.nombre_del_procedimiento || ""} ${lic.descripci_n_del_procedimiento || ""}`;
  const t = norm(texto);
  ...
  if (!pertinencia.ok) {
    const r = conCodigos(salida(false, "no_pertinente", unspsc, pertinencia, false, pertinencia.motivo));
...
function objeto_valido(lic, perfil, conocimiento) { return evaluarObjeto(lic, perfil, conocimiento).ok; }
...
function clasesDelRup(setRup) { return indiceDe(setRup).clases; }
function unspscCubierto(clases, setRup) { return cubiertoPorPrefijo(clases, indiceDe(setRup)); }
...
module.exports = {
  ...
  esSegmentoDeBienes, anclaObra, esSuministroPuro, SEGMENTOS_SERVICIOS_NO_CONSTRUCTIVOS,
  VERBO_ADQUISICION_RE, VERBO_OBRA_RE,
  admisibleParaIngesta,
  evaluarObjeto, objeto_valido,
  unspscClasesDe, claseDe, clasesDelRup, unspscCubierto,
};
