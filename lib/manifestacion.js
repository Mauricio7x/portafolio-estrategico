...
/* La fecha del SORTEO que trae el cronograma del pliego (8-sep-2026). MISMA vía
   y misma poda que las dos de arriba: `guardarFechaCronograma` y
   `leerFechasCronograma` reciben la clave, así que no hay una segunda copia de
   la escritura ni de la lectura. Es la ÚNICA fecha de sorteo que se puede
   afirmar: la norma dice que la entidad PUEDE sortear, no cuándo. */
const CLAVE_CRONOGRAMA_SORTEO = "cronograma:sorteo";
...
/* Días de calendario entre dos DÍAS. `Date.UTC` sí es seguro —se construye y se
   lee siempre en UTC, así que no hay huso que corra la fecha—; lo prohibido es
   `new Date("2026-09-13")` y después `.getDate()` (la doctrina del calendario).
   null si alguna de las dos no es un día operable. */
function diasEntre(desde, hasta) {
  const a = fechaOperable(soloFecha(desde)), b = fechaOperable(soloFecha(hasta));
  if (!a || !b) return null;
  const ms = (f) => { const [y, m, d] = f.split("-").map(Number); return Date.UTC(y, m - 1, d); };
  return Math.round((ms(b) - ms(a)) / 86400000);
}

/* ══ EL SORTEO: QUÉ SE PUEDE AFIRMAR Y QUÉ NO (8-sep-2026) ══════════════════
   Encargo del dueño: «cuando un proceso esté en estado "Presentando oferta", la
   plataforma debe mostrar cuánto tiempo falta para la realización del sorteo (si
   aplica); si el sorteo no aplica para ese tipo de proceso, debe indicarse
   claramente».

   Lo que la norma dice (num. 2 del artículo que cita NORMA, arriba): en menor
   cuantía con manifestación de interés, si se presentan MÁS DE DIEZ interesados
   la entidad PUEDE sortear diez. Tres consecuencias que no hay que re-aprender:

   · EL SORTEO NO SIEMPRE OCURRE. Depende de cuántos manifiesten, y eso no se
     sabe hasta que se cierra el plazo de manifestación. Por eso hay tres
     respuestas y no dos: «sí» (el pliego publicó el día), «puede que sí» (la
     modalidad lo contempla y todavía no se sabe) y «no» (esta modalidad no
     limita oferentes por sorteo). Un booleano habría obligado a mentir en el
     caso de en medio, que es el más frecuente.
   · LA FECHA SOLO SALE DEL PLIEGO. La norma fija que el plazo de ofertas empieza
     el día hábil siguiente al informe del sorteo, no cuándo es el sorteo:
     calcularlo sería inventar el dato más caro de la pantalla. Sin cronograma
     leído, `fecha` es null y se dice por qué.
   · EL DÍA NO ES LA HORA. El cronograma publica el día; la hora de una audiencia
     no viaja en ninguna fuente. Cualquier cuenta atrás sobre esta fecha se dice
     en DÍAS, nunca en horas (la regla que ya rige la manifestación: el
     cronograma da el día, no la hora).

   `l` es la fila del corpus; `fechaCronograma` es lo leído del pliego para ESTE
   proceso (hash `cronograma:sorteo`). Devuelve siempre un objeto: la pantalla
   tiene que poder decir «aquí no hay sorteo» con las mismas palabras. */
function sorteoDe(l, hoy, { fechaCronograma = null, externo = false } = {}) {
  /* `fechaOperable` valida el AÑO con `Date.parse`, que acepta «2026-02-31» y lo
     corre al 3 de marzo: no sirve de filtro para una fecha que va a un calendario.
     La guarda buena ya existe y se llama (`diaValido` de lib/cronograma, exportada
     el 7-sep-2026 para las fechas del casillero): es el hermano vivo de aquella. */
  const fecha = require("./cronograma.js").diaValido(soloFecha(fechaCronograma));
  const contempla = exigeManifestacion(l || {});
  /* UN PROCESO QUE AGREGÓ USTED NO ES UN PROCESO DE SECOP II. Su modalidad la
     escribió el usuario y su cronograma no está publicado en ninguna parte: ni
     «sí», ni «puede», ni «no» son afirmables. Se dice que no se sabe, que es el
     único dato verdadero — la cuarta respuesta existe por la misma razón que
     existen las otras tres. */
  if (externo) {
    return { desde_manifestaciones: MAX_MANIFESTACIONES_SIN_SORTEO, norma: NORMA, fecha: null, origen: null, dias: null, paso: null,
      aplica: "no_se_sabe",
      frase: "Este proceso lo agregó usted, así que la aplicación no conoce su cronograma: si hay sorteo, la fecha está en el pliego que le dio la entidad. Apúntela en el cuaderno de este proceso y le avisará.",
      motivo: "proceso agregado a mano: no viene de SECOP II" };
  }
  const dia = soloFecha(hoy) || habiles.hoyColombia();
  const base = { desde_manifestaciones: MAX_MANIFESTACIONES_SIN_SORTEO, norma: NORMA, fecha: null, origen: null, dias: null, paso: null };
  const legible = (f) => habiles.fechaLegible(f);
  if (fecha) {
    const dias = diasEntre(dia, fecha);
    return { ...base, aplica: "si", fecha, origen: "pliego", dias, paso: dias != null && dias < 0,
      frase: dias == null ? `El pliego fija el sorteo para el ${legible(fecha)}.`
        : dias < 0 ? `El sorteo era el ${legible(fecha)}: mire en SECOP II si quedó dentro de los ${MAX_MANIFESTACIONES_SIN_SORTEO} que siguen en carrera.`
          : dias === 0 ? `El sorteo es HOY (${legible(fecha)}). El cronograma da el día, no la hora: entre a SECOP II.`
            : `${dias === 1 ? "Falta 1 día" : `Faltan ${dias} días`} para el sorteo (${legible(fecha)}). El cronograma del pliego da el día, no la hora.`,
      motivo: "fecha publicada en el cronograma del pliego" };
  }
  if (contempla) {
    return { ...base, aplica: "posible",
      frase: `Puede haber sorteo: en esta modalidad, si manifiestan interés más de ${MAX_MANIFESTACIONES_SIN_SORTEO}, la entidad sortea ${MAX_MANIFESTACIONES_SIN_SORTEO}. La fecha no está en ninguna fuente abierta; sale del cronograma del pliego cuando se lee.`,
      motivo: "la modalidad lo contempla y el cronograma del pliego todavía no se ha leído" };
  }
  return { ...base, aplica: "no",
    frase: "En esta modalidad no hay sorteo para limitar el número de oferentes: se presenta quien quiera y todos son evaluados.",
    motivo: "la modalidad no es de menor cuantía con manifestación de interés" };
}
...
module.exports = {
  ...
  guardarFechaCronograma, leerFechasCronograma, CLAVE_CRONOGRAMA, CLAVE_CRONOGRAMA_ADJUDICACION, CLAVE_CRONOGRAMA_SORTEO, MAX_FECHAS_CRONOGRAMA,
  sorteoDe,
