...
const { norm, MANIFESTACION_INTERES_RE, FASE_ANTES_MANIFESTACION_RE, FASE_TRAS_MANIFESTACION_RE } = require("./semantica.js");
...
/* ══ LA SEÑAL PUBLICADA: SECOP II SÍ DICE SI EL PLAZO ESTÁ CORRIENDO ══════════
   ...
   publicado gana a un calculado. Se usa en UNA dirección: para NO dar por
   pasado un plazo que SECOP II tenía recibiendo. La otra dirección —«Evaluación»
   = cerró— ya la aplica `lib/filtros.estado_abierto` (Evaluación cuenta como
   cerrado por prefijo) y esas filas no llegan al listado: son exactamente las
   que el dueño pidió no ver.
...
function senalSecop(l) {
  // el mismo recorte del prefijo «Proceso » que se aplica al estado (por si la fase lo trae)
  const fase = norm(String((l && l.fase) || "")).replace(/^proceso(?:\s+de)?\s+/, "");
  const faseLegible = String((l && l.fase) || "").trim() || null;
  const estadoLegible = String((l && l.estado_del_procedimiento) || "").trim() || null;
  const estado = norm(estadoLegible || "").replace(/^proceso(?:\s+de)?\s+/, "");
  ...
  /* «antes» SOLO si la columna autoritativa dice que esa fase está recibiendo
     (la precedencia del 20-ago: `estado_del_procedimiento` manda y `fase` solo
     habla cuando aquella calla). «Presentación de observaciones» en «Evaluación»
     (45 en la medición) ya pasó de largo: no se afirma «todavía no abre». */
  if (FASE_ANTES_MANIFESTACION_RE.test(fase)) {
    if (/^(?:publicado|abierto|convocado)\b/.test(estado)) return { recibiendo: null, fecha, posicion: "antes", fase: faseLegible, estado: estadoLegible };
    return { recibiendo: null, fecha, posicion: null, fase: faseLegible, estado: estadoLegible };
  }
...
function filaManifestacion(l, hoy, { fechaCronograma = null, ahora = null } = {}) {
  ...
  const notaFase = estado === "vencida" && origenVencimiento === "fase_secop"
    ...
    : estado === "por_abrir"
      ? `Según SECOP II, el ${secop.fecha ? legible(secop.fecha) : "último día visto"} este proceso estaba en la fase «${secop.fase || "anterior"}»: el plazo para avisar que le interesa todavía no ha abierto. Abre con el pliego definitivo y la ley le da como máximo ${PLAZO_MANIFESTACION_HABILES} días hábiles desde entonces, así que siga el cronograma del proceso en SECOP II.`
      : null;
      ...
      : !v.desde
        ...
        : estado === "pudo_vencer"
          ...
          /* «Nadie ha publicado» SOLO si de verdad nadie publicó nada: cuando se
             ...
             corta. */
          ? `Ábralo en SECOP II y mire el «Plazo para manifestación de Interés»: si dice tiempo transcurrido, cerró; si no, todavía puede avisar. ${fueraDeRango ? "No hay ninguna fecha límite que se pueda afirmar" : "Nadie ha publicado la fecha límite de este proceso"} y el máximo que da la ley (${PLAZO_MANIFESTACION_HABILES} días hábiles desde la apertura del ${legible(v.apertura)}) ya pasó, así que el plazo PUDO cerrarse y no consta que lo hiciera.`
...
module.exports = {
  exigeManifestacion, senalSecop, aplicarSenalSecop, porAbrir, yaRecibeOfertas, enSorteo, faseEnDuda, aperturaDe, ventanaDe, estadoDeVentana, filaManifestacion, manifestacionDeFila,
