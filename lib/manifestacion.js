...
function filaManifestacion(l, hoy, { fechaCronograma = null } = {}) {
  ...
  const fueraDeRango = !!(propuesta && v.apertura && (propuesta <= v.apertura || propuesta > techoLegal));
  ...
  const avisoDescarte = fueraDeRango
    ? ` En el pliego se leyó «${propuesta}» como fecha límite, pero no puede serlo: la ley la sitúa entre el día siguiente a la apertura (${v.apertura}) y el ${techoLegal}. Se descarta: no se usa como fecha límite.` : "";
...
/* Guarda la fecha límite leída del pliego. `hoy` inyectable para poder podar y
   ...
   una mejora de la lectura, no puede tumbar la del pliego. */
async function guardarFechaCronograma(redis, idProceso, fecha, { hoy = null } = {}) {
  const id = String(idProceso || "").trim();
  const f = soloFecha(fecha);
  if (!redis || !id || !f) return { guardada: false, podadas: 0 };
  const dia = /^\d{4}-\d{2}-\d{2}$/.test(String(hoy)) ? String(hoy) : habiles.hoyColombia();
  try {
    await redis.hset(CLAVE_CRONOGRAMA, { [id]: JSON.stringify({ fecha: f, escrito: dia }) });
    ...
    let podadas = 0;
    const n = Number(await redis.hlen(CLAVE_CRONOGRAMA)) || 0;
    if (n > MAX_FECHAS_CRONOGRAMA) {
      const todo = await redis.hgetall(CLAVE_CRONOGRAMA);
      ...
      if (podadas) { await redis.del(CLAVE_CRONOGRAMA); await redis.hset(CLAVE_CRONOGRAMA, vivos); }
    }
...
async function leerFechasCronograma(redis) {
  if (!redis) return {};
  try {
    const todo = await redis.hgetall(CLAVE_CRONOGRAMA);
    const out = {};
...
module.exports = {
  ...
  guardarFechaCronograma, leerFechasCronograma, CLAVE_CRONOGRAMA, MAX_FECHAS_CRONOGRAMA,
