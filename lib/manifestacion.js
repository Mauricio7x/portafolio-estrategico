/* lib/manifestacion.js · La MANIFESTACIÓN DE INTERÉS de la selección abreviada de
   ...
   valores, no dos (la lista completa, que el propio código enumera):
     ...
     · `vencida`       CONSTA que venció: hay fecha publicada y ya pasó.
...
const { norm, MANIFESTACION_INTERES_RE } = require("./semantica.js");
...
function senalSecop(l) {
  const fase = norm(String((l && l.fase) || ""));
  if (!MANIFESTACION_INTERES_RE.test(fase)) return { recibiendo: null, fecha: null };
  const estado = norm(String((l && l.estado_del_procedimiento) || "")).replace(/^proceso(?:\s+de)?\s+/, "");
  const t = Date.parse(String((l && l[":updated_at"]) || ""));
  const fecha = Number.isFinite(t) ? habiles.hoyColombia(t) : null;
  if (/^(?:publicado|abierto|convocado)\b/.test(estado)) return { recibiendo: true, fecha };
  if (/^(?:en\s+)?evaluacion\b/.test(estado)) return { recibiendo: false, fecha };
  return { recibiendo: null, fecha };
}
...
function filaManifestacion(l, hoy, { fechaCronograma = null, ahora = null } = {}) {
  ...
  const secopContradice = secop.recibiendo === true && (
    ventana.estado === "pudo_vencer" || ventana.estado === "sin_fecha"
    || (ventana.estado === "vencida" && confirmada && secop.fecha && secop.fecha > confirmada));
  const { estado, accion } = secopContradice ? { estado: "por_confirmar", accion: "verifique_ya" } : ventana;
...
function manifestacionDeFila(l, hoy, opciones) {
  ...
  return {
    aplica: true,
    apertura: f.apertura,
    estado: f.estado,                                 // abierta | por_confirmar | pudo_vencer | vencida | sin_fecha
...
module.exports = {
  exigeManifestacion, senalSecop, aperturaDe, ventanaDe, estadoDeVentana, filaManifestacion, manifestacionDeFila,
