...
function guiaDe({ fila = null, foto = null, perfil, ctx = {} } = {}) {
  ...
  if (manif && manif.aplica) {
    ...
    const detalleM = yaAvisoEtapa
      ...
      : m.estado === "vencida" && m.origen_vencimiento === "fase_secop"
        ...
        : porAbrir(m)
          ? `${m.nota} Avise el mismo día que abra: puede durar solo unas horas.`
          : m.estado === "vencida"
  ...
  if (manif && manif.aplica && etapa === "manifestado") {
    ...
    if (porAbrir(manif)) paso("Avise que le interesa en SECOP II el día que abra el plazo", null, `Según SECOP II el proceso todavía está en «${manif.secop_fase || "una fase anterior"}»: el plazo para avisar abre con el pliego definitivo y puede durar solo unas horas. Siga el cronograma. Sin este aviso no hay oferta.`);
