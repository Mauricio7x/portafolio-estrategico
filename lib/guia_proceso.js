...
function guiaDe({ fila = null, foto = null, perfil, ctx = {} } = {}) {
  ...
  /* LOS PASOS SE PIENSAN EN UN ORDEN Y OCURREN EN OTRO. La garantía de seriedad son cinco días
     HÁBILES antes del cierre y las observaciones siete CALENDARIO: basta un festivo en esa semana
     para que la garantía caiga ANTES que las observaciones y la lista muestre las fechas hacia
     atrás. Reproducido con el cierre del 13-oct-2026 (festivo el 12): observaciones 06-oct,
     garantía 05-oct. No se toca ninguna de las dos fechas —las dos son correctas, las fija la ley,
     no esta lista—: se ordena lo que el usuario LEE. El orden es estable (los empates conservan el
     orden en que se pensaron, que es el didáctico: presentar antes que verificar) y los pasos sin
     fecha —traslado y adjudicación— se quedan al final, que es donde se emiten. */
  ...
  const enOrden = conFecha.concat(sinFecha);
  enOrden.forEach((p, i) => { p.orden = i + 1; });
  pasos.length = 0;
  for (const p of enOrden) pasos.push(p);
