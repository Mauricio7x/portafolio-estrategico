...
function guiaDe({ fila = null, foto = null, perfil, ctx = {} } = {}) {
  ...
  /* EL PASO A PASO ES UNA LÍNEA DE TIEMPO, NO EL ORDEN EN QUE SE ESCRIBIÓ EL
     CÓDIGO (13-sep-2026). Cada paso calcula su fecha con una regla distinta
     —unos en días calendario, otros en días HÁBILES— y un festivo entre medias
     las cruza: con cierre el 13-oct-2026 y el 12 festivo, la garantía de
     seriedad (cinco hábiles antes) cae el 5 y las observaciones (siete
     calendario antes) el 6, pero se empujaban al revés. El contratista leía un
     paso a paso que RETROCEDE en el tiempo y podía pedir la póliza tarde.
     Se ordena el CONJUNTO por fecha, no el par que se reprodujo: cualquier otro
     par de reglas distintas se cruza igual en cuanto cambie el calendario. Los
     pasos SIN fecha (traslado, adjudicación) van al final, que es su sitio —
     ocurren después del cierre—, y el desempate conserva el orden de escritura,
     que es el que sabe que «presente la oferta» va antes que «verifique que
     diga Presentada» el mismo día. */
  pasos.sort((a, b) => {
    if (!a.cuando && !b.cuando) return a.orden - b.orden;
    if (!a.cuando) return 1;
    if (!b.cuando) return -1;
    return a.cuando < b.cuando ? -1 : a.cuando > b.cuando ? 1 : a.orden - b.orden;
  });
