...
/* Los requisitos que un consorcio puede cubrir (Ley 80 art. 7: los integrantes
   suman experiencia y capacidad; los indicadores se ponderan). El aviso de
...
function guiaDe({ fila = null, foto = null, perfil, ctx = {} } = {}) {
  ...
  if (puertas) {
    ...
    const detalleCaja = !p3.sin_dato
      ...
      : p3.sin_dato_de === "anticipo"
        ? `${hAnt && hAnt.anticipo === "no" ? `El pliego leído dice que no hay anticipo (${citaDe(hAnt)})` : "El proceso no publica si tiene anticipo"}: sin anticipo tendría que financiar cerca de ${millones(p3.financiacion_requerida)} antes del primer cobro. Confírmelo en la forma de pago del pliego antes de fijar el precio.`
  ...
  /* EL ORDEN DE UNA LISTA DE PASOS ES LA INSTRUCCIÓN (13-sep-2026). Los pasos se
     empujan en el orden en que se piensan, no en el que se hacen, y dos de ellos
     se calculan con relojes DISTINTOS: las observaciones a cierre − 7 días
     CALENDARIO y la garantía de seriedad a cierre − 5 días HÁBILES. Basta un
     festivo dentro de esa ventana —cerca de un tercio del año en Colombia— para
     que la garantía caiga ANTES que las observaciones y el paso a paso le diga
     al contratista que pida la póliza después de una fecha que ya pasó.
     Reproducido con la función real: cierre 2026-10-13 daba observaciones el
     6-oct y garantía el 5-oct, por el festivo del 12 de octubre.
     Se ORDENA por fecha, que es el único orden que el usuario puede seguir. Los
     pasos SIN fecha (traslado y adjudicación: dependen de que la entidad
     publique) se quedan al final, en su orden, porque ocurren después de todo lo
     fechado. `sort` es estable en Node, así que dos pasos del mismo día
     conservan el orden en que se pensaron —presentar antes que verificar—. */
  pasos.sort((a, b) => (a.cuando ? (b.cuando ? (a.cuando < b.cuando ? -1 : a.cuando > b.cuando ? 1 : 0) : -1) : (b.cuando ? 1 : 0)));
  ...
  const dinero = {
    ...
    contribucion_obra_5pct_cop: presupuesto && aplicaContribucion(tipoTrabajo) ? Math.round(presupuesto * CONTRIBUCION_OBRA_PCT / 100) : null,
...
function exigenciasDe({ hechoDe, perfilObj = null, lectura, hAnt = null, anticipo = 0, anticipoLegible = null, idProceso = null }) {
  ...
  for (const e of EXIGENCIAS) {
    ...
    x.accion = accionDeCasilla(x, h, tieneSuyo ? Number(propio) : null, idProceso, fmtValor);
    cierra(x);
  }
  return salida;
}
/* ── de la casilla en rojo al socio que la cubre (6-sep-2026, M-COMP-02) ──
   ...
   indicadores se ponderan por participación, lib/consorcio). `diferencia` es la
...
function accionDeCasilla(x, h, propio, idProceso, fmtValor) {
  ...
  const diferencia = sentido === "max" ? propio - Number(h.valor) : Number(h.valor) - propio;
  ...
  return {
    ...
    frase: sentido === "max"
      ? `Se pasa ${legible} de lo que permite el pliego: con un socio que tenga la cifra baja, la del consorcio se pondera por participación.`
...
module.exports = { guiaDe, exigenciasDe, resumenExigencias, accionDeCasilla, modalidadEnLlano, tamanoDe, filaDesdeFoto, ESTADOS_REQUISITO, ESTADOS_EXIGENCIA, EXIGENCIAS, REQUISITOS_CON_SOCIO, CONTRIBUCION_OBRA_PCT, GARANTIA_SERIEDAD_PCT, REFERENCIA_FINANCIERA, VERSION };
