...
function guiaDe({ fila = null, foto = null, perfil, ctx = {} } = {}) {
  ...
  /* LOS PASOS CON FECHA SALEN EN ORDEN CRONOLÓGICO, SE CALCULEN COMO SE CALCULEN (13-sep-2026).
     «Envíe observaciones» se cuenta en días de CALENDARIO (cierre − 7) y «Pida la garantía» en
     días HÁBILES (cierre − 5). Con un festivo dentro de la ventana los hábiles se estiran y la
     garantía cae ANTES que las observaciones, así que el orden de EMISIÓN mentía: con cierre el
     13-oct-2026 (el 12 es festivo) salían 6-oct y luego 5-oct. No se permuta ese par a mano
     —dejaría hermanos vivos en cuanto otra regla con fecha entre en juego—: se ordena el bloque
     entero, con orden ESTABLE para que los dos pasos que comparten el día anterior al cierre
     conserven su secuencia, y los pasos sin fecha se quedan al final, que es donde ya estaban. */
  const conFecha = pasos.filter((p) => p.cuando), sinFecha = pasos.filter((p) => !p.cuando);
  conFecha.sort((a, b) => (a.cuando < b.cuando ? -1 : a.cuando > b.cuando ? 1 : 0));
  const enOrden = [...conFecha, ...sinFecha].map((p, i) => ({ ...p, orden: i + 1 }));
  pasos.length = 0;
  pasos.push(...enOrden);
