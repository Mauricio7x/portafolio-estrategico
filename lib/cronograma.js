...
const valida = (a, m, d) => m >= 1 && m <= 12 && d >= 1 && d <= 31 && a >= 2000 && a <= 2100;
...
function extraerHitos(texto) {
  ...
  for (const linea of lineas) {
    for (const h of HITOS) {
      ...
      if (!f) { sinFecha++; break; }
      hitos.push({ id: h.id, etiqueta: h.etiqueta, fecha: f, origen: "pliego", evidencia: linea.trim().slice(0, 160) });
      vistos.add(h.id);
      break;
    }
  }
...
function ics(hitos, { proceso = "", entidad = "", uidBase = "detekta" } = {}) {
  ...
  for (const h of hitos) {
    ...
    lineas.push("BEGIN:VEVENT",
      ...
      `DESCRIPTION:${escIcs(`${entidad ? entidad + ". " : ""}Fuente: ${h.origen === "pliego" ? "cronograma del pliego" : "SECOP II"}. ${h.evidencia || ""}`)}`);
    for (const d of [7, 3, 1]) {
