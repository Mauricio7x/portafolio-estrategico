...
/* …PERO LO QUE QUEDA NO PUEDE LLEGAR A CERO (7-sep-2026). La regla de arriba, al
   pie de la letra, deja el plazo del intento en 1 ms cuando el presupuesto ya se
   consumió antes de la primera petición: el intento se aborta solo, la tanda
   avanza CERO páginas y la siguiente invocación repite el cuadro. Una cadena
   reanudable que nunca avanza no termina: la suite en GitHub Actions (corredor
   más lento que esta máquina) cayó en «la extracción histórica no converge» tras
   400 invocaciones con presupuesto de 200 ms, mientras en local pasaba —el fallo
   era del reloj, no del corredor—. El presupuesto puede ENCOGER la ventana de un
   intento hasta este suelo, nunca por debajo: así toda invocación tiene derecho a
   un intento de verdad y la cadena progresa al menos una página. No lo aplica
   sobre `opts.timeoutMs`: un llamador que pide 50 ms quiere fallar rápido y manda.
   2 s porque es lo que tarda una página real de Socrata (el tope normal sigue
   siendo 20 s: este suelo solo aparece con el presupuesto agotado). */
const PISO_INTENTO_MS = 2000;
...
function crearCliente(opts = {}) {
  ...
  async function pedir(params, etiqueta, { plazoMs } = {}) {
    ...
    /* Dos plazos DISTINTOS, y el suelo solo vale para uno (7-sep-2026). `plazoDe()` es el
       presupuesto de una TANDA que pagina y se reanuda: si se agota, encoger el intento hasta
       cero estanca la cadena, así que ahí manda `PISO_INTENTO_MS`. `{ plazoMs }` es una CONSULTA
       SUELTA a la que su llamador le dio su tiempo (documentos, seguimiento): no hay cadena que
       hacer converger y respetarlo es justo el propósito, así que corta duro. Confundirlos era
       lo que ponía una cota de 200 ms en 2 s. `restante()` (retrocesos y «tope entero») sigue
       mirando los dos: para decidir si aún hay tiempo, manda el más apretado. */
    const restanteTanda = () => {
      if (!plazoDe) return Infinity;
      const p = Number(plazoDe());
      return Number.isFinite(p) ? p : Infinity;
    };
    const restanteConsulta = () => (Number.isFinite(plazoMs) ? plazoMs - (Date.now() - tArranque) : Infinity);
    const restante = () => Math.min(restanteTanda(), restanteConsulta());
    const corte = () => {
      const ms = Math.max(1, Math.min(timeoutMs, restanteConsulta(), Math.max(PISO_INTENTO_MS, restanteTanda())));
