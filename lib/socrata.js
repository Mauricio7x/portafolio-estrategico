...
function crearCliente(opts = {}) {
  ...
  async function pedir(params, etiqueta) {
    ...
    /* CUOTA AGOTADA, EN LENGUAJE DE USUARIO (6-sep-2026, M-DGF-04). Este texto
       lo pegan al `motivo` que ve la persona cinco módulos en nueve sitios (socio,
       ejecucion, proponentes, documentos, seguimiento): con un 429 al final decía «{etiqueta}:
       agotados 5 intentos (HTTP 429 en {etiqueta})», o sea un id de dataset y un
       código HTTP donde hacía falta «vuelva a intentarlo». El 429 es el ÚNICO caso
       con mensaje propio —lo dice datos.gov.co, no un fallo nuestro— y se hereda
       sin tocar a los llamadores (una lista de sitios dejaría huecos). `status`
       viaja para quien quiera distinguirlo y `detalle` conserva el texto técnico
       para el registro. */
    const detalle = `${etiqueta}: agotados ${MAX_INTENTOS} intentos (${ultimo && ultimo.message})`;
    const limitado = !!(ultimo && ultimo.status === 429);
    const err = new Error(limitado ? "datos.gov.co limitó las consultas por unos minutos; vuelva a intentarlo" : detalle);
    if (ultimo && ultimo.status) err.status = ultimo.status;
    err.detalle = detalle;
    throw err;
  }
  ...
  async function contarMes(mes) {
    ...
    /* «SIN DATO» ≠ 0 (6-sep-2026): con `parseInt(... || "0")` un cuerpo vacío, [{}]
       o HTML publicaba 0 esperados y un count no numérico publicaba NaN. La ausencia
       se descarta ANTES de convertir (Number(null) === 0) y lo que no es un entero
       es null: «no auditado», que sync e historico publican como null. */
