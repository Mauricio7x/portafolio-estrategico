...
const RE_SMMLV_ANTES = /(\d[\d.,]*)\s*(?:smmlv|salarios?\s+minimos?)/g;
const RE_SMMLV_DESPUES = /(?:smmlv|salarios?\s+minimos?)\s*[:=]?\s*(\d[\d.,]*)/g;
  ...
  if (codigos.length > MAX_CODIGOS) {
    return {
      ...
      error: `Se detectaron ${codigos.length} códigos UNSPSC, muy por encima de lo que trae un certificado RUP real. `
        + "El documento no parece un RUP; verificá el PDF o cargá el perfil a mano desde /admin.html.",
