...
const { MAX_EXPERIENCIA_SMMLV, TAMANOS_EMPRESA } = require("./config_rup.js");
...
/* El TAMAÑO DE EMPRESA que publica el certificado. No es cosmético: decide si
   un proponente plural cabe en una convocatoria limitada a Mipyme, donde TODOS
   los integrantes tienen que serlo (art. 2.2.1.2.4.2.2 del Decreto 1082 de
   2015, modificado por el Decreto 1860 de 2021).
   Es un dato PUBLICADO: se lee o se deja en `null`. Jamás se deduce del
   patrimonio ni del número de contratos — el umbral legal es de ingresos y
   cambia con el salario mínimo, así que deducirlo sería inventarlo, y un tamaño
   inventado puede dejar una oferta fuera o meterla donde no cabe. */
const TAMANOS = Object.freeze({
  microempresa: /\bmicro\s*empresa\b|\bmicroempresa\b/,
  pequena: /\bpeque[nñ]a\s+empresa\b/,
  mediana: /\bmediana\s+empresa\b/,
  gran_empresa: /\bgran\s+empresa\b|\bgrande\s+empresa\b/,
});
/* CENSO, no lista: si el esquema admite un tamaño que aquí no se sabe leer, el
   certificado que lo traiga saldría con `null` sin que nadie lo note. La suite
   recorre TAMANOS_EMPRESA y exige que cada uno se lea; esta constante es la que
   compara. */
const TAMANOS_QUE_SE_LEEN = Object.freeze(Object.keys(TAMANOS));
function tamanoEnTexto(n) {
  for (const [clave, re] of Object.entries(TAMANOS)) if (re.test(n)) return clave;
  return null;
}
function leerTamanoEmpresa(lineas) {
  for (let i = 0; i < lineas.length; i++) {
    const n = norm(lineas[i]);
    if (!/tama[nñ]o\s+de\s+empresa/.test(n)) continue;
    /* En los tres certificados medidos (11-sep-2026) el valor va en la MISMA
       línea, tras los dos puntos: «TAMAÑO DE EMPRESA:MICROEMPRESA». */
    const tras = n.slice(n.indexOf("empresa") + "empresa".length);
    const mismo = tamanoEnTexto(tras);
    if (mismo) return mismo;
    /* El hermano vivo: otra cámara —u otro extractor de PDF— puede dejar el
       rótulo en una línea y el valor en la siguiente. Se mira UNA línea más y
       ninguna más: si ahí tampoco está, es «sin dato», no una búsqueda ancha
       que acabe recogiendo la palabra «microempresa» de cualquier otra parte
       del certificado. */
    const siguiente = i + 1 < lineas.length ? tamanoEnTexto(norm(lineas[i + 1])) : null;
    if (siguiente) return siguiente;
  }
  return null;   // sin dato: la puerta de convocatoria limitada no afirmará nada
}
...
function extraerRupDeTexto(texto) {
  ...
  const tamanoEmpresa = leerTamanoEmpresa(lineas);
  ...
  /* SIN TOPE POR DEFECTO (26-sep-2026, decisión del dueño: «lo que diga la
     ley»). Antes se ponía 2 × el mayor contrato, un apetito que escondía
     procesos que la capacidad de contratación —calculada con este mismo RUP—
     sí alcanza. */
  const topeSMMLV = null;
...
module.exports = { extraerRupDeTexto, extraerCodigosUnspsc, fechaDe, unirNumerosPartidos, numerosDe, leerTamanoEmpresa, TAMANOS_QUE_SE_LEEN, PLAUSIBLE_MIN, MIN_TEXTO };
