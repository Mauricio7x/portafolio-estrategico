...
function leerIndicador(lineasNorm, re, { excluir = null } = {}) {
  for (const ln of lineasNorm) {
    ...
    const m = re.exec(ln);
    if (!m) continue;
    const numeros = numerosDe(ln.slice(m.index + m[0].length));
...
/* ══════════════════ 3 · Códigos UNSPSC ══════════════════ */
const RE_SECCION_ON = /(clasificaci|clasificador)[^]*?(bienes|servicios|unspsc)|bienes y servicios|segmento\s+familia\s+clase|codigos?\s+unspsc/;
...
function extraerCodigosUnspsc(lineas) {
  ...
  for (const linea of lineas) {
    const ln = norm(linea);
    if (RE_SECCION_ON.test(ln)) enSeccion = true;
...
function leerExperienciaYK(lineasNorm) {
  ...
  for (const ln of lineasNorm) {
    ...
    if (!/smmlv|salarios?\s+minimos?/.test(ln)) continue;
    for (const { valor } of numerosDe(ln)) {
      if (valor > 0 && valor <= MAX_EXPERIENCIA_SMMLV) {
...
function extraerRupDeTexto(texto) {
  ...
  if (crudo.trim().length < MIN_TEXTO) {
    return {
      ...
      error: "El texto extraído del PDF está vacío o es demasiado corto. Si el certificado es un escaneo "
        + "(imagen sin capa de texto), pdf.js no puede leerlo: descargá el RUP en PDF directamente del "
  ...
  if (!codigos.length) {
    return {
      ok: false,
      error: "No se pudo leer la columna de códigos UNSPSC. Verificá que el PDF sea tu certificado RUP "
        + "vigente (el Registro Único de Proponentes que expide la Cámara de Comercio) y que incluya la "
        + "sección «Clasificación de bienes y servicios». Si el problema persiste, cargá el RUP a mano "
        + "desde /admin.html.",
  ...
  const liquidez = leerIndicador(lineasNorm, /(indice\s+de\s+)?liquidez/);
  const endeudamiento = leerIndicador(lineasNorm, /(indice\s+de\s+|nivel\s+de\s+)?endeudamiento/);
  const cobertura = leerIndicador(lineasNorm, /(razon\s+de\s+)?cobertura\s+de\s+intereses/);
  ...
  const faltantes = [];
  if (!liquidez) faltantes.push("índice de liquidez");
  if (!endeudamiento) faltantes.push("índice de endeudamiento");
  if (!patrimonio) faltantes.push("patrimonio");
  if (!utilidad) faltantes.push("utilidad operacional (ni la rentabilidad del patrimonio para derivarla)");
  if (experiencia_smmlv == null) faltantes.push("experiencia acreditada en SMMLV");
  if (faltantes.length) {
    return {
      ok: false,
      error: `Se leyeron ${codigos.length} códigos UNSPSC, pero faltan indicadores del certificado: `
        + `${faltantes.join(", ")}. Sin ellos no se puede calcular tu capacidad de contratación. `
        + "Verificá que el PDF incluya la sección de información financiera, o cargá esos valores a mano desde /admin.html.",
      diagnostico: {
  ...
  if (vigencia.verificar_vigencia) {
    advertencias.push(`La última fecha del certificado (${ultimaFecha}) es de un año anterior: el RUP se renueva `
      + "cada año antes del quinto día hábil de abril. Verificá que esté vigente antes de presentarte.");
  }
  if (!nombre) advertencias.push("No se pudo leer la razón social: el perfil queda con un nombre genérico (editable desde /admin.html).");

  /* ---- los dos supuestos declarados ---- */
  advertencias.push("El RUP no publica la planta de profesionales: se asume 1 (el factor CT de la capacidad K queda en su mínimo, 20). Si tu equipo es mayor, corregilo cargando el RUP desde /admin.html.");
  const topeSMMLV = Math.ceil(experiencia_smmlv * 2);
  advertencias.push(`Tope estratégico por defecto: ${topeSMMLV} SMMLV (2 × tu mayor contrato acreditado). Por encima de esa cuantía no se te mostrarán procesos; ajustalo si tu apetito es otro.`);
  ...
  const config = {
    ...
    unspsc: codigos,
    indicadores: {
      liquidez: liquidez.valor,
      endeudamiento: comoRatio(endeudamiento, "endeudamiento", advertencias),
      cobertura_intereses: coberturaValor,
      patrimonio: Math.round(patrimonio.valor),
      utilidad_operacional: Math.round(utilidad.valor),
  ...
  return {
    ...
    diagnostico: {
      ...
      utilidad_derivada: Boolean(utilidad.derivada),
    },
  };
}
