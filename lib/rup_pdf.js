...
const RE_COLA_PARTIDA = /(\d[\d.,]*?)([.,])\s*$/;
const RE_CABEZA_NUMERO = /^\s*(\d[\d.,]*)/;
...
function unirNumerosPartidos(lineas) {
  ...
  for (let i = 0; i < lineas.length; i++) {
    ...
    let cola = RE_COLA_PARTIDA.exec(ln);
    while (cola && i + 1 < lineas.length) {
      const sig = String(lineas[i + 1]);
      const cab = RE_CABEZA_NUMERO.exec(sig);
      if (!cab) break;
      const union = `${cola[1]}${cola[2]}${cab[1].replace(/[.,]$/, "")}`;
      if (!RE_NUMERO_BIEN_FORMADO.test(union)) break;
      ln = ln.replace(/\s+$/, "") + sig.replace(/^\s+/, "");
      i++;
      cola = RE_COLA_PARTIDA.exec(ln); // un número partido en tres líneas
    }
    salida.push(ln);
  }
  return salida;
...
const MOTIVO_PARTIDO = (token) => "En el certificado la cifra aparece partida o incompleta "
  + `(se leyó «${token}», que no es una cifra completa): escríbala usted tal como figura en el certificado.`;
...
function leerIndicador(lineasNorm, re, { excluir = null, minPlausible = null } = {}) {
  for (const ln of lineasNorm) {
    ...
    if (n.colgante === "," || (n.colgante === "." && (minPlausible == null || bajo)) || bajo) {
      const token = n.colgante ? (ln.match(/(\d[\d.,]*)\s*$/) || [])[1] : null;
      return { partido: true, token: token || String(n.valor), linea: ln.trim().slice(0, 160) };
    }
...
/* → { experiencia_smmlv, k_declarada_smmlv, experiencia_partida }.
   `experiencia_partida` es el token de una cifra que quedó partida sin poder
   unirse: la línea con la unidad EMPIEZA por el número y la anterior termina
   en un número con separador colgando («12.» / «5000 SMMLV»). Ese contrato es
   el que decide el máximo y no se sabe cuánto valía: la experiencia se pide
   (con lo leído a la vista), no se guarda el trozo ni el máximo de los demás. */
function leerExperienciaYK(lineasNorm) {
  let maxSMMLV = null;
  let kDeclarada = null;
  let partida = null;
  let anteriorColgante = null; // token numérico con separador colgando en la línea previa
  for (const ln of lineasNorm) {
    const colgaAqui = RE_COLA_PARTIDA.exec(ln);
    const empiezaPorNumero = /^\s*\d/.test(ln);
    if (RE_K_DECLARADA.test(ln)) {
      ...
      anteriorColgante = colgaAqui ? colgaAqui[1] + colgaAqui[2] : null;
    ...
    for (const re of [RE_SMMLV_ANTES, RE_SMMLV_DESPUES]) {
      ...
      while ((m = re.exec(ln)) !== null) {
        if (anteriorColgante && empiezaPorNumero && m.index === 0 && re === RE_SMMLV_ANTES) {
          if (!partida) partida = `${anteriorColgante} / ${m[1]}`;
          continue;
        }
...
function extraerRupDeTexto(texto) {
  ...
  const partidos = {};
  const leido = (campo, x) => {
    if (x && x.partido) { partidos[campo] = x.token; return null; }
    return x;
  };
  ...
  if (cifrasPartidas.length) {
    advertencias.push(`${cifrasPartidas.length === 1 ? "Una cifra del certificado aparece partida" : `${cifrasPartidas.length} cifras del certificado aparecen partidas`} `
      + `en un salto de línea y no se guardó el trozo (${cifrasPartidas.map((c) => `${c}: «${partidos[c]}»`).join("; ")}): `
