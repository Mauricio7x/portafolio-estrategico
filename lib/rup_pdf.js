...
function extraerRupDeTexto(texto) {
  ...
  if (codigos.length > MAX_CODIGOS) {
    return {
      ...
      error: `Se detectaron ${codigos.length} códigos UNSPSC, muy por encima de lo que trae un certificado RUP real. `
        + "El documento no parece un RUP; verificá el PDF o cargá el perfil a mano desde la pestaña «Mi empresa».",
  ...
  advertencias.push("El RUP no publica la planta de profesionales: se asume 1 (el factor CT de la capacidad K queda en su mínimo, 20). Si tu equipo es mayor, corregilo cargando el RUP desde la pestaña «Mi empresa».");
