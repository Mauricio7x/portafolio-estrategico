...
function extraerRupDeTexto(texto) {
  ...
  const CAMPOS_PEDIBLES = [
    { campo: "liquidez", ok: !!liquidez, etiqueta: "Índice de liquidez",
      ...
      sin_el: "No se puede calcular tu capacidad financiera (uno de los factores de la K)." },
    { campo: "endeudamiento", ok: !!endeudamiento, etiqueta: "Nivel de endeudamiento (%)",
      donde: "Sección «Información financiera». Escribilo como porcentaje: 13 para 13 %.",
    ...
    { campo: "patrimonio", ok: !!patrimonio, etiqueta: "Patrimonio (pesos)",
      ...
      sin_el: "No se puede calcular ni tu capacidad de contratación ni si podés financiar la obra." },
    { campo: "utilidad_operacional", ok: !!utilidad, etiqueta: "Utilidad operacional (pesos)",
      donde: "Sección «Información financiera». Los certificados de persona natural muchas veces NO la traen: "
        + "tomala de tu declaración de renta o de tu estado de resultados del último año.",
      sin_el: "No se puede estimar tu capacidad organizacional (el factor CO de la K)." },
    { campo: "experiencia_smmlv", ok: experiencia_smmlv != null, etiqueta: "Experiencia acreditada (SMMLV)",
      donde: "Sección de experiencia del certificado: es el valor total en SMMLV de los contratos que inscribiste.",
      sin_el: "No se puede comparar tu experiencia contra la que pide cada proceso." },
  ];
  ...
  if (!completo) {
    advertencias.push(`Del certificado se leyeron ${codigos.length} códigos y el resto de los datos, pero `
      ...
      + `${faltan.map((f) => f.etiqueta).join(", ")}. Escribilos una vez y quedan guardados en tu perfil.`);
  }
