...
const { desenlaceDe } = require("./indice_competencia.js");
...
async function auditarCobertura(redis, perfil, { terminos = null, log = () => {} } = {}) {
  ...
  for (const lic of filas) {
    /* a. solo procesos ADJUDICADOS: un desierto no dice con qué código se
       contrata de verdad, dice que nadie llegó. Se pregunta por el DESENLACE
       (remate B9b-H3, 6-sep-2026): `esAdjudicado` a secas se contentaba con una
       fecha de adjudicación y dejaba pasar justo el desierto que este filtro
       existe para excluir. */
    if (desenlaceDe(lic) !== "adjudicado") { embudo.sin_adjudicacion++; continue; }
