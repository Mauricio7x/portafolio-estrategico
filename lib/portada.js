/* lib/portada.js · El pulso del mercado y la manifestación de interés (Fase 9 · Detekta v4)
   ...
   · MANIFESTACIÓN DE INTERÉS · lo que el dataset SÍ y NO dice (censo real
     ...
     cuenta regresiva sobre una fecha deducida sin decirlo. La apertura es
     `fecha_de_publicacion` (la fase de manifestación, publicada por SECOP II)
     cuando viene, y si no `fecha_de_publicacion_del` como supuesto declarado
     (22-sep-2026, `lib/manifestacion.aperturaDe`); el enlace al SECOP va en cada fila.
...
const { estado_abierto, selloReglaIngesta } = require("./filtros.js");
...
/* LA HISTORIA DEL MERCADO (6-sep-2026, M-DGF-20). `portada:resumen` es una foto
   que cada reconstrucción SOBRESCRIBE; aquí se guarda, aparte, un punto por
   día (hora de Colombia) con las tres cifras del teaser y el sello de la regla
   de ingesta, acotado a HISTORIA_MAX puntos. La portada pública solo enseña
   tendencia con base (≥ 30 puntos en la ventana y un solo sello): una serie
   que se mueve porque cambió la regla no es el mercado. */
const CLAVE_HISTORIA = "portada:historia";
const HISTORIA_MAX = 120;
...
const { exigeManifestacion, aperturaDe, ventanaDe, estadoDeVentana, filaManifestacion, sigueValiendoLaPena, noConstaVencida, PLAZO_MANIFESTACION_HABILES, PLAZO_MINIMO_HABILES, MAX_MANIFESTACIONES_SIN_SORTEO, NORMA } = require("./manifestacion.js");
...
/* DOS CONJUNTOS, Y NO UNO CON DOS NOMBRES (15-sep-2026, encargo del dueño):
   · `sinVencer` — todo aquello en lo que NO CONSTA que el plazo haya vencido.
     Es lo que se GUARDA en la ventana precalculada y lo que la pantalla de
     «Avisar que le interesa» enseña: el dueño pidió ver todo a lo que todavía
     se pueda presentar, y esconder por una deducción es lo que se corrigió.
   · `abiertos` ⊂ `sinVencer` — aquellos de los que se puede AFIRMAR que siguen
     vivos. Es lo que CUENTA el teaser de la portada, porque ahí la cifra va
     dentro de la frase «todavía puede avisar que le interesa» y esa frase es
     una afirmación. Contarlos juntos sería la confusión de
     `total_procesos`/`procesos_contados`, con el precio de prometer un plazo
...
function manifestacionDe(abiertas, { ahora = Date.now() } = {}) {
  ...
  for (const l of abiertas) {
    ...
    const f = filaManifestacion(l, hoy, { ahora });
    if (noConstaVencida(f)) sinVencer.push(f);           // los predicados viven UNA vez, en lib/manifestacion
  }
  sinVencer.sort((a, b) => (a.habilesHastaElTecho ?? 99) - (b.habilesHastaElTecho ?? 99) || (b.valor || 0) - (a.valor || 0));
  return { hoy, sinVencer, abiertos: sinVencer.filter(sigueValiendoLaPena) };
}
...
function agregar(filas, { indiceBaja = null, ahora = Date.now(), paa = null } = {}) {
  ...
  let valorTotal = 0;
  /* Cuántos procesos NO publican presupuesto: su cuantía vale 0 (la regla de
     «0 = sin dato» del dataset) y suman 0 al total, así que «$4,7 billones en
     juego» es una COTA INFERIOR. Sin decir cuántos son, el visitante lee un
     total exacto donde hay un mínimo. */
  let sinCuantia = 0;
  ...
  return {
    ...
    // el total es cota inferior: estos procesos están abiertos y no publican valor
    procesosSinCuantia: sinCuantia,
    ...
    porDepartamento,
    manifestacion: {
      abiertos: manif.abiertos.length,          // los AFIRMABLES: es la cifra de una frase que afirma
      sin_vencer: manif.sinVencer.length,       // en los que no consta que el plazo venciera
    ...
    _manifestacion: manif.sinVencer,
    _paa: paa && Array.isArray(paa.muestra) ? paa.muestra : null,
  };
}

/* Un punto por fecha: si el día ya está, se sustituye (la última reconstrucción
   del día es la foto del día); se ordena por fecha y se recorta a HISTORIA_MAX
   por el principio. Función pura: la suite la ejecuta con listas sintéticas. */
const FECHA_RE = /^\d{4}-\d{2}-\d{2}$/;
function anexarPunto(historia, punto, max = HISTORIA_MAX) {
  const validos = (Array.isArray(historia) ? historia : []).filter((x) => x && FECHA_RE.test(String(x.fecha || "")) && x.fecha !== punto.fecha);
  validos.push(punto);
  validos.sort((a, b) => (a.fecha < b.fecha ? -1 : a.fecha > b.fecha ? 1 : 0));
  return validos.slice(-max);
}
function puntoDe(publica, { ahora = Date.now(), sello = null } = {}) {
  return {
    fecha: habiles.hoyColombia(ahora),
    procesosAbiertos: Number.isInteger(publica.procesosAbiertos) ? publica.procesosAbiertos : null,
    valorTotal: Number.isFinite(publica.valorTotal) ? publica.valorTotal : null,
    entidadesActivas: Number.isInteger(publica.entidadesActivas) ? publica.entidadesActivas : null,
    sello,
  };
}
...
async function leerHistoria(redis) {
  const h = await leerJSON(redis, CLAVE_HISTORIA);
...
async function reconstruirPortada(redis, { cargarCorpus, consultarPaa = null, ahora = Date.now(), presupuestoPaaMs = 6000 } = {}) {
  ...
  /* `sin_vencer` es la lista buena desde el 15-sep-2026; `abiertos` se escribe
     con lo MISMO solo para que una app desplegada contra una ventana recién
     escrita por la versión anterior siga leyendo algo (desplegar no puede
     exigir reconstruir). El lector prefiere `sin_vencer`. */
  await escribirJSON(redis, CLAVE_MANIFESTACION, { generado: publica.generado, hoy: publica.manifestacion.hoy, sin_vencer: _manifestacion, abiertos: _manifestacion, proximos: _paa });
  /* la historia: el punto de HOY (hora de Colombia) con el sello de la regla
     de ingesta vigente; con su propio try, que una historia que no se pudo
     escribir no tumba la portada que sí se escribió */
  try {
    const historia = anexarPunto(await leerHistoria(redis), puntoDe(publica, { ahora, sello: selloReglaIngesta() }));
    await escribirJSON(redis, CLAVE_HISTORIA, historia);
  } catch { /* la portada ya está escrita */ }
  return portada;
}

module.exports = {
  agregar, manifestacionDe, filaManifestacion, exigeManifestacion, aperturaDe, ventanaDe, estadoDeVentana, sigueValiendoLaPena, noConstaVencida, leerPortada, leerManifestacion, reconstruirPortada,
  leerHistoria, anexarPunto, puntoDe,
  CLAVE_PORTADA, CLAVE_ENTIDADES, CLAVE_MANIFESTACION, CLAVE_HISTORIA, HISTORIA_MAX, PLAZO_MANIFESTACION_HABILES, PLAZO_MINIMO_HABILES, MAX_MANIFESTACIONES_SIN_SORTEO, NORMA, TOP_ENTIDADES, BAJA_MIN_PROCESOS,
};
