...
const { exigeManifestacion, aperturaDe, ventanaDe, estadoDeVentana, filaManifestacion, PLAZO_MANIFESTACION_HABILES, PLAZO_MINIMO_HABILES, MAX_MANIFESTACIONES_SIN_SORTEO, NORMA } = require("./manifestacion.js");
...
function manifestacionDe(abiertas, { ahora = Date.now() } = {}) {
  ...
  for (const l of abiertas) {
    ...
    if (f.estado === "abierta" || f.estado === "por_confirmar") abiertosAhora.push(f);
  }
...
module.exports = {
  agregar, manifestacionDe, filaManifestacion, exigeManifestacion, aperturaDe, ventanaDe, estadoDeVentana, leerPortada, leerManifestacion, reconstruirPortada,
