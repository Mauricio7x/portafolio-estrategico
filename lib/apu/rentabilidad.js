...
/* σ que usa la curva de precio cuando el índice NO publica un IQR utilizable
   (celda concentrada: el histograma es de puntos porcentuales enteros y la
   colapsa). Es un SUPUESTO SIN FUENTE, y hay que decirlo: decide cuánta
   probabilidad pierde una oferta que no llega al centro del mercado, y con
   mediana 35 % y baja máxima 20 % lleva el factor al suelo de 0,01, mientras
   que con la dispersión observada en producción (σ ≈ 25 pp en una muestra de
   extremos) el mismo caso daría ≈ 0,56.

   NO se ha recalibrado a propósito: hacerlo con una muestra de extremos sería
   la «suavización» que este proyecto prohíbe. `lib/indice_baja.medirDispersion`
   lo mide sobre el corpus entero y lo publica en `indice:baja:meta.dispersion`
   (importa ESTA constante, no la copia); con esa medición delante se decide. */
...
function multiplicadorPrecio({
  ...
  /* El ORIGEN viaja con la cifra: una dispersión sin su procedencia no se puede
     discutir, igual que `granularidad_utilizada` en el índice de baja o
     `origen_b_max` aquí al lado. Medido en producción (24-ago-2026): 404 de
     1 555 celdas (26 %) no tienen dispersión medible y usan el supuesto, y sin
     este campo un σ supuesto de 4 pp es indistinguible de uno medido de 4 pp. */
  let origen_sigma = "supuesta";
  const p25 = numero(baja_p25), p75 = numero(baja_p75);
  if (p25 != null && p75 != null && p75 > p25) {
    sigma = Math.max(0.5, (p75 - p25) / 1.349);
    origen_sigma = "medida";
  }
  ...
  return {
    ...
    mu, b, sigma, origen_sigma, n, pi_menor_valor,
...
module.exports = { DISPERSION_DEFECTO,
