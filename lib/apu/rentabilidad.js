...
function pGanarPorPrecio({
  ...
  if (mu == null || b == null) return sinDato;

  /* EL NÚMERO DE OFERENTES NO ENTRA AQUÍ, Y ES DELIBERADO. La FORMA de la
     curva se evalúa con un `n` de REFERENCIA fijo; el NIVEL lo pone `p_base`,
     que ya cae con los rivales (1/(1+rivales)) en lib/probabilidad.
  ...
  const mult = enCentro > 0 ? enOferta / enCentro : 1;

  const base = p_base != null ? p_base : enOferta;
  ...
  return {
    ...
    p_menor_valor: redondear(menorValor(b), 4),
    p_central: redondear(central(b), 4),
...
function flujoCaja({
  ...
  const T = Math.max(1, Math.round(Number(plazo_meses) || 12));
  const L = Math.max(0, Math.round(Number(cola_liquidacion_meses) || 0));
  ...
  for (let t = 1; t <= T + L; t++) {
    ...
    const ingreso = (t === 1 ? V * alfa : 0) + cobro + (t === T + L ? retegarantiaTotal : 0);
  ...
  return {
    ...
    plazo_meses: T, cola_liquidacion_meses: L, dso_meses: dsoMeses,
...
function rentabilidad(entrada = {}) {
  ...
  const roic = flujo.k_max > 0
    ? redondear(Math.pow(1 + utilidad / flujo.k_max, 12 / (flujo.plazo_meses + flujo.cola_liquidacion_meses)) - 1, 4)
    : null;
...
module.exports = {
  rentabilidad, desdePresupuesto, pGanarPorPrecio, ajusteCompetitivo, precioPiso, primaMaldicion,
