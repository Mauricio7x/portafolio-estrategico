...
const COMO_LEERLO = {
  ...
  limite: "El barrido NO corrige que el precio entre dos veces en la probabilidad "
    + "(§2.5c del análisis de probabilidad del proyecto). Ese defecto mueve el NIVEL del VEG, no el precio elegido: "
...
function optimizarPrecioOferta(proceso = {}, costo_directo_total = 0, config = {}) {
  ...
  if (sinPuntoRentable) {
    // «Atención:» y no un pictograma: esta cadena viaja por la API hasta la pantalla y un
    // emoji lo dibuja el sistema operativo (la misma regla de lib/apu/calculo, 6-sep-2026)
    alertas.push("Atención: NINGÚN precio del rango tiene valor esperado positivo: con este costo directo y este "
