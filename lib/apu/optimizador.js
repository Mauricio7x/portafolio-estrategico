/* ============================================================================
   ...
   `docs/PROBABILIDAD_MEJORADA.md` §2.5c documenta que el precio se cobra DOS
   VECES: `lib/probabilidad` ya multiplica por un factor de baja de mercado, y
   `pGanarPorPrecio` vuelve a modular por precio sobre esa misma `p` como
   `p_base`. Los pasos A4/A5 de ese plan (separar `p` de `p_sin_precio`) siguen
   pendientes y este módulo NO los adelanta.
   Lo que sí se puede afirmar —y hay prueba— es que el defecto **no mueve el
   precio recomendado**: el factor de baja es constante a lo largo del barrido
   (depende de la mediana de la entidad, no del precio que uno elija), y
   `argmax_d [k·f(d) − c]` no depende de `k > 0`. Afecta al NIVEL del VEG
   publicado, no al argmax. Decir «queda arreglado» sería falso; decir «invalida
...
/* ════════════════════════ optimizarPrecioOferta ════════════════════════
   `proceso`   presupuesto_oficial · baja (registro de bajaDeMercado) ·
               competencia (registro de competenciaDe) · p_base (la `p` de
               lib/probabilidad) · precio_venta · precio_actual · plazo_meses ·
               entidad · id_proceso
