...
/* La ruta llega como VARIABLE, así que quien garantiza que el JSON viaje al
   despliegue NO es el tracer de Vercel sino `includeFiles: "data/**"` de
   vercel.json (declarado en los 6 routers). El comentario decía lo contrario y
   eso importa: con ruta dinámica y sin `includeFiles`, el archivo no viajaría y
   la degradación de abajo dejaría el clasificador mudo SOLO EN PRODUCCIÓN. Se
   degrada a vacío en vez de tumbar la consulta, y `meta()` publica cuántos
   archivos no se pudieron cargar para que no sea una degradación silenciosa. */
