...
function costoDirecto(item, cat = SEMILLA, regionId = cat._meta.region_base) {
  ...
  for (const linea of item.insumos || []) {
    ...
    if (insumo.tipo === "mano_obra") {
      ...
      cantidad = 1 / Number(linea.rendimiento); // días (u horas) por unidad de obra
  ...
  return { total: Math.round(total), capitulos: cap, lineas, region: regionId };
}
...
function itemPorCodigo(cat, codigo) {
  ...
  const directo = items.find((i) => String(i.codigo) === c);
  if (directo) return directo;
  const nuevo = CODIGOS_RENUMERADOS[c];
  return nuevo ? items.find((i) => String(i.codigo) === nuevo) || null : null;
}

module.exports = {
  SEMILLA, TIPOS, CODIGOS_RENUMERADOS, itemPorCodigo,
