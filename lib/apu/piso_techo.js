...
function bajaUtilizable(baja) {
  ...
  /* EL RANGO EN QUE CAYÓ LA MITAD DE LAS ADJUDICACIONES VIAJA CON LA MEDIANA
     (6-sep-2026, M-DGF-01). `lib/indice_baja` ya lo publica (`baja_p25` /
     `baja_p75`, encogidos con la misma mezcla que la mediana) y el motor de
     precio ya lo USA —`lib/apu/rentabilidad.multiplicadorPrecio` deriva σ de
     (p75 − p25) / 1,349—: aquí se LEE del mismo registro, no se recalcula. Sin
     él, el panel donde se fija el precio enseña un centro sin decir si lo
     normal es un rango estrecho o ancho. Puede faltar (un índice anterior al
     encogimiento): entonces es `null`, nunca 0, y quien dibuje decide con la
     ausencia, no con un cero creíble. */
  return {
    mediana, procesos: n, granularidad: baja.granularidad_utilizada, modalidad: baja.modalidad_utilizada || null,
    p25: numero(baja.baja_p25), p75: numero(baja.baja_p75),
  };
}
...
/* Cómo se dice la baja según DE DÓNDE salió (la cascada solo baja en
   ...
   DOS CORRECCIONES DEL 23-sep-2026, las dos por llamar a la regla en vez de
   tener una propia:
   · El signo, con `<= 0` como `mensajeDe` (lib/indice_baja): se comparaba
     `=== 0`, y con una mediana de −2 el panel decía «Esta entidad suele bajar
     −2 %» mientras el techo, acotado al presupuesto, decía en la misma pantalla
     que el precio ES el presupuesto. Una mediana negativa es «no bajó», como la
     cero. `b` sale de `bajaUtilizable`, que descarta la mediana ausente, y
     aun así se vuelve a exigir aquí porque la función se exporta: en JS
     `null <= 0` es TRUE, y un «sin dato» leído como «no bajó» sería el cero
     creíble que este repositorio prohíbe. Sin mediana, null (no hay frase).
   · El lugar sale de `dondeSeMidio`, la derivación del servidor que ya
     publica la tarjeta (`baja_donde`): estas tres ramas propias decían «esta
     entidad» sin distinguir la familia de todo lo que contrata, y «esta zona»
     sobre un departamento con nombre. `require` diferido: `indice_baja` carga
     `almacen` y el índice de competencia, y este panel es una capa pura. */
function fraseBaja(b) {
  const mediana = numero(b && b.mediana);
  if (mediana == null) return null;
  const n = `${b.procesos} proceso${b.procesos === 1 ? "" : "s"}`;
  const mod = b.modalidad ? ` en ${b.modalidad}` : "";
  const donde = require("../indice_baja.js").dondeSeMidio(b.granularidad);
  if (mediana <= 0) {
    return `En ${donde}, quien ganó no bajó el precio: se suele adjudicar por el presupuesto oficial (${n}${mod}).`;
  }
  return `En ${donde}, quien ganó suele bajar ${pct(mediana)} del presupuesto oficial (${n}${mod}).`;
}
...
function pisoTecho(entrada = {}) {
  ...
  /* EL TECHO NUNCA SUPERA EL PRESUPUESTO OFICIAL (22-sep-2026). Una mediana de baja
     NEGATIVA existe en el índice (la entidad adjudicó por encima del oficial en su
     historial: adendas, ajustes) y daba un techo de 1.050 M sobre un presupuesto
     de 1.000 M, que `lib/ganancia` tomaba como precio esperado: una «ganancia»
     sobre un precio que SECOP II no admite. La mediana medida se publica tal cual
     (`baja_esperada_pct`); el precio al que se adjudica se acota al oficial. */
  const techo = b ? red(po * (1 - Math.max(0, b.mediana) / 100)) : null;
  ...
  /* ── lo que NO SE PUDO LEER (24-sep-2026) ── el mismo centinela que ya
     reconocen la tarjeta y la ganancia (`motivo: "no_se_leyo"`): falló la
     consulta, no faltan datos. Sin esto el panel decía «no hay procesos
     anteriores comparables» y «no tenemos historial suficiente de esta
     entidad» sobre un índice que nadie pudo mirar. */
  const bajaNoLeida = !b && !!baja && baja.motivo === "no_se_leyo";
  const compNoLeida = !comp && !!competencia && competencia.motivo === "no_se_leyo";
  ...
  const cifras = {
    ...
    /* Mismos números que `baja_p25`/`baja_p75` del índice, con el sufijo `_pct`
       de este bloque (como `baja_esperada_pct` lo es de `baja_mediana`): no son
       dos cosas, es la misma cifra bajo la convención del panel. Sin base
       utilizable van `null` junto con la mediana: un rango sin los 5 procesos
       del techo sería una precisión que nadie midió. */
    baja_p25_pct: b ? b.p25 : null,
    baja_p75_pct: b ? b.p75 : null,
    baja_procesos: b ? b.procesos : null,
    baja_granularidad: b ? b.granularidad : null,
    /* el lugar en palabras de pantalla, derivado UNA vez aquí (23-sep-2026): el panel de
       Precios decía «lo que suele bajar aquí» sobre una baja que podía ser del departamento */
    baja_donde: b ? require("../indice_baja.js").dondeSeMidio(b.granularidad) : null,
    baja_modalidad: b ? b.modalidad : null,
    /* LA BAJA QUE NO SE PUDO LEER NO ES «NO HAY» (24-sep-2026): con
       `motivo: "no_se_leyo"` (lib/indice_baja.SIN_LECTURA_BAJA) nadie contó nada,
       así que el conteo va null —antes un `|| 0` lo volvía «0 comparables» y el
       panel decía «no hay procesos anteriores»— y `baja_motivo` lo dice para
       quien arma su propia frase (el margen de la lista). La ausencia se
       descarta ANTES de convertir: `numero(null)` no es 0. */
    baja_motivo: bajaNoLeida ? "no_se_leyo" : null,
    /* el gemelo para los oferentes (24-sep-2026): el panel decía «Menos de 5 procesos
       con oferentes contados» cuando nadie pudo contar nada */
    oferentes_motivo: compNoLeida ? "no_se_leyo" : null,
    baja_procesos_vistos_sin_base: !b && baja && !bajaNoLeida ? numero(baja.procesos_contados) : null,
  ...
  const frases = {
    ...
    baja: b ? fraseBaja(b)
      // la frase de la lectura fallida es UNA, la del índice: se llama, no se reescribe
      : bajaNoLeida ? require("../indice_baja.js").SIN_LECTURA_BAJA.mensaje
      : (vistos > 0
    ...
    oferentes: comp
      ...
      : compNoLeida
        ? "Cuántos se presentan aquí: no se pudo consultar en esta carga, no es falta de datos. Vuelva a cargar la página en unos minutos."
  ...
  if (piso > po) {
    ...
    veredicto = bajaNoLeida
      ? `Su precio mínimo es ${cop(piso)}. No se pudo consultar a cuánto se suele adjudicar en esta entidad.`
      : `Su precio mínimo es ${cop(piso)}. No tenemos historial suficiente de esta entidad para estimar a cuánto se suele adjudicar.`;
    detalle = frases.baja;
  } else if (techo < piso) {
