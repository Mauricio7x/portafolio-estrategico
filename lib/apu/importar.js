...
function mapearFilasImportadas(filas, catalogo, opciones = {}) {
  ...
  const enBogota = /bogot/i.test(sinTildes);
  ...
  const rango = (it) => {
    ...
    if (it.es_invias) return enBogota || enCundinamarca ? 2 : 1;
  ...
  const salida = (filas || []).map((cruda, i) => {
    ...
    if (terminos.length) {
      ...
      /* …Y CON DESCRIPCIONES LARGAS UN SOLO TÉRMINO NO PODA NADA (ago 2026).
         ...
         pasar el 36 % de los 6 588 candidatos de los cinco bancos, y 400 filas
         así tardaban más de 60 s: el `maxDuration` de la función. Medido con
         descripciones REALES del propio banco IDU (673 pasan de 200 caracteres).
         A partir de tres términos se exigen DOS en común, que sigue sin poder
         cambiar ninguna decisión —un candidato con un solo término compartido no
         alcanza el umbral de aceptación ni mueve el margen de un firme— y acota
         el trabajo. Con una o dos palabras se conserva el criterio de siempre,
         porque ahí exigir dos dejaría filas legítimas sin candidato.
      ...
      candidatos = todos
        ...
        .sort((a, b) => b.score - a.score || a._rango - b._rango || String(a.codigo).localeCompare(String(b.codigo)));
    }
    ...
    const variantes = mejor && BANCOS_OFICIALES.has(mejor.fuente_mapeo)
      ? candidatos.slice(1).filter((c) => c.fuente_mapeo === mejor.fuente_mapeo && c._familia.clave === mejor._familia.clave)
        .map((c) => ({ codigo: c.codigo, descripcion: c.descripcion, unidad: c.unidad }))
      : [];
