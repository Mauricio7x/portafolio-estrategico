...
/* ═══════════ 1-bis · ¿Se pueden COMPARAR las dos unidades? ═══════════════
   Una unidad que no se puede leer no contradice nada. 144 de los 6 414 ítems
   de los bancos traen la unidad rota por el corrimiento de columnas del PDF de
   origen («CONSTRUCCIÓNM2», un precio, un número suelto): 142 del ICCU, 1 del
   IDU y 1 del FFIE. Quién decide si una unidad es legible NO se reescribe
   aquí —una segunda definición divergiría de la primera corrección—: es
   `unidadIlegible`, la regla que ya censó esos 144 en `lib/apu/iccu_items`.
   El `require` va DIFERIDO y memoizado: ese módulo carga un JSON de 1 234
   ítems y sin unidad que comparar no hace falta abrirlo. */
let _unidadIlegible = null;
const unidadIlegibleRef = (u) => {
  if (!_unidadIlegible) _unidadIlegible = require("./iccu_items.js").unidadIlegible;
  return _unidadIlegible(u);
};
/** true SOLO si las dos unidades se leen: ausente o rota ⇒ no hay comparación. */
function unidadesComparables(unidadFila, unidadCatalogo) {
  if (unidadCanonica(unidadFila) == null || unidadCanonica(unidadCatalogo) == null) return false;
  return !unidadIlegibleRef(unidadFila) && !unidadIlegibleRef(unidadCatalogo);
}
...
function mapearFilasImportadas(filas, catalogo, opciones = {}) {
  ...
  const precioDe = (codigo) => {
    ...
    try {
      ...
      /* ⚠️ EL `regionId` TAMBIÉN VIAJA (27-ago-2026): sin él, el nivel
         `catalogo` de la cascada caía a la región base (Bogotá) aunque el
         usuario hubiera declarado su departamento — la vista previa enseñaba
         un precio y «Calcular» otro del MISMO ítem (medido: hasta 10 % en la
         Costa), y `variantes[].precio` es justo la cifra con la que se elige
         variante. Es la MISMA resolución del handler de `calcular`
         (`regionDeDepartamento`, el punto único de paso). */
      const { regionDeDepartamento } = require("./tipologias.js");
      const reg = regionDeDepartamento(String(departamento || ""));
      /* ⚠️ SIN REGIÓN MAPEADA SE CAE A LA REGIÓN BASE, NUNCA A `null` (la
         revisión adversaria del propio arreglo, 27-ago-2026): un `regionId:
         null` EXPLÍCITO apaga el nivel `catalogo` de la cascada (`costoDirecto`
         solo aplica su default ante undefined) y la vista previa perdía el
         precio de TODOS los ítems del catálogo justo en el estado por defecto
         —sin departamento elegido— y en los 19 departamentos sin factor
         regional, Tolima incluido. Es lo mismo que hace el motor: sin región
         cotizada calcula con la base y lo dice. */
      const regionId = reg && reg.estado === "mapeado"
        ? reg.region
        : ((catalogo && catalogo._meta && catalogo._meta.region_base) || "bogota_sabana");
      const r = cotizarItem({ item_id: k, cantidad: 1 }, { cat: catalogo, regionId, departamento });
  ...
  const salida = (filas || []).map((cruda, i) => {
    ...
    /* Variantes de la misma cabecera que el elegido: se publican, no se
       esconden — y solo LAS QUE SE MIDEN IGUAL, que es lo que este comentario
       decía desde siempre y el filtro no hacía.
       POR QUÉ IMPORTA (mismo defecto que el de arriba, otro campo): la vista
       previa pinta con estas variantes un RANGO de precio («de X a Y · se tomó
       Z»), que es justo la cifra con la que la persona elige. Con una variante
       de otra unidad ese rango compara peras con manzanas: medido sobre los
       cinco bancos, 3 de 34 filas con variantes traían alguna de otra unidad, y
       la peor es «DESMONTE Y LIMPIEZA EN BOSQUE» — elegido por hectárea a
       $9.337.434 y acompañado de INVIAS:200,1,2 a $1.059 el m², que se leería
       como un ahorro de 8 817 veces que no existe.
       No se descarta por ignorancia: si las dos unidades son igual de
       ilegibles, `unidadCanonica` da lo mismo en las dos y la variante entra.
       Las apartadas se CUENTAN (`variantes_otra_unidad`), que es lo contrario
       de esconderlas en silencio. */
    const mismaUnidadQueElElegido = (c) => unidadCanonica(c.unidad) === unidadCanonica(mejor.unidad);
    const hermanas = mejor && BANCOS_OFICIALES.has(mejor.fuente_mapeo)
      ? candidatos.slice(1).filter((c) => c.fuente_mapeo === mejor.fuente_mapeo && c._familia.clave === mejor._familia.clave)
      : [];
    const variantes = hermanas.filter(mismaUnidadQueElElegido)
      .map((c) => ({ codigo: c.codigo, descripcion: c.descripcion, unidad: c.unidad, precio: precioDe(c.codigo) }));
    const variantesOtraUnidad = hermanas.length - variantes.length;
    ...
    /* ═══ LA UNIDAD CRUZADA NO DA UN EMPAREJAMIENTO AUTOMÁTICO ═══════════════
       La unidad pesa 0,13 sobre 1,00, así que perderla no impedía llegar a
       «firme»: 100 m² de grouting casaban en firme con IDU:3730, que el IDU
       paga por m³ a $661.296, y salían presupuestados en $66.129.600 con la
       pantalla y el Excel perfectamente cuadrados («m3 · 100 · 661.296»).
       Medido sobre los bancos reales: 25 de 25 filas con la unidad cruzada a
       propósito salían en firme. Aquí el falso positivo es el caro y se paga en
       pesos, así que el emparejamiento CAE A «revisar» —se sigue usando, con
       su sugerencia y su precio, pero lo confirma la persona en la vista
       previa—, que es la doctrina que este módulo ya aplica al resto.

       SOLO CUANDO LAS DOS UNIDADES SE PUEDEN LEER. Degradar por una unidad
       ilegible convertiría «no sé» en «está mal», que es el error simétrico:
       144 de los 6 414 ítems de los bancos traen la unidad rota por el
       corrimiento de columnas del PDF de origen (142 del ICCU, 1 del IDU,
       1 del FFIE), y sus filas seguirían siendo el mismo ítem. Quién decide si
       una unidad es legible NO se reescribe aquí: es `unidadIlegible`, la regla
       que ya censó esos 144 (`lib/apu/iccu_items`).
       La vía de COBERTURA TOTAL no se toca: ya exigía `unidadIgual`. */
    const unidadComparable = acepta && unidadesComparables(unidad, mejor.unidad);
    /* MISMO significado que en la ruta del lector de pliegos (`lib/apu_mapeo`):
       hay unidad en la fila y no es la del catálogo. La grafía se compara
       canónica («UND»≈«un», «M»≈«ml») porque aquí la escribe un Excel. */
    const unidadDiscrepante = !!(acepta && unidadCan != null && unidadCan !== unidadCanonica(mejor.unidad));
    ...
    const firme = acepta && mejor.score >= UMBRAL_FIRME && mejor.coincidencias.length >= MIN_COINCIDENCIAS_FIRME
      ...
      && !(unidadComparable && unidadDiscrepante);
    ...
    return {
      ...
      /* hermanas de la misma cabecera que se miden en OTRA unidad: no entran al
         rango de precio porque no son alternativas comparables, pero se dicen */
      variantes_otra_unidad: acepta ? variantesOtraUnidad : 0,
      ...
      unidad_discrepante: unidadDiscrepante,
      /* «distinta» y «no se pudo comparar» son DOS cosas, y con un solo campo
         se leían igual: `unidad_comparable` dice si las dos unidades eran
         legibles. Es lo que separa degradar de no degradar, y lo que permite a
         la pantalla decir «el catálogo la mide en m3» en vez de callar. */
      unidad_comparable: unidadComparable,
      ...
      entrada_calculo: {
        ...
        descripcion,
        unidad,
        /* LA UNIDAD DEL CATÁLOGO VIAJA AL LADO DE LA DEL PLIEGO, no en lugar de
           ella. `unidad` sigue siendo la del PLIEGO —es la que la entidad va a
           pagar y la única que no es una suposición nuestra— y no se convierte
           jamás (pasar m² a m³ exige un espesor que el catálogo no conoce).
           Antes la discrepancia moría aquí: la fila la publicaba y la entrada al
           cálculo no, así que la pantalla que pinta el presupuesto ya no tenía
           con qué avisar y el ítem salía cuadrado con la unidad del banco. */
        unidad_catalogo: acepta ? mejor.unidad : null,
        unidad_discrepante: unidadDiscrepante,
        unidad_comparable: unidadComparable,
        capitulo: String(fila.capitulo || "").slice(0, 160) || null,
  ...
  const conUnidadDiscrepante = salida.filter((f) => f.unidad_discrepante).length;

  /* AVISOS DE LA IMPORTACIÓN, como ya los publica la ruta hermana del lector de
     pliegos (`lib/apu_extraer`): una fila puede traer su marca y aun así nadie
     mirarla, porque en una tabla de 150 filas lo que se lee es el resumen.
     La frase de la unidad es LA MISMA que la de esa ruta —dos redacciones para
     la misma advertencia se contradicen a la primera corrección— con la
     consecuencia propia de aquí añadida. */
  const avisos = [];
  if (conUnidadDiscrepante) {
    avisos.push(`${conUnidadDiscrepante} fila(s) traen una unidad distinta a la del catálogo. Se conserva la `
      + "DEL PLIEGO y no se convierte: pasar m² a m³ exigiría un espesor que el catálogo no conoce. "
      + "Revise esas filas antes de calcular: con la unidad cruzada, el precio del banco se cobraría por una "
      + "medida que no es la que paga la entidad.");
  }
  ...
  return {
    ...
    resumen_mapeo: {
      ...
      /* MISMO nombre que en `lib/apu_mapeo.mapearTabla`: es el mismo hecho
         contado en la otra ruta, y dos nombres para lo mismo divergen. */
      con_unidad_discrepante: conUnidadDiscrepante,
