...
function mapearFilasImportadas(filas, catalogo, opciones = {}) {
  ...
  const todos = conInvias ? preparados.concat(candidatosInvias()) : preparados;
  ...
  const familiaDe = (it) => {
    if (it.es_invias) {
      ...
      return { clave: `inv:${cab}`, articulo: art || null };
    }
  ...
  // desempate a puntaje IGUAL: catálogo (contrato/derivación) > INVIAS oficial > estimación del catálogo
  const rango = (it) => (it.es_invias ? 1 : it.fuente === "estimado" ? 2 : 0);
  ...
  const salida = (filas || []).map((cruda, i) => {
    ...
    if (terminos.length) {
      candidatos = todos
        .map((it) => ({ codigo: it.codigo, descripcion: it.descripcion, unidad: it.unidad, fuente_mapeo: it.es_invias ? "invias" : "catalogo",
    ...
    const variantes = mejor && mejor.fuente_mapeo === "invias"
      ? candidatos.slice(1).filter((c) => c.fuente_mapeo === "invias" && c._familia.clave === mejor._familia.clave)
