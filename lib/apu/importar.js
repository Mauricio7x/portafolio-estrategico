...
function mapearFilasImportadas(filas, catalogo, opciones = {}) {
  ...
  const todos = conInvias ? preparados.concat(candidatosInvias(), candidatosIdu()) : preparados;
  ...
  const familiaDe = (it) => {
    ...
    if (it.es_idu) return { clave: `oficial:${norm(String(it.descripcion || "").split("(")[0])}`, articulo: null };
    if (it.es_invias) {
  ...
  const enBogota = /bogot/i.test(String(departamento || "").normalize("NFD").replace(/[̀-ͯ]/g, ""));
  const rango = (it) => (it.es_idu ? (enBogota ? 1 : 2) : it.es_invias ? (enBogota ? 2 : 1) : it.fuente === "estimado" ? 3 : 0);
  ...
  const salida = (filas || []).map((cruda, i) => {
    ...
    if (terminos.length) {
      ...
      candidatos = todos
        .filter((it) => terminos.some((x) => it._set.has(x)))
        .map((it) => ({ codigo: it.codigo, descripcion: it.descripcion, unidad: it.unidad, fuente_mapeo: it.es_invias ? "invias" : it.es_idu ? "idu" : "catalogo",
    ...
    const variantes = mejor && (mejor.fuente_mapeo === "invias" || mejor.fuente_mapeo === "idu")
