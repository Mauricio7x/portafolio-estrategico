...
/* La mediana es la de `lib/estadistica`, no una copia: esta promediaba el par
   central y lo REDONDEABA a peso entero ahí mismo (una cifra redondeada para
   mostrar no puede decidir; aquí se redondea una sola vez, al devolver). */
const { mediana } = require("../estadistica.js");
...
function precioReferencia(codigoOId, { municipio = null, departamento = null } = {}) {
  ...
  if (hallado && porProv[hallado.provincia]) {
    ...
    /* EXCEPCIÓN DECLARADA a «sin dato ≠ cero»: aquí el 0 NO es un dato. Un
       ítem de obra a $0 no existe, y el capturador ya lo zanjó —
       `tests/capturar_iccu_apu.js:90` solo acepta lecturas `> 0`, así que una
       celda en blanco o ilegible del PDF nunca llega como 0 sino como ausencia
       (medido: 0 ceros en las 14.138 medianas por provincia del banco). Dejar
       entrar un 0 hundiría la mediana y publicaría un precio de referencia más
       barato de lo real, que en APU es el falso caro: el falso POSITIVO. */
    const medianas = Object.values(porProv).map((r) => r.mediana).filter((n) => n > 0);
    precio = mediana(medianas);   // sin redondear: el redondeo va al devolver
