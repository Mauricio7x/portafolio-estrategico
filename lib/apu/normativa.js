...
function normativaAplicada(catalogo, regionId) {
  ...
  try {
    ...
    /* Las dos grafías de la MISMA clave: la semilla del repositorio la llama
       `id` y el hash de Redis la sirve como `region` (lib/apu/catalogo.
       regionDeHash). Buscando solo por `id`, con el catálogo cargado —o sea en
       producción— no encontraba ninguna región y el panel decía «sin factor
       prestacional» mientras el motor multiplicaba los jornales por 1,55. */
    const region = regionId ? lista.find((r) => r && (r.id === regionId || r.region === regionId)) : null;
