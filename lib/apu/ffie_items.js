...
/* La mediana es la de `lib/estadistica`, no una copia: esta tomaba `todos[n>>1]`
   y redondeaba el par central a peso entero AL CALCULAR. El redondeo va al
   devolver, una sola vez. */
...
function precioReferencia(codigoOId, departamento = null) {
  ...
  if (literal && it.precios[literal] > 0) {
    ...
    /* EXCEPCIÓN DECLARADA a «sin dato ≠ cero», la misma del ICCU: el FFIE
       publica un precio TOPE por departamento y un tope de $0 no existe. El
       capturador (`tests/capturar_ffie_apu.js:122`) ni siquiera escribe la
       clave si la celda no es `> 0`, así que un 0 aquí sería una lectura
       ilegible, no un dato (medido: 0 ceros en los 34.386 precios del banco). */
    const todos = Object.values(it.precios).filter((n) => n > 0);
    precio = mediana(todos);   // sin redondear: el redondeo va al devolver
