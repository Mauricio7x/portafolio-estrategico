/* ============================================================================
   ...
   · NO entra en `costoDirecto`. El costo directo sale del catálogo calibrado
     ...
     negociar, con rezago declarado (vigencia 2025-1: la 2025-2 de la API está
     corrupta en origen — ver tests/capturar_invias.js).
...
function referenciaInvias(insumoId, departamento = null) {
  ...
  if (locales && locales.length) {
    ...
    alcance = dep && dep.startsWith("BOGOTA")
      ...
      : dep
        ? "mediana nacional — tu departamento no está en la captura"
        : "mediana nacional";
  }
...
function coberturaInvias(departamento) {
  ...
  return { cubierto: false, motivo: "Tu departamento no está en la captura INVIAS: la referencia que se enseña es la mediana nacional, declarada." };
}
