...
function provinciasPara(it, departamento) {
  ...
  let usadas = (d ? INDICES_POR_DEP.get(d.codigo) || [] : []).filter((i) => Number.isFinite(it.precios[i]));
  let nivel = "departamento";
  if (!usadas.length) {
    usadas = (DATOS.provincias || []).map((_, i) => i).filter((i) => Number.isFinite(it.precios[i]));
    nivel = "nacional";
  }
