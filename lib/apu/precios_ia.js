...
async function despertarRutina({ id, perfil, fetchImpl = null, tiempoMs = RUTINA_TIEMPO_MS } = {}) {
  ...
  /* `motivo` es para la PANTALLA, en palabras del usuario y sin nombres del sistema;
     `detalle` es para quien configura (sale por op=ia y va en la guía de variables).
     La revisión adversaria del 13-sep-2026 encontró «Vercel», «token» y
     «claude.ai/code/routines» en la pantalla: era un solo campo. */
  ...
  if (!RUTINA_URL_RE.test(url)) return fallo(null, "la búsqueda automática no está bien configurada", "RUTINA_PRECIOS_URL no tiene la forma de la dirección de disparo de una rutina (…/v1/claude_code/routines/<id>/fire)");
  /* el texto lleva SOLO el identificador y el perfil: la sesión lee lo demás de la cola */
  const texto = `Solicitud de Precios de Detekta: id_borrador=${id} perfil=${perfil}. Ejecute /precios ${id} ${perfil}.`;
  ...
  try {
    ...
    if (nombre === "TimeoutError" || nombre === "AbortError") return fallo(null, "la búsqueda automática no respondió a tiempo y puede haber arrancado igual", `tiempo agotado (${tiempoMs} ms) en el disparo de la rutina`, { indeterminada: true });
    return fallo(null, "no se pudo llamar a la búsqueda automática", `no se pudo llamar a la rutina (${e && e.message ? e.message : e})`);
  }
  let json = null;
  try { json = await r.json(); } catch { json = null; }
  const status = Number(r.status);
  if (!r.ok) {
    if (status === 401) return fallo(status, "el acceso a la búsqueda automática fue rechazado", "401: el token de la rutina fue rechazado — regenérelo en claude.ai/code/routines y péguelo en Vercel como RUTINA_PRECIOS_TOKEN");
    if (status === 403) return fallo(status, "la cuenta no tiene permiso para la búsqueda automática", "403 (permission_error): las rutinas están desactivadas para la cuenta o el token no es de esta rutina");
    if (status === 404) return fallo(status, "la búsqueda automática no está bien configurada", "404: la rutina no existe con ese identificador — revise RUTINA_PRECIOS_URL");
    if (status === 429) return fallo(status, "se agotó por hoy el cupo de búsquedas automáticas", "429: tope diario de corridas de rutinas o cuota de la suscripción agotada");
    return fallo(status, "la búsqueda automática no respondió bien", `la rutina respondió ${status}`);
  }
