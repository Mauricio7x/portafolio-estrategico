...
function calcularItem(entrada, cat, regionId, preciosUsuario = null, departamento = null, opcionesCosto = {}) {
  ...
  const refIdu = !def && !apuInvias && IduItems.esCodigoIdu(codigo) ? IduItems.precioReferencia(codigo, departamento) : null;
  ...
  if (manual !== null && manual > 0) {
    ...
    return {
      ...
      descripcion: descripcionEntrada || (def ? def.descripcion : null) || (apuInvias ? apuInvias.descripcion : null) || (refIdu ? refIdu.descripcion : null),
      unidad: unidadEntrada || (def ? def.unidad : null) || (apuInvias ? apuInvias.unidad : null) || (refIdu ? refIdu.unidad : null),
      capitulo: capituloEntrada || (def ? def.capitulo : null) || (apuInvias ? apuInvias.capitulo : null) || (refIdu ? refIdu.capitulo : null) || null,
      fuente: null,
      cantidad,
      sin_apu: true,
      ...
      cascada: require("./precios.js").explicarCascada(codigo, {
        preciosUsuario, enCatalogo: !!def, enInvias: !!apuInvias, enIdu: !!refIdu, elegido: origenManual,
      }),
      ...
      cd_catalogo: cdCat && !cdCat.error ? cdCat.total : (apuInvias ? unitarioInvias(apuInvias) : (refIdu ? refIdu.precio : null)),
...
function calcularPresupuesto({ items = [], departamento = "", config = {}, catalogo = null, preciosUsuario = null, parametros = null } = {}) {
  ...
  const valorAdministracion = red(baseAiu * (cfg.aiu_pct / 100));
  const valorImprevistos = red(baseAiu * (cfg.imprevistos_pct / 100));
  const valorUtilidad = red(baseAiu * (cfg.utilidad_pct / 100));
  ...
  const manualesPropios = manuales.filter((i) => i.origen_precio !== "idu");
