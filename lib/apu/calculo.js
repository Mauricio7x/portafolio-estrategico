...
function calcularItem(entrada, cat, regionId) {
  ...
  const def = (cat.items || []).find((i) => String(i.codigo) === codigo) || null;
  const cantidad = numero(entrada && entrada.cantidad, 0);

  if (!def) {
    return {
      item_id: codigo || null, descripcion: null, unidad: null, cantidad,
      incompleto: true, motivo: "item_desconocido",
      mensaje: `El ítem «${codigo}» no existe en el catálogo cargado.`,
  ...
  return {
    ...
    detalle: {
      ...
      insumos: (def.insumos || []).map((l) => {
        const ins = (cat.insumos || []).find((x) => x.id === l.insumo_id) || null;
        return {
          insumo_id: l.insumo_id,
          nombre: ins ? ins.nombre : null,
          unidad: ins ? ins.unidad : null,
          tipo: ins ? ins.tipo : null,
          cantidad_por_unidad: l.cantidad_por_unidad ?? null,
          rendimiento: l.rendimiento ?? null,
