...
function calcularItem(entrada, cat, regionId) {
  ...
  const def = codigo ? (cat.items || []).find((i) => String(i.codigo) === codigo) || null : null;
  ...
  const manual = numero(entrada && entrada.precio_manual, null);
  ...
  return {
    ...
    detalle: {
      ...
      insumos: cd.lineas.map((l) => ({
        ...
        cantidad: l.cantidad,
        cantidad_por_unidad: l.rendimiento != null ? null : l.cantidad,
