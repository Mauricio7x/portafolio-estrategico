...
function normalizarConfig(cruda = {}) {
  ...
  return {
    ...
    /* LA CONTRIBUCIÓN DEL 5 % NO SE LE COBRA A TODO (12-sep-2026). El art. 120
       de la Ley 418/1997 grava los contratos de OBRA PÚBLICA; una interventoría
       o una consultoría son contratos de consultoría (Ley 80/1993 art. 32
       num. 2) y no la causan, y el usuario puede declarar que su administración
       ya la lleva dentro. Este módulo NO decide eso —no conoce el tipo de
       trabajo— así que RECIBE el porcentaje ya resuelto por la única regla que
       existe (`lib/ganancia.aplicaContribucion`, que es la que ya honra
       `lib/handlers/apu/editor` para el panel Piso/Techo y la que
       `lib/baja_maxima.contribucionPctDe` aplica al listado). Escribir aquí una
       segunda lista de «esto no es obra» pondría a la tarjeta, al piso y al
       margen a decir tres cifras del mismo proceso.
       AUSENTE ≠ 0: sin declararlo se cobra el 5 %, que es el comportamiento
       anterior y el prudente (ante la duda, no prometer). Un 0 EXPLÍCITO es el
       único que la apaga. */
    contribucion_pct: acotar(numero(c.contribucion_pct, CONTRIBUCION_OBRA_PUBLICA), 0, 100),
...
function calcularItem(entrada, cat, regionId, preciosUsuario = null, departamento = null, opcionesCosto = {}) {
  ...
  /* ═══ LA UNIDAD QUE SE PUBLICA ES LA DEL PLIEGO (13-sep-2026) ════════════
     La referencia de precio que respondió la cascada, resuelta UNA vez: el
     catálogo o el banco que ganó. Son EXCLUYENTES por construcción (cada uno
     exige `!` de los anteriores), así que `ref` es exactamente el que contestó.

     EL DEFECTO QUE CIERRA: las seis ramas publicaban `unidad: <referencia>.unidad
     || unidadEntrada` (y la del catálogo, `def.unidad` a secas), o sea la unidad
     de la REFERENCIA, borrando la del PLIEGO — que es la única que no es una
     suposición nuestra, porque es por la que la entidad va a pagar. Medido: 100 m²
     de grouting contra IDU:3730 (m³, $661.296) salían presupuestados en
     $66.129.600 con la pantalla y el Excel perfectamente cuadrados —«m3 · 100 ·
     661.296»— y el m² del pliego desaparecido. La doctrina ya existía en las dos
     rutas de mapeo (`lib/apu_mapeo`, `lib/apu/importar`): un dato PUBLICADO gana
     a uno de referencia, y la unidad NO se convierte jamás. Faltaba el motor.

     Quién decide «¿son la misma unidad?» y «¿se pueden leer las dos?» NO se
     reescribe aquí: son `importar.unidadCanonica` (la que ya tolera «UND»≈«un»,
     «M»≈«ml» y las compuestas «m2/mes») e `iccu_items.unidadIlegible` (la que ya
     censó los 144 ítems con la unidad rota por el corrimiento del PDF de origen).
     Dos definiciones divergirían a la primera corrección.

     AUSENCIA ≠ CONTRADICCIÓN (R1): si falta cualquiera de las dos unidades no
     hay discrepancia que declarar, y `unidad` cae a la de la referencia como
     hasta ahora. `unidad_comparable` separa «distinta» de «no se pudo comparar»,
     con el MISMO significado y los MISMOS nombres que ya publica la importación,
     para que la pantalla los lea una sola vez. */
  const ref = def || apuInvias || apuEpc || refIdu || refFfie || refIccu || null;
  const uni = (() => {
    const canonica = require("./importar.js").unidadCanonica;
    const ilegible = require("./iccu_items.js").unidadIlegible;
    const unidadRef = (ref && ref.unidad) || null;
    const canFila = canonica(unidadEntrada);
    const canRef = canonica(unidadRef);
    return {
      unidad: unidadEntrada || unidadRef,          // el PLIEGO manda; su grafía, tal cual
      unidad_catalogo: unidadRef,                  // la de la referencia, al lado y nunca en su lugar
      unidad_discrepante: !!(canFila != null && canRef != null && canFila !== canRef),
      unidad_comparable: canFila != null && canRef != null
        && !ilegible(unidadEntrada) && !ilegible(unidadRef),
    };
  })();
  ...
  if (manual !== null && manual > 0) {
    ...
    return {
      ...
      /* `unidad` (la del pliego) + la de la referencia + las dos marcas. Aquí NO
         bloquea: el precio lo puso el usuario y se cotiza por la unidad que él
         escribió, que es justo la salida que deja abierta la rama de abajo. Se
         declara igual porque `cd_catalogo` enseña al lado el precio de la
         referencia, y comparar $/m² con $/m³ como si fuera un ahorro es el
         mismo error de categoría con otro nombre. */
      ...uni,
  ...
  /* ═══ CON LA UNIDAD CRUZADA NO SE PRESUPUESTA (13-sep-2026) ══════════════
     Publicar la unidad del pliego arregla el rótulo, no la aritmética: el
     unitario del banco es por SU unidad, así que `cantidad × precio` con la
     cantidad del pliego sigue siendo un error de categoría — y ahora, encima,
     bien maquetado. No hay factor que lo salve: pasar m² a m³ exige un espesor
     que ninguna referencia publica, y un espesor inventado es exactamente la
     cifra creíble y equivocada que este módulo existe para evitar.
     Aquí el falso caro es el POSITIVO (la doctrina de APU/precios, al revés que
     en oportunidades): ante la duda NO se presupuesta. El ítem sale
     `incompleto`, no suma al total y dice qué hacer — la misma forma que
     `sin_precio`, donde ya está escrito que un 0 sería un precio inventado.
     La referencia NO se esconde: viaja el precio en `cd_catalogo` y el bloque
     `referencia_*_apu` entero, para que la persona vea la cifra y decida.
     LA SALIDA ESTÁ ARRIBA y es la de siempre: un precio propio, del archivo o
     tecleado por la unidad del pliego manda sobre la referencia y no se bloquea.
     SOLO CUANDO LAS DOS UNIDADES SE LEEN: degradar por una unidad rota
     convertiría «no sé» en «está mal», que es el error simétrico. */
  if (uni.unidad_discrepante && uni.unidad_comparable) {
    const cdRef = def ? costoDirecto(def, cat, regionId, opcionesCosto) : null;
    const precioRef = def ? (cdRef && !cdRef.error ? cdRef.total : null)
      : apuInvias ? unitarioInvias(apuInvias)
        : apuEpc ? unitarioEpc(apuEpc)
          : refIdu ? refIdu.precio
            : refFfie ? refFfie.precio
              : refIccu ? refIccu.precio : null;
    const deQuien = def ? "el catálogo de costeo" : apuInvias ? "el INVIAS" : apuEpc ? "EPC"
      : refIdu ? "el IDU" : refFfie ? "el FFIE" : "el ICCU";
    return {
      item_id: def ? def.codigo : (codigo || null),
      codigo: codigoArchivo,
      descripcion: descripcionEntrada || (ref ? ref.descripcion : null),
      ...uni,
      capitulo: capituloEntrada || (ref ? ref.capitulo : null) || null,
      fuente: null,
      cantidad,
      ...(cantidadIlegible ? { cantidad_ilegible: true } : {}),
      sin_apu: true,
      origen_precio: null,
      incompleto: true,
      motivo: "unidad_no_aplicable",
      mensaje: `La entidad paga este ítem por ${uni.unidad} y ${deQuien} lo cotiza por `
        + `${uni.unidad_catalogo}${precioRef == null ? "" : ` (${precioRef.toLocaleString("es-CO")} COP por ${uni.unidad_catalogo})`}. `
        + `Ese precio no se puede cobrar por ${uni.unidad} y no se convierte: pasar de una medida a la `
        + `otra exigiría un dato que nadie publica (de m² a m³, el espesor). Escriba el precio unitario `
        + `por ${uni.unidad} o asígnele otro ítem. Mientras tanto NO suma al total: cobrar por una medida `
        + "que no es la que paga la entidad sería una cifra inventada.",
      /* la referencia se VE, para poder decidir: el precio y su bloque entero */
      cd_catalogo: precioRef == null ? null : red(precioRef),
      referencia_invias_apu: apuInvias ? resumenInvias(apuInvias) : null,
      referencia_epc_apu: apuEpc ? resumenEpc(apuEpc) : null,
      referencia_idu_apu: refIdu || null,
      referencia_ffie_apu: refFfie || null,
      ...
      referencia_tienda: referenciaTiendaDirecta(),
      cascada: require("./precios.js").explicarCascada(codigo, {
        preciosUsuario, enCatalogo: !!def, enInvias: !!apuInvias, enEpc: !!apuEpc, enIdu: !!refIdu, enFfie: !!refFfie, enIccu: !!refIccu, elegido: null,
      }),
      rendimiento_dia: null,
      rendimiento_es_override: false,
      costo_material_unitario: null,
  ...
  if (cd.error) {
    return {
      item_id: def.codigo, descripcion: def.descripcion, ...uni, cantidad,
      ...(cantidadIlegible ? { cantidad_ilegible: true } : {}),
...
function calcularPresupuesto({ items = [], departamento = "", config = {}, catalogo = null, preciosUsuario = null, parametros = null } = {}) {
  ...
  const contribucion = red(precioFinal * (cfg.contribucion_pct / 100));
  ...
  if (fuenteCatalogo === "semilla") {
    alertas.push("El catálogo de precios NO está cargado en Redis: se usó la semilla del repositorio. Cárguelo en Mi empresa → Catálogo de precios de referencia → «Cargar catálogo APU» para trabajar con la versión vigente.");
  }
  ...
  if (cfg.aplicar_ajuste_competitivo && margenFinal <= 0) {
    alertas.push("Atención: con esa baja el precio final NO cubre ni el costo directo: presentarse sería trabajar a pérdida.");
  ...
  /* ESTA ALERTA INVITABA A COBRAR EL 5 % DOS VECES (12-sep-2026). Decía
     «Cárguelas en deducciones de acta (%)» refiriéndose a la contribución Y a
     las estampillas, mientras el pie del propio campo en la pantalla dice desde
     ago 2026 «Solo lo que se PIERDE: estampillas y ReteICA del pliego… La
     contribución del 5 % ya se descuenta aparte». Dos textos contradictorios
     sobre el mismo campo, y obedecer al de aquí sumaba la contribución al τ que
     `lib/apu/rentabilidad` y `lib/apu/piso_techo` ya le cargan: medido, cargar
     un 7 % sobre un precio de $308.500.000 subía τ a $37.020.000 —$15.425.000
     de sobrecargo— y hundía la utilidad esperada de un presupuesto sano. Ahora
     pide SOLO lo que se pierde y dice dónde está ya contada la contribución. */
  if (cfg.deducciones_pct === null) {
    alertas.push("Las deducciones de acta no están cargadas: el margen que ve es una COTA SUPERIOR. "
      + "Escriba en «deducciones de acta (%)» SOLO lo que se PIERDE de cada pago: las estampillas y el "
      + "ReteICA que fije el pliego."
      + (cfg.contribucion_pct > 0
        ? ` La contribución especial de obra pública (${cfg.contribucion_pct} % del valor del contrato, `
          + `Ley 418/1997) ya se descuenta aparte en el precio mínimo y en la rentabilidad —sobre este precio `
          + `sería ${contribucion == null ? "—" : contribucion.toLocaleString("es-CO")} COP—, así que escribirla `
          + "también aquí la cobraría dos veces."
        : " Este contrato no causa la contribución especial de obra pública, así que tampoco va aquí."));
  }
  ...
  return {
    ...
    configuracion: {
      ...
      /* CON QUÉ PORCENTAJE SE CALCULÓ LA CONTRIBUCIÓN. Viaja para poder
         discutirlo: un 0 en `contribucion_obra_publica` es «este contrato no la
         causa» y no «se me olvidó», y sin declararlo las dos lecturas son
         indistinguibles. */
      contribucion_pct: cfg.contribucion_pct,
      cuantia_cop: cfg.cuantia_cop,
    },
    validaciones,
    items: calculados,
