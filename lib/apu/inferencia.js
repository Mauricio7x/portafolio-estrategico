...
const { TIPOLOGIAS, tipologiaPorCodigo, itemsDeTipologia, itemPorCodigo, tipologiasDeFamilia } = require("./catalogo.js");
...
function inferir(objeto, opts = {}) {
  ...
  const items = itemsDeTipologia(mejor.codigo).map((c) => {
    const it = itemPorCodigo(c);
    return it ? { codigo: it.codigo, descripcion: it.descripcion, unidad: it.unidad, cantidad: null } : null;
  }).filter(Boolean);
