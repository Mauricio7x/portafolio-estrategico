...
function coberturaRetail(departamento) {
  ...
  if (motivo) return { cubierto: false, motivo };
  /* LA COBERTURA SE DERIVA DE LO QUE DECIDE EL PRECIO, no de una lista paralela
     (ago 2026). La lista solo enumera los 8 departamentos donde la captura
     FALLÓ; Cundinamarca no está en ninguno de los dos grupos (su capital es
     Bogotá, que tiene clave propia), así que se respondía «cubierto: true»
     mientras cada precio decía «Bogotá — sin captura retail en su
     departamento»: dos mitades de la misma respuesta contradiciéndose, y en el
     departamento donde este módulo más se usa. Cubierto es, exactamente, tener
     captura propia. */
  const dep = norm(departamento);
  const tieneCaptura = (DATOS.referencias || []).some((ref) => ref.alcance === "por_capital"
    && Object.keys(ref.precios_por_departamento || {}).some((k) => norm(k) === dep));
  return tieneCaptura
    ? { cubierto: true, motivo: null }
    : { cubierto: false, motivo: "no se capturó la capital de este departamento: la referencia sale del precio de Bogotá" };
}
...
function resolverPrecio(ref, departamento) {
  if (ref.alcance === "por_capital") {
    ...
    return {
      precio: bogota,
      ambito: dep
        ? "Bogotá — sin captura retail en su departamento"
        : "Bogotá (referencia)",
    };
  }
