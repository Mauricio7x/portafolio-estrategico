...
/* ─────────────────── nivel 4 · el catálogo con factor regional ─────────────
   ...
   tiempo de carga (R5). */
function delCatalogo(itemId, cat, regionId) {
  const def = ((cat && cat.items) || []).find((i) => String(i.codigo) === String(itemId));
  if (!def) return { hay: false, motivo: MOTIVOS.no_esta_en_catalogo };
  const { costoDirecto } = require("./catalogo.js");
  const cd = costoDirecto(def, cat, regionId);
...
function cotizarItem(item, { cat, regionId, preciosUsuario = null } = {}) {
  ...
  if (!elegido) {
    ...
    elegido = registrar("catalogo", delCatalogo(itemId, cat, regionId));
  }
...
/* ═══════════════════════ cotizar una LISTA ════════════════════════════════ */
function cotizar({ items = [], catalogo = null, region = null, preciosUsuario = null } = {}) {
...
module.exports = {
  cotizar, cotizarItem, referenciaDeMercado, medianaDe,
