/* ============================================================================
   ...
   El encargo pedía cinco niveles y ponía en el tercero «precios estimados del
   ...
   ══════════════════════ LA CASCADA · 12 NIVELES ═════════════════════════════

   LA LISTA DE ABAJO NO SE ESCRIBE A MANO: una prueba la EXTRAE de este
   comentario y la compara, id a id y EN ORDEN, contra `NIVELES`. Decía «LOS
   CINCO NIVELES» y enumeraba cinco cuando ya eran doce, con `catalogo` en el
   puesto 4 (es el 6) y `sin_precio` en el 5 (es el 12): quien leyera esta
   cabecera para saber el orden de la cascada leía un orden FALSO, y `retail` e
   `invias` no aparecían. Documentación falsa dentro del módulo que fija el
   precio de una oferta. Si añade un banco y no toca esta lista, la suite cae.
     ...
     4 · retail      Techo de tienda o lista de fabricante. Es un techo POR
                     INSUMO y jamás un precio de ítem: se declara siempre con
                     `techo_de_insumo`.
     5 · invias      Referencia oficial del banco de INSUMOS del INVIAS, por
                     provincia. También por insumo: `referencia_oficial_insumo`.
     6 · catalogo    El catálogo base (contrato Nogal 4) con el factor de la
                     región. Es lo que había antes de este módulo.
     7 · invias_apu  APU de referencia del INVIAS para el ítem de pago, en la
                     provincia representativa del departamento. CON composición.
     8 · epc_apu     APU de Empresas Públicas de Cundinamarca. CON composición.
     9 · idu_apu     Precio de referencia del IDU (Bogotá). SIN composición, y
                     fuera de Bogotá va sin ajuste: las dos cosas se declaran.
    10 · ffie_apu    Precio TOPE del FFIE, cobertura nacional. SIN composición.
                     Es un techo, no una cotización: leerlo como precio de
                     mercado es el mismo error de categoría que el retail. Por
                     eso va detrás de los demás bancos.
    11 · iccu_apu    Precio de referencia de la Gobernación de Cundinamarca,
                     con el nivel usado (municipio, provincia o departamento).
                     SIN composición.
    12 · sin_precio  No hay dato. El ítem NO suma al total y se pide el precio.
                     Un 0 aquí sería un precio inventado (R1).

   Los bancos oficiales (7-11) tienen CADA UNO su propio espacio de códigos
   (`INVIAS:…`, `IDU:…`), así que para un ítem dado solo responde uno: su orden
   relativo aquí NO decide qué fuente se usa. Quien lo decide es el MAPEO
   (`lib/apu/importar.js:rango`), que es donde el INVIAS cede por licencia.
...
const NIVELES = Object.freeze([
  {
    ...
    etiqueta: "Su precio",
    confianza: "alta",
    porque: "Lo corrigió usted para este ítem: es de su mercado y de su proveedor.",
  },
  {
    id: "pliego",
  ...
  },
  {
    ...
    confianza: "media",
    porque: "Precio oficial de referencia del insumo en su provincia (banco INVIAS, con rezago declarado): sirve para contrastar y negociar insumos, no para costear un ítem de obra.",
  },
  {
    id: "catalogo",
    etiqueta: "Catálogo de referencia",
    confianza: "media",
    porque: "Precio del contrato Nogal 4 (Bogotá 2025) ajustado por el factor de su región.",
  },
  {
    ...
    confianza: "media",
    porque: "Costo directo oficial del INVIAS para ese ítem de pago en la provincia representativa de su departamento (vigencia declarada). Es una referencia, no una cotización.",
  },
  {
    /* Los 440 APU de EPC (Cundinamarca, ago 2026): acueducto, alcantarillado y
       su obra civil, CON composición. Mismo criterio de orden que los otros dos
       bancos: espacio de códigos propio (`EPC:`), así que para un ítem dado solo
       responde uno y el orden entre ellos no decide nada. */
    id: "epc_apu",
    etiqueta: "APU de referencia EPC (Cundinamarca)",
    confianza: "media",
    porque: "Costo directo oficial de Empresas Públicas de Cundinamarca para esa actividad, con su composición (vigencia declarada). Es una referencia, no una cotización; fuera de Cundinamarca va sin ajuste y se dice.",
  },
  {
  ...
  {
    /* El precio TOPE del FFIE (edificación, 33 departamentos · ago 2026). VA
       ÚLTIMO entre los bancos a propósito: es un TECHO reconocido por una
       entidad, no un precio de mercado —el contrato adjudicado del dueño está
       POR ENCIMA—, así que cualquier otra fuente que responda es mejor. */
    id: "ffie_apu",
    etiqueta: "Precio tope FFIE",
    confianza: "baja",
    porque: "Precio TOPE que el FFIE reconoce para ese ítem de edificación en su departamento (vigencia declarada; sin composición). Es un techo de referencia, no una cotización: un contrato real puede estar por encima.",
  },
  {
    /* La lista del ICCU (Cundinamarca, por municipio · ago 2026). Va con los
       demás bancos: espacio de códigos propio (`ICCU:`), así que para un ítem
       dado solo responde uno. */
    id: "iccu_apu",
    etiqueta: "Precio de referencia ICCU (Cundinamarca)",
    confianza: "media",
    porque: "Precio de referencia de la Gobernación de Cundinamarca para ese ítem (vigencia declarada; sin composición), con el nivel usado —municipio, provincia o mediana del departamento— dicho en cada respuesta.",
  },
  {
    id: "sin_precio",
    etiqueta: "Sin precio",
...
const MOTIVOS = Object.freeze({
  ...
  no_cargado: "Todavía no ha corregido el precio de este ítem.",
  fuente_no_disponible: "Fuente no disponible en este despliegue.",
  ...
  no_es_item_epc: "No es una actividad del banco de APU de Empresas Públicas de Cundinamarca (acueducto, alcantarillado y su obra civil).",
  no_es_item_ffie: "No es un ítem del listado de precios tope del FFIE (edificación).",
  no_es_item_iccu: "No es un ítem de la lista de precios del ICCU (Cundinamarca).",
...
/* ─────────────────── nivel 1 · usuario · los precios del contratista ─────────────────
...
/* ─────────────────── nivel 6 · catalogo · el catálogo con factor regional ─────────────
...
/* ─────────────────── nivel 7 · invias_apu · el APU de referencia INVIAS ──────────────────
...
/* ─────────────────── nivel · el APU de referencia EPC (Cundinamarca) ───────
   Llama a `apuParaDepartamento`, la MISMA función que usa `calcular` (R2): la
   cotización y el cálculo no pueden dar dos unitarios distintos del mismo ítem. */
function delEpcApu(itemId, departamento) {
  const E = require("./epc_items.js");
  if (!E.esCodigoEpc(itemId)) return { hay: false, motivo: MOTIVOS.no_es_item_epc };
  const a = E.apuParaDepartamento(itemId, departamento || null);
  if (!a || !Number.isFinite(a.precio) || a.precio <= 0) return { hay: false, motivo: MOTIVOS.no_es_item_epc };
  return {
    hay: true,
    precio: E.unitarioMotor(a),
    detalle: {
      unidad: a.unidad || null, vigencia: a.vigencia, provincia: a.provincia,
      departamento: a.departamento, ajuste_regional: a.ajuste_regional, nota: a.nota_regional,
    },
  };
}

/* ─────────────────── nivel 9 · idu_apu · el precio de referencia IDU (Bogotá) ────────
...
/* ─────────────────── nivel · el precio TOPE del FFIE ───────────────────────
   Llama a `precioReferencia`, la MISMA función que usa `calcular` (R2). */
function delFfieApu(itemId, departamento) {
  const F = require("./ffie_items.js");
  if (!F.esCodigoFfie(itemId)) return { hay: false, motivo: MOTIVOS.no_es_item_ffie };
  const r = F.precioReferencia(itemId, departamento || null);
  if (!r || !Number.isFinite(r.precio) || r.precio <= 0) return { hay: false, motivo: MOTIVOS.no_es_item_ffie };
  return {
    hay: true,
    precio: r.precio,
    detalle: {
      unidad: r.unidad || null, vigencia: r.vigencia, nivel: r.nivel,
      departamento: r.departamento, departamento_usado: r.departamento_usado,
      es_tope: true, nota: [r.nota_tope, r.nota].filter(Boolean).join(" "),
    },
  };
}

/* ─────────────────── nivel · el precio de referencia ICCU ──────────────────
   Llama a `precioReferencia`, la MISMA función que usa `calcular` (R2). */
function delIccuApu(itemId, departamento, municipio) {
  const I = require("./iccu_items.js");
  if (!I.esCodigoIccu(itemId)) return { hay: false, motivo: MOTIVOS.no_es_item_iccu };
  const r = I.precioReferencia(itemId, { departamento: departamento || null, municipio: municipio || null });
  if (!r || !Number.isFinite(r.precio) || r.precio <= 0) return { hay: false, motivo: MOTIVOS.no_es_item_iccu };
  return {
    hay: true,
    precio: r.precio,
    detalle: {
      unidad: r.unidad || null, vigencia: r.vigencia, nivel: r.nivel,
      provincia: r.provincia, municipio: r.municipio, departamento: r.departamento,
      ajuste_regional: r.ajuste_regional, nota: [r.nota, r.nota_regional].filter(Boolean).join(" ") || null,
    },
  };
}
...
function cotizarItem(item, { cat, regionId, preciosUsuario = null, opcionesCosto = {}, departamento = null } = {}) {
  ...
  if (!elegido) {
    elegido = registrar("epc_apu", delEpcApu(itemId, departamento));
  }
  if (!elegido) {
    elegido = registrar("idu_apu", delIduApu(itemId, departamento));
  }
  if (!elegido) {
    elegido = registrar("ffie_apu", delFfieApu(itemId, departamento));
  }
  if (!elegido) {
    elegido = registrar("iccu_apu", delIccuApu(itemId, departamento, item && item.municipio));
  }
...
/* La mediana es la de `lib/estadistica`, no una copia: esta redondeaba el par
   central a 2 decimales AL CALCULAR (una cifra redondeada para mostrar no puede
   decidir) y devolvía NaN si un valor no era legible. Se re-exporta porque ya
   forma parte de la superficie de este módulo. */
...
function referenciaDeMercado(procesos = [], { familia = null, departamento = null } = {}) {
  /* El valor adjudicado se lee con la MISMA función del índice de competencia
     ...
     nombre a una sola. */
  const { numero, primero, desenlaceDe, CAMPOS_VALOR_ADJUDICADO } = require("../indice_competencia.js");
  ...
  /* LA FAMILIA SE LEE CON LA REGLA DEL ÍNDICE DE BAJA (23-sep-2026), no con un
     `replace(/\D/g)` propio: SECOP II publica el código con prefijo de versión
     («V1.72141500») y quitar los no-dígitos dejaba «172141500», familia
     «1721»: ningún contrato casaba y la referencia decía «No hay contratos
     adjudicados con valor para esa familia» sobre un corpus que los tiene
     (reproducido). `familiaDe` retira el prefijo con `extraerCodigos` y rellena
     la jerarquía; la familia pedida pasa por la misma función, así que «7214»,
     «72141500» y «V1.72141500» piden lo mismo, y una ilegible no filtra. */
  const { familiaDe } = require("../indice_baja.js");
  const depBuscado = departamento ? norm(departamento) : null;
  const famBuscada = familia ? familiaDe({ codigo_principal_de_categoria: familia }) : null;
  ...
  for (const p of procesos) {
    if (!p) continue;
    mirados++;
    /* el DESENLACE, no `esAdjudicado` a secas (remate B9b-H3): aquí el falso
       caro es el POSITIVO, y un proceso que dice «desierto» no aporta un precio
       de mercado aunque traiga fecha o valor */
    if (desenlaceDe(p) !== "adjudicado") { descartados++; continue; }
    if (depBuscado && norm(p.departamento_entidad || "") !== depBuscado) continue;
    if (famBuscada && familiaDe(p) !== famBuscada) continue;
  ...
  return {
    ...
    que_es: "Valor mediano del CONTRATO COMPLETO de obras parecidas ya adjudicadas en este departamento. "
      + "Sirve para contrastar el total de su presupuesto — NO es el precio unitario de ningún ítem.",
  };
}
...
/* EL MISMO LECTOR QUE `calcularItem`, y no `num()` (ago 2026). El campo es el
   MISMO —`precio_manual`— y se leía con dos convenciones distintas: el motor
   con `numeroColombiano` (punto = MILES, la regla de los pliegos colombianos) y
   el aprendizaje con `Number` estricto. «74.596» se calculaba como $74.596 y se
   APRENDÍA como $74,596: mil veces menos, guardado sin TTL y sirviéndose desde
   entonces como «Su precio» —nivel 1 de la cascada, por encima del catálogo— en
   todos los presupuestos siguientes. Es la familia de «375.0000» con
   consecuencia en pesos, y la regla que este proyecto ya tenía escrita: dos
   lecturas del mismo campo divergen a la primera corrección. */
function precioManual(crudo) {
  if (typeof crudo === "string" && crudo.trim() !== "") {
    return require("../apu_pliego.js").numeroColombiano(crudo);
  }
  return num(crudo);
}
...
function normalizarPreciosParaGuardar(items = [], { region = null, ahora = null } = {}) {
  ...
  for (const it of Array.isArray(items) ? items : []) {
    ...
    const precio = precioManual(it.precio_manual);
    /* Sin ítem del catálogo no hay a qué volver; un precio <= 0 no es un precio
       (R1) y guardarlo haría que la próxima cotización valiera cero. Un
       `precio_manual` escrito que NO se pudo leer se CUENTA: descartarlo en
       silencio dejaría al usuario creyendo que su precio quedó guardado. */
    if (!itemId || precio == null || precio <= 0) {
      if (it.precio_manual !== undefined && it.precio_manual !== null && String(it.precio_manual).trim() !== "") descartados++;
      continue;
    }
    nuevos[itemId] = {
      precio,
...
/* `ttl` (segundos, 0 = sin caducidad): los precios corregidos de un perfil FIJO son
   conocimiento y no caducan; los de un perfil que puede desaparecer (rup_… de 45
   días, cons_… borrable) caducan con él y se renuevan en cada guardado — si no,
   cada visitante abandonado dejaría un hash eterno en Redis (6-sep-2026). */
async function guardarPreciosUsuario(redis, perfil, items, { region = null, ttl = 0 } = {}) {
  ...
  if (ttl > 0) await redis.expire(CLAVES.apuPreciosUsuario(perfil), ttl);
...
/* ═══════ LA CASCADA, EXPLICADA SIN VOLVER A CALCULARLA ═════════════════════
   ...
   es una instrucción. */
function explicarCascada(itemId, { preciosUsuario = null, enCatalogo = false, enInvias = false, enEpc = false, enIdu = false, enFfie = false, enIccu = false, elegido = null } = {}) {
  ...
  push("epc_apu", elegido === "epc_apu", enEpc
    ? "Hay APU de referencia EPC, pero manda el que está por encima."
    : MOTIVOS.no_es_item_epc);
  push("idu_apu", elegido === "idu_apu", enIdu
    ? "Hay precio de referencia IDU, pero manda el que está por encima."
    : MOTIVOS.no_es_item_idu);
  push("ffie_apu", elegido === "ffie_apu", enFfie
    ? "Hay precio tope del FFIE, pero manda el que está por encima."
    : MOTIVOS.no_es_item_ffie);
  push("iccu_apu", elegido === "iccu_apu", enIccu
    ? "Hay precio de referencia del ICCU, pero manda el que está por encima."
    : MOTIVOS.no_es_item_iccu);
  ...
  if (elegido === "manual" || elegido === "archivo") {
    return {
      ...
      motivo: elegido === "archivo"
        ? "El precio viene del archivo que usted importó y manda sobre todas las fuentes."
        : "Usted escribió este precio a mano, así que manda sobre todas las fuentes.",
      pasos,
    };
  }
