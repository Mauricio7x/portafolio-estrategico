...
function crp(perfil, presupuestoCOP) {
  ...
  // si a alguno le falta la CO, la suma tampoco se conoce (null, no «la de los otros»)
  if (perfil.integrantes) {
    let acc = 0;
    for (const i of perfil.integrantes) {
      const k = crp(i.perfil, presupuestoCOP);
  ...
  if (co == null) return null; // K sin dato: la puerta P2 lo declara y deja pasar
  ...
  if (sinDato(perfil.expSMMLV) || sinDato(perfil.profesionales) || sinDato(perfil.liquidez)) return null;
  ...
  return Math.max(co * (e + ct + cf) / 100 - sce, 0);
}
