...
/* EL PREFIJO «Proceso » / «Proceso de » que SECOP II pega a estados y fases («Proceso
   adjudicado y celebrado», «Proceso Presentación de observaciones»): UNA sola copia
   (22-sep-2026; vivía en lib/filtros y se reescribía en lib/manifestacion y en
   lib/filtros_lista). Se recorta antes de comparar; nunca coincidencia por subcadena. */
const PREFIJO_ESTADO_RE = /^proceso(?:\s+de)?\s+/;
const nucleoEstado = (valor) => String(valor || "").replace(PREFIJO_ESTADO_RE, "");
...
/* 3-salud · PRESTACIÓN DE SERVICIOS DE SALUD (23-sep-2026, captura del dueño).
   «PRESTACIÓN DEL SERVICIO DE SALUD EN ANESTESIOLOGÍA HOSPITALARIA PARA SALAS DE
   CIRUGÍA Y GINECO OBSTETRICIA; CONSULTA PRE-ANESTÉSICA E INTERCONSULTAS…» llegó a
   la lista de un contratista de obra: su código casa con una clase de servicios
   que los RUP inscriben (85101500 de Génesis, 80111600 de Helder…), y ningún
   término de la lista de arriba la reconocía, así que quedaba en ámbar
   «Verificar objeto» y hasta salía rescatada «con socio».

   DESCARTA SOLO LO QUE SE CONTRATA: el término ENCABEZA el objeto (23-sep-2026,
   revisión adversaria). Casi todas estas palabras nombran también el LUGAR donde
   se hace una obra o una consultoría legítima, y ahí no pueden descartar:
   «READECUACIÓN FÍSICA DE LA UNIDAD DE HEMODIÁLISIS», «GERENCIA INTEGRAL DEL
   PROYECTO TORRE DE ESPECIALIDADES MÉDICAS», «PINTURA Y ARREGLOS LOCATIVOS DEL
   ÁREA DE CONSULTA PREANESTÉSICA», «SUMINISTRO E INSTALACIÓN DE CIELO RASO…
   CONSULTORIOS DE MÉDICOS ESPECIALISTAS». Y la prestación en una cláusula de
   FINALIDAD nombra el destino del edificio, no lo que se contrata: «GERENCIA DE
   PROYECTO DEL NUEVO HOSPITAL PARA LA PRESTACIÓN DEL SERVICIO DE SALUD»,
   «AVALÚO DEL PREDIO DESTINADO A LA PRESTACIÓN DE SERVICIOS DE SALUD». Con las
   alternativas sueltas en medio del texto, esos objetos (visibles en ámbar
   antes del 23-sep) desaparecían sin volver ni atenuados: el falso negativo caro
   de oportunidades. Una lista de lugares o de marcadores de finalidad deja el
   hermano de al lado vivo; la ESTRUCTURA no: el objeto es lo que se contrata
   cuando lo nombra al principio.

   Por eso la especialidad o el servicio de salud tiene que ir al INICIO del
   objeto normalizado, con delante solo un prefijo de contratación
   («contrato de», «contratar», «contratación de») y de servicio («prestación /
   prestar / operación / operar», «del / de los… servicio(s)», «profesionales,
   integrales, asistenciales, especializados», «de / del / en»). La captura del
   dueño («PRESTACIÓN DEL SERVICIO DE SALUD EN ANESTESIOLOGÍA…»), «SERVICIO DE
   HEMODIÁLISIS PARA PACIENTES…» o «MÉDICOS ESPECIALISTAS PARA LA UNIDAD…» caen;
   lo demás llega al paso 4 en ámbar, que se muestra.

   Quién la aplica y con qué guardas vive en lib/filtros.evaluarPertinencia: va
   DESPUÉS del verbo de obra (con vocabulario de obra en el objeto no descarta) y
   DESPUÉS de la clase de obra pura (72/77/81/95 inscrita gana: el mantenimiento
   de un hospital con 72101500 sigue en verde). No va en la blacklist de la
   ingesta: cambiaría el sello de la regla y sacaría la fila del diagnóstico.

   TERCERA RONDA (23-sep-2026, verificación final). El ancla a la cabecera se
   queda y se afina por los dos lados:
   · LOS HERMANOS SEGUÍAN SERVIDOS, con la promesa del socio: «CONTRATAR LA
     OPERACIÓN DE LA UNIDAD DE CUIDADOS INTENSIVOS», urología, patología y
     citología, vacunación, apoyo diagnóstico, «TERCERIZACIÓN DEL PROCESO DE
     ENFERMERÍA», y la captura misma con un preámbulo delante («CONTRATAR CON UNA
     IPS LA…», «SELECCIONAR AL CONTRATISTA PARA LA…»). Se amplían los preámbulos
     y el vocabulario. NUNCA «patología» sola: la patología estructural es
     ingeniería; cuentan «patología clínica», «anatomía patológica» y la citología.
   · Y UNA ETIQUETA DE ÁREA NO ES EL SERVICIO: «SALUD: GERENCIA DEL PROYECTO DE LA
     NUEVA TORRE», «HOSPITALIZACIÓN: PLAN DE REGULARIZACIÓN Y MANEJO» (consultorías
     en ámbar en e1e46e0) caían en rojo porque la palabra encabezaba. Dos reglas:
     (1) las palabras que TAMBIÉN nombran un área del hospital (AREA_DE_SALUD:
     «salud», «hospitalización», «urgencias», «cirugía», «enfermería», «UCI»…)
     solo cuentan detrás de un prefijo de servicio —«prestación / operación /
     tercerización de», «servicio(s) de»—, jamás desnudas; las especialidades
     (ESPECIALIDAD_DE_SALUD: «anestesiología», «urología»…) sí cuentan solas,
     porque así no se titula una obra;
     (2) una cabecera que termina en «:» sin un verbo de contratación delante es
     la etiqueta del área, no el objeto: el objeto empieza detrás de los dos
     puntos y ahí se vuelve a mirar (prestacionDeSalud, la ÚNICA entrada). */
const PREAMBULO_SALUD = "(?:contrato\\s+de|contratacion\\s+de|"
  + "contratar(?:\\s+(?:con|a)\\s+una\\s+(?:i\\.?p\\.?s|e\\.?s\\.?e)\\.?)?|"
  + "seleccionar\\s+al\\s+contratista\\s+para|seleccion\\s+del\\s+contratista\\s+para|adquirir|adquisicion\\s+de)";
const PREFIJO_CONTRATACION_SALUD = "(?:" + PREAMBULO_SALUD + "\\s+)?(?:(?:la|el|los|las)\\s+)?";
const VERBO_SERVICIO_SALUD = "(?:prestacion|prestar|operacion|operar|tercerizacion|tercerizar)";
const SUSTANTIVO_SERVICIO_SALUD = "servicios?\\s+(?:(?:profesional(?:es)?|integral(?:es)?|asistencial(?:es)?|especializad[oa]s?)\\s+)?";
// con al menos el verbo o la palabra «servicio»: es lo que deja contar a una palabra de ÁREA
const PREFIJO_SERVICIO_SALUD = "(?:" + VERBO_SERVICIO_SALUD + "\\s+(?:(?:del?|de\\s+los|de\\s+las|los|las|el|la)\\s+)?"
  + "(?:" + SUSTANTIVO_SERVICIO_SALUD + ")?|" + SUSTANTIVO_SERVICIO_SALUD + ")";
const ENLACE_SALUD = "(?:(?:de|del|en)\\s+)?(?:(?:la|el|los|las)\\s+)?";
const ESPECIALIDAD_DE_SALUD = "(?:"
  + "medic[oa]s?|medicina|anestesi\\w*|consultas?\\s+pre\\W*anestesicas?|interconsultas?|"
  + "odontologia|fisioterapia|terapias?|radiologia|imagenes\\s+diagnosticas|hemodialisis|"
  + "ginecolog\\w*|obstetric\\w*|gastroenterolog\\w*|pediatria|"
  + "psicologia|nutricion|optometria|fonoaudiologia|bacteriologia|"
  + "urolog\\w*|ortopedi\\w*|otorrinolaringolog\\w*|cardiolog\\w*|neurolog\\w*|nefrolog\\w*|neumolog\\w*|"
  + "dermatolog\\w*|oftalmolog\\w*|oncolog\\w*|hematolog\\w*|psiquiatr\\w*|traumatolog\\w*|neonatolog\\w*|fisiatr\\w*|"
  + "patologia\\s+clinica|anatomia\\s+patologica|(?:patologia\\s+y\\s+)?citologias?|"
  + "vacunacion|aplicacion\\s+de\\s+vacunas|procesos?\\s+(?:asistencial(?:es)?\\s+)?de\\s+enfermeria|"
  + "hospitalizacion\\s+domiciliaria|"
  + "(?:sub)?procesos\\s+(?:y\\s+subprocesos\\s+)?asistenciales|"
  + "atencion\\s+(?:domiciliaria|prehospitalaria|medica|en\\s+salud)"
  + ")";
const AREA_DE_SALUD = "(?:"
  + "salud|enfermeria|urgencias|hospitalizacion|cirugia|laboratorio\\s+clinico|especialidades\\s+medicas|"
  + "consulta\\s+externa|apoyo\\s+diagnostico|(?:toma\\s+(?:e\\s+interpretacion\\s+)?de\\s+)?ayudas\\s+diagnosticas|ambulancias?|uci|(?:unidad(?:es)?\\s+de\\s+)?cuidados?\\s+(?:intensivos?|intermedios?)"
  + ")";
const SERVICIO_DE_SALUD = "(?:" + ESPECIALIDAD_DE_SALUD + "|" + AREA_DE_SALUD + ")";
// el ancla va en un lookbehind para que el término que se cita no arrastre las comillas o el guion de delante
const TERMINOS_PRESTACION_SALUD = new RegExp(
  "(?<=^\\W*)" + PREFIJO_CONTRATACION_SALUD
  + "(?:" + PREFIJO_SERVICIO_SALUD + ENLACE_SALUD + SERVICIO_DE_SALUD + "|" + ENLACE_SALUD + ESPECIALIDAD_DE_SALUD + ")\\b");
/* La cabecera con dos puntos: hasta 80 caracteres sin «;» antes del primer «:».
   Si empieza por un preámbulo o un verbo de servicio («PRESTACIÓN DE SERVICIOS DE
   SALUD EN LAS SIGUIENTES ESPECIALIDADES: …») ES el objeto y no se salta. */
const CABECERA_CON_DOS_PUNTOS = /^\W*([^:;]{1,80}?)\s*:\s*/;
const CABECERA_QUE_CONTRATA = new RegExp("^\\W*(?:" + PREAMBULO_SALUD + "|" + VERBO_SERVICIO_SALUD + ")\\b");
function prestacionDeSalud(textoNorm) {
  let t = String(textoNorm || "");
  for (let i = 0; i < 3; i++) { // «SALUD: HOSPITALIZACIÓN: …» — hasta tres etiquetas seguidas
    const m = t.match(CABECERA_CON_DOS_PUNTOS);
    if (!m || CABECERA_QUE_CONTRATA.test(m[1])) break;
    t = t.slice(m[0].length);
  }
  return t.match(TERMINOS_PRESTACION_SALUD);
}
/* El MISMO vocabulario, en cualquier punto del texto: NO descarta nada (lo que
   descarta es la regla de arriba). Existe para que la cerradura cuente, con las
   palabras reales y no con una copia, cuántos objetos llegan al paso 3-salud
   nombrando la salud como lugar o finalidad y aun así siguen en ámbar. */
const MENCION_DE_SALUD = new RegExp("\\b" + SERVICIO_DE_SALUD + "\\b");
...
/* «MANIFESTACIÓN DE INTERÉS» en un `estado` o una `fase` de SECOP II, sobre el
   valor ya normalizado (15-sep-2026). Vive aquí —y no en lib/filtros— porque la
   consultan dos módulos que no pueden requerirse entre sí: lib/filtros (la
   cerca del estado abierto) y lib/manifestacion (la señal publicada del plazo),
   que es HOJA a propósito. Una expresión, y no una lista de literales: cubre
   singular, plural, con prefijo y con sufijo de una vez. */
const MANIFESTACION_INTERES_RE = /manifest(?:aci[oó]n|aciones)\s+de\s+inter[eé]s/;

/* DÓNDE ESTÁ EL PROCESO RESPECTO A LA MANIFESTACIÓN, según la `fase` que SECOP
   II publica (22-sep-2026). Medido por el dueño el 15-sep-2026 sobre las menores
   cuantías publicadas desde el 1-sep: ANTES de la manifestación el proceso está
   en «Presentación de observaciones» (262 recibiendo · 45 en evaluación) o en
   borrador; DESPUÉS está en «Presentación de oferta» (253), «Oferta» (145) o
   «Fase de ofertas» (9), y de ahí a «Selección», evaluación y adjudicación. Son
   dos cercas sobre el valor normalizado, no listas de literales (una lista deja
   vivo el hermano de al lado), y viven aquí por la misma razón que la de arriba:
   las consultan lib/filtros y lib/manifestacion, que no pueden requerirse entre
   sí. Lo que no casa con ninguna —«Pré-Calificación», «Clarification
   submission», vacío— NO dice nada: la app sigue calculando la ventana como
   siempre, sin afirmar ni negar. */
/* «antes» se cierra al literal medido: «Presentación de observaciones AL INFORME
   de evaluación» es una fase POSTERIOR y casaba por prefijo (revisión adversaria
   del 22-sep). «tras» incluye lo que sigue a la adjudicación (celebrado, en
   ejecución, terminado, liquidado, desierto) porque ahí la manifestación cerró
   sin duda; NO incluye cancelado, suspendido, revocado ni anulado: un proceso
   parado no es un plazo cerrado y de él no se afirma nada (la cascada de
   `lib/filtros` ya lo saca del listado). */
/* «Selección de ofertas (borrador)» es el proyecto de pliego de una licitación (8 filas medidas el 22-sep): va
   ANTES, y la cerca de después la excluye a propósito para que «Selección» no se la trague. */
const FASE_ANTES_MANIFESTACION_RE = /^(?:presentaci[oó]n\s+de\s+observaciones(?!\s+al\s+informe)|observaciones\s+al\s+proyecto|borrador|proyecto\s+de\s+pliego|selecci[oó]n\s+de\s+ofertas\s*\(borrador\))/;
const FASE_TRAS_MANIFESTACION_RE = /^(?:presentaci[oó]n\s+de\s+ofertas?|fase\s+de\s+ofertas?|fase\s+de\s+selecci[oó]n|ofertas?\b|recepci[oó]n\s+de\s+ofertas?|selecci[oó]n(?!\s+de\s+ofertas\s*\(borrador\))|(?:en\s+)?evaluaci[oó]n|informe|adjudicaci[oó]n|adjudicad[oa]|celebrad[oa]|(?:en\s+)?ejecuci[oó]n|terminad[oa]|liquidaci[oó]n|liquidad[oa]|(?:declarad[oa]\s+)?desiert[oa])/;

module.exports = {
  norm, PREFIJO_ESTADO_RE, nucleoEstado, MANIFESTACION_INTERES_RE, FASE_ANTES_MANIFESTACION_RE, FASE_TRAS_MANIFESTACION_RE,
  BLACKLIST_OBJETO, WHITELIST_OBRA,
  INFRAESTRUCTURA,
  VERBOS_DE_OBRA_FUERTES, VERBOS_DE_OBRA_CONDICIONADOS, VERBOS_DE_OBRA_CONDICIONADOS_INV,
  TERMINOS_NO_PERTINENTES, prestacionDeSalud, MENCION_DE_SALUD, TERMINOS_BLOQUEANTES, TERMINOS_ESTRUCTURACION,
