...
/* 3-salud · PRESTACIÓN DE SERVICIOS DE SALUD (23-sep-2026, captura del dueño).
   ...
   SOLO palabras que nombran el SERVICIO, nunca el LUGAR. «Consulta externa»,
   «enfermería», «ginecología», «obstetricia», «gastroenterología», «imágenes
   diagnósticas», «laboratorio clínico» o «atención hospitalaria» sueltas nombran
   también el ÁREA del hospital donde se hace una obra o una consultoría
   legítima («MANTENIMIENTO LOCATIVO DE LAS ÁREAS DE ENFERMERÍA», «SUPERVISIÓN
   TÉCNICA DEL ÁREA DE CONSULTA EXTERNA», «PLAN DE REGULARIZACIÓN Y MANEJO DEL
   HOSPITAL, ATENCIÓN HOSPITALARIA…»: medidas, y caían a rojo con un prototipo
   que las listaba). Por eso la especialidad solo cuenta detrás de «prestación
   del servicio de…», que es lo que la vuelve un servicio.
...
const TERMINOS_PRESTACION_SALUD = new RegExp(
  "\\b(?:"
  + "prestacion\\s+(?:del?|de\\s+los)\\s+servicios?\\s+(?:\\w+\\s+){0,2}?de\\s+"
  + "(?:salud|odontologia|fisioterapia|terapias?|radiologia|imagenes\\s+diagnosticas|enfermeria|"
  + "laboratorio\\s+clinico|medicina|urgencias|hospitalizacion|ginecolog\\w*|obstetric\\w*|pediatria|"
  + "cirugia|anestesi\\w*|psicologia|nutricion|optometria|fonoaudiologia|bacteriologia)|"
  + "anestesiolog\\w*|consultas?\\s+pre\\W*anestesicas?|interconsultas?|"
  + "servicios?\\s+medicos?|medicos?\\s+(?:generales?|especialistas?)|especialidades\\s+medicas|"
  + "hemodialisis|(?:sub)?procesos\\s+(?:y\\s+subprocesos\\s+)?asistenciales|"
  + "atencion\\s+(?:domiciliaria|prehospitalaria)"
  + ")\\b");
...
module.exports = {
  ...
  TERMINOS_NO_PERTINENTES, TERMINOS_PRESTACION_SALUD, TERMINOS_BLOQUEANTES, TERMINOS_ESTRUCTURACION,
