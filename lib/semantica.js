...
const BLACKLIST_OBJETO = new RegExp(String.raw`\b((?<=\b(?:adquisici[oó]n|compra|suministro|adiestramiento|entrenamiento|alimento|concentrado|esterilizaci[oó]n|vacunaci[oó]n|manejo|transporte)\s+(?:de\s+|del\s+|los\s+|las\s+|el\s+|la\s+)*)(?:canin[oa]s?|perr[oa]s?|semovientes?|equin[oa]s?|bovin[oa]s?|porcin[oa]s?|ganado\b)|aves\s+de\s+corral|adiestramiento|(?<=\b(?:suministro|prestaci[oó]n|servicios?|entrega|contrataci[oó]n|operador(?:es)?|raciones?|complementos?)\b[\s\S]{0,25})alimentaci[oó]n\s+(?:escolar|pae|adulto|infantil|complement)|aliment(?:os|aria|ario)\s+(?:escolar|pae|nutric)|paquetes?\s+alimentari|refrigerios?|v[ií]veres|(?<=\b(?:suministro|entrega|compra|adquisici[oó]n|operaci[oó]n|paquetes?|bonos?|kits?|distribuci[oó]n)\b[\s\S]{0,40})mercad(?:o|os)\s+(?:campesin|familiar)|almuerz(?:o|os)|desayun(?:o|os)|loncher|vestuario|uniformes?|dotaci[oó]n\s+(?:de\s+)?(?:personal|empleados|escolar|trabajadores|polici|militar)|calzado|prendas\s+(?:de\s+vestir|escolar)|medicament(?:o|os|aria)|f[aá]rmacos?|insumos?\s+m[eé]dic(?:o|os|al)|insumos?\s+hospital|dispositivos?\s+m[eé]dic|(?<=\b(?:insumos?|equipos?|servicios?|unidad(?:es)?|sill[oó]n(?:es)?|material(?:es)?|elementos?|dispositivos?|instrumental|suministros?|atenci[oó]n)\b[\s\S]{0,40})odontol[oó]gic|farmac[eé]utic|papeler[ií]a|[uú]tiles\s+de\s+oficina|t[oó]ner|impresi[oó]n\s+(?:de\s+formul|de\s+document)|combustible|gasolina|acpm\b|lubricantes|aceite\s+de\s+motor|adquisici[oó]n\s+de\s+veh[ií]culos|compra\s+de\s+veh[ií]culos|motocicletas?|cuatrimoto|(?<=\b(?:parque|reparaci[oó]n|seguros?|taller(?:es)?|veh[ií]culos?|repuestos?|lavado|alquiler|arrendamiento)\b[\s\S]{0,25})automotor(?:es)?|busetas?|camion(?:es|eta)|armament|munici[oó]n(?:es)?|dotaci[oó]n\s+polici|chaleco\s+antibalas|software|licenci(?:a|as)\s+(?:inform[aá]tic|de\s+software|microsoft|adobe|office|antiviru)|ofim[aá]tica|hosting|dominio\s+web|(?<=\b(?:servicios?\s+(?:de|para)(?:\s+la)?|jornadas?\s+de|programas?\s+de|cursos?\s+de|talleres?\s+de|procesos?\s+de)\s+)capacitaci[oó]n|congres(?:o|os)|seminari(?:o|os)|hospedaje|(?<=\bservicios?\s+de\s+)alojamiento|alojamiento(?=\s+y\s+alimentaci[oó]n)|tiquetes?\s+a[eé]reos?|transporte\s+(?:de\s+)?personal|transporte\s+escolar|fumigaci[oó]n|control\s+(?:de\s+)?(?:vectores|plagas|roedores)|aseo\s+(?:y\s+)?(?:cafeter[ií]a|general|integral|institucion)|vigilancia\s+(?:armada|privada|y\s+seguridad)|seguridad\s+privada|seguros?\s+(?:generales|todo\s+riesgo|automotor|vida|salud|estudiantil(?:es)?|de\s+(?:vida|salud|bienes|responsabilidad|accidentes|da[ñn]os|veh[ií]culos|autom[oó]viles|automotores|manejo|cumplimiento|todo\s+riesgo|la\s+entidad|los\s+bienes|exequias|hogar|incendio|infraestructura|maquinaria|transporte))|programa\s+de\s+seguros|p[oó]liza|intermediaci[oó]n\s+de\s+seguros|corretaje|telefon[ií]a|` + CONECTIVIDAD_DE_TELECOM + String.raw`|plan\s+de\s+datos|publicidad|impresos\s+(?:y|institucion)|pendones?|plotter|jur[ií]dic(?:o|os|a|as)\s+(?:asesor|representac|defensa)|defensa\s+judicial|representaci[oó]n\s+judicial|auditor[ií]a\s+(?:contable|financiera|forense|de\s+cumplimiento|interna|de\s+gesti)|libros\b|bibliogr[aá]fic|(?<=\b(?:dotaci[oó]n|adquisici[oó]n|compra|suministro|material(?:es)?|elementos)\b[\s\S]{0,40})biblioteca|juguetes?|navide[ñn](?:o|a|os|as)|premios?\s+(?:y\s+est[ií]mul|escolar|deportiv)|bonos?\s+(?:navide|nutric|alimentari|escolar)|becas?\b|fertilizantes?|abonos?\s+(?:org[aá]nicos|qu[ií]micos)|insumos?\s+agr[ií]cola|(?<=\b(?:suministro|adquisici[oó]n|compra|entrega|insumos?|kits?|maquinaria|herramientas?|semillas?|asistencia\s+t[eé]cnica|extensi[oó]n|fomento|proyectos?\s+productivos?|unidades?\s+productivas?)\b[\s\S]{0,25})agropecuari|fungicidas?|herbicidas?|plaguicidas?|kits?\s+(?:escolar|estudiantil|alimentari|nutric|hi?gi[eé]nic|deportiv|biosanitar)|elementos?\s+de\s+protecci[oó]n\s+(?:personal|biosanitar)|epp\b|interpretaci[oó]n\s+de\s+lenguaje|traducci[oó]n)`, "i");
