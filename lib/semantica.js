...
const BLACKLIST_OBJETO = new RegExp(String.raw`\b(canin[oa]s?|perr[oa]s?|semovientes?|equin[oa]s?|bovin[oa]s?|porcin[oa]s?|aves\s+de\s+corral|ganado|adiestramiento|alimentaci[oó]n\s+(?:escolar|pae|adulto|infantil|complement)|aliment(?:os|aria|ario)\s+(?:escolar|pae|nutric)|paquetes?\s+alimentari|refrigerios?|v[ií]veres|mercad(?:o|os)\s+(?:campesin|familiar)|almuerz(?:o|os)|desayun(?:o|os)|loncher|vestuario|uniformes?|dotaci[oó]n\s+(?:de\s+)?(?:personal|empleados|escolar|trabajadores|polici|militar)|calzado|prendas\s+(?:de\s+vestir|escolar)|medicament(?:o|os|aria)|f[aá]rmacos?|insumos?\s+m[eé]dic(?:o|os|al)|insumos?\s+hospital|dispositivos?\s+m[eé]dic|odontol[oó]gic|farmac[eé]utic|papeler[ií]a|[uú]tiles\s+de\s+oficina|t[oó]ner|impresi[oó]n\s+(?:de\s+formul|de\s+document)|combustible|gasolina|acpm\b|lubricantes|aceite\s+de\s+motor|adquisici[oó]n\s+de\s+veh[ií]culos|compra\s+de\s+veh[ií]culos|motocicletas?|cuatrimoto|automotor(?:es)?|busetas?|camion(?:es|eta)|armament|munici[oó]n(?:es)?|dotaci[oó]n\s+polici|chaleco\s+antibalas|software|licenci(?:a|as)\s+(?:inform[aá]tic|de\s+software|microsoft|adobe|office|antiviru)|ofim[aá]tica|hosting|dominio\s+web|(?<=\b(?:servicios?\s+(?:de|para)(?:\s+la)?|jornadas?\s+de|programas?\s+de|cursos?\s+de|talleres?\s+de|procesos?\s+de)\s{1,3})capacitaci[oó]n|congres(?:o|os)|seminari(?:o|os)|hospedaje|(?<=\bservicios?\s+de\s{1,3})alojamiento|alojamiento(?=\s+y\s+alimentaci[oó]n)|tiquetes?\s+a[eé]reos?|transporte\s+(?:de\s+)?personal|transporte\s+escolar|fumigaci[oó]n|control\s+(?:de\s+)?(?:vectores|plagas|roedores)|aseo\s+(?:y\s+)?(?:cafeter[ií]a|general|integral|institucion)|vigilancia\s+(?:armada|privada|y\s+seguridad)|seguridad\s+privada|seguros?\s+(?:de\s+|generales|todo\s+riesgo|automotor|vida|salud)|p[oó]liza|intermediaci[oó]n\s+de\s+seguros|corretaje|telefon[ií]a|` + CONECTIVIDAD_DE_TELECOM + String.raw`|plan\s+de\s+datos|publicidad|impresos\s+(?:y|institucion)|pendones?|plotter|jur[ií]dic(?:o|os|a|as)\s+(?:asesor|representac|defensa)|defensa\s+judicial|representaci[oó]n\s+judicial|auditor[ií]a\s+(?:contable|financiera|forense|de\s+cumplimiento|interna|de\s+gesti)|libros|bibliogr[aá]fic|(?<=\b(?:dotaci[oó]n|adquisici[oó]n|compra|suministro|material(?:es)?|elementos)\b[\s\S]{0,40})biblioteca|juguetes?|navide[ñn](?:o|a|os|as)|premios?\s+(?:y\s+est[ií]mul|escolar|deportiv)|bonos?\s+(?:navide|nutric|alimentari|escolar)|becas?|fertilizantes?|abonos?\s+(?:org[aá]nicos|qu[ií]micos)|insumos?\s+agr[ií]cola|agropecuari|fungicidas?|herbicidas?|plaguicidas?|kits?\s+(?:escolar|estudiantil|alimentari|nutric|hi?gi[eé]nic|deportiv|biosanitar)|elementos?\s+de\s+protecci[oó]n\s+(?:personal|biosanitar)|epp\b|interpretaci[oó]n\s+de\s+lenguaje|traducci[oó]n)`, "i");
