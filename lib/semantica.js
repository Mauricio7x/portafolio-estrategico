...
const PREFIJO_CONTRATACION_SALUD = "(?:(?:contrato\\s+de|contratar|contratacion\\s+de)\\s+)?(?:(?:la|el|los|las)\\s+)?";
const PREFIJO_SERVICIO_SALUD = "(?:(?:prestacion|prestar|operacion|operar)\\s+(?:(?:del?|de\\s+los|de\\s+las|los|las|el|la)\\s+)?)?"
  + "(?:servicios?\\s+(?:(?:profesional(?:es)?|integral(?:es)?|asistencial(?:es)?|especializad[oa]s?)\\s+)?)?"
  + "(?:(?:de|del|en)\\s+)?(?:(?:la|el|los|las)\\s+)?";
const SERVICIO_DE_SALUD = "(?:"
  + "salud|medic[oa]s?|medicina|anestesi\\w*|consultas?\\s+pre\\W*anestesicas?|interconsultas?|"
  + "odontologia|fisioterapia|terapias?|radiologia|imagenes\\s+diagnosticas|enfermeria|hemodialisis|"
  + "laboratorio\\s+clinico|ginecolog\\w*|obstetric\\w*|gastroenterolog\\w*|pediatria|urgencias|"
  + "hospitalizacion|cirugia|psicologia|nutricion|optometria|fonoaudiologia|bacteriologia|"
  + "especialidades\\s+medicas|(?:sub)?procesos\\s+(?:y\\s+subprocesos\\s+)?asistenciales|"
...
const TERMINOS_PRESTACION_SALUD = new RegExp(
  "(?<=^\\W*)" + PREFIJO_CONTRATACION_SALUD + PREFIJO_SERVICIO_SALUD + SERVICIO_DE_SALUD + "\\b");
...
module.exports = {
  ...
  TERMINOS_NO_PERTINENTES, TERMINOS_PRESTACION_SALUD, MENCION_DE_SALUD, TERMINOS_BLOQUEANTES, TERMINOS_ESTRUCTURACION,
