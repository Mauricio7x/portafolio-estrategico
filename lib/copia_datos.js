/* ============================================================================
   ...
   se dice; con él, se reemplaza ENTERO, para que no queden campos que la copia
   no traía — pero por un SWAP ATÓMICO (clave de trabajo + `rename`), nunca
   borrando primero: un corte a mitad borraba los precios corregidos que la
   restauración venía a reemplazar (ver `escribirEntrada`).
   ========================================================================== */
"use strict";
...
/* Clave de trabajo del SWAP ATÓMICO de abajo. Va fuera del censo de `PATRONES`
   a propósito: una exportación simultánea no puede llevarse un hash a medias. */
const claveDeTrabajo = (clave) => `copia:restaurando:${clave}`;

/* Un hash se reescribe ENTERO —la copia manda, no pueden quedar campos que no
   traía—, pero `del` + `hset` son DOS viajes REST contra Upstash y un corte
   entre ellos dejaba la clave BORRADA: el único hash que viaja es
   `apu:precios:*`, los precios que el dueño corrigió a mano, que es justo lo
   que ninguna reconstrucción puede devolver. Se usa el SWAP ATÓMICO que ya
   emplean `lib/indice_competencia` y `lib/indice_baja`: el reemplazo se escribe
   completo en una clave de trabajo y UN solo `rename` la pone encima de la
   vigente. El único comando destructivo llega cuando el nuevo ya está entero.

   A diferencia de los índices, aquí NO hay camino de respaldo con `del` + `hset`
   si el `rename` falla: en un índice ese respaldo es barato porque el dato se
   reconstruye desde el corpus, y aquí sería volver a poner el defecto que se
   está cerrando. El error sube, `restaurarCopia` lo informa con lo hecho hasta
   ahí, y lo que el usuario ya tenía sigue en su sitio. El hash vacío tampoco
   llega: `validarCopia` exige «valor» no vacío, así que nunca se renombra desde
   una clave inexistente. */
async function escribirEntrada(redis, e) {
  if (e.tipo === "hash") {
    const trabajo = claveDeTrabajo(e.clave);
    await redis.del(trabajo);            // resto de un intento anterior cortado
    await redis.hset(trabajo, e.valor);
    try {
      await redis.rename(trabajo, e.clave);
    } catch (err) {
      try { await redis.del(trabajo); } catch { /* el siguiente intento lo limpia */ }
      throw err;
    }
    /* El TTL se pone DESPUÉS del rename y solo si la copia lo traía: la clave
       de trabajo nunca lleva expiración, porque `RENAME` se la llevaría a los
       precios (que no caducan). Sin `ttl_seg` la clave queda sin TTL. */
...
async function restaurarCopia(redis, copia, { sobrescribir = false } = {}) {
  ...
  try {
    for (const e of entradas) {
      ...
      try {
        if (candado) await conCandado(redis, candado, CANDADO_CORTO_TTL_SEG, () => escribirEntrada(redis, e), { accion: "restaurar" });
        else await escribirEntrada(redis, e);
