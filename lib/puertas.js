...
const { crp, faltantesK } = require("./capacidad.js"); // `calcCRPC`/`plazoMesesDe` ya no se llaman aquí: los envuelve `cargaK`
...
/* Patrimonio que respalda la FINANCIACIÓN. Para un proponente plural NO es el
   patrimonio del perfil del consorcio de lib/perfiles (ese es el indicador
   habilitante, y con un pliego que pondera sale ponderado): es la SUMA de los patrimonios de los integrantes, porque quien pone la caja
...
/* EL CÓDIGO CASA, PERO SOLO POR UNA CLASE DE SERVICIOS QUE NO SON OBRA
   ...
   DICE LO QUE SE MIDIÓ, NO MÁS (23-sep-2026, revisión adversaria): lo medido es
   una AUSENCIA —ninguna lista de vocabulario de obra casa con el objeto—, y
   «el objeto no describe una obra» la volvía un hecho negativo, falso en
   «IMPERMEABILIZACIÓN DE CUBIERTAS DEL HOSPITAL» con 80101600: el «sin dato ≠
   cero» pasado a texto. Una sola redacción para las tres salidas (P1, la línea
   de la tarjeta, que la lee de aquí vía `p1_rup.mensaje`, y la frase del socio,
   que la llama con el nombre del socio): tres redacciones del mismo hecho ya
   habían divergido en el orden de las cláusulas. */
const mensajeCasaSoloPorServicio = (quien) =>
  `${quien} tiene el código de este proceso, pero el objeto no dice que sea una obra: confírmelo en el pliego antes de contar con él.`;
const MENSAJE_CASA_SOLO_POR_SERVICIO = mensajeCasaSoloPorServicio("Su registro");
...
function p2K(lic, perfil) {
  ...
  if (crpVal == null) {
    ...
    /* La lista sale de lib/capacidad.faltantesK, la MISMA que decide que la K no
       se puede calcular (25-sep-2026): con dos listas, la de aquí nombraba como
       ausente una liquidez indeterminada y callaba el total del segmento 72. */
    const FRASE_FALTANTE = {
      co: "la utilidad (o el ingreso) operacional",
      liquidez: "el índice de liquidez",
      experiencia_segmento72: "el valor total de sus contratos de construcción (segmento 72) inscritos en el registro",
      profesionales: "el personal profesional",
    };
    const faltan = faltantesK(perfil).map((k) => FRASE_FALTANTE[k] || k);
...
function p4Competencia(competencia) {
  ...
  /* el índice NO SE PUDO LEER (23-sep-2026, lib/indice_competencia.SIN_LECTURA):
     decir «no hay histórico suficiente de esta entidad» sería la misma afirmación
     falsa que el chip de la tarjeta dejó de hacer; y el conteo es null, no 0 */
  const noLeido = !conBase && c.motivo === "no_se_leyo";

  return {
    pasa: true, // por diseño: la competencia no hace inviable un proceso
    /* SIN BASE ES «?», NO «✓» (24-sep-2026). Con `pasa: true` y sin `sin_dato`
       la tarjeta pintaba «Competencia ✓» en verde al lado de «Sin datos de
       cuántos compiten» y de «No se pudo consultar»: un visto bueno sobre un dato
       que nadie midió. `sin_dato` solo cambia el chip (gris «?»); `pasa` sigue en
       true —P4 nunca bloquea— y ningún consumidor lee `sin_dato` de P4 para
       decidir viabilidad o socio (medido con op=listar antes y después). */
    sin_dato: !conBase,
    nivel: conBase ? nivel : "sin_dato",
    promedio_oferentes: conBase ? promedio : null,
    total_procesos: noLeido ? null : procesos,
    advertencia: conBase && nivel === "alta",
    /* LA FRASE NEUTRA (24-sep-2026): «esta entidad no publica cuántos ofertaron»
       se decía también sin índice, con el índice vacío y con un nombre que no
       casa, donde nadie miró qué publica la entidad. Lo cierto en los tres
       casos, y en el de la entidad que de verdad no publica el conteo, es lo
       que le falta a Detekta. Las mismas palabras que FUENTE_P (public/app.js). */
    mensaje: noLeido
      ? "No se pudo consultar la competencia de esta entidad en esta carga: vuelva a cargar. No significa que la entidad no tenga datos."
      : !conBase
      ? "No hay datos suficientes de cuántos compiten en esta entidad."
      : nivel === "alta"
...
module.exports = {
  evaluarPuertas,
  FRACCION_FINANCIACION, TIERS_SOLIDOS, TIERS_CON_ADVERTENCIA, MENSAJE_CASA_SOLO_POR_SERVICIO, mensajeCasaSoloPorServicio,
