...
const umbralOk = (c, v) => (c.mayoritaria ? v > 50 : c.porcentaje == null ? true : v >= Number(c.porcentaje));
function clausulaCumple(c, suya, conf) {
  ...
  switch (c.forma) {
    ...
    case "lider_mayoria": return Math.max(d, so) > 50;
...
/* ── LOS INDICADORES FINANCIEROS CON LAS TRES FÓRMULAS (26-sep-2026) ──
   ...
   fórmula en 130 de 141 pliegos donde afirma una (medido contra la cosecha del
   25-sep): un 8 % de error que el dueño no acepta («jamás»). Por eso un reparto
...
function fronteraReparto({ dueno, socio, presupuestoCOP, crpc = null, crpcMinimo = null, exigidaSMMLV = null, tipoContrato = null, pliego = null }) {
  ...
  const clausulas = pliego && Array.isArray(pliego.clausulas) ? pliego.clausulas : null;
  ...
  const fallasEn = (s, umbral = crpc, metodos = METODOS_TODOS) => {
    ...
    if (c) f.push({ clave: "participacion", clausula: c });
  ...
  return {
    ...
    participacion: clausulas == null ? { leida: false } : { leida: true, documentos_leidos: pliego.leidos || 0, clausulas },
    financiero: reqFin.length ? { metodos: METODOS_TODOS, metodo_leido: pliego && pliego.metodo ? pliego.metodo : null, en_rojo_con_cualquier_reparto: siempreEnRojo, suya_sin_ponderar: suyaSinPonderar } : null,
    frase: fraseFrontera({ suya, deja, regla, socio, kEnSuya, crpc, midenK, choque, nadaMedido, validos, familiaImposible, clausulas, primeraFallaCon: (x) => fallasEn(x) }),
    avisos: avisosDe({ regla, midenK, kSinDato, hueco, suya, suyaConAnticipo, pliego, clausulas, suyaSinPonderar,
      financiero: reqFin.length ? { metodo_leido: pliego && pliego.metodo ? pliego.metodo : null, en_rojo_con_cualquier_reparto: siempreEnRojo } : null }),
  };
}
...
function avisosDe({ regla, midenK = true, kSinDato = false, hueco = [], suya = null, suyaConAnticipo = null, pliego = null, clausulas = null, suyaSinPonderar = null, financiero = null }) {
  ...
  if (clausulas == null) a.push(AVISO_PROVISIONAL, AVISO_MINIMO);
    ...
    for (const c of clausulas) {
      a.push(c.forma === "otro"
        ...
        : suya != null ? `Esta recomendación ya cumple lo que exige ${dondeDe(c)}: ${fraseClausula(c)}` : `Lo que exige ${dondeDe(c)}: ${fraseClausula(c)}`);
    }
  }
  if (financiero) {
    const leido = financiero.metodo_leido;
    const conPliegoTipo = suyaSinPonderar != null && (suya == null || suyaSinPonderar > suya);
    if (conPliegoTipo) {
    ...
    const titulos = [...new Set(ponderadasRojas.map((r) => r.titulo))];
    if (titulos.length) a.push(`Si el pliego pondera por la participación, ${titulos.join(" y ")} no se cumple con ningún reparto: confirme en el capítulo financiero del pliego qué fórmula usa.`);
  }
...
function fraseFrontera({ suya, deja, regla, socio, kEnSuya, crpc, midenK, choque = false, nadaMedido = false, familiaImposible = null, clausulas = null, primeraFallaCon = null }) {
  ...
  if (suya == null) {
    ...
    if (choque && primeraFallaCon && (clausulas && clausulas.some((c) => c.forma !== "otro") || primeraFallaCon(50).some((x) => x.clave === "financiero"))) {
      return `Con ${nombre} no hay un reparto que cumpla todo a la vez: lo que un porcentaje arregla (${[...new Set(primeraFallaCon(50).map((x) => NOMBRE_FALLA[x.clave]))].join(", ")}) lo rompe otra regla.`;
    }
    if (choque) {
  ...
  if (deja.length) {
    const porque = deja.map((d) => d.clave === "capacidad"
      ...
      : d.clave === "participacion" ? `lo que exige ${dondeDe(d.clausula)}: ${minus(fraseClausula(d.clausula))}`
