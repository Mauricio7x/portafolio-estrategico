...
async function crearPerfilDinamico(redis, { perfil, meta = {}, ttl = PERFIL_DINAMICO_TTL_SEG } = {}) {
  ...
  try {
    ...
    if (vivos.length >= MAX_PERFILES_DINAMICOS) {
      return {
        ok: false, status: 503,
        error: "Se alcanzó el tope de perfiles creados sin cuenta. Intente de nuevo en unos días (los perfiles "
          + "sin uso caducan solos) o contacte al administrador del sitio.",
      };
    }
  } catch (e) {
