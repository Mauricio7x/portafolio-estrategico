...
async function crearPerfilDinamico(redis, { perfil, meta = {}, ttl = PERFIL_DINAMICO_TTL_SEG } = {}) {
  ...
  try {
    ...
    if (vivos.length >= MAX_PERFILES_DINAMICOS) {
      return {
        ok: false, status: 503,
        error: "Se alcanzó el tope de perfiles creados sin cuenta. Intentá de nuevo en unos días (los perfiles "
          + "sin uso caducan solos) o contactá al administrador del sitio.",
      };
    }
  } catch (e) {
    return { ok: false, status: 503, error: `No se pudo consultar Redis. Reintentá. (${e.message})` };
  }
  ...
  try {
    ...
    return { ok: false, status: 503, error: `No se pudo guardar el perfil. Reintentá. (${e.message})` };
  }
...
module.exports = {
  ID_DINAMICO_RE, MAX_INYECTADOS, MAX_PERFILES_DINAMICOS,
