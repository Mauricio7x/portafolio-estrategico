/* ============================================================================
   ...
   proyecto. Es la misma vía por la que Mi empresa (#/admin) encadena la full desde el
...
/* QUÉ FILA DE TOTAL ES CUÁL: la regla vive UNA vez, en `public/xlsx_lectura.js`
   (UMD, hoja), y la comparten los DOS lectores de formularios de cantidades del
   proyecto. Aquí no estaba, y `TOTALES_RE` metía en el mismo saco «SUBTOTAL
   CAPITULO 1» y «COSTO DIRECTO»: el total del DOCUMENTO acababa como subtotal
   del ÚLTIMO capítulo (que salía «no_cuadra» por la diferencia con los demás)
   y encima se PERDÍA como total, así que el documento no se podía cuadrar
   contra su propia cifra. Mismo patrón que `lib/costos.js`: se requiere, no se
   copia. */
const { clasificarRotuloTotal } = require("../public/xlsx_lectura.js");
...
/* Filas que ESTÁN dentro de la tabla, con unidad y cifras, pero que NO son costo
   ...
   …Y EL ANCLAJE NO BASTABA PARA SALVARLA (ago 2026). El comentario de arriba
   prometía justo eso y era falso: «administracion delegada de obra» EMPIEZA por
   la palabra, así que el `^` no la protege y la fila se descartaba en silencio.
   La administración delegada es una modalidad de contratación real (el
   contratista gestiona la obra por un honorario) y, si el pliego la paga como
   partida, perderla ABARATA el presupuesto — que es el error caro de este
   módulo. El discriminante no es la posición sino lo que SIGUE a la palabra: la
   «A» del AIU va sola o con su porcentaje, mientras que la partida real la
   califica («delegada», «de obra»). Y si la línea trae un `%`, manda el AIU
   aunque venga calificada: «Administración de obra 15 %» es la A, no un ítem. */
const CALIFICADOR_DE_OBRA = "(?!\\s+(?:delegada|de\\s+(?:la\\s+)?obras?)\\b)";
const NO_ES_COSTO_DIRECTO_RE = new RegExp(
  "^(?:a\\.?i\\.?u\\.?|administracion" + CALIFICADOR_DE_OBRA
  + "|imprevistos|utilidad|iva|impuesto|retencion|estampilla|contribucion)\\b");
/* La excepción de la excepción: con porcentaje en la línea vuelve a ser AIU. */
const ADMIN_CON_PORCENTAJE_RE = /^administracion\b[^%]{0,40}%/;
const noEsCostoDirecto = (desc) => NO_ES_COSTO_DIRECTO_RE.test(desc) || ADMIN_CON_PORCENTAJE_RE.test(desc);
...
function filaPorPosicion(celdas, mapa, { conTab = true } = {}) {
  ...
  /* LA MISMA CELDA VACÍA, PERO CON ESPACIOS COMO SEPARADOR (ago 2026). La
     ...
     falta.
     LO QUE ESTA REGLA NO PUEDE RESOLVER, dicho para que nadie lo dé por cerrado:
     una fila con la cantidad comida y SIN unitario ni total («2.1 SUBBASE M3
     95.000», separada por espacios) tiene exactamente la misma forma que una
     fila benigna con su cantidad y los precios en blanco («1.1 EXCAVACION M3
     10»). Cuatro celdas las dos, y lo único que las separa es la MAGNITUD del
     número, que sería una heurística inventada. Se elige la lectura benigna a
     propósito: la entidad publica las cantidades y deja los precios al oferente
     —nunca al revés—, así que ese caso es el corriente y el otro, patológico.
     Si algún día hay que cerrarlo, la señal está en el DOCUMENTO (cuántas celdas
     traen las demás filas), no en la fila. */
...
function parsearPliego(texto, opciones = {}) {
  ...
  let vistoAiu = false;            // ¿ya pasó la fila del AIU? («TOTAL» después ya no es la suma)
  let totalDeclarado = null;       // el total que el DOCUMENTO declara, si lo declara
  ...
  for (const lineaCruda of lineas) {
    ...
    if (fila) {
      ...
      if (noEsCostoDirecto(desc)) { descartadas.no_costo_directo++; continue; }
    ...
    /* A PARTIR DEL AIU, UN «TOTAL» YA NO ES LA SUMA DE LOS ÍTEMS sino el precio
       con AIU. La bandera se enciende aquí, en UN solo punto, porque el AIU
       aparece de las dos formas: como fila con unidad y como rótulo suelto
       («AIU 25 % … 52.500»), y la segunda no llega a la rama de las filas.
       Se reutiliza `noEsCostoDirecto`, que ya distingue «ADMINISTRACION 15 %»
       de «ADMINISTRACION DELEGADA DE OBRA»: una segunda lista de «esto es AIU»
       divergiría de la primera. Encenderla de más solo hace que un «TOTAL»
       posterior no sirva de ancla («no sé»); de menos, que un precio CON AIU se
       tome por la suma y fabrique un «no cuadra». El error cae del lado bueno. */
    if (noEsCostoDirecto(tNorm)) vistoAiu = true;

    /* ---- subtotal de capítulo declarado ---- */
    if (TOTALES_RE.test(tNorm)) {
      /* ¿Es el total del DOCUMENTO o el subtotal de un CAPÍTULO? La regla es la
         de `xlsx_lectura`, compartida: «COSTO(S) DIRECTO(S)» es la suma de los
         ítems por definición y manda; un «TOTAL» a secas vale solo ANTES del
         AIU. Cualquier otro rótulo devuelve null y sigue tratándose como hasta
         hoy: esto DESVÍA los casos inequívocos, no reclasifica el resto. */
      /* El rótulo va SIN su cola de cifras: `clasificarRotuloTotal` ancla el
         «TOTAL» seco a FIN de cadena (es lo que lo separa de «TOTAL CAPITULO
         1»), y aquí la línea siempre arrastra el valor —«TOTAL 262.500»—, así
         que sin recortar la cola esa rama no se dispararía NUNCA. Es la misma
         limpieza que ya usa `nombrado` dos líneas más abajo. */
      const rotulo = tNorm.replace(/[\d.,$\s]+$/, "").trim();
      const clase = clasificarRotuloTotal(rotulo, { vistoAiu });
      if (clase === "aiu") vistoAiu = true;
      if (clase === "precio_con_aiu") {
        /* Es el precio CON AIU, no la suma de un capítulo. Asignárselo al
           capítulo en curso le fabricaba un «no_cuadra» — el mismo defecto que
           el total del documento, por la otra puerta. */
        descartadas.totales++;
        continue;
      }
      if (clase === "costo_directo" || clase === "total_documento") {
        const nums = compactas.map(numeroColombiano).filter((n) => n != null);
        const valor = nums.length ? nums[nums.length - 1] : null;
        /* El «COSTO DIRECTO» explícito reemplaza a un «TOTAL» seco leído antes;
           nunca al revés. */
        if (valor != null && (totalDeclarado == null
            || (clase === "costo_directo" && !totalDeclarado.explicito))) {
          totalDeclarado = {
            valor, etiqueta: linea.replace(/\s+/g, " ").trim().slice(0, 80),
            explicito: clase === "costo_directo", pagina: paginaActual,
          };
        }
        descartadas.totales++;
        continue;
  ...
  /* ═════ EL ANCLA QUE EL PROPIO DOCUMENTO TRAE, Y QUE SE ESTABA TIRANDO ═════
     El nivel Documento solo sabía cuadrar contra el `precio_base` EXTERNO (el
     presupuesto oficial del proceso), que no siempre se conoce. Pero el pliego
     suele declarar su propio «COSTO DIRECTO», y esa es la MISMA cifra que la
     suma de los ítems, sin AIU de por medio y sin depender de ninguna hipótesis
     sobre cómo presupuesta la entidad: es el ancla más limpia que hay, y estaba
     yendo a parar al subtotal de un capítulo.
     Es exactamente el «cuadre de control» que `xlsx_lectura` ya hace con los
     Excel; aquí faltaba. Se publica SIEMPRE que exista, aunque además haya
     precio_base, porque responden a preguntas distintas: este dice «leí bien el
     documento», aquél dice «el documento cuadra con el presupuesto oficial».
     Sin total declarado NO se inventa nada: `null` y el motivo. */
  const cuadreInterno = { estado: "sin_datos", motivo: null, total_declarado: null,
    etiqueta: null, pagina: null, suma_items: conTotal ? sumaTotales : null,
    filas_sumadas: conTotal, filas_sin_valor: items.length - conTotal };
  if (totalDeclarado == null) {
    cuadreInterno.motivo = "El documento no declara su costo directo: no hay contra qué contrastar la lectura.";
  } else {
    cuadreInterno.total_declarado = totalDeclarado.valor;
    cuadreInterno.etiqueta = totalDeclarado.etiqueta;
    cuadreInterno.pagina = totalDeclarado.pagina;
    cuadreInterno.explicito = totalDeclarado.explicito;
    if (!conTotal) {
      /* Comparar una suma incompleta contra el total declarado daría un
         «no cuadra» que solo dice que faltan valores, no que la lectura falle. */
      cuadreInterno.estado = "no_comparable";
      cuadreInterno.motivo = `El documento declara «${totalDeclarado.etiqueta}» pero ningún ítem trae valor total legible: la suma no se puede comparar.`;
    } else {
      const desv = Math.abs(sumaTotales - totalDeclarado.valor);
      const tol = Math.max(TOL_CAPITULO_ABS, TOL_DOCUMENTO_REL * Math.abs(totalDeclarado.valor));
      cuadreInterno.desviacion = desv;
      cuadreInterno.tolerancia = tol;
      cuadreInterno.estado = desv <= tol ? "cuadra" : "no_cuadra";
      if (cuadreInterno.estado === "no_cuadra") {
        cuadreInterno.motivo = "Los ítems leídos no suman lo que el documento declara: falta un capítulo, sobra una fila o una cantidad se leyó mal.";
      }
    }
  }
  ...
  const luz = semaforo({
    ...
    /* DOS VÍAS AL NIVEL DOCUMENTO, y la interna es la MÁS limpia. La externa
       compara la suma contra el `precio_base` del proceso, lo que obliga a
       suponer cómo presupuesta la entidad (con IVA sobre la utilidad o sin él)
       y exige una cifra que el documento no trae. La interna la compara contra
       el «COSTO DIRECTO» que el PROPIO pliego declara: misma magnitud, sin AIU
       de por medio y sin ninguna hipótesis. Un pliego que cuadra al peso con su
       propia cifra salía ÁMBAR solo por no conocerse la cuantía del proceso.
       Las tres guardas del verde (ninguna cantidad ilegible, ≥5 filas
       validadas, ≥50 % de cobertura) siguen intactas: son las que impiden que
       un cuadre afortunado sobre cuatro filas se lea como «se usa
       automáticamente». */
    totalCuadraDeclarado: (documento.estado === "cuadra" && documento.via === "aiu_declarado")
      || cuadreInterno.estado === "cuadra",
  });
  ...
  return {
    ...
    validacion: {
      ...
      /* el cuadre contra la cifra que el PROPIO documento declara */
      cuadre_interno: cuadreInterno,
      documento,
    },
    diagnostico: {
