...
function filaPorPosicion(celdas, mapa, { conTab = true } = {}) {
  ...
  /* LA MISMA CELDA VACÍA, PERO CON ESPACIOS COMO SEPARADOR (ago 2026). La
     ...
     La señal es exacta y barata: si la fila aporta menos celdas de las que la
     cabecera declaró, algún hueco se comió el separador y el mapa ya no dice
     qué columna es cuál. Entonces las cifras posicionales son ILEGIBLES —null,
     jamás el número de al lado—, la fila se conserva (nunca se descarta) y el
     guardián de cantidades ilegibles hace su trabajo. */
...
function parsearPliego(texto, opciones = {}) {
  ...
  for (const lineaCruda of lineas) {
    ...
    const esProsaCorta = items.length && compactas.length === 1
