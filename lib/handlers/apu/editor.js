...
/* ══ EL PERFIL DEL EDITOR SE RESUELVE COMO EN EL LISTADO Y EL DICTAMEN (6-sep-2026) ══
   ...
   · `fijo` distingue los perfiles fijos del dueño de los que pueden desaparecer
...
/* EL PORCENTAJE DE CONTRIBUCIÓN SE RESUELVE UNA SOLA VEZ, AQUÍ (12-sep-2026).
   `lib/ganancia.aplicaContribucion` decide por TIPO DE TRABAJO (el art. 120 de
   la Ley 418/1997 grava los contratos de OBRA PÚBLICA: una interventoría es
   consultoría) y el usuario puede declarar que su administración ya la lleva
   dentro. Hasta hoy esta regla solo llegaba al panel Piso/Techo: `lib/apu/calculo`
   cobraba el 5 % SIEMPRE y `lib/apu/rentabilidad` lo sumaba a τ tal cual, así
   que la MISMA respuesta servía dos pisos distintos ($308.500.000 y
   $324.736.842) y el veredicto cambiaba de «ningún precio rentable» a uno que sí
   lo era. Se calcula una vez y se PASA a los dos motores.
   Sin `tipo_trabajo` (una llamada a mano, sin el enlace de la tarjeta) se degrada
   a cobrarla, que es el comportamiento anterior y el prudente. */
function contribucionPctDe(datos) {
  const cfg = (datos && datos.config && typeof datos.config === "object") ? datos.config : {};
  return (aplicaContribucion(datos && datos.tipo_trabajo) && !cfg.contribucion_en_administracion)
    ? CONTRIBUCION_PCT : 0;
}
...
async function presupuestoDe(redis, datos, perfil, { conCredencial = true } = {}) {
  ...
  const presupuesto = calcularPresupuesto({
    ...
    /* El porcentaje de contribución viaja DENTRO de la configuración del
       presupuesto: es el único sitio por el que `calcularPresupuesto` puede
       enterarse de que este contrato no la causa. */
    config: { ...(datos.config || {}), contribucion_pct: contribucionPctDe(datos) },
    catalogo,
    parametros: par,
  });
...
module.exports = async function handler(req, res) {
  ...
  if (accion === "parametros") {
    ...
    if (metodoCrudo === "POST") {
      ...
      /* TOKEN PRESENTE E INVÁLIDO = 401, TAMBIÉN AQUÍ (12-sep-2026). Esta rama
         retornaba ANTES del bloque de token opcional de más abajo y era la ÚNICA
         lectura del árbol que se saltaba la regla: con un token malo respondía
         200 byte a byte igual que sin token, mientras `catalogo`, `listar` y
         `pulso` respondían 401. Quien se molestó en mandarlo tiene que enterarse
         de que está mal. Sin token la lectura sigue abierta: los parámetros
         normativos son públicos. */
  ...
  if (accion === "catalogo") {
    try {
      ...
      const base = {
        ...
        /* ¿EL CATÁLOGO DE REDIS QUEDÓ ATRÁS? (12-sep-2026). La comparación ya
           existía —`lib/handlers/admin/cargar_catalogo` la hace igual— pero vivía
           en un endpoint que la pantalla NUNCA consulta por GET (`public/app.js`
           solo llama a admin por POST), así que un catálogo viejo cotizaba en
           silencio: medido, Redis en 2.0.0 servía un `precio_base` de 16.752
           donde la semilla desplegada dice 33.504. Es un dato del CATÁLOGO, no
           una cifra del perfil: no hay nada que redactar y viaja sin credencial.
           Sin versión en Redis (una carga vieja, leída por hashes) viaja `null`:
           «no sé qué versión hay» no es «está desactualizado». */
        version_semilla: SEMILLA._meta.version,
        desactualizado: m.version_catalogo == null ? null : m.version_catalogo !== SEMILLA._meta.version,
        aviso: AVISO,
  ...
  if (accion === "rentabilidad") {
    ...
    if (redis) {
      try {
        /* LA REGLA DE LECTURA DEL LISTADO (24-sep-2026), la misma que ya llama el desglose: un
           `Promise.all` con un solo `catch` perdía la baja (que sí se había leído) cuando fallaba
           la competencia, y las dos llegaban en null SIN `motivo`: el panel decía «no hay
           procesos anteriores comparables» y «menos de 5 procesos con oferentes contados» sobre
           una entidad cuya tarjeta, en la misma carga, decía «3 % bajaron los que ganaron» (medido
           en Chromium). Cada índice que no se pudo leer viaja como su centinela y responde su
           registro «no se leyó»; sin meta no hay índice, como en la tarjeta. `require` diferido:
        ...
        const iComp = lecturaComp.leido ? lecturaComp.indice : INDICE_NO_LEIDO;
        const iBaja = lecturaBaja.leido ? lecturaBaja.indice : INDICE_NO_LEIDO;
        if (!lic.entidad && lic.nit_entidad && iComp) {
        ...
        mercado = {
          ...
          no_se_leyo: [lecturaComp.leido ? null : "competencia", lecturaBaja.leido ? null : "baja"].filter(Boolean),
    ...
    /* LA MISMA función que ya usó `presupuestoDe` para calcular el presupuesto:
       el piso del panel y el margen del presupuesto tienen que descontar la
       contribución con el MISMO porcentaje o son dos cifras del mismo proceso. */
    const contribucionPct = contribucionPctDe(datos);
  ...
  if (accion === "guardar") {
    ...
    /* ═══ EL SISTEMA APRENDE DE LOS PRECIOS QUE VOS CORREGÍS ═════════════════
       ...
       Los precios de los perfiles fijos del dueño NO llevan TTL aunque el
  ...
  /* ══════════════════ ia · precios buscados por una sesión de Claude Code ══════════════════
     ...
     cableado: seis entradas sobre un borrador GUARDADO (sin borrador no hay
     nada que buscar ni dónde dejar la respuesta): GET estado · GET expediente
     (la sesión) · GET encargo (el texto para pegar en un chat) · POST solicitar
     (encola y DESPIERTA la rutina si está configurada) · POST motor sesión
     (progreso o propuesta) · POST motor pegado (la respuesta de un chat).
     Exige token: la solicitud y la propuesta son de un perfil concreto. */
  if (accion === "ia") {
    ...
    /* el expediente (la sesión) y el encargo (el chat) salen de la MISMA cotización:
       dos unitarios distintos del mismo ítem es el defecto que este proyecto ya pagó */
    const expedienteDe = async () => {
      ...
      return IA.armarExpediente({ presupuesto: registro, cotizacion: r });
    };
    if (metodo === "GET" && String(q.expediente || "") === "1") {
      const exp = await expedienteDe();
    ...
    /* ── el ENCARGO para pegar en un chat (13-sep-2026) ──
       El dueño saca los APU pegando su prompt en un chat: aquí va ese prompt
       YA ARMADO (contexto, ítems y esquema) en un solo texto. La respuesta
       vuelve por POST {motor:"pegado", texto} y pasa por la misma verificación
       que la de la sesión. Funciona sin ninguna variable configurada. */
    if (metodo === "GET" && String(q.encargo || "") === "1") {
      const exp = await expedienteDe();
      return res.status(200).json({ ok: true, encargo: true, perfil, id, version: exp.version, texto: IA.encargoDe(exp), resumen: exp.entrada.resumen,
        como_devolver: { metodo: "POST", url: "/api/apu?op=ia", cuerpo: { perfil, id, motor: "pegado", texto: "<la respuesta completa del chat>" }, cabecera: "x-historico-token" } });
    }
    if (metodo === "GET") {
      ...
      /* ── La solicitud que ENVEJECE en silencio (5-sep-2026) ──
         ...
         `pendientes=1` (la cola que se atiende con /precios) NO se toca: allí
         sigue valiendo «en_cola», que es lo que esa sesión busca. */
      /* Con la rutina DESPERTADA el plazo es otro (13-sep-2026): la sesión arranca
         en minutos y su primer «progreso {hecho:0}» saca la solicitud de «en_cola»;
         media hora sin esa señal es una sesión que no llegó (red del entorno
         cerrada, cuota agotada, token caducado), no una cola que espera turno. */
      const fueDespertada = !!(sol && sol.despertada && sol.despertada.ok);
      const guardado = sol ? sol.estado : prop ? "listo" : "sin_solicitud";
      /* Y una sesión que ENMUDECIÓ a medio camino (revisión adversaria del 13-sep-2026):
         «buscando» con el último progreso de hace más de SESION_VIVA_MIN también es
         «sin_atender» — si no, la barra se quedaba en «67 %» para siempre y el botón
         Buscar, deshabilitado mientras «buscando», no dejaba pedirlo otra vez. */
      const UMBRAL_SIN_ATENDER_MIN = guardado === "buscando" ? IA.SESION_VIVA_MIN : fueDespertada ? 30 : 180;   // dos horas sin progreso · media hora despertada · tres horas sin nadie
      ...
      const tProg = sol && sol.progreso && sol.progreso.actualizado_el ? Date.parse(sol.progreso.actualizado_el) : NaN;
      const sinSenalMin = Number.isFinite(tProg) ? Math.max(0, Math.floor((Date.now() - tProg) / 60000)) : null;
      const estado = guardado === "en_cola" && edadMin != null && edadMin > UMBRAL_SIN_ATENDER_MIN ? "sin_atender"
        : guardado === "buscando" && sinSenalMin != null && sinSenalMin > UMBRAL_SIN_ATENDER_MIN ? "sin_atender"
        : guardado;
    ...
    if (datos.solicitar === true) {
      ...
      /* ── DESPERTAR a quien atiende la cola (13-sep-2026) ──
         Hasta hoy la solicitud esperaba a que alguien escribiera /precios a mano.
         Con la rutina configurada (RUTINA_PRECIOS_URL + RUTINA_PRECIOS_TOKEN),
         «Buscar» la dispara por HTTP y la sesión arranca sola. Un segundo «Buscar»
         dentro de RUTINA_REPETIR_MIN sobre una solicitud ya despertada y viva NO
         dispara otra sesión (cada disparo gasta una corrida del día) y devuelve
         la que hay. `despertada` es OBSERVACIÓN: null sin rutina; ok:false con
         motivo cuando no se pudo; nunca la promesa de que alguien vendrá. */
      const previa = await leer(claveSol);
      /* Dos formas de estar VIVA (la segunda la encontró la revisión adversaria del
         13-sep-2026: medir solo el disparo pisaba a una sesión que llevaba 16 min
         trabajando y había mandado progreso hacía 1 min): en cola y despertada hace
         menos de RUTINA_REPETIR_MIN, o «buscando» con progreso más reciente que
         SESION_VIVA_MIN — venga de la rutina o de un /precios a mano. Una fecha
         ilegible o del futuro no cuenta como viva: se dispara, que es el lado seguro. */
      const minDesde = (iso) => { const t = Date.parse(iso || ""); return Number.isFinite(t) ? (Date.now() - t) / 60000 : null; };
      const enColaDespertada = previa && previa.estado === "en_cola" && previa.despertada && (previa.despertada.ok || previa.despertada.indeterminada) ? minDesde(previa.despertada.el) : null;
      const trabajando = previa && previa.estado === "buscando" && previa.progreso ? minDesde(previa.progreso.actualizado_el) : null;
      const viva = (enColaDespertada != null && enColaDespertada >= 0 && enColaDespertada < IA.RUTINA_REPETIR_MIN)
        || (trabajando != null && trabajando >= 0 && trabajando < IA.SESION_VIVA_MIN);
      if (viva) {
        const hace = Math.floor(trabajando != null && trabajando >= 0 ? trabajando : enColaDespertada);
        return res.status(200).json({ ok: true, encolada: true, repetida: true, perfil, id, estado: previa.estado,
          solicitud: { ...previa, despertada: previa.despertada ? { ...previa.despertada, repetida: true } : null },
          nota: previa.estado === "buscando" ? `Ya hay una sesión trabajando en este borrador (último avance hace ${hace} min): no se abrió otra.`
            : `Ya hay una búsqueda en marcha para este borrador (despertada hace ${hace} min): no se abrió otra.` });
      }
      /* Y el candado ATÓMICO del disparo (revisión adversaria, 13-sep-2026): tres
         «Buscar» simultáneos leían la misma `previa` y abrían tres sesiones. SET NX
         con la ventana de «repetida»; si el disparo falla se suelta para poder
         reintentar; si expira (indeterminado) se conserva. Sin rutina no hay candado. */
      const candado = CLAVES.apuIaDisparo(perfil, id);
      let disparoTomado = true;
      if (IA.hayRutina()) {
        try { disparoTomado = (await redis.set(candado, "1", { nx: true, ex: IA.RUTINA_REPETIR_MIN * 60 })) === "OK"; }
        catch { disparoTomado = true; }   // sin Redis para el candado, mejor disparar que callar
      }
      if (!disparoTomado) {
        const sol = previa || { id, perfil, nombre: registro.nombre || null, estado: "en_cola", solicitado_el: null, respondida_el: null, progreso: null, despertada: null };
        return res.status(200).json({ ok: true, encolada: true, repetida: true, perfil, id, estado: sol.estado,
          solicitud: { ...sol, despertada: { ...(sol.despertada || { ok: true, el: null, motivo: null, sesion_url: null }), repetida: true } },
          nota: "Otra pulsación de «Buscar» acaba de disparar la búsqueda para este borrador: no se abrió otra." });
      }
      const despertada = await IA.despertarRutina({ id, perfil });
      if (despertada && despertada.ok === false && !despertada.indeterminada) { try { await redis.del(candado); } catch { /* el candado caduca solo */ } }
      const sol = { id, perfil, nombre: registro.nombre || null, objeto: String(registro.objeto || "").slice(0, 200) || null, departamento: registro.departamento || null, ciudad: ciudad || registro.ciudad || null,
        filas: Array.isArray(registro.items) ? registro.items.length : 0, estado: "en_cola", solicitado_el: new Date().toISOString(), respondida_el: null, progreso: null, despertada };
      ...
      return res.status(200).json({ ok: true, encolada: true, perfil, id, estado: "en_cola", solicitud: sol, despertada,
        nota: despertada && despertada.ok ? "La solicitud quedó en cola y la rutina que la atiende fue despertada: los precios aparecen aquí con su fuente, su fecha y su enlace; usted decide cuáles usar."
          : despertada ? `La solicitud quedó en cola, pero la rutina no se pudo despertar: ${despertada.motivo}.`
          : "La solicitud quedó en cola. Este despliegue no tiene rutina configurada: la atiende una sesión de Claude Code (/precios) o usted mismo con el encargo (op=ia&encargo=1) y la respuesta pegada (motor «pegado»)." });
    }
    ...
    /* UNA guardada para los dos orígenes (la sesión y lo pegado de un chat): la
       misma verificación, la misma forma en Redis, el mismo «listo». */
    const guardarPropuesta = async (crudo, origen, queHacer) => {
      const v = IA.verificarPropuesta(crudo, registro);
      if (!v.ok) return { status: 400, cuerpo: { ok: false, motivo: "forma", detalle: v.detalle, error: `La respuesta no tiene la forma del esquema: ${v.detalle.join(" ")}`, que_hacer: queHacer } };
      const guardada = { ...v.propuesta, guardada_el: new Date().toISOString(), origen, apartados: v.apartados, resumen: v.resumen };
      try {
      ...
      } catch (e) { return { status: 503, cuerpo: { ok: false, error: `No se pudo guardar la propuesta. Reintente. (${e.message})` } }; }
      return { status: 200, cuerpo: { ok: true, guardado: true, perfil, id, estado: "listo", origen, resumen: v.resumen, apartados: v.apartados } };
    };
    if (datos.motor === "sesion") {
      const r = await guardarPropuesta(datos.propuesta, "sesion:claude-code", "Devuelva un objeto con la forma exacta de «esquema» del expediente.");
      return res.status(r.status).json(r.cuerpo);
    }
    /* la respuesta de un CHAT, pegada tal cual (13-sep-2026): se saca el objeto de
       entre la prosa y las vallas; nada legible es 400 con qué hacer, jamás una
       propuesta a medias */
    if (datos.motor === "pegado") {
      const crudo = IA.extraerJSON(datos.texto);
      if (!crudo) return res.status(400).json({ ok: false, motivo: "ilegible", error: "En lo pegado no hay un objeto con la forma del encargo. Pegue la respuesta completa del chat, desde la primera llave «{» hasta la última «}».", que_hacer: "Si el chat respondió con tablas y no con el objeto, pídale «responda solo con el objeto JSON del esquema» y vuelva a pegar." });
      /* el «qué hacer» le habla al contratista, no a una sesión (revisión adversaria, 13-sep-2026) */
      const r = await guardarPropuesta(crudo, "pegado:chat", "Pídale al chat que responda solo con el objeto del esquema, completo y sin cambiar los nombres de los campos, y vuelva a pegar la respuesta entera.");
      return res.status(r.status).json(r.cuerpo);
    }
    return res.status(400).json({ ok: false, error: "El cuerpo debe traer «solicitar: true» (encolar), «motor: \"sesion\"» con «progreso» (avance) o con «propuesta» (los APU), o «motor: \"pegado\"» con «texto» (la respuesta de un chat)." });
  }
