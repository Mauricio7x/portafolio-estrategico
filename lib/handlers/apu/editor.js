...
const { CLAVES, APU_TTL_SEG, escribirJSONComprimido, leerJSONComprimido, descomprimir } = require("../../almacen.js");
const { idCanonico, PERFILES } = require("../../perfiles.js");
const {
  ...
  return {
    ...
    como_leerlo: {
      motor: "El catálogo cotiza la mano de obra POR DÍA (jornales calibrados con un contrato adjudicado) y no divide "
        ...
        + "los días de mano de obra por unidad; el EPP como % de la mano de obra. Ver docs/metodologia.md.",
...
module.exports = async function handler(req, res) {
  ...
  if (accion === "ia") {
    ...
    /* la cola, para la sesión: todas las solicitudes vivas de todos los perfiles */
    if (metodo === "GET" && String(q.pendientes || "") === "1") {
      let claves = [];
    ...
    if (metodo === "GET") {
      ...
      return res.status(200).json({ ok: true, perfil, id, estado: sol ? sol.estado : prop ? "listo" : "sin_solicitud", solicitud: sol, propuesta: prop,
        como_leerlo: "«en_cola»: una sesión de Claude Code atenderá la solicitud y los precios aparecerán aquí con su fuente; «listo»: la propuesta está y cada precio se acepta fila a fila." });
    }
    if (datos.solicitar === true) {
      const sol = { id, perfil, nombre: registro.nombre || null, objeto: String(registro.objeto || "").slice(0, 200) || null, departamento: registro.departamento || null,
        filas: Array.isArray(registro.items) ? registro.items.length : 0, estado: "en_cola", solicitado_el: new Date().toISOString(), respondida_el: null };
    ...
    return res.status(400).json({ ok: false, error: "El cuerpo debe traer «solicitar: true» (encolar) o «motor: \"sesion\"» con «propuesta» (devolver precios)." });
  }
