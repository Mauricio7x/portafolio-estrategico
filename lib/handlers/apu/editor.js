...
const ACCIONES = ["catalogo", "inferir", "calcular", "cotizar", "rentabilidad", "guardar", "cargar", "listar",
  "importar", "extraer-texto", "descargar", "parametros"];
...
function respuestaParametros(P, C, lectura, historial) {
  ...
  return {
    ...
    como_leerlo: {
      ...
      verificacion: "«verificado» = contrastado con la norma; «referencia» = metodología sectorial (IDU/INVIAS) vía "
        + "fuentes secundarias, pendiente de contraste con el manual original; «supuesto» = decisión declarada.",
...
module.exports = async function handler(req, res) {
  ...
  if (accion === "calcular") {
    ...
    try {
      ...
      const px = await require("../../apu/precios.js").leerPreciosUsuario(redis, perfil);
      /* Los parámetros normativos (jornada, EPP) se leen AQUÍ, no en el motor:
         el motor es aritmética pura. Con Redis caído sirven los DEFAULTS y la
         respuesta lo declara en `parametros_costo.fuente`. */
