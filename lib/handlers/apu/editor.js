...
async function presupuestoDe(redis, datos, perfil) {
...
module.exports = async function handler(req, res) {
  ...
  if (accion === "calcular") {
    ...
    try {
      ...
      const { presupuesto: r, catalogo } = await presupuestoDe(redis, datos, perfil);
  ...
  if (accion === "rentabilidad") {
    ...
    try {
      ...
      ({ presupuesto } = await presupuestoDe(redis, datos, perfil));
    } catch (e) {
      return res.status(500).json({ ok: false, error: `No se pudo calcular: ${e.message}` });
    }
