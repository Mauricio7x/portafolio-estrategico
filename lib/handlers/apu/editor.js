...
module.exports = async function handler(req, res) {
  ...
  if (accion === "ia") {
    ...
    if (metodo === "GET") {
      ...
      const UMBRAL_SIN_ATENDER_MIN = fueDespertada ? 30 : 180;   // media hora despertada · tres horas sin nadie
    ...
    if (datos.solicitar === true) {
      ...
      const viva = previa && previa.despertada && previa.despertada.ok && ["en_cola", "buscando"].includes(previa.estado)
        ? (Date.now() - Date.parse(previa.despertada.el)) / 60000 : null;
      if (viva != null && Number.isFinite(viva) && viva >= 0 && viva < IA.RUTINA_REPETIR_MIN) {
        return res.status(200).json({ ok: true, encolada: true, repetida: true, perfil, id, estado: previa.estado, solicitud: { ...previa, despertada: { ...previa.despertada, repetida: true } },
          nota: `Ya hay una búsqueda en marcha para este borrador (despertada hace ${Math.floor(viva)} min): no se abrió otra.` });
      }
      const despertada = await IA.despertarRutina({ id, perfil, nombre: registro.nombre || null });
    ...
    const guardarPropuesta = async (crudo, origen) => {
      const v = IA.verificarPropuesta(crudo, registro);
      if (!v.ok) return { status: 400, cuerpo: { ok: false, motivo: "forma", detalle: v.detalle, error: `La respuesta no tiene la forma del esquema: ${v.detalle.join(" ")}`, que_hacer: "Devuelva un objeto con la forma exacta de «esquema» del expediente." } };
    ...
    if (datos.motor === "sesion") {
      const r = await guardarPropuesta(datos.propuesta, "sesion:claude-code");
    ...
    if (datos.motor === "pegado") {
      ...
      const r = await guardarPropuesta(crudo, "pegado:chat");
      return res.status(r.status).json(r.cuerpo);
    }
