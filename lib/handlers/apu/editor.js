...
const ERROR_SIN_PERFIL = "Falta el perfil con el que está trabajando: elija uno en la barra de Licitaciones "
  + "(o entre con su RUP) y vuelva a intentarlo. Se manda como «perfil» en el cuerpo o en la dirección.";
async function resolverPerfil(redis, q, cuerpo, accion) {
  ...
  if (!crudo) {
    ...
    return { ok: false, status: 400, error: ERROR_SIN_PERFIL, perfiles: perfilesFijos() };
  }
...
module.exports = async function handler(req, res) {
  ...
  let perfil = null, perfilFijo = true;
  {
    ...
    if (!r.ok) {
      return res.status(r.status).json({ ok: false, error: r.error, perfil_caducado: !!r.perfil_caducado, ...(r.perfiles ? { perfiles: r.perfiles } : {}) });
    }
