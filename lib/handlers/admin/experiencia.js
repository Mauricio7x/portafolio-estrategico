...
const { PERFILES, idCanonico, recargarPerfiles } = require("../../perfiles.js");
const {
...
/* ⚠️ DE QUIÉN ES LA EXPERIENCIA QUE SE CARGA (28-ago-2026).
   El archivo del repositorio es literalmente el de Génesis —lo dice su nombre y
   lo dicen sus 106 contratos—, así que `?origen=repositorio` fija `genesis` sin
   preguntar: dejarlo al criterio de quien pulse el botón es cómo se acabó
   atribuyéndole a Helder una experiencia que no es suya. La carga manual PIDE
   `?perfil=`; sin él se guarda sin dueño (`null`) y entonces no se le atribuye
   a nadie —«no consta de quién es» no es «de todos»—, que es lo que hace que la
   omisión se note en la pantalla en vez de mentir en silencio. */
const PERFIL_DEL_REPOSITORIO = "genesis";
...
module.exports = async function handler(req, res) {
  ...
  /* El dueño va en la QUERY por la misma razón que el origen: `validarContratos`
     ignora las claves extra de la raíz del cuerpo, así que un `perfil` mal
     escrito DENTRO del JSON se guardaría como `null` con un 200 idéntico —dueño
     equivocado y sin síntoma—. En la query, un valor desconocido es un 400
     ruidoso. */
  const perfilPedido = String(q.perfil || "").trim();
  ...
  if (metodo === "GET" && desdeRepositorio) {
    ...
    return res.status(405).json({
      ...
      como_hacerlo: "Abra /admin.html y pulse «1 · Cargar Experiencia Génesis» (o «Hacer los tres pasos»). "
  ...
  if (metodo === "GET") {
    ...
    if (!guardada || !Array.isArray(guardada.contratos) || !guardada.contratos.length) {
      return res.status(200).json({
        ...
        perfil: null, perfil_nombre: null,
    ...
    /* De quién es. `null` = no consta (cargada antes del 28-ago-2026): se dice
       así, no se rellena con el primer perfil que haya. */
    const duenoId = meta.perfil ? idCanonico(meta.perfil) : null;
    // el nombre que se enseña es el del perfil VIGENTE (un RUP cargado manda
    // sobre el del repositorio), no el del respaldo
    if (duenoId) { try { await recargarPerfiles(redis); } catch { /* se sirve el respaldo */ } }
    const dueno = duenoId && PERFILES[duenoId] ? PERFILES[duenoId] : null;
    return res.status(200).json({
      ok: true, cargada: true,
      version: meta.version || null,
      cargado: meta.cargado || null,
      perfil: duenoId,
      perfil_nombre: dueno ? dueno.nombre : null,
  ...
  /* El archivo del repositorio ES el de Génesis: su dueño no se negocia ni se
     puede sobrescribir desde la query. En la carga manual, un perfil que no
     existe se rechaza ANTES de escribir: guardar contratos con un dueño
     inventado es peor que no guardarlos. */
  let perfilDueno = PERFIL_DEL_REPOSITORIO;
  if (!desdeRepositorio) {
    perfilDueno = perfilPedido ? idCanonico(perfilPedido) : null;
    if (perfilDueno && !Object.prototype.hasOwnProperty.call(PERFILES, perfilDueno)) {
      return res.status(400).json({
        ok: false, origen,
        error: `No existe el perfil «${perfilPedido}»: no se guardó nada.`,
        perfiles_validos: Object.keys(PERFILES),
        como_hacerlo: "POST /api/admin/experiencia?perfil=helder (o genesis, o consorcio) con el JSON de contratos en el cuerpo.",
      });
    }
  }
  ...
  let guardado = null;
  try {
    guardado = await guardarExperiencia(redis, v.contratos, perfilDueno);
  } catch (e) {
  ...
  return res.status(200).json({
    ...
    // de quién quedó: quien carga tiene que poder comprobarlo sin volver a leer
    ...
    nota: desdeRepositorio
      ? `Experiencia cargada desde «${ARCHIVO_REPOSITORIO}» del repositorio, a nombre de Génesis (es su archivo). Ejecute la auditoría de cobertura para ver qué códigos le faltan basados en su experiencia real.`
      : guardado.perfil
        ? "Experiencia cargada. Ejecute la auditoría de cobertura para ver qué códigos le faltan basados en su experiencia real."
        : "Experiencia cargada SIN DUEÑO: no se atribuye a ningún perfil y la auditoría de cobertura no la usará. Vuelva a cargarla con ?perfil=… para decir de quién es.",
  });
};
