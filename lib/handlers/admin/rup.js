...
const { validarConfig, validarPerfilDinamico, derivarUnspsc } = require("../../config_rup.js");
const {
  ...
  restablecerPerfiles, IDS, ID_DUENO, CANDIDATOS_CONSORCIO, derivarPlural,
} = require("../../perfiles.js");
const { extraerRupDeTexto } = require("../../rup_pdf.js");
const { generarIdDinamico, crearPerfilDinamico, MAX_PERFILES_DINAMICOS, guardaDeAltaPublica } = require("../../perfil_dinamico.js");
/* La redacción única de «sin presupuesto no hay UNA capacidad» (lib/consorcio.js).
   Require de primer nivel sin ciclo: lib/consorcio solo carga
...
const { ID_DINAMICO_RE } = require("../../perfil_dinamico.js");
// las cuatro whitelists derivadas que escribe cada carga (fija o dinámica)
/* Los cuatro sufijos y la lista de claves de un perfil dinámico se IMPORTAN
   de `lib/perfil_dinamico`, que es quien los escribe: una copia local se
   desincroniza el día que se añada un quinto. */
const { SUFIJOS_UNSPSC, clavesDePerfilDinamico } = require("../../perfil_dinamico.js");
/* El tope de perfiles dinámicos vivos (MAX_PERFILES_DINAMICOS) y la creación
   viven en lib/perfil_dinamico: la puerta de entrada (Fase 2) crea perfiles
   por el mismo camino, y dos copias del tope o de la escritura divergirían. */

/* Resumen legible de lo que quedó cargado (lo que enseña «Perfil actual»).
   REHECHO el 25-sep-2026 por encargo del dueño: enseñaba tres fichas iguales
   —Helder, Génesis y el consorcio fijo 50/50 con un tope de 11.000 salarios—,
   sin PRODIAC ni ningún «Helder + PRODIAC». Ahora dice lo que es el negocio:
   SU EMPRESA (el dueño), sus SOCIOS POSIBLES (el censo `CANDIDATOS_CONSORCIO`,
   no una lista escrita aquí) y, por cada socio, el CONSORCIO calculado con la
   MISMA regla que usa toda la app (`derivarPlural`, fórmula del pliego tipo), sin
   tope: desde el 26-sep-2026 el consorcio no lleva ninguno salvo que se fije a
   propósito (lo que la ley mide es la capacidad y los habilitantes). Los indicadores del consorcio no dependen del reparto,
   así que se enseñan una vez. */
function fichaDe(p) {
  const d = derivarUnspsc([...p.unspsc]);
  return {
    id: p.id, nombre: p.nombre, tamano_empresa: p.tamanoEmpresa != null ? p.tamanoEmpresa : null,
    codigos: p.unspsc.size, clases: d.clases.length, familias: d.familias.length, segmentos: d.segmentos.length,
    liquidez: p.liquidez ?? null, endeudamiento: p.endeudamiento ?? null, cobertura_intereses: p.coberturaIntereses ?? null,
    patrimonio: p.patrimonio ?? null, capital_trabajo: p.capitalTrabajo ?? null,
    contratos: p.contratosRup ?? null, experiencia_segmento72_smmlv: p.expSeg72SMMLV ?? null,
    profesionales: p.profesionales ?? null, tope_smmlv: p.topeSMMLV ?? null,
  };
}
function resumenPerfiles() {
  const dueno = PERFILES[ID_DUENO];
  if (!dueno) return { empresa: null, socios: [], consorcios: [] };
  const socios = CANDIDATOS_CONSORCIO.filter((id) => PERFILES[id]).map((id) => PERFILES[id]);
  return {
    empresa: fichaDe(dueno),
    socios: socios.map(fichaDe),
    consorcios: socios.map((s) => {
      const pl = derivarPlural([
        { perfil: dueno, perfilId: dueno.id, participacion: 0.5 },
        { perfil: s, perfilId: s.id, participacion: 0.5 },
      ], { nombre: `${dueno.nombre} + ${s.nombre}` });
      return {
        con: s.id, ...fichaDe({ ...pl, id: `${dueno.id}+${s.id}` }),
        metodo: pl.metodoIndicadores, metodo_texto: pl.metodoIndicadoresTexto,
        falta_balance_de: pl.indicadoresFaltaBalanceDe,
        indeterminados: pl.indicadoresIndeterminados,
      };
    }),
  };
}
...
async function cargarRupDesdePdf(req, res) {
  ...
  /* CUOTA POR CONEXIÓN, ANTES DE LEER EL CUERPO (M-SEG-07, 6-sep-2026). La otra
     alta pública. MISMA función que la puerta de entrada de tres datos —una sola
     regla, en lib/perfil_dinamico— para que las dos no diverjan a la primera
     corrección del tope. El desalojo del más viejo se conserva intacto: la cuota
     limita a UNA conexión, no cierra la puerta. */
  const guarda = await guardaDeAltaPublica(crearRedis({}), req, req.query || {});
  ...
  if (completar && typeof completar === "object" && !Array.isArray(completar)) {
    ...
    for (const [campo, valor] of Object.entries(completar)) {
      ...
      if (campo === "experiencia_smmlv") {
        ...
        // sin tope por defecto: lo que decide es la capacidad de contratación (26-sep-2026)
  ...
  if (!extraccion.completo) {
    return res.status(200).json({
      ...
      como_seguir: "Escriba los datos que faltan y vuelva a enviar el mismo certificado junto con «completar». "
        + "Quedan guardados en su perfil: solo se piden una vez.",
    });
  }

  const v = validarPerfilDinamico("rup", extraccion.config);
  if (!v.ok) {
    ...
    return res.status(400).json({
      ...
      error: `Un dato del certificado no pasa la validación y no se guardó nada: ${legibles.join(" ")} `
        + "Corrija ese valor al completar los datos, o cargue el RUP a mano desde la pestaña «Mi empresa».",
  ...
  /* guardado con TTL por la vía ÚNICA de lib/perfil_dinamico (ids del servidor,
     cuatro whitelists, tope de vivos, perfil escrito al final). */
  const creado = await crearPerfilDinamico(redis, { perfil: v.perfil, meta: { origen: "pdf", vigencia: extraccion.vigencia } });
  if (!creado.ok) return res.status(creado.status).json({ ok: false, error: creado.error });
  const d = creado.derivado;

  /* LA K TECHO YA NO SE PUBLICA (13-sep-2026). Aquí salía `crp(creado.perfil, 0)`
     y el comentario de esta misma línea NOMBRABA el defecto —«K estimada (techo):
     … factor E en su máximo»— y la servía igual: sin presupuesto el factor E de
     la Guía CCE-EICP-GI-22 se va al mejor escalón, y la cifra queda por encima
     del mayor contrato que de verdad pasa la puerta P2 (medido ejecutando `p2K`:
     +12,9 % en helder, +50,0 % en prodiac). La K frente a CADA proceso la calcula
     la app al servir, y esa sí es una capacidad. Sin licitación elegida viaja
     `null` con el motivo que redacta lib/consorcio.js (una sola copia). Lo que
     el certificado SÍ publica —la K declarada en salarios mínimos— sigue
     viajando aparte en `k_declarada_smmlv`: ese es un dato, no una estimación. */
  const id = creado.id;
  ...
  return res.status(200).json({
    ...
    k: null,
    k_motivo: MOTIVO_CAPACIDAD_SIN_PRESUPUESTO,   // por qué `k` va vacía: nunca un hueco mudo
...
async function eliminarRup(res, redis, q) {
  ...
  if (ID_DINAMICO_RE.test(crudo)) {
    ...
    /* La lista de claves la da `lib/perfil_dinamico`, que es donde se CREAN:
       el desalojo por tope lleno necesita exactamente la misma y dos listas
       divergen a la primera clave que alguien añada — la que se quedara corta
       dejaría basura huérfana con el perfil ya borrado. Son claves LITERALES
       (borrar una ausente es un no-op), así que no se depende de la semántica
       de comodines de SCAN para un id conocido. */
    const claves = clavesDePerfilDinamico(crudo);
    let borradores = [];
    try {
    ...
    return res.status(200).json({
      ...
      perfil: crudo,
      tipo: "dinamico",
      claves_eliminadas: claves.length + borradores.length,
...
/* la pantalla «Perfil actual» se prueba con la función real (tests/e2e.js) */
module.exports.resumenPerfiles = resumenPerfiles;
