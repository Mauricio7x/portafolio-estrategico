...
function adjudicacionDeFila(diaCierre, { fechaPliego = null, plazo = null } = {}) {
  const delPliego = fechaPliego && /^\d{4}-\d{2}-\d{2}$/.test(String(fechaPliego)) ? String(fechaPliego) : null;
  ...
  const desfase = desfasada ? { pliego_desfasado: delPliego } : {};
  if (!diaCierre || !plazo || typeof plazo !== "object") return desfasada ? { fecha: null, origen: null, ...desfase } : null;
  ...
  if (med == null || !Number.isFinite(med) || med < 0 || !(base >= minimo)) {
    return desfasada ? { fecha: null, origen: null, ...desfase } : null;
  }
  return {
