...
function diasRestantes(fechaIso, ahora) {
  const t = Date.parse(fechaIso);
  if (!Number.isFinite(t)) return null;
  // el dataset publica hora Colombia flotante: se compara contra ahora − 5 h
  return Math.floor((t - (ahora - OFFSET_COLOMBIA_MS)) / 86400000);
}
...
async function contarOportunidades(redis, perfilId, perfilVivo, ahora = Date.now()) {
  ...
  const muestra = viables
    .map(({ l }) => ({
      ...
      objeto: String(l.descripcion || l.objeto || l.nombre_del_procedimiento || "").slice(0, 220),
