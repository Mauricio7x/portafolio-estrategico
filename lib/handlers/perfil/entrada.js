...
function adjudicacionDeFila(diaCierre, { fechaPliego = null, plazo = null } = {}) {
  if (fechaPliego && /^\d{4}-\d{2}-\d{2}$/.test(String(fechaPliego))) return { fecha: String(fechaPliego), origen: "pliego" };
  if (!diaCierre || !plazo || typeof plazo !== "object") return null;
  ...
  if (med == null || !Number.isFinite(med) || med < 0 || !(base >= minimo)) return null;
  return {
