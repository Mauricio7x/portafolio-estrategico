...
const { crearPerfilDinamico, guardaDeAltaPublica } = require("../../perfil_dinamico.js");
const { configDesdeManual, ACTIVIDADES } = require("../../perfil_manual.js");
/* La redacción de «sin presupuesto no hay UNA capacidad» viaja desde
   lib/consorcio.js: una sola copia para todas las pantallas que la dicen. El
   require es de primer nivel y NO cierra ciclo: lib/consorcio solo carga
   perfiles/capacidad/almacen arriba y difiere el resto dentro de sus funciones. */
const { MOTIVO_CAPACIDAD_SIN_PRESUPUESTO } = require("../../consorcio.js");
...
// M-DGF-08: el plazo de adjudicación de la entidad, tal como lo publica el índice
const { hechosDeEntidad } = require("../../indice_competencia.js");
...
async function contarOportunidades(redis, perfilId, perfilVivo, ahora = Date.now()) {
  ...
  /* La cascada compartida ya deja pasar lo que el dueño alcanza CON SOCIO y
     dice con cuál (`conSocio`): este conteo no tiene que saber nada de socios,
     solo no descartar lo que ella rescató. Si lo descartara, «Hoy hay N» y
     «Ver las N» volverían a decir una N que la lista no enseña — la brecha del
     17-ago-2026, por el otro lado. */
  ...
  for (const l of cascada.visibles) {
    ...
    if (!puertas.pasa_todas && !cascada.conSocio.get(l)) continue;
  ...
  /* CUÁNDO ADJUDICAN (M-DGF-08, 6-sep-2026): la fecha de adjudicación que el
     pliego publicó (guardada al leerlo, un HGETALL) y, para lo que el pliego
     no dice, el plazo que la ENTIDAD tardó en adjudicar según el índice de
     competencia (`hechosDeEntidad`, sobre el índice ya cargado por el
     listado). Las dos son best-effort: sin ellas el calendario dice que no se
     sabe, y nunca se cae por esto. */
  let fechasAdjudicacion = {};
  try { fechasAdjudicacion = await Manifestacion.leerFechasCronograma(redis, { clave: Manifestacion.CLAVE_CRONOGRAMA_ADJUDICACION }); } catch { fechasAdjudicacion = {}; }
  let plazoDe = null;
  try {
    const { cargarIndice } = require("../procesos/listar.js");
    const { indice } = await cargarIndice(redis);
    if (indice) plazoDe = (l) => hechosDeEntidad(indice, l).plazo_adjudicacion;
  } catch { plazoDe = null; }
  ...
  return {
    ...
    agregados: agregarPulso(viables.map((v) => v.l), ahora, { fechasCronograma, fechasAdjudicacion, plazoDe }),
  };
}
...
function agregarPulso(procesos, ahora = Date.now(), { fechasCronograma = {}, fechasAdjudicacion = {}, plazoDe = null } = {}) {
  ...
  return {
    ...
    calendario: calendarioDeCierres(procesos, ahora, { fechasCronograma, fechasAdjudicacion, plazoDe }),
...
/* CUÁNDO ADJUDICAN una fila del calendario (M-DGF-08). Un publicado gana a un
   calculado: la fecha del cronograma del pliego, si se leyó; si no, el cierre
   más la mediana de días hábiles que la entidad tardó en adjudicar (solo con
   la base mínima que el índice publica; `sumarHabiles` es la misma cuenta de
   lib/habiles con la que se midió el plazo); si no hay ninguna, null — el
   calendario dice que no se sabe. Una fecha ilegible del pliego no se toma.

   NI UNA ANTERIOR O IGUAL AL CIERRE VIGENTE (remate B9b-H1, 6-sep-2026 · un
   defecto reproducido): el índice YA rechazaba ese mismo par como
   `no_posterior_al_cierre` («adjudicar el mismo día del cierre o antes no es un
   plazo») y aquí la fecha del pliego se tomaba sin mirar el cierre — hermano
   vivo de la guarda. Pasa de verdad: la entidad prorroga el cierre después de
   que el usuario leyó el pliego (la propia aplicación mide esas prórrogas) y la
   fecha guardada queda atrás. Cuando ocurre no se calla: se cae al histórico si
   lo hay, y la fila lleva `pliego_desfasado` para que la ficha diga que esa
   fecha quedó antes del cierre y hay que verificarla en el cronograma. */
function adjudicacionDeFila(diaCierre, { fechaPliego = null, plazo = null } = {}) {
  const cruda = fechaPliego && /^\d{4}-\d{2}-\d{2}$/.test(String(fechaPliego)) ? String(fechaPliego) : null;
  /* ⚠️ Y TAMPOCO UNA DE UN AÑO QUE NO ES DE ESTE CALENDARIO (13-sep-2026 · un
     defecto reproducido). La guarda de arriba solo miraba hacia ATRÁS: hacia
     DELANTE no había cota ninguna, y `2202-01-01` o `9999-12-31` —el hito se
     extrae del pliego por REGEX de línea, y el dataset trae años imposibles de
     verdad: el `1970` del timestamp nulo, el `2202` documentado— se publicaban
     como `origen: "pliego"` y, por ser un PUBLICADO, desplazaban a la estimación
     por el histórico. Se copia el criterio de la hermana, que ya estaba bien
     hecha: `lib/manifestacion` descarta la fecha del cronograma cuyo año no es
     operable (`anioImposible`) y la hace VIAJAR en `fecha_cronograma_descartada`
     para que el descarte se pueda auditar. Aquí se llama a la MISMA guarda
     —`habiles.fechaOperable`, la que el índice aplica a esta misma fecha en
     `plazoAdjudicacionDe`— y lo descartado viaja en `pliego_fuera_de_calendario`.
     NO SE INVENTA UN TOPE de días: una adjudicación lejana pero dentro del
     calendario es un plazo largo, no un dato corrupto, y el propio índice dejó
     escrito que un tope a los plazos largos sería un umbral inventado. */
  const fueraDeCalendario = cruda != null && !habiles.fechaOperable(cruda);
  const delPliego = fueraDeCalendario ? null : cruda;
  // sin día de cierre no hay con qué comparar: la del pliego es lo único que hay
  const desfasada = delPliego != null && diaCierre != null && delPliego <= String(diaCierre);
  if (delPliego && !desfasada) return { fecha: delPliego, origen: "pliego" };
  /* lo descartado viaja SIEMPRE, con o sin estimación detrás: el usuario ya leyó
     esa fecha en el pliego y tiene que poder ver que no se usó, y por qué */
  const desfase = {
    ...(desfasada ? { pliego_desfasado: delPliego } : {}),
    ...(fueraDeCalendario ? { pliego_fuera_de_calendario: cruda } : {}),
  };
  const descartada = desfasada || fueraDeCalendario;
  if (!diaCierre || !plazo || typeof plazo !== "object") return descartada ? { fecha: null, origen: null, ...desfase } : null;
  const med = plazo.mediana_dias_habiles == null ? null : Number(plazo.mediana_dias_habiles);
  const base = Number(plazo.base), minimo = Number(plazo.min_procesos) || 5;
  if (med == null || !Number.isFinite(med) || med < 0 || !(base >= minimo)) {
    return descartada ? { fecha: null, origen: null, ...desfase } : null;
  }
  return {
    fecha: habiles.sumarHabiles(diaCierre, med),
    origen: "historico",
    dias_habiles: med,
    base,
    adjudicados: plazo.adjudicados == null ? null : Number(plazo.adjudicados),
    ...desfase,
  };
}

function calendarioDeCierres(procesos, ahora = Date.now(), { fechasCronograma = {}, fechasAdjudicacion = {}, plazoDe = null } = {}) {
  ...
  for (const l of procesos) {
    ...
    let plazo = null;
    if (typeof plazoDe === "function") { try { plazo = plazoDe(l); } catch { plazo = null; } }
    const fila = {
      id,
      ...
      // M-DGF-08: cuándo adjudican — pliego > histórico de la entidad > null
      adjudicacion: adjudicacionDeFila(dia, { fechaPliego: (id && fechasAdjudicacion[id]) || null, plazo }),
    };
...
function paraCalendario(m) {
  ...
  return {
    ...
    estado: m.estado,                                   // abierta | por_confirmar | pudo_vencer | vencida | sin_fecha | por_abrir
    /* la señal publicada de SECOP II (fase × estado), que el calendario ya
       sabía leer y esta lista blanca no dejaba pasar (22-sep-2026) */
    secop_recibia: m.secop_recibia, secop_fecha_legible: m.secop_fecha_legible,
    secop_posicion: m.secop_posicion, secop_fase: m.secop_fase, secop_en_sorteo: m.secop_en_sorteo, contradiccion: m.contradiccion,
    secop_observaciones_cerradas: m.secop_observaciones_cerradas,
    origen_vencimiento: m.origen_vencimiento,          // "cronograma" | "fase_secop" | null
    ...
    hora_limite_legible: m.hora_limite_legible,         // solo si el pliego publicó la hora de corte
...
function resumenPerfil(perfilVivo, derivado) {
  return {
    ...
    /* LA K SOLO EXISTE FRENTE A UN PRESUPUESTO (13-sep-2026). Aquí se publicaba
       `crp(perfilVivo, 0)`, que NO es la capacidad: es el TECHO. Sin presupuesto
       `lib/capacidad.factorE` se va al mejor escalón («sin presupuesto no hay
       ratio que exigir»), así que la cifra salía POR ENCIMA del mayor contrato
       que de verdad pasa la puerta P2 —medido ejecutando `p2K`: +12,9 % en
       helder, +50,0 % en prodiac— y esta es la PRIMERA cifra que ve alguien que
       acaba de subir su RUP. `lib/puertas.js:p2K`, `lib/consorcio.js` y
       `handlers/perfil/pulso.js` ya se guardaban de esto; esta era una de las dos
       hermanas que quedaban vivas (la otra, la carga de RUP). No hay fórmula que escribir: sin licitación elegida la
       capacidad viaja en null CON SU MOTIVO —la redacción única de
       lib/consorcio.js— para que la pantalla diga por qué está vacía en vez de
       enseñar un hueco mudo. El par de nombres es el MISMO que ya publica el
       simulador de consorcio (`capacidadContratacion` / `capacidadMotivo`): dos
       nombres distintos para lo mismo divergirían a la primera corrección. */
    capacidadContratacion: null,
    capacidadMotivo: MOTIVO_CAPACIDAD_SIN_PRESUPUESTO,
...
module.exports = async function handler(req, res) {
  ...
  /* CUOTA POR CONEXIÓN, ANTES DE LEER EL CUERPO (M-SEG-07, 6-sep-2026). Es una de
     las dos altas públicas: sin este freno, una serie de altas desde una misma
     conexión desaloja a los 300 visitantes que ya subieron su certificado y gasta
     el reconocimiento de imágenes de pago. Va delante de `leerCuerpo` a propósito:
     quien está fuera de cuota no consigue que el servidor acepte 6 MB. El 429 dice
     que los datos SÍ llegaron y cuándo volver; la regla vive una sola vez en
     lib/perfil_dinamico y la comparte la carga de RUP en PDF. */
  const redis = crearRedis({});
  const guarda = await guardaDeAltaPublica(redis, req, req.query || {});
  if (!guarda.permitido) return res.status(guarda.status).json(guarda.cuerpo429);
...
module.exports.calendarioDeCierres = calendarioDeCierres;
module.exports.adjudicacionDeFila = adjudicacionDeFila;
/* se exporta para que la cerradura EJECUTE la función real (el patrón de
   `adjudicacionDeFila` y `calendarioDeCierres`): una regla escrita no es una
   cerradura, y comprobar por regex que una función se llama no prueba lo que
   publica. */
module.exports.resumenPerfil = resumenPerfil;
