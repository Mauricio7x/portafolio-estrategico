...
function agregarPulso(procesos, ahora = Date.now()) {
  ...
  return {
    ...
    sinEntidad,
    top: TOP_PULSO,
    /* ═══ TRES REPARTOS MÁS PARA GRAFICAR (encargo del ingeniero, ago 2026) ═══
       «Datos verdaderos, certeros, claros… que el cliente pueda obtener con una
       sola mirada». Los tres salen de `FiltrosLista.facetas`, que es la MISMA
       función que cuenta las facetas del listado: una segunda definición de
       «esto es obra» o de «esta modalidad» divergiría a la primera corrección,
       y además haría que el pulso y la lista discreparan sobre el mismo corpus.
       Cada cubeta es EXACTAMENTE un filtro de public/filtros.js, así que cada
       barra lleva a su lista —igual que `porCierre` y `porCuantia`—; una
       partición más bonita daría barras que no llevan a ninguna parte.
       `manifestacion.urgentes` es el dato más accionable del sistema y no
       estaba en la pestaña: son los procesos donde hay que avisar HOY que le
       interesa, y el plazo puede ser de horas. */
    ...(() => {
      try {
        const f = FiltrosLista.facetas(procesos, FiltrosLista.crearClasificador({ ahora }));
        return { porTipo: f.tipo, porModalidad: f.modalidad, manifestacion: f.manifestacion };
