...
const { esConsorcio, cargarConsorcio, MOTIVO_CAPACIDAD_SIN_PRESUPUESTO } = require("../../consorcio.js");
...
/* ═══ LA EMPRESA EN CIFRAS (Mi empresa como pestaña principal, ago 2026) ═══
   ...
   mínimos), contratos acreditados y tope. Patrimonio, capacidad de
   contratación, indicadores financieros y NIT son CIFRAS (y datos) DEL PERFIL
   (la regla del token): viajan solo con
...
function empresaEnCifras(perfil, p, finanzasVisibles) {
  ...
  return {
    ...
    /* EL TOPE ES UNA CIFRA DEL PERFIL (12-sep-2026). `lib/publico.js` anula
       `tope_cop` con su motivo escrito —«revela el tamaño de contrato que la
       empresa se permite»— y aquí salía el MISMO tope en salarios mínimos, sin
       credencial: 4.000 × SMMLV da exactamente el `tope_cop` que se redactaba
       allí. Va detrás de la misma puerta que las demás cifras. La vista de
       visitante no pierde nada: para un perfil `rup_…` `finanzasVisibles` ya es
       true (quien tiene el id subió su propio RUP, y está declarado arriba). */
    tope_smmlv: finanzasVisibles ? numONull(p.topeSMMLV) : null,
    finanzas_visibles: !!finanzasVisibles,
    patrimonio: finanzasVisibles ? numONull(p.patrimonio) : null,
    /* LA K SOLO EXISTE FRENTE A UN PRESUPUESTO (12-sep-2026). Aquí se publicaba
       `crp(p, 0)`, y con presupuesto 0 el factor E de la Guía CCE-EICP-GI-22 se
       va al mejor escalón («sin presupuesto no hay ratio que exigir»): la cifra
       salía POR ENCIMA del mayor contrato que de verdad pasa la puerta P2
       (medido: Helder 5.798.971.989 frente a 5.134.946.589; Prodiac +50,0 %), y
       la pantalla la rotula «Puede facturar hasta». `lib/puertas.js:p2K` ya se
       guardó de esto; esta era su hermana viva. No hay fórmula que escribir: sin
       proceso la capacidad viaja en null CON SU MOTIVO, para que la pantalla
       diga por qué está vacía en vez de enseñar un hueco mudo. */
    capacidad_contratacion: null,
    capacidad_contratacion_motivo: finanzasVisibles ? MOTIVO_CAPACIDAD_SIN_PRESUPUESTO : null,
    /* LOS DEMÁS DATOS DEL REGISTRO, DETRÁS DE LA MISMA PUERTA (7-sep-2026,
       M-COMP-07). La ficha en Excel los necesita para que el dueño los copie a
       los formatos que exija cada pliego, y salen de aquí porque `empresa` ya
       se resuelve POR PETICIÓN (fuera de la caché `pulso:{perfil}`) y ya aplica
       la regla del token. El NIT entra en el mismo lado que el patrimonio a
       propósito: identifica a una persona natural por su documento, así que
       tiene menos derecho a salir sin credencial que una cifra, no más. */
    nit: finanzasVisibles ? (p.nit ? String(p.nit) : null) : null,
    liquidez: finanzasVisibles ? numONull(p.liquidez) : null,
    endeudamiento: finanzasVisibles ? numONull(p.endeudamiento) : null,
    cobertura_intereses: finanzasVisibles ? numONull(p.coberturaIntereses) : null,
    capital_trabajo: finanzasVisibles ? numONull(p.capitalTrabajo) : null,
    utilidad_operacional: finanzasVisibles ? numONull(p.utilidadOp) : null,
...
module.exports = async function handler(req, res) {
  ...
  try {
    let cacheado = null;
    /* `?refrescar=1` SOLO CON CREDENCIAL: es el único salto de caché público del
       repositorio y detrás hay la lectura más cara que existe (SCAN + MGET del
       corpus entero, inflate y cascada). Sin token es INERTE —nunca 401, para no
       romper un enlace— igual que `?baja_max=`. Ninguna pantalla lo usa. */
    const refrescar = String(q.refrescar || "") === "1" && finanzasVisibles;
    if (!refrescar) {
