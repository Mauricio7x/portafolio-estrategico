...
async function alertasDelPerfil(redis, perfil, ahora, { conGuia = false, guardados = null } = {}) {
  ...
  const procesos = ids.map((id) => {
    const e = S.enriquecer(g.procesos[id], vivas.get(id) || null, ahora, { fechaManifestacionCronograma: cron[id] || null });
    return conGuia ? { ...e, guia: guiaPara(g.procesos[id], vivas.get(id) || null, perfil, ctxG, ahora, cron[id] || null, docsPorId.get(id) || null) } : e;
  }).sort((a, b) => {
  ...
  return { procesos, alertas, resumen };
}
...
module.exports = async function handler(req, res) {
  ...
  const { procesos, alertas, resumen } = await alertasDelPerfil(redis, perfil, Date.now(), { conGuia: true, guardados: g });
