...
async function detalleCompetencia(idProceso, guardado, { log = () => {} } = {}) {
  ...
  if (codigoEntidad && nits.length) {
    ...
    }, "veces ante la entidad"), TIEMPO_MAX_MS).then((r) => { for (const g of r || []) veces.set(String(g.nit_proveedor), { veces: num(g.veces), ultima: String(g.ultima || "").slice(0, 10) || null }); })
  ...
  if (nitEntidad && nits.length) {
    ...
    }, "ganadas ante la entidad"), TIEMPO_MAX_MS).then((r) => { for (const g of r || []) ganadas.set(String(g.nit_del_proveedor_adjudicado), { ganadas: num(g.ganadas), ultima: String(g.ultima || "").slice(0, 10) || null, valor: num(g.valor) != null && num(g.valor) > 0 ? Math.round(num(g.valor)) : null }); })
  ...
  return {
    ...
    proponentes: lista.map((p) => S.fichaCompetidor(p, { veces: p.nit ? veces.get(p.nit) : null, ganadas: p.nit ? ganadas.get(p.nit) : null, vigentes: p.nit ? vigentes.get(p.nit) : null })),
...
module.exports = async function handler(req, res) {
  ...
  const resumen = { total: procesos.length, abiertos: procesos.filter((p) => p.cerrado === false).length, cerrados: procesos.filter((p) => p.cerrado === true).length,
    presentados: procesos.filter((p) => p.estado === "presentado").length, avisos_proximos: procesos.flatMap((p) => p.avisos).filter((a) => a.dias_antes <= 7).length };
  return res.status(200).json({ ok: true, perfil, resumen, procesos, estados: S.ESTADO_ETIQUETA,
    como_leerlo: "Sus procesos guardados con la versión VIVA del corpus: estado, días para el cierre, hitos y avisos a 7/3/1 días. Cuando el proceso cierra, «detalle» trae quiénes se presentaron y la ficha de cada uno." });
};
