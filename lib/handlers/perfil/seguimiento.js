...
/* ═══ CON QUIÉN CONVIENE IR, CONGELADO AL GUARDAR (11-sep-2026) ═══
   Encargo del dueño: «que al momento de dar guardar en mis procesos me diga con
   quién conviene más y la justificación técnica simple». La lista sigue
   enseñando TODO lo alcanzable —solo o con cualquiera de las dos socias—; el
   VEREDICTO, que es lo que hay que leer despacio, se calcula aquí y viaja al
   expediente, que es donde el navegador entra justo después de guardar.

   SE CONGELA, por el mismo motivo que la predicción (F0-7): es el consejo del
   día en que DECIDIÓ. Si se recalculara en cada lectura, cargar mañana el
   registro de otra socia —o corregir un tope— cambiaría en silencio el consejo
   de un proceso que ya estudió, y no habría forma de saber sobre qué decidió.
   Lleva su fecha para que se vea.

   NO CUESTA UNA SOLA LECTURA DE MÁS: la fila viva del corpus ya está en la mano
   en el mismo punto donde se congela la predicción, y el módulo es puro (0,239
   ms por fila, medido). Por eso va FUERA del candado, como aquella.

   Best-effort con la misma regla: si falla, `null` CON su motivo y el guardado
   sigue. Perder el proceso por no poder calcular un consejo sería cambiar el
   producto por su adorno. */
function congelarSocio(perfilId, viva) {
  const enBlanco = (motivo) => ({ recomendacion: null, frase: null, motivo, congelado_el: new Date().toISOString() });
  try {
    const { ID_DUENO, CANDIDATOS_CONSORCIO } = require("../../perfiles.js");
    /* solo para el perfil del dueño: un visitante con su RUP subido no tiene
       socias que ofrecerle, y suponerle unas sería inventarle un consorcio */
    if (perfilId !== ID_DUENO || !CANDIDATOS_CONSORCIO.length) return null;
    if (!viva) return enBlanco("el proceso no está en el corpus");
    const { socioPorProceso } = require("../../socio_por_proceso.js");
    const r = socioPorProceso({ fila: viva, base: perfilId, candidatos: CANDIDATOS_CONSORCIO });
    if (!r || !r.recomendacion) return enBlanco("no se pudo decidir con la información del proceso");
    return { ...r, motivo: null, congelado_el: new Date().toISOString() };
  } catch (e) { return enBlanco(`no se pudo calcular: ${e.message}`); }
}
...
function guiaPara(guardado, fila, perfil, ctx, ahoraMs, fechaManifestacionCronograma = null, documentos = null) {
  try {
    /* la ETAPA del guardado entra a la guía (22-sep-2026): a quien ya avisó que le
       interesa no se le pide avisar, y el requisito del sorteo se juzga por ella */
    return guiaDe({ fila: fila || null, foto: guardado.foto || null, perfil, ctx: { conocimiento: ctx.conocimiento, competencia: fila ? ctx.compDe(fila) : null, baja: fila ? ctx.bajaDe(fila) : null, ahoraMs, fechaManifestacionCronograma, documentos, etapa: guardado.estado || null } });
...
/* Los perfiles que TIENEN procesos guardados (6-sep-2026, M-COMP-03). Es un
   CENSO por la clave, no la lista de los perfiles fijos: un perfil
...
module.exports = async function handler(req, res) {
  ...
  if (metodo === "POST") {
    ...
    const socioNuevo = d.enterado === true || pre ? null : congelarSocio(p, viva);
    let r;
    try {
      r = await conCandado(redis, claveCandado(p), CANDADO_CORTO_TTL_SEG, async () => {
        ...
        if (!existente && viva) {
          ...
          foto = { ...(foto && foto.nombre ? foto : fv), estado_secop: fv.estado_secop, fase: fv.fase, secop_visto: fv.secop_visto };
        }
        ...
        /* el veredicto de socio sigue la MISMA regla: solo al crear, y lo que
           venga del cliente se ignora — lo calcula el servidor */
        const socio = existente ? (existente.socio || null) : (socioNuevo || congelarSocio(p, viva));
        if (prediccion) {
        ...
        g.procesos[id] = {
          ...
          prediccion, socio, carpeta, tareas, documentos,
        };
        await escribirJSON(redis, clave(p), g);
