...
module.exports = async function handler(req, res) {
  ...
  const resumen = { total: procesos.length, abiertos: procesos.filter((p) => p.cerrado === false).length, cerrados: procesos.filter((p) => p.cerrado === true).length,
    ...
    manifestaciones_abiertas: procesos.filter((p) => p.manifestacion && p.manifestacion.vencida === false).length,
    manifestaciones_urgentes: procesos.filter((p) => p.manifestacion && p.manifestacion.vencida === false && p.manifestacion.quedan_habiles != null && p.manifestacion.quedan_habiles <= 2).length,
