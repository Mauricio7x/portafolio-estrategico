...
/* EL DÍA DE COLOMBIA LO FIJA EL SERVIDOR, nunca el reloj del navegador: el
   calendario del cronograma marca «hoy» con él, y cinco horas de diferencia
   corren la fecha un día entero — que en esta app es la diferencia entre
   llegar y no llegar. Es la misma regla que el calendario de cierres. */
...
module.exports = async function handler(req, res) {
  ...
  const procesos = ids.map((id) => S.enriquecer(g.procesos[id], vivas.get(id) || null, ahora, { fechaManifestacionCronograma: cron[id] || null }))
    .sort((a, b) => {
  ...
  return res.status(200).json({ ok: true, perfil, hoy: hoyColombia(ahora), resumen, alertas, procesos, estados: S.ESTADO_ETIQUETA, orden_estados: S.ESTADOS,
