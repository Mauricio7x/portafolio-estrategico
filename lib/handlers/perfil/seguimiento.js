...
function guiaPara(guardado, fila, perfil, ctx, ahoraMs, fechaManifestacionCronograma = null) {
  try {
    return guiaDe({ fila: fila || null, foto: guardado.foto || null, perfil, ctx: { conocimiento: ctx.conocimiento, competencia: fila ? ctx.compDe(fila) : null, baja: fila ? ctx.bajaDe(fila) : null, ahoraMs, fechaManifestacionCronograma } });
...
module.exports = async function handler(req, res) {
  ...
  if (metodo === "POST") {
    ...
    /* F0-7 · la predicción se congela al CREAR y NO se vuelve a calcular: es la
       ...
       cuerpo se IGNORA: la calcula el servidor (ver congelarPrediccion). */
    let prediccion = existente ? (existente.prediccion || null) : await congelarPrediccion(redis, p, id);
    if (prediccion) {
    ...
    /* la guía viaja en la MISMA respuesta del guardado: es lo que hace que «al
       ...
       segunda pulsación. Solo al crear (al cambiar de etapa no hace falta). */
    let guia = null;
    if (!existente) {
      const vivas = await filaViva(redis, [id]);
      const ctxG = await contextoGuia(redis, p);
      let cronG = {}; try { cronG = await leerFechasCronograma(redis); } catch { cronG = {}; }
      guia = guiaPara(g.procesos[id], vivas.get(id) || null, p, ctxG, Date.now(), cronG[id] || null);
    }
    return res.status(200).json({ ok: true, perfil: p, id, guardado: g.procesos[id], total: Object.keys(g.procesos).length, guia });
  }
  ...
  const procesos = ids.map((id) => ({ ...S.enriquecer(g.procesos[id], vivas.get(id) || null, ahora, { fechaManifestacionCronograma: cron[id] || null }), guia: guiaPara(g.procesos[id], vivas.get(id) || null, perfil, ctxG, ahora, cron[id] || null) }))
    .sort((a, b) => {
