...
const { CLAVES, leerChunksDedup, leerJSON, leerJSONComprimido } = require("../../almacen.js");
const { PERFILES, ALIAS_PERFIL, evaluarRup } = require("../../rup.js");
const { modalidad_competitiva, estado_abierto, evaluarObjeto, filtrarProcesosVisibles } = require("../../filtros.js");
const {
  codigosDeLicitacion, normalizarCodigo, indiceDe, emparejar, cubiertoPorPrefijo, claseDe,
} = require("../../unspsc.js");
const { leerEquivalencias, leerEquivalenciasMeta, explicarEquivalencias } = require("../../equivalencias.js");
const { vocabularioActivo } = require("../../texto_unspsc.js");
const { SMMLV, recargarPerfiles } = require("../../perfiles.js");
const { leerIndice, competenciaDe, INDICE_NO_LEIDO } = require("../../indice_competencia.js");
const { evaluarPuertas } = require("../../puertas.js");
const { censarColumnasHistoricas } = require("../../columnas_historicas.js");
const { rastrear } = require("../../rastreo.js");
const { leerIndiceBaja, leerIndiceBajaMeta, bajaDeMercado, MIN_PROCESOS: MIN_BAJA } = require("../../indice_baja.js");
...
module.exports = async function handler(req, res) {
  ...
  const anticipoMin = q.anticipo_min !== undefined ? parseFloat(q.anticipo_min) || 0 : require("../../filtros.js").ANTICIPO_MIN_DEFAULT;   // la MISMA constante que la cascada, no un 20 a mano

  const redis = crearRedis({});
  const t0 = Date.now();
  let filas, meta, clavesAct, clavesHist, equivalencias = null, eqMeta = null, vocabRedis = null, vocMeta = null;
  let indiceComp = null, indiceBaja = null, bajaMeta = null, censoIngesta = null;
  let errorLecturaIndice = null; // texto del fallo si el índice de competencia no se pudo leer
  let errorLecturaBaja = null; // ídem, el índice de baja (23-sep-2026)
  try {
    ...
    /* NO LEÍDO no es NO CONSTRUIDO (23-sep-2026): el fallo viaja como centinela
       (competenciaDe responde `motivo: "no_se_leyo"`) y se publica abajo. */
    try { indiceComp = await leerIndice(redis); } catch (e) {
      indiceComp = INDICE_NO_LEIDO;
      const { tacharClave } = require("../../apu_ocr.js");
      errorLecturaIndice = tacharClave(String((e && e.message) || e || "fallo sin mensaje")).slice(0, 160);
    }
    eqMeta = await leerEquivalenciasMeta(redis);
    if (eqMeta && !eqMeta.vacio) equivalencias = await leerEquivalencias(redis);
    /* NO LEÍDO no es NO CONSTRUIDO, también en la baja (23-sep-2026): el fallo
       viaja como el mismo centinela (`bajaDeMercado` responde «no_se_leyo») y
       `baja_de_mercado` deja de contar 0 entidades y 0 % de cobertura. */
    try { indiceBaja = await leerIndiceBaja(redis); bajaMeta = await leerIndiceBajaMeta(redis); }
    catch (e) {
      indiceBaja = INDICE_NO_LEIDO;
      const { tacharClave } = require("../../apu_ocr.js");
      errorLecturaBaja = tacharClave(String((e && e.message) || e || "fallo sin mensaje")).slice(0, 160);
    }
    // censo de descartes de la INGESTA: es lo único que ve lo que el embudo
    // NO puede ver, porque el embudo censa el corpus ya guardado
    try { censoIngesta = await leerJSON(redis, CLAVES.censoIngesta); }
    catch { /* diagnóstico opcional: nunca tumba el diagnóstico */ }
  ...
  /* ─────────── ¿POR QUÉ NO ESTÁ ESTE PROCESO? (`?buscar=`) ───────────
     Responde ANTES de construir el embudo: es otra pregunta y no necesita el
     recorrido entero. El juicio NO se reimplementa — se le inyecta a
     lib/rastreo el `evaluarRup`/`evaluarPuertas` que ya usa la app, para que
     la explicación sea la de la decisión que de verdad se tomó. */
  if (q.buscar !== undefined) {
    /* «Servido» se decide con la MISMA `filtrarProcesosVisibles` que sirve
       /api/oportunidades y /api/resumen, sobre la fila suelta y con las
       opciones por defecto del listado. Un segundo criterio «equivalente hoy»
       diría «apartado por el juicio» de un proceso que la app está enseñando
       —lo cazó la suite: un proceso servido volvía como `en_corpus`—, y una
       herramienta de diagnóstico que contradice a la app no sirve para
       verificar nada. Es la regla de la casa: la cascada vive UNA vez. */
    const evaluar = (l) => {
      const { visibles, veredictos, descartes } = filtrarProcesosVisibles([l], perfilId, conocimiento);
      const rup = veredictos.get(l) || evaluarRup(l, perfilId, conocimiento);
      if (!visibles.length) {
        /* EN QUÉ PASO EXACTO MURIÓ. La cascada ya lo publica y se estaba
           tirando: con una sola fila, la cubeta que vale 1 es la que lo
           descartó. Sin ella el caso más frecuente —ya cerró— se responde con
           «no pasa el juicio para este perfil», que además es falso. */
        const descarte = Object.keys(descartes || {}).find((k) => descartes[k] > 0) || null;
        return { rup, puertas: null, visible: false, descarte };
      }
      const puertas = evaluarPuertas(l, perfilId, { rup, competencia: competenciaDe(indiceComp, l) });
      return { rup, puertas, visible: !!puertas.pasa_todas };
    };
    // LOS DOS censos, no uno: un proceso que tiró la carga completa no tiene
    // por qué estar en el del último delta, y mirar solo uno respondería
    // «no consta» sobre un descarte que sí consta.
    const censos = ["delta", "full"]
      .filter((k) => censoIngesta && censoIngesta[k] && censoIngesta[k].censo)
      .map((k) => ({ origen: k, censo: censoIngesta[k].censo }));
    /* `campo` acota DÓNDE buscar (todo | entidad | objeto | proceso) y
       `modalidad` filtra por cómo se adjudica. Los dos son opcionales y un
       valor desconocido es INERTE — lo resuelve `lib/rastreo`, que es quien
       tiene la lista: validarlos aquí sería una segunda copia. */
    const r = rastrear(q.buscar, { filas, evaluar, censos, campo: q.campo, modalidad: q.modalidad });
    return res.status(r.ok ? 200 : 400).json({
      ...r, perfil: perfilId,
      censo_ingesta: censoIngesta,
      como_leerlo: "`donde` dice en cuál de los cuatro sitios está el proceso: `servido` (la app lo enseña), "
        + "`en_corpus` (guardado pero apartado por el juicio, con su motivo), `descartado_en_ingesta` "
        + "(la cascada de ingesta lo tiró, con el motivo del censo) o `no_consta` (la app no lo ha leído). "
        + "`no_consta` NUNCA significa que el proceso no exista en SECOP II: significa que la app no lo tiene.",
    });
  }
  ...
  const embudo = {
    ...
    fuera_anticipo: 0,
    visibles: 0,
    /* de las visibles, cuántas están ahí porque las alcanza un socio y no el
       dueño solo. NO es un `fuera_*`: es una porción de `visibles`. */
    rescatadas_con_socio: 0,
  ...
  const distribucion_puertas = {
    fallan_solo_caja: 0,   // visibles NO rescatadas que solo cierra la caja (P3)
  ...
  /* `ninguno` es el tier de una fila que el registro del DUEÑO no cubre y que
     está visible porque la alcanza un socio (11-sep-2026). Tiene que existir
     como casilla o el reparto deja de sumar los visibles, y ese cuadre es lo
     que permite que este endpoint verifique a la app. */
  const porTier = { clase: 0, familia: 0, equivalente: 0, texto: 0, ninguno: 0 };
  /* `sin_dato` es el nivel de una fila rescatada con socio: el juicio de
     pertinencia del objeto no llegó a emitirse para el dueño porque su registro
     no cubre esa actividad. Tiene que existir como casilla o el reparto deja de
     sumar los visibles (11-sep-2026). */
  const porPertinencia = { verde: 0, amarillo: 0, rojo: 0, sin_dato: 0 };
  ...
  /* Los plurales del dueño, derivados una vez; el predicado es el MISMO que usa
     la cascada compartida, para que las dos cuenten igual por construcción. Los
     `require` van diferidos: cerrarían un ciclo. */
  const pluralesDiag = (() => {
    try {
      const { ID_DUENO, CANDIDATOS_CONSORCIO } = require("../../perfiles.js");
      if (perfilId !== ID_DUENO || !CANDIDATOS_CONSORCIO.length) return [];
      return require("../../socio_por_proceso.js").pluralesDe(perfilId, CANDIDATOS_CONSORCIO);
    } catch { return []; }
  })();
  const rescataSocio = (l) => pluralesDiag.length
    ? !!require("../../socio_por_proceso.js").socioQueAlcanza(l, pluralesDiag, { conocimiento })
    : false;
  ...
  for (const l of filas) {
    ...
    /* RESCATE CON SOCIO (11-sep-2026). La cascada compartida deja pasar lo que
       el dueño alcanza con alguna de sus combinaciones; este embudo tiene que
       contar IGUAL, o el panel y el diagnóstico dirían cifras distintas de lo
       mismo. Las rescatadas van a `visibles` con su propio contador, así que la
       identidad «los fuera_* más visibles suman el total» se conserva. */
    /* Se marca aquí, pero NO se cuenta aquí: una fila rescatada sigue el camino
       normal y todavía puede caerse por el anticipo. Contarla en el punto de
       rescate inflaba el contador con filas que nunca llegaron a verse. */
    let rescatadaPorSocio = ev.paso === "unspsc" && rescataSocio(l);
    if (ev.paso === "unspsc" && !rescatadaPorSocio) { embudo.fuera_unspsc++; continue; }
    ...
    /* `topeSMMLV` puede ser null = SIN TOPE DECLARADO (un socio cuyo apetito no
       nos consta). `null * SMMLV` da 0, y con eso TODO quedaría «sobre el
       tope»: la ausencia se descarta ANTES de convertir. */
    const dentroTope = perfil.topeSMMLV == null ? true : (l.cuantia_cop || 0) <= perfil.topeSMMLV * SMMLV;
    if (!rup.capacidad_ok) {
      if (!rescatadaPorSocio && rescataSocio(l)) {
        rescatadaPorSocio = true;                            // sigue el camino normal
      } else {
    ...
    embudo.visibles++;
    if (rescatadaPorSocio) embudo.rescatadas_con_socio++;   // ya es visible de verdad
    ...
    /* LOS VISIBLES SE PARTEN EN TRES CONJUNTOS QUE NO SE SOLAPAN, y esa
       partición es lo que permite que este endpoint verifique a la app:
         pasan_todas + rescatadas_con_socio + fallan_solo_caja = visibles
       Una fila rescatada no puede pasar todas las puertas (si pasara, no
       habría hecho falta rescatarla), así que cuenta en su conjunto y no en
       el de caja aunque además falle P3: contarla en los dos la duplicaba. */
    if (!rescatadaPorSocio && !puertas.pasa_todas) distribucion_puertas.fallan_solo_caja++;
    ...
    if (muestra.length < nMuestra) {
      muestra.push({
        ...
        /* Una fila rescatada con socio no tiene etiqueta de pertinencia propia:
           el juicio del objeto no llegó a emitirse para el dueño porque su
           registro no cubre esa actividad. Se dice, en vez de dejar el campo
           vacío — un campo vacío parece un fallo del diagnóstico. */
        pertinencia: (ev.pertinencia && ev.pertinencia.etiqueta)
          || (rescatadaPorSocio ? "La alcanza un socio, no su registro" : null),
        rescatada_con_socio: !!rescatadaPorSocio,
  ...
  const baja_de_mercado = (() => {
    // sin leer no hay nada que contar: null con el motivo, jamás «0 entidades · 0 % cubierto»
    if (errorLecturaBaja) return { leido: false, error_lectura: errorLecturaBaja, construido: bajaMeta ? bajaMeta.generado : null };
  ...
  return res.status(200).json({
    ...
    /* EL PUNTO CIEGO QUE ESTE BLOQUE CIERRA: el `embudo` de arriba censa el
       corpus YA GUARDADO, así que un proceso que la cascada de INGESTA tiró no
       aparece en ninguna de sus cubetas. `censo_ingesta` es lo que ve ese
       tramo: cuántos procesos se descartaron antes de guardar y por qué motivo,
       con ejemplos. Sin él, «¿por qué no está este proceso?» no tenía respuesta
       posible (defecto del 20-ago-2026, UNIVERSIDAD PEDAGÓGICA NACIONAL).
       `null` = todavía no ha corrido ninguna sincronización desde que existe el
       censo; NO es un cero. */
    censo_ingesta: censoIngesta,
    /* ¿se pudo leer el índice de competencia? Solo alimenta P4 (que nunca
       cierra), pero sin esto un fallo dejaba la competencia «sin dato» muda. */
    lectura_indice_competencia: { leido: !errorLecturaIndice, error: errorLecturaIndice },
    lectura_indice_baja: { leido: !errorLecturaBaja, error: errorLecturaBaja },
    distribuciones,
    unspsc_cobertura,
    ...
    muestra,
    como_leerlo: "`censo_ingesta` responde lo que el embudo NO puede: por qué un proceso ni siquiera llegó al corpus "
      + "(el embudo solo censa lo ya guardado). Y `?buscar=<referencia|entidad|NIT>` lo dice para UN proceso concreto. "
      + "embudo va en orden: cada `fuera_*` son procesos que murieron en ESE paso y no llegaron al siguiente. "
