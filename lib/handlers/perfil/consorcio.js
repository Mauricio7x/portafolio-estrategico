/* lib/handlers/perfil/consorcio.js · /api/perfil?op=consorcio | op=consorcio-simular (Fase 10)
   ─────────────────────────────────────────────────────────────────────────────
     GET    ?op=consorcio                    → los consorcios guardados y `candidatos`:
                                               los perfiles individuales con los que se
                                               puede armar uno (el dueño y sus socias).
                                               La barra ofrece SOLO al dueño, así que el
                                               armador no puede sacarlos de ahí.
     ...
     POST   ?op=consorcio-simular {integrantes, proceso?, origen?, recomendar?}
                                             (con `recomendar: true` y dos integrantes
                                               sin partes: la simulación en el reparto
                                               que más le deja al primero, con
                                               `recomendacion` —frontera, motivos y
                                               avisos—, lib/consorcio.recomendarReparto)
                                             → indicadores del pliego tipo (truncados a 2),
...
module.exports = async function handler(req, res, { op = "consorcio" } = {}) {
  ...
  if (op === "consorcio-simular") {
    ...
    /* EL MISMO CONOCIMIENTO QUE EL LISTADO (12-sep-2026). El simulador juzgaba
       con `conocimiento: {}` en duro, así que las capas 6 (equivalencias) y 7
       (vocabulario) de la cascada no disparaban y el MISMO consorcio ante el
       MISMO proceso daba un veredicto OPUESTO al de la tarjeta. Se carga con la
       MISMA función memoizada por sello que ya reutiliza la puerta de entrada
       (`cargarConocimiento` de listar.js): cero cálculo nuevo. Require diferido
       como el de `procesoDe`, para no cerrar un ciclo. Si no se puede cargar, se
       sigue sin él —igual que el listado—: es opcional por construcción. */
    let conocimiento = {};
    try {
      const { cargarConocimiento } = require("../procesos/listar.js");
      conocimiento = (await cargarConocimiento(redis)).conocimiento || {};
    } catch { conocimiento = {}; }
    /* `recomendar: true` (25-sep-2026): los dos integrantes sin partes, y el
       servidor devuelve la simulación en el reparto que más le deja al dueño,
       con la frontera y sus motivos (lib/consorcio.recomendarReparto). */
    if (datos.recomendar === true) {
      const ids = (Array.isArray(datos.integrantes) ? datos.integrantes : []).map((i) => String((i && i.perfilId) || ""));
      if (ids.length !== 2) return res.status(400).json({ ok: false, error: "Para recomendar el reparto hacen falta exactamente dos integrantes: usted y el socio." });
      /* el proceso pedido ya no está en la lista viva: la misma respuesta que la
         simulación con número (`proceso_encontrado: false`), que la pantalla sabe
         decir, en vez de un «elija un proceso» a quien sí eligió uno */
      if (!proceso) return res.status(200).json({ ok: true, integrantes: [], exigencias: null, recomendacion: null, proceso: datos.proceso || null, proceso_encontrado: false, origen });
      const rr = await C.recomendarReparto(redis, { dueno: ids[0], socio: ids[1], proceso, documentos, conocimiento });
      if (!rr.ok) return res.status(rr.status || 400).json({ ok: false, error: rr.error });
      return res.status(200).json({ ...rr, proceso: datos.proceso || null, proceso_encontrado: !!proceso, origen });
    }
    const r = await C.simular(redis, { integrantes: datos.integrantes, proceso, documentos, conocimiento, contar: contarOportunidades });
  ...
  if (metodo === "GET") {
    ...
    /* CON QUIÉN SE PUEDE IR, servido aparte de DESDE QUIÉN SE NAVEGA (11-sep-2026).
       La barra ofrece un solo perfil —el dueño— porque es el único que es
       nuestro; las socias son un RECURSO, no una identidad desde la que mirar
       el mercado. Pero el armador de consorcios y el simulador «¿y con un
       socio?» leían sus nombres de esa misma barra, así que al podarla se
       quedaban sin candidatas y decían «cargue el registro del socio» teniendo
       dos cargadas. Van aquí, en una respuesta QUE YA PIDE LLAVE (autorizarToken
       arriba): son las socias del dueño, y un visitante no tiene por qué verlas
       en el fuente de la página. Se incluye al dueño para que la lista se baste
       a sí misma. */
    const { PERFILES, ID_DUENO, CANDIDATOS_CONSORCIO } = require("../../perfiles.js");
    const candidatos = [ID_DUENO, ...CANDIDATOS_CONSORCIO]
      .filter((id) => PERFILES[id])
      .map((id) => ({ id, nombre: PERFILES[id].nombre || id }));
    return res.status(200).json({
      ok: true,
      consorcios: Object.values(todos).sort((a, b) => String(b.guardado).localeCompare(String(a.guardado))),
      candidatos,
    });
  }
  if (metodo === "DELETE") {
