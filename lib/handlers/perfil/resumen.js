...
const { CLAVES, RESUMEN_TTL_SEG, leerChunksDedup, leerJSON, leerJSONComprimido } = require("../../almacen.js");
...
const { plazoMesesDe } = require("../../rup.js");
const { ANTICIPO_PLENO_PCT } = require("../../negocio.js");
const { leerIndice, leerIndiceMeta, competenciaDe, MIN_PROCESOS_DEPTO, INDICE_NO_LEIDO } = require("../../indice_competencia.js");
const { leerIndiceBaja, leerIndiceBajaMeta } = require("../../indice_baja.js");
...
/* Etiquetas de la banda de competencia, EXACTAMENTE las de public/app.js
   (COMPETENCIA_ENTIDAD): el tablero y la tarjeta tienen que decir lo mismo con
   las mismas palabras. Sin pictograma (6-sep-2026): el color lo pone la pantalla
   con su clase; un emoji servido por la API lo dibujaría el sistema operativo. */
const COMPETENCIA = {
  baja: { titulo: "Poca competencia" },
  media: { titulo: "Competencia media" },
  alta: { titulo: "Alta competencia" },
  /* «Sin datos de cuántos compiten» y no «sin datos históricos» (22-sep-2026): lo
     que falta es el dato de OFERENTES; la misma tarjeta puede estar midiendo la
     baja de esa entidad sobre ocho contratos dos centímetros más abajo, y decir
     que no hay histórico sería falso. */
  sin_dato: { titulo: "Sin datos de cuántos compiten en esta entidad" },
  /* el índice NO SE PUDO LEER (23-sep-2026): no es que la entidad no tenga datos.
     Las mismas palabras que el chip de la tarjeta (public/app.js, COMPETENCIA_ENTIDAD). */
  no_se_leyo: { titulo: "No se pudo consultar la competencia de esta entidad · vuelva a cargar" },
};
...
function badgeCompetencia(c) {
  ...
  const d = conBase ? (COMPETENCIA[nivel] || COMPETENCIA.sin_dato)
    : c && c.motivo === "no_se_leyo" ? COMPETENCIA.no_se_leyo : COMPETENCIA.sin_dato;
  if (!conBase) return d.titulo;
  return `${d.titulo} — promedio ${numEs(promedio)} oferentes en ${procesos} proceso${procesos === 1 ? "" : "s"}`;
}
...
/* `por_anio` y la ventana electoral tal como los midió el índice (lib/
   indice_competencia.medirPeriodos): aquí no se recalcula ni se redondea. */
function competenciaPeriodos(indiceMeta) {
  const per = indiceMeta && indiceMeta.periodos;
  if (!per || typeof per !== "object" || !per.por_anio || typeof per.por_anio !== "object") return null;
  return {
    por_anio: per.por_anio,
    ventana_garantias_2026: per.ventana_garantias_2026 || null,
    min_procesos: MIN_PROCESOS_DEPTO,
    construido: indiceMeta.construido || null,
  };
}
...
function totalesVacios() {
  return {
    ...
    /* `ninguno` es el tier de una fila que el registro del dueño no cubre y que
       está visible porque la alcanza un socio (11-sep-2026). Sin esa casilla el
       reparto deja de sumar los visibles, y ese cuadre es lo que permite que el
       panel y el diagnóstico se verifiquen el uno al otro. */
    por_tier_unspsc: { clase: 0, familia: 0, equivalente: 0, texto: 0, ninguno: 0 },
    por_modalidad: {},
    ...
    // las que no superan la capacidad solas pero las alcanza un socio (11-sep-2026)
    rescatadas_capacidad: 0,
  };
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Pero NO CONSTRUIDO no es NO LEÍDO (23-sep-2026): un fallo de lectura ya no
    // se traga — cada proceso sale con `motivo: "no_se_leyo"`, el reparto lo
    // cuenta aparte, la respuesta lo avisa y no se guarda en caché.
    try {
      indiceMeta = await leerIndiceMeta(redis);
      if (indiceMeta) indice = await leerIndice(redis);
    } catch (e) {
      indice = INDICE_NO_LEIDO;
      const { tacharClave } = require("../../apu_ocr.js");
      errorLecturaIndice = tacharClave(String((e && e.message) || e || "fallo sin mensaje")).slice(0, 160);
    }
    /* SIN CONSTRUIR sigue siendo opcional; NO LEÍDO no (23-sep-2026): con el
       hash sin leer, la tarjeta del panel contaba 0 entidades clasificadas y
       decía «El índice de baja no se ha construido todavía. Pulse
       «Reconstruir»» —una reconstrucción entera para un fallo pasajero—, y lo
       guardaba cinco minutos en caché. Ahora viaja `leido: false` y no se cachea. */
    try {
  ...
  totales.rescatadas_capacidad = cascada.capacidad.rescatadas_capacidad || 0;
  ...
  for (const v of visibles) {
    ...
    else totales.por_tier_unspsc.ninguno++;
    ...
    /* un proceso cuya entidad no se pudo CONSULTAR no es uno «sin histórico»:
       va a su propio cubo, que solo aparece si hubo fallo (la suma sigue cuadrando) */
    const nivel = comp && comp.motivo === "no_se_leyo" ? "no_se_leyo" : comp && comp.nivel ? comp.nivel : "sin_dato";
    totales.por_nivel_competencia_entidad[nivel] = (totales.por_nivel_competencia_entidad[nivel] || 0) + 1;
  ...
  const top_entidades = [...porEntidad.entries()]
    ...
    .map(([entidad, e]) => ({
      ...
      // sin dato NO es cero: con el índice sin leer el conteo es null
      procesos_historicos: e.comp && e.comp.total_procesos != null ? e.comp.total_procesos : null,
  ...
  const cuerpo = {
    ...
    indice_competencia: indiceMeta || errorLecturaIndice
      ? {
        construido: indiceMeta ? indiceMeta.construido : null,
        entidades: indiceMeta ? indiceMeta.clasificadas : null,
        min_procesos: indiceMeta ? indiceMeta.min_procesos : null,
        leido: !errorLecturaIndice,
        error_lectura: errorLecturaIndice,
      }
      : null,
    /* El aviso que faltaba: sin él, «de N no hay histórico» salía del panel con
       el índice sin leer. null cuando se leyó (o no está construido). */
    aviso_competencia: errorLecturaIndice
      ? `No se pudo consultar cuántos compiten en cada entidad (${errorLecturaIndice}). `
        + "Los procesos afectados van en totales.por_nivel_competencia_entidad.no_se_leyo, no en «sin_dato»: "
        + "no significa que sus entidades no tengan datos. Vuelva a cargar en unos minutos."
      : null,
    /* con el índice sin leer no hay cifras que dar (null, jamás 0 entidades):
       `leido: false` es lo que la pantalla lee para no pedir una reconstrucción */
    baja_mercado: errorLecturaBaja
      ? { leido: false, error_lectura: errorLecturaBaja, construido: bajaMeta ? bajaMeta.generado || null : null,
        entidades_clasificadas: null, baja_mediana_global: null, mas_descuentan: null, menos_descuentan: null }
      : bajaMercadoPanel(indiceBaja, bajaMeta),
    /* CUÁNTA GENTE COMPITIÓ, AÑO A AÑO Y EN EL PERÍODO ELECTORAL (6-sep-2026,
       M-DGF-14). El índice lo MIDE en cada reconstrucción (`meta.periodos`,
       sección «B2 MEDIDO Y CERRADO SIN SEGMENTAR») y solo lo servía op=historico,
       el endpoint del dueño: el tablero no lo veía nadie. Son agregados del
       mercado —ni una cifra del perfil— y viajan tal como el índice los publicó;
       el suelo para pintar un año es el MISMO que el índice exige a un
       departamento (`MIN_PROCESOS_DEPTO`), no un número nuevo. Sin índice o sin
       medición: null, nunca un objeto vacío que se lea como «0 oferentes». */
    competencia_periodos: competenciaPeriodos(indiceMeta),
  ...
  /* ---------- paso 9 · guardar en caché (TTL 300 s) ----------
     Una respuesta con el índice SIN LEER no se guarda: serviría el fallo cinco
     minutos a todo el que abriera el panel, y la petición siguiente puede leer bien. */
  if (!errorLecturaIndice && !errorLecturaBaja) {
    try {
