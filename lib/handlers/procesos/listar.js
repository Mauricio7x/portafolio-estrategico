...
const { leerIndiceBaja, leerIndiceBajaMeta, bajaDeMercado, bajaSegmentoDe } = require("../../indice_baja.js");
...
/* Borradores de APU del perfil con costo directo guardado, indexados por el
   `id_proceso` de SECOP: es la única fuente del «costo calculado» que exige
   el orden por margen (Fase 8 §8.3). SCAN + MGET sobre las propias claves,
   igual que el listado de borradores de /api/apu (la clave es la fuente de
   verdad). Un borrador guardado ANTES de que se guardara el costo directo no
   sirve para el margen y se cuenta en `sin_costo`: hay que recalcular y
   volver a guardar, y el frontend lo dice. Solo se lee cuando alguien pide
   ese orden — no encarece el listado normal. */
async function cargarCostosPorProceso(redis, perfil) {
  const porProceso = new Map();
  let claves = [];
  try { claves = await redis.scan(CLAVES.patronApuPerfil(perfil)); } catch { return { porProceso, borradores: 0, sin_costo: 0 }; }
  ...
  for (let i = 0; i < claves.length; i += 8) {
    ...
    for (const v of valores) {
      ...
      // si hay varios borradores del mismo proceso manda el más reciente
  ...
  return { porProceso, borradores, sin_costo: sinCosto };
}
...
module.exports = async function handler(req, res) {
  ...
  const evalDe = (l) => {
    ...
    const detalle = estimarPDetalle(l, {
      ...
      /* A4: la baja máxima que el dueño soporta, si la DECLARÓ (`?baja_max=`).
         Sin declarar el factor de precio es neutro: penalizar por no saber
         hasta dónde puede bajar sería inventar. */
      baja_maxima_pct: bajaMax,
    });
  ...
  if (ordenarPor === "margen") {
    costos = await cargarCostosPorProceso(redis, perfil);
  }
  const margenDe = (l) => {
    if (!costos) return null;
    const idProc = l.id_del_proceso || null;
    const c = idProc ? costos.porProceso.get(idProc) : null;
    if (!c) {
    ...
    const pt = pisoTecho({
      ...
      baja: e.baja, competencia: compDe(l),
