...
module.exports = async function handler(req, res) {
  ...
  /* ¿EL CORTE ES FRESCO? (6-sep-2026, M-INF-10). El navegador disparaba
     op=sync&modo=auto tras CADA búsqueda —cargar, cambiar de perfil, cada
     filtro, cada página— y el servidor tomaba y soltaba el candado —y tras
     «al día» corría además el índice de baja: decenas de comandos de Redis,
     la suite imprime la cifra— solo para responder «al día». Se publica el HECHO
     con la MISMA constante que decide el sync (FRESCO_MS, llamada, no
     copiada): true = el sync respondería alDia; false = toca disparar; null =
     sin corte conocido o ilegible (el navegador dispara como hasta hoy, porque
     la cadena de la full puede haber muerto). Require diferido: servir el
     listado no carga el módulo de sincronización hasta que hace falta. */
  const instanteCorte = meta && meta.last_sync ? Date.parse(meta.last_sync) : NaN;
  const { FRESCO_MS } = require("./sync.js");
  const sincronizadoFresco = Number.isFinite(instanteCorte) ? (Date.now() - instanteCorte) <= FRESCO_MS : null;
