...
/* Índice de baja de mercado. Mismo contrato que el de competencia: si no se ha
   ...
   relee) y `leido`/`error` suben a `indice_baja` de la respuesta. */
async function cargarIndiceBaja(redis) {
  let meta = null;
  try { meta = await leerIndiceBajaMeta(redis); } catch (e) { return { indice: null, meta: null, leido: false, error: textoDeFallo(e) }; }
  ...
  let indice;
  try { indice = await leerIndiceBaja(redis); } catch (e) { return { indice: null, meta, leido: false, error: textoDeFallo(e) }; }
