/* ============================================================================
   ...
   GET /api/oportunidades?perfil=helder|genesis|juntos   (alias: consorcio)
     &anticipo_min=0             (excluye anticipos DECLARADOS bajo el mínimo; 0 = sin filtro, el defecto desde el 15-sep-2026;
...
const { CLAVES, leerChunksDedup, leerJSON, leerVariosJSON, leerJSONComprimido, descomprimir } = require("../../almacen.js");
...
const { recargarPerfiles, ID_DUENO, CANDIDATOS_CONSORCIO } = require("../../perfiles.js");

/* Lo MÍNIMO que la tarjeta necesita del veredicto: si hace falta socio y si con
   él se abren todas las puertas que faltaban. Ni el nombre del socio ni el
   reparto ni el porqué: eso se lee en el expediente al guardar, y mandarlo en
   cada fila de cada página era el 14,4 % del cuerpo de la respuesta.
   `cierra_todo` NO se puede perder: es lo que distingue «con un socio sí» de
   «con un socio se acerca pero puede no bastar», y sin esa diferencia la
   tarjeta prometería una habilitación que el pliego no confirma. */
function resumenSocio(v) {
  const r = v && v.recomendacion;
  if (!r) return null;
  return { tipo: r.tipo, cierra_todo: r.tipo === "con_socio" ? !!r.cierra_todo : null };
}
const { socioPorProceso } = require("../../socio_por_proceso.js");
/* La validación y la carga del perfil (alias, fijo, `rup_…` dinámico, `cons_…`
   consorcio) viven en lib/perfil_resolver.js desde el 2-sep-2026: el dictamen
   del pliego resuelve el perfil POR LA MISMA VÍA que este listado. */
const { validarIdPerfil, cargarPerfilResuelto, baseDelPerfil } = require("../../perfil_resolver.js");
...
const { leerIndiceBaja, leerIndiceBajaMeta, bajaDeMercado, bajaSegmentoDe, bajaDepartamentoDe, encogerBaja, dondeSeMidio } = require("../../indice_baja.js");
...
const { tipoPrecio, presupuestoOficialDe } = require("../../negocio.js");
const { evaluarZona, nombreDeBase } = require("../../accesibilidad.js");
...
const { gananciaDeProceso } = require("../../ganancia.js");
...
const ORDEN_CAMPOS = {
  ...
  /* «Lo que más deja»: ganancia estimada del contrato (lib/ganancia) PONDERADA
     por la opción de ganar. Se ordena por la ponderada y no por la del contrato
     porque es la que decide dónde poner el tiempo: un contrato enorme que se
     gana una vez de cada veinte deja menos, por intento, que uno mediano que se
     gana una de cada tres. Sin cifra va al final (−1e18), jamás con un cero que
     se leería como «no deja nada». */
  /* ⚠️ SOLO LAS QUE YA COSTEÓ (24-ago-2026) · ORDENABA LA OBRA AL REVÉS.
     Sin APU costeado, `lib/ganancia` cierra el costo por la IDENTIDAD del
     precio (CD = V / (1 + (A+I+U)/100)), así que `valor` se reduce
     algebraicamente a `V × k` con `k = U/(1+A+I+U) − τ`: una CONSTANTE que
     depende de la estructura que tecleó el usuario y del tipo de trabajo, no
     del proceso. Con la estructura de referencia (15/5/5) y la contribución
     del 5 %, `k = −1 %` para OBRA y `+…` para consultoría, de modo que la
     clave era negativa por construcción en toda la obra. Medido ejecutando el
     orden real: «Lo que más deja» ponía las tres interventorías por delante de
     TODA la obra, y dentro de la obra el reparto quedaba perfectamente
     invertido —$12.000 M el último, $120 M el primero—. Quien pide «lo que más
     deja» recibía lo que menos deja.
     La cifra informa si y solo si el costo está MEDIDO, así que este orden
     sigue la doctrina que `margen` ya aplica encima: solo entran los procesos
     con borrador costeado y el resto va al final SIN cifra, jamás con un cero
     ni con una clave derivada de la cuantía que se leería como una medición.
     Para «cuánto dinero hay en juego» ya está `ve` («Mayor contrato
     esperado»), que es otro orden y se llama distinto. */
  ganancia: (l, ctx) => {
    const g = ctx.ganancia;
    if (!g || g.valor == null || g.base !== "apu") return -1e18;
    return g.por_intento != null ? g.por_intento : g.valor;
  },
};
...
/* `medicion` es OPCIONAL: quien lo pasa recibe en él cuántos chunks se
   escanearon y si el corpus salió de la memoria de la instancia (la métrica
   que ARQUITECTURA_MULTITENANT §4.3 pide vigilar antes de optimizar el SCAN). */
async function cargarCorpus(redis, meta, medicion) {
  ...
  if (medicion) { medicion.chunks = claves.length; medicion.instancia_caliente = false; }
  ...
  if (sello && _mem.sello === sello && _mem.filas) {
    ...
    if (medicion) medicion.instancia_caliente = true;
    return _mem.filas;
  }
...
/* Índice de competencia por entidad. Si nunca se construyó (no se ha corrido
   ...
   ---------------------------------------------------------------------------
   «NO SE PUDO LEER» NO ES «NO HAY» (23-sep-2026). Un fallo de la meta o del
   HGETALL se tragaba con un `catch` vacío: todas las filas salían «sin datos de
   cuántos compiten», con HTTP 200 y `indice_competencia` afirmando que el índice
   existía —lo mismo que ve el dueño cuando una entidad de verdad no tiene
   histórico—. Ahora el fallo viaja: `indice` es el centinela INDICE_NO_LEIDO
   (falso a propósito: ver lib/indice_competencia), `competenciaDe` responde
   `motivo: "no_se_leyo"` en cada fila, y `leido`/`error` suben a la respuesta.
   Tres reglas, y las tres las prueba la suite («unidad índice que no se pudo leer»):
   · el fallo NUNCA se memoiza: la petición siguiente vuelve a leer y se cura;
   · un hash VACÍO tampoco se memoiza —con la meta intacta, un hash borrado un
     instante se quedaba pegado a la memoria caliente hasta la siguiente
     reconstrucción—; releer un hash vacío cuesta un comando barato;
   · `_ultimaLecturaIndice` guarda cómo salió la última vez que ESTA instancia
     necesitó el índice (op=salud la repite, cero comandos). Un acierto de la
     memoria caliente también cuenta como lectura buena: si no, un fallo viejo
     se quedaría anunciado en una instancia que ya sirve bien. */
const { INDICE_NO_LEIDO } = require("../../indice_competencia.js");
let _ultimaLecturaIndice = null;
/* El texto del fallo viaja en respuestas públicas: pasa por el CENSO de secretos
   (lib/apu_ocr.tacharClave, el mismo del error de sincronización que publica
   op=salud) y se recorta. require DIFERIDO: el listado no carga el OCR. */
const textoDeFallo = (e) => {
  const { tacharClave } = require("../../apu_ocr.js");
  return tacharClave(String((e && e.message) || e || "fallo sin mensaje")).slice(0, 160);
};
/* Un campo de una meta que puede faltar (meta null o versión anterior sin el
   campo): null, jamás `undefined` —que JSON tira y deja la clave sin decir nada—. */
const campoDe = (meta, k) => (meta && meta[k] != null ? meta[k] : null);
async function cargarIndice(redis) {
  const fallo = (e, meta) => {
    const error = textoDeFallo(e);
    _ultimaLecturaIndice = { ts: new Date().toISOString(), ok: false, campos: null, error };
    return { indice: INDICE_NO_LEIDO, meta, leido: false, error };
  };
  let meta = null;
  try { meta = await leerIndiceMeta(redis); } catch (e) { return fallo(e, null); }
  if (!meta) {
    _memIndice = { sello: null, indice: null, meta: null };
    _ultimaLecturaIndice = { ts: new Date().toISOString(), ok: true, campos: null, error: null };
    return { indice: null, meta: null, leido: true, error: null };
  }
  const sello = `${meta.construido}|${meta.entidades}`;
  if (_memIndice.sello === sello && _memIndice.indice) {
    const campos = Object.keys(_memIndice.indice).length;
    logDev(`índice desde memoria caliente (${campos} claves)`);
    _ultimaLecturaIndice = { ts: new Date().toISOString(), ok: true, campos, error: null };
    return { indice: _memIndice.indice, meta, leido: true, error: null };
  }
  let indice;
  try { indice = await leerIndice(redis); } catch (e) { return fallo(e, meta); }
  const campos = Object.keys(indice).length;
  _memIndice = campos ? { sello, indice, meta } : { sello: null, indice: null, meta: null };
  _ultimaLecturaIndice = { ts: new Date().toISOString(), ok: true, campos, error: null };
  logDev(`índice leído de Redis: ${campos} claves`);
  return { indice, meta, leido: true, error: null };
}
...
/* Índice de baja de mercado. Mismo contrato que el de competencia: si no se ha
   ...
   El MISMO catch mudo que el de competencia (23-sep-2026): un fallo de lectura
   ya no se confunde con «no construido». No se memoiza (la siguiente petición
   relee) y `leido`/`error` suben a `indice_baja` de la respuesta. Y el índice
   viaja como el MISMO centinela INDICE_NO_LEIDO: `bajaDeMercado` responde
   `motivo: "no_se_leyo"` con el conteo en null en cada fila, y la ganancia no
   presupone el presupuesto oficial como precio (lib/indice_baja.SIN_LECTURA_BAJA).
   Los demás lectores del índice (segmento, departamento, encogimiento) lo
   tratan como falso: no pintan nada, que es lo cierto. */
async function cargarIndiceBaja(redis) {
  let meta = null;
  try { meta = await leerIndiceBajaMeta(redis); } catch (e) { return { indice: INDICE_NO_LEIDO, meta: null, leido: false, error: textoDeFallo(e) }; }
  if (!meta) { _memBaja = { sello: null, indice: null, meta: null }; return { indice: null, meta: null, leido: true, error: null }; }
  const sello = `${meta.generado}|${meta.procesos_analizados}`;
  if (_memBaja.sello === sello && _memBaja.indice) return { indice: _memBaja.indice, meta, leido: true, error: null };
  let indice;
  try { indice = await leerIndiceBaja(redis); } catch (e) { return { indice: INDICE_NO_LEIDO, meta, leido: false, error: textoDeFallo(e) }; }
...
function dispararSync(req, modo) {
  try {
    ...
    // el pase del muro de Vercel y, con la guarda del cron activa, el Bearer (lib/auth)
...
/* La última medición del listado servida por ESTA instancia (op=salud la
   repite sin gastar un comando; null si la instancia aún no sirvió ninguna). */
...
module.exports = async function handler(req, res) {
  ...
  /* Aquí solo se valida el FORMATO del perfil (alias, fijo, `rup_…`, `cons_…`):
     un id malformado es un 400 como cualquier perfil inexistente, no un viaje
     a Redis. La carga contra Redis va más adelante, cuando ya hay cliente. */
  const idPerfil = validarIdPerfil(q.perfil);
  if (!idPerfil.ok) return res.status(idPerfil.status).json({ ok: false, error: idPerfil.error });
  const perfil = idPerfil.perfil;
  /* la base desde la que se mide la zona es del PERFIL (lib/perfil_resolver):
     Bogotá/Ibagué solo para los del dueño; un RUP subido o un consorcio a la
     medida no la tienen y su distancia viaja declarada «sin calcular» */
  const baseZona = baseDelPerfil(perfil);
  if (!hayCredenciales()) {
    return res.status(503).json({ ok: false, error: "Faltan credenciales de Upstash Redis en el despliegue." });
  }

  const redis = crearRedis({});
  let meta, progresoSync = null, filas, indice = null, indiceMeta = null, indiceBaja = null, indiceBajaMeta = null, conocimiento = {}, conocimientoMeta = null;
  // ¿se pudieron LEER los dos índices? (un fallo ya no se disfraza de «no construido»)
  let lecturaComp = { leido: true, error: null }, lecturaBaja = { leido: true, error: null };
  const medicionCorpus = {};
  try {
    ...
    /* el perfil dinámico y el consorcio se releen en CADA petición (un GET de
       un valor pequeño): así re-subir un RUP tiene efecto inmediato y un perfil
    ...
    if (!cargado.ok) return res.status(cargado.status).json({ ok: false, perfil_caducado: true, error: cargado.error });
    // meta y progreso en UN comando: el progreso solo sirve para decir si el
    // sync respondería «al día» (remate B3b-H1) y no puede costar un GET más
    [meta, progresoSync] = await leerVariosJSON(redis, [CLAVES.meta, CLAVES.progreso]);
    filas = await cargarCorpus(redis, meta, medicionCorpus);
    if (filas) {
      ({ indice, meta: indiceMeta, ...lecturaComp } = await cargarIndice(redis));
      ({ indice: indiceBaja, meta: indiceBajaMeta, ...lecturaBaja } = await cargarIndiceBaja(redis));
  ...
  /* Borradores de APU del perfil (SCAN + MGET, unos pocos comandos): SOLO CON
     TOKEN —de ahí sale la baja máxima que el dueño soporta en cada proceso
     (A4: `b_max` del APU manda sobre la declarada)—. Sin credencial no se leen
     siquiera: además de las cifras, el conjunto de borradores revela EN QUÉ
     procesos está trabajando el dueño. `ordenar_por=margen` sin token queda
     inerte más abajo. Un fallo de lectura deja `costos` en null: la lista
     sirve igual, sin b_max del APU. */
  let costos = null;
  if (filas && conFinanzas) {
  ...
  /* `?administracion_pct=` `?imprevistos_pct=` `?utilidad_pct=`
     `?contribucion_en_administracion=1` — LA ESTRUCTURA DE PRECIO QUE EL USUARIO
     DECLARA en el detalle de «lo que deja este contrato». Misma doctrina que
     `?baja_max=`: solo con credencial (son las cifras del negocio del dueño) y
     un valor ilegible o fuera de [0, 100] es INERTE, jamás un 400 — un enlace
     guardado no puede vaciarle la lista a nadie.
     Viajan por la URL y no por una clave nueva de Redis a propósito: así la
     cuenta que el usuario acaba de ajustar en el detalle y la que ORDENA la
     lista son la misma, calculada en un solo sitio. Guardarlas en el servidor
     habría creado un segundo estado que se desincroniza con el del navegador. */
  const pctDeclarado = (v) => {
    if (v === undefined || v === "") return null;
    const n = parseFloat(v);
    return conFinanzas && Number.isFinite(n) && n >= 0 && n <= 100 ? n : null;
  };
  const aiuDeclarado = {
    administracion_pct: pctDeclarado(q.administracion_pct),
    imprevistos_pct: pctDeclarado(q.imprevistos_pct),
    utilidad_pct: pctDeclarado(q.utilidad_pct),
  };
  const hayAiuDeclarado = Object.values(aiuDeclarado).some((v) => v != null);
  /* «Declarada» significa que RESPONDIÓ, no que respondiera que sí: es la misma
     distinción «sin dato vs cero» de todo el repositorio, y aquí decide si la
     tarjeta puede AFIRMAR una pérdida o tiene que decir «depende». */
  const contribEnAdmin = conFinanzas && (q.contribucion_en_administracion === "1" || q.contribucion_en_administracion === "true");
  const contribDeclarada = conFinanzas && q.contribucion_en_administracion !== undefined && q.contribucion_en_administracion !== "";
  ...
  /* Lista blanca en las DOS direcciones: `?ubicacion_valida=zzz` (o «TRUE», o
     un «si» de un enlace viejo) se leía como `false` = «fuera de mi zona» y
     escondía filas sin ficha ni aviso (1-sep-2026). Lo desconocido es inerte. */
  const vUbicacion = String(q.ubicacion_valida ?? "").toLowerCase();
  const fUbicacion = ["true", "1"].includes(vUbicacion) ? true : ["false", "0"].includes(vUbicacion) ? false : null;
  ...
  const { visibles, veredictos, noViables, conSocio } = filtrarProcesosVisibles(filas, perfil, conocimiento, {
  ...
  /* El consejo de socio solo tiene sentido para el perfil del DUEÑO: a un
     proponente plural no se le pregunta con quién se asocia, y un perfil de
     onboarding (`rup_…`) no tiene socios que ofrecerle. */
  const socioAplica = perfil === ID_DUENO && CANDIDATOS_CONSORCIO.length > 0;
  /* Qué socio rescató esta fila. Lo decidió la CASCADA, que es quien puede
     hacerlo una sola vez para el listado, el panel y el conteo del pulso. */
  const alcanzaConSocio = (l) => (conSocio && conSocio.get(l)) || null;
  ...
  const evalDe = (l) => {
    ...
    const detalle = estimarPDetalle(l, {
      ...
      prorroga_medida: indiceMeta && indiceMeta.prorroga ? indiceMeta.prorroga : null,
    });
  ...
  const estrecha = (l) => {
    ...
    if (fZonaFacil && evaluarZona(l, baseZona).puntos < 2) return false;
    return true;
  };
  ...
  if (soloViables) {
    /* UN PROCESO QUE SE ALCANZA CON SOCIO NO SE ESCONDE (11-sep-2026, encargo
       del dueño). Antes se retiraba todo lo que el dueño solo no pasaba, y ahí
       se iban justamente las obras para las que existe un consorcio. Ahora se
       queda lo que pasa solo Y lo que pasa con alguna de sus combinaciones; la
       tarjeta dice con cuál. Lo que no alcanza NI con socio sigue fuera: eso no
       es esconder, es no ofrecer lo que no se puede tomar. */
    lista = lista.filter((l) => evalDe(l).puertas.pasa_todas || !!alcanzaConSocio(l));
  } else {
  ...
  /* PELDAÑO 1 DE LA MANIFESTACIÓN: las fechas límite REALES leídas de pliegos
     (`manifestacion:cronograma`, escritas por /api/pliego?op=cronograma). UN
     comando, y solo si el corpus trae alguna de menor cuantía — en un listado
     sin manifestaciones sería una lectura que no cambia nada. Con la fecha del
     pliego la tarjeta dice el día exacto en vez de la ventana calculada; sin
     ella todo sigue exactamente igual que antes. */
  const Manif = require("../../manifestacion.js");
  const fechasCronograma = lista.some((l) => Manif.exigeManifestacion(l))
    ? await Manif.leerFechasCronograma(redis) : {};
  const clasificar = FiltrosLista.crearClasificador({ rupDe, ahora, fechasCronograma });
  const base = lista;
  const totalSinFiltros = base.length;
  lista = FiltrosLista.aplicar(base, estadoFiltros, clasificar);
  /* `totalPorDefecto` = la base con SOLO los filtros por defecto (suministro
     apagado): es la N que enseña el pulso y la que la lista tiene al abrirse
     sin tocar nada. «126 de 827» decía una base que nadie ve; «126 de 771» es
     la lista contra la que el usuario acaba de filtrar. */
  const totalPorDefecto = FiltrosLista.aplicar(base, Filtros.leerEstado({}), clasificar).length;
  ...
  /* ---------- orden ---------- */
  /* `ordenar_por=margen` SOLO CON CREDENCIAL, por el mismo motivo que
     `?baja_max=`: su cifra (`margen_estimado`) lleva dentro el piso rentable
     —derivado del costo directo del APU guardado del dueño— y el techo
     competitivo, del que se despeja EXACTA la mediana de baja de la entidad
     que `lib/publico` acaba de tapar en la misma respuesta. Sin token el orden
     es INERTE (cae al default) y `margen_ignorado` lo dice: un enlace guardado
     no puede vaciar la lista ni quedarse mudo. */
  const margenIgnorado = !conFinanzas && q.ordenar_por === "margen";
  /* Y `ganancia` es su hermano (1-sep-2026): sin credencial `gananciaDe` da null
     en TODAS las filas, el comparador caía al desempate por VE y la respuesta
     seguía diciendo `ordenado_por: "ganancia"` — un orden que no existía, mudo. */
  const gananciaIgnorada = !conFinanzas && q.ordenar_por === "ganancia";
  const ordenarPor = !margenIgnorado && !gananciaIgnorada && Object.prototype.hasOwnProperty.call(ORDEN_CAMPOS, q.ordenar_por)
    ? q.ordenar_por : ORDEN_DEFAULT;
  ...
  const margenDe = (l) => {
    ...
    if (!r) {
      return { valor: null, piso: null, techo: null, motivo: "Sin referencia — todavía no ha calculado el costo de este proceso en Precios." };
    }
    const c = r.borrador, pt = r.pt;
    const cif = (pt && pt.cifras) || {};
    /* LA AUSENCIA SE DESCARTA ANTES DE CONVERTIR. `Number(null)` es 0 y
       `Number.isFinite(0)` es true, así que un `techo_competitivo: null` —lo
       que publica `pisoTecho` cuando la entidad no llega a 5 adjudicaciones
       conocidas, o sea el caso NORMAL mientras nadie reconstruya el índice—
       entraba como techo CERO: la guarda de abajo no disparaba, su mensaje no
       salía nunca y la tarjeta afirmaba «el techo al que suele adjudicar esta
       entidad ($0) está POR DEBAJO de su piso: aquí no da». Un «no sé»
       convertido en un go/no-go. Es la trampa que `lib/probabilidad` y
       `lib/apu/normativa` ya documentan, aquí sin resolver. */
    const cifra = (v) => {
      if (v === null || v === undefined || v === "" || typeof v === "boolean") return null;
      const n = Number(v);
      return Number.isFinite(n) ? n : null;
    };
    const piso = cifra(cif.piso_rentable);
    const techo = cifra(cif.techo_competitivo);
    if (piso == null || techo == null) {
      /* la baja que NO SE PUDO LEER (24-sep-2026) no es «no hay techo con base»:
         `pisoTecho` lo publica en `baja_motivo` y aquí se propaga con la frase del
         índice (lib/indice_baja.SIN_LECTURA_BAJA), la misma del panel */
      const fraseNoLeida = techo == null && cif.baja_motivo === "no_se_leyo" && pt.frases && pt.frases.baja;
      return { valor: null, piso, techo, borrador: c.id, motivo: fraseNoLeida
        ? `Sin referencia — ${fraseNoLeida.charAt(0).toLowerCase()}${fraseNoLeida.slice(1)}`
        : techo == null
    ...
    /* `donde` sale de la MISMA derivación que la tarjeta (`dondeSeMidio`, 22-sep-2026):
       la cascada de la baja baja de la entidad al departamento sin avisar, y el
       recorrido decía «esta entidad» sobre un techo de todo el departamento */
    return { valor: Math.round(techo - piso), piso, techo, borrador: c.id, motivo: null, donde: dondeSeMidio(cif.baja_granularidad) };
  };
  const _margen = new Map();
  const margenMemo = (l) => { if (!_margen.has(l)) _margen.set(l, margenDe(l)); return _margen.get(l); };

  /* ---------- GANANCIA ESTIMADA DEL CONTRATO (lib/ganancia) ----------
     La tercera cifra de la tarjeta. Reemplaza al valor esperado, que era
     correcto y se leía al revés: el contratista lo tomaba por «lo que me
     queda». Aquí lo que se sirve ES lo que le queda.

     Tres entradas y ninguna inventada:
     · el costo directo MEDIDO del borrador de este proceso, si lo costeó;
     · su estructura de precio — la del borrador de ESTE proceso si existe, si
       no la del último borrador que guardó (`config_reciente`), y solo si no
       hay ninguno, el defecto documentado, que se DECLARA como supuesto;
     · el tipo de trabajo ya clasificado, porque la contribución del 5 % es de
       los contratos de obra pública y no de una interventoría.

     Sale gratis: reutiliza el piso/techo y la baja ya memoizados por fila, y
     los borradores ya están leídos para `b_max`. Solo con credencial: la
     estructura de precio y el costo son del dueño (lib/publico lo vuelve a
     anular como defensa en profundidad). */
  const cfgReciente = (costos && costos.config_reciente) || null;
  const gananciaDe = (l) => {
    if (!conFinanzas) return null;
    const r = pisoTechoDe(l);
    const cfg = (r && r.borrador && r.borrador.config) || cfgReciente || {};
    const declarada = !!(r && r.borrador && r.borrador.config) || !!cfgReciente || hayAiuDeclarado;
    const e = evalDe(l);
    return gananciaDeProceso({
      presupuesto_oficial: presupuestoOficialDe(l),
      tipo_trabajo: clasificar(l).tipo,
      baja: e.baja,
      competencia: compDe(l),
      costo_directo: r && r.borrador ? r.borrador.costo_directo : null,
      borrador: r && r.borrador ? r.borrador.id : null,
      /* Lo DECLARADO en el detalle manda sobre el borrador: es la respuesta más
         reciente del usuario a la misma pregunta, y es la que acaba de ver en
         pantalla. Lo que no declaró conserva lo del borrador. */
      aiu: {
        administracion_pct: aiuDeclarado.administracion_pct ?? cfg.aiu_pct,
        imprevistos_pct: aiuDeclarado.imprevistos_pct ?? cfg.imprevistos_pct,
        utilidad_pct: aiuDeclarado.utilidad_pct ?? cfg.utilidad_pct,
        modo: cfg.modo_aiu,
      },
      aiu_origen: declarada ? "suyo" : null,
      deducciones_pct: cfg.deducciones_pct ?? null,
      contribucion_en_administracion: contribEnAdmin || !!cfg.contribucion_en_administracion,
      contribucion_declarada: contribDeclarada || !!cfg.contribucion_en_administracion,
      p_ganar: e.p_ganar,
    });
  };
  const _ganancia = new Map();
  const gananciaMemo = (l) => { if (!_ganancia.has(l)) _ganancia.set(l, gananciaDe(l)); return _ganancia.get(l); };
  ...
  const ctxDe = (l) => {
    ...
    if (ordenarPor === "ganancia" && e.ganancia === undefined) e.ganancia = gananciaMemo(l);
    return e;
  };
  lista.sort((a, b) => {
    ...
    /* ⚠️ UN TRÁMITE QUE VENCE HOY NO PUEDE ESTAR EN LA PÁGINA 3 (15-sep-2026).
       El orden por defecto miraba viabilidad, zona y valor esperado, y nada
       más. La selección abreviada de MENOR cuantía es, por definición de la
       modalidad, el escalón bajo de presupuesto, así que su valor esperado es
       pequeño y toda la clase se hundía por debajo de las licitaciones grandes:
       medido con 60 procesos, el único cuyo plazo para avisar vencía al día
       siguiente salía en la posición 60 — página 3 de 3, con `por_pagina = 20`.
       Y avisar que le interesa es el trámite SIN EL CUAL NO SE PUEDE OFERTAR:
       perderlo no cuesta una oportunidad peor, cuesta la oportunidad entera.
       La urgencia se lee con `esUrgente`, que ya existe en lib/manifestacion y
       ya la usan las facetas y los avisos: aquí no se inventa un segundo
       criterio de urgencia, se llama al que hay. Solo mueve `atractividad`
       —quien pide otro orden pidió ESE orden— y solo entre procesos igual de
       viables y con la misma zona, así que no puede subir algo que no se puede
       tomar. */
    if (ordenarPor === "atractividad") {
      const ua = Manif.esUrgente(clasificar(a).manifestacion) ? 1 : 0;
      const ub = Manif.esUrgente(clasificar(b).manifestacion) ? 1 : 0;
  ...
  const resultados = lista.slice((pagina - 1) * porPagina, (pagina - 1) * porPagina + porPagina)
    ...
    .map((l) => {
      ...
      const salida = {
        ...
        /* «Cómo se adjudica en su departamento» (M-COMP-01, 6-sep-2026): la
           lectura del hash de departamento, que solo se ENSEÑA junto a la baja
           que decide (no entra en su cascada). Null sin departamento en el
           proceso; se anula sin token con los otros tres (lib/publico). */
        baja_departamento: bajaDepartamentoDe(indiceBaja, l),
        puertas: e.puertas,
        p_ganar: e.p_ganar,
        ...
        filtro: (() => { const c = clasificar(l); return { tipo: c.tipo, modalidad: c.modalidad, departamento: c.departamento, rango: c.rango, dias_cierre: c.dias, ventana: c.ventana, admite_ofertas: c.admite_ofertas }; })(),
        /* Avisar que le interesa (selección abreviada de menor cuantía): la
           VENTANA en la que el plazo puede cerrar y su estado. La ley fija un
           MÁXIMO de 3 días hábiles desde la apertura (D. 1082/2015 art.
           2.2.1.2.1.2.20), no un plazo: quien lo fija es la entidad, en el
           pliego. Con la fecha del pliego ya leída (`fechasCronograma`) el
           estado pasa a confirmado y sí hay cuenta atrás. null cuando la
           modalidad no la exige. Misma regla que la portada y Mis procesos. */
        manifestacion: clasificar(l).manifestacion,
        ...(ordenarPor === "margen" ? { margen_estimado: margenMemo(l) } : {}),
        /* Lo que deja el contrato si lo gana (lib/ganancia): la cifra que la
           tarjeta enseña en la tercera celda. Viaja con su nivel (`apu` =
           costeado por el usuario · `estructura_de_precio` = cerrado por su AIU
           declarado), su precio de referencia, sus descuentos y sus supuestos,
           para que se pueda auditar sin abrir otra pantalla. Sin credencial es
           `null`: sale del costo y de la estructura de precio del dueño. */
        ganancia: gananciaMemo(l),
        ...
        /* CON CUÁL DE MIS SOCIOS CONVIENE ESTE PROCESO (11-sep-2026). Solo
           para el perfil del dueño: preguntarle a un plural con qué socio se
           asocia no tiene sentido, y a un perfil de onboarding tampoco (no
           tiene socios). Reaprovecha el veredicto y las puertas ya calculadas
           de esta misma fila: no vuelve a evaluar el perfil base. */
        /* Qué socio la alcanza, si hace falta alguno. Sale del MISMO predicado
           que decidió que la fila se quedara en la lista: si el listado la dejó
           pasar, la tarjeta puede decir por qué. */
        ...(socioAplica ? { alcanzable_con_socio: alcanzaConSocio(l) } : {}),
        /* EL VEREDICTO COMPLETO YA NO VIAJA POR FILA (11-sep-2026, encargo del
           dueño: «para que sea más sencillo, pon que cuando lo guarde el proceso
           me diga con quién conviene más»). La lista contesta a «¿a qué me
           puedo presentar?» y por eso sigue enseñando TODO lo alcanzable, solo o
           con cualquiera de las dos socias; el CON QUIÉN se lee despacio en el
           expediente, donde el navegador entra justo al guardar, y allí llega
           congelado del servidor (lib/handlers/perfil/seguimiento.congelarSocio).
           Aquí queda lo mínimo para que la tarjeta pueda explicar por qué se
           enseña un proceso que el dueño solo no alcanza — sin eso la fila sale
           con el chip rojo de «no viable» y sin motivo, que es el falso negativo
           que en oportunidades cuesta caro.
           Medido sobre 210 filas servidas: el veredicto entero pesaba 1.220 B
           por fila (14,4 % del cuerpo; 15,0 % ya comprimido) y este resumen pesa
           30 B. */
        ...(socioAplica ? { socio: resumenSocio(socioPorProceso({
          fila: l, base: perfil, candidatos: CANDIDATOS_CONSORCIO,
          ctx: { rup: rupDe(l), puertas: evalDe(l).puertas, conocimiento, competencia: compDe(l), incluirTextoDebil: opciones.incluirTextoDebil },
        })) } : {}),
      };
  ...
  /* Reparto por solidez del match: le dice al dueño cuántas de las que ve son
     suyas de plano y cuántas hay que verificar en el pliego. `con_socio` es la
     casilla NUEVA (11-sep-2026): filas que su registro no cubre y que están en
     la lista porque las alcanza una de sus combinaciones. Tiene que existir
     como casilla propia —y no repartirse entre las otras— porque el reparto
     SUMA EXACTAMENTE el total, y sin ella la cuenta quedaría corta y muda. */
  /* `no_encaja` (23-sep-2026): con «Solo las que cumplen» desmarcado vuelven, atenuadas,
     filas que su registro no cubre y que NINGÚN socio alcanza; `con_socio` las contaba
     igual, y con PRODIAC —que no tiene socios— salía «con_socio: 2». Casilla propia: la
     suma sigue cuadrando con el total sin decir que un socio las alcanza. */
  const por_match = { clase: 0, familia: 0, equivalente: 0, texto: 0, con_socio: 0, no_encaja: 0 };
  for (const l of lista) {
    /* El reparto es por TIER, y tiene que seguir cuadrando con el filtro
       `?match=…`, que también filtra por tier: si aquí se contara otra cosa,
       «clase: 351» y la lista de `?match=clase` dirían cifras distintas de lo
       mismo. `con_socio` recoge lo que no tiene tier propio —el registro del
       dueño no cubre ese objeto— y está en la lista porque lo alcanza un socio;
       lo que no tiene tier y ningún socio alcanza va a `no_encaja`. */
    const t = rupDe(l).tier;
    if (t in por_match && t !== "con_socio" && t !== "no_encaja") por_match[t]++;
    else if (alcanzaConSocio(l)) por_match.con_socio++;
    else por_match.no_encaja++;
  }
  ...
  /* MEDICIÓN DEL LISTADO (6-sep-2026, M-INF-04): filas del corpus, chunks
     escaneados, duración e instancia caliente — cero comandos extra. Nombres
     distintos de `total`/`totalSinFiltros` a propósito: son tamaños del corpus
     y de la petición, no conteos de la lista. Se guarda en la instancia para
     que op=salud la repita. */
  const medicion = {
    filas_corpus: filas.length,
    chunks: Number.isInteger(medicionCorpus.chunks) ? medicionCorpus.chunks : null,
    duracion_ms: Date.now() - t0,
    instancia_caliente: medicionCorpus.instancia_caliente === true,
  };
  _ultimaMedicion = { ...medicion, ts: new Date().toISOString() };
  /* ¿EL SYNC RESPONDERÍA «AL DÍA»? (6-sep-2026, M-INF-10 y remate B3b-H1). El
     navegador disparaba op=sync&modo=auto tras CADA búsqueda —cargar, cambiar
     de perfil, cada filtro, cada página— y el servidor tomaba y soltaba el
     candado —y tras «al día» corría además el índice de baja: decenas de
     comandos de Redis, la suite imprime la cifra— solo para responder «al
     día». Se publica el HECHO con la MISMA decisión que ejecuta el sync
     (`decidirAuto`, llamada, no copiada: copiar solo la rama del corte fresco
     decía «al día» con una full a medias, con el año cambiado o con la full de
     higiene vencida, y el sync las lanzaba igual): true = respondería alDia;
     false = toca disparar; null = sin corte conocido o ilegible (el navegador
     dispara como hasta hoy). Require diferido: servir el listado no carga el
     módulo de sincronización hasta que hace falta. */
  const instanteCorte = meta && meta.last_sync ? Date.parse(meta.last_sync) : NaN;
  const { decidirAuto } = require("./sync.js");
  const sincronizadoFresco = Number.isFinite(instanteCorte) ? decidirAuto({ meta, progreso: progresoSync }) === "al_dia" : null;
  return res.status(200).json({
    ok: true, total, resultados, pagina, por_pagina: porPagina, perfil,
    sincronizado: meta ? meta.last_sync : null,
    sincronizado_fresco: sincronizadoFresco,
    /* el último intento de sincronizar FALLÓ (lo escribe sync.js y lo borra la
       siguiente corrida buena): la barra lo dice en vez de un ámbar mudo */
    ultimo_error: meta && meta.ultimo_error ? meta.ultimo_error : null,
    medicion,
    ordenado_por: ordenarPor,
    /* desde dónde se midió la zona de cada fila: «Bogotá / Ibagué» para los
       perfiles del dueño; null cuando el perfil no ha dicho desde dónde opera
       (la pantalla rotula con eso el filtro «Solo cerca de mi zona») */
    zona_base: nombreDeBase(baseZona),
    ...
    ...(margenIgnorado ? { margen_ignorado: "requiere credencial: el margen sale del costo directo de sus presupuestos guardados" } : {}),
    ...(gananciaIgnorada ? { ganancia_ignorada: "requiere credencial: la ganancia sale de su costo y de su estructura de precio" } : {}),
    por_match, incluye_sin_unspsc: opciones.incluirTextoDebil,
    /* LOS DOS ÍNDICES DICEN SI SE LEYERON (23-sep-2026). null solo cuando no hay
       índice construido Y la lectura fue bien; si falló, el objeto viaja con
       `leido: false` y el texto del fallo aunque no se haya podido leer ni la meta
       —es lo que el dueño mira pegando esta dirección en Chrome—. La meta es la
       de ESTA petición, no la de la memoria del módulo: tras un fallo la memoria
       puede guardar la de otra. */
    indice_baja: indiceBajaMeta || !lecturaBaja.leido
      ? {
        generado: campoDe(indiceBajaMeta, "generado"),
        entidades: campoDe(indiceBajaMeta, "entidades_clasificadas"),
        min_procesos: campoDe(indiceBajaMeta, "min_procesos"),
        leido: lecturaBaja.leido,
        error_lectura: lecturaBaja.error,
      }
      : null,
    /* `entidades` conserva lo que publicó siempre —las CLASIFICADAS— para no
       romper a quien ya lo lea; pero en la meta y en op=historico «entidades»
       son las que tienen algún proceso contado, así que las dos cifras viajan
       además con nombres que no se confunden. */
    indice_competencia: indiceMeta || !lecturaComp.leido
      ? {
        construido: campoDe(indiceMeta, "construido"),
        entidades: campoDe(indiceMeta, "clasificadas"),
        clasificadas: campoDe(indiceMeta, "clasificadas"),
        entidades_con_procesos: campoDe(indiceMeta, "entidades"),
        min_procesos: campoDe(indiceMeta, "min_procesos"),
        leido: lecturaComp.leido,
        error_lectura: lecturaComp.error,
      }
      : null,
...
// la guía de Mis procesos (lib/guia_proceso) reutiliza los DOS índices memoizados:
// una segunda lectura del hash por petición sería pagar dos veces lo mismo
module.exports.cargarIndice = cargarIndice;
module.exports.cargarIndiceBaja = cargarIndiceBaja;
// op=salud repite la última medición del listado de esta instancia (null si no hubo)
module.exports.ultimaMedicion = () => _ultimaMedicion;
// …y cómo salió la última vez que esta instancia necesitó el índice de competencia
module.exports.ultimaLecturaIndice = () => _ultimaLecturaIndice;
