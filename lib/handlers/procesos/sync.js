...
async function senalesDeCierre(redis, historico) {
  ...
  for (const r of historico) {
    ...
    const previo = g.fecha_cierre ? Date.parse(g.fecha_cierre) : NaN;
    const inicial = g._cierre_inicial ? Date.parse(g._cierre_inicial) : NaN;
...
module.exports = async function handler(req, res) {
  ...
  return res.status(200).json({ ok: true, modo, duracionMs: Date.now() - t0, comandosRedis: redis.comandos(), ...r, baja, portada });
};
