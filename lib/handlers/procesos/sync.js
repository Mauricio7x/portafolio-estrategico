...
async function extraerFull(redis, socrata, { presupuestoMs, reiniciar }) {
  const t0 = Date.now();
  /* «rendirse solo después de avanzar» (7-sep-2026, lib/presupuesto): una
     invocación cuya preparación consuma el presupuesto entero devolvía cero
     páginas, y una cadena reanudable que nunca avanza no converge. */
  ...
  /* UNA sola salida por presupuesto agotado, para las tres puertas: entre páginas,
     dentro del count y dentro de una página (el transporte marca
     `presupuesto_agotado` cuando lo que le quedaba a esta invocación cortó los
     reintentos, remate V-B3a-01): se persiste el cursor y la siguiente invocación
     continúa. Un count cortado se vuelve a pedir (esperadosMes sigue null). */
  const cortar = async () => {
    await escribirJSON(redis, CLAVES.progreso, p);
  ...
  while (p.mesIdx < p.meses.length) {
    ...
    if (p.esperadosMes == null) {
      ...
      catch (e) { if (e && e.presupuesto_agotado) return cortar(); p.esperadosMes = -1; }
    ...
    let finDeMes = false;
    while (!finDeMes) {
      if (tanda.agotado()) return cortar();
      let filas;
      try {
        ...
        if (e && e.presupuesto_agotado) return cortar();
        throw e;
      }
      if (filas.length) {
      ...
      tanda.avanzo();   // página guardada: solo ahora se puede rendir el presupuesto
    }
  ...
  /* EL MARCADOR DE «HECHO» VA DESPUÉS DEL HECHO, Y LA HIGIENE NO ES UN DATO
     (12-sep-2026). Antes este cierre iba: `terminado: true` → purga → meta. Los
     dos órdenes costaban lo mismo que en el histórico:
       · si algo fallaba en la purga o al escribir `meta`, el progreso ya decía
         «terminado» sin `last_full`, y entonces `decidirAuto` no responde
         «continuar_full» (mira `!progreso.terminado`) sino «full»: la siguiente
         invocación REINICIA el año entero en vez de cerrar lo que ya estaba
         bajado — medido: 9 de 9 meses bajados y vueltos a bajar;
       · y la purga —73 de 121 comandos, sin mirar el reloj de la tanda ni una
         vez— podía tumbar una carga completa BUENA por higiene.
     Ahora: purga en best-effort (el mismo trato que `guardarCenso`, y si falla
     se dice: `purgadas: null` —jamás 0, que sería «no había nada que purgar»—),
     después el DATO (`meta`), y el marcador al FINAL. Un fallo al sellar deja
     `terminado: false` con el bucle de meses agotado: la siguiente invocación
     entra, no baja nada y escribe la meta que faltaba. */
  ...
  let retenidos = [], purgadas = null, purgaFallida = null;
  try {
    ...
    purgadas = muertas.length;
  } catch (e) {
    // la purga es HIGIENE: su fallo no puede tumbar una carga completa que ya bajó
    // el año entero. Se dice (`purga_fallida`) y la siguiente full vuelve a intentarlo.
    purgaFallida = String((e && e.message) || e);
    // «no se pudo» no es «no había nada»: ni 0 claves purgadas ni cero meses retenidos
    retenidos = null; purgadas = null;
  }
  ...
  await escribirJSON(redis, CLAVES.meta, {
    ...
    porMes: p.porMes,
    // del año anterior, con procesos aún abiertos (el delta los sigue leyendo por SCAN, no por esta lista).
    // `null` cuando la purga falló: no se llegó a mirar, y una lista vacía afirmaría que no hay ninguno.
    meses_retenidos: retenidos,
  ...
  // el DATO ya está escrito: ahora sí se marca «hecho»
  p.terminado = true;
  await escribirJSON(redis, CLAVES.progreso, p);
  return {
    done: true, total: totalGuardadas, leidas: totalLeidas, porMes: p.porMes,
    purgadas, censo_ingesta: censoFull,
    ...(purgaFallida ? { purga_fallida: purgaFallida } : {}),
  };
}
...
async function extraerDelta(redis, socrata, { presupuestoMs }) {
  ...
  /* HERMANO DE LA GUARDA DE `decidirAuto` (12-sep-2026, el censo del lote): el
     `||` no cubre una fecha ILEGIBLE —cualquier cadena es «truthy», así que
     `last_sync: "ayer"` NO cae a `last_full`—, y entonces `Date.parse` da NaN y
     `new Date(NaN).toISOString()` revienta con RangeError: el delta muere en 502
     cada vez, para siempre. Antes ese estado era inalcanzable en modo auto
     porque `decidirAuto` respondía «al_dia» (el defecto que este lote arregla);
     ahora se llega, así que la guarda va aquí también. Una fecha que no se puede
     leer es «no sé»: se cae al sello de la última carga completa y, si ese
     tampoco se lee, no hay desde dónde arrancar un delta y se hace una full. */
  const anclaDelta = Number.isFinite(Date.parse(meta.last_sync)) ? meta.last_sync
    : (Number.isFinite(Date.parse(meta.last_full)) ? meta.last_full : null);
  if (!anclaDelta) return extraerFull(redis, socrata, { presupuestoMs, reiniciar: false });
  const desdeEsperado = new Date(Date.parse(anclaDelta) - SOLAPE_DELTA_MS).toISOString();
  ...
  /* «rendirse solo después de avanzar» (7-sep-2026, lib/presupuesto) */
  const tandaDelta = relojDeTanda(t0, presupuestoMs);
  while (!fin) {
    if (tandaDelta.agotado()) { completo = false; break; }
    let filas;
    try {
      ...
      // el presupuesto cortó la página por dentro: mismo corte que entre páginas (el cursor se guarda)
      if (e && e.presupuesto_agotado) { completo = false; break; }
      throw e;
    }
...
/* ─────────── ¿qué toca en modo auto? (pura, con prueba) ───────────
   Las CUATRO ramas de la decisión en un solo sitio (remate B3b-H1, 6-sep-2026):
   el listado publicaba `sincronizado_fresco` copiando SOLO la última (corte
   < FRESCO_MS) y decía «fresco» —el navegador no disparaba— en tres estados en
   los que este handler SÍ lanzaba una full: full a medias con la cadena muerta,
   `meta.ano` de otro año y full de higiene vencida (medido con los handlers
   reales: 26 y 125 comandos donde el listado prometía «al día»). El listado la
   LLAMA y publica `=== "al_dia"`; dos copias divergen a la primera corrección.
   Devuelve "continuar_full" | "full" | "delta" | "al_dia". */
function decidirAuto({ meta, progreso, ahora = Date.now() } = {}) {
  const m = meta || {};
  if (progreso && progreso.tipo === "full" && !progreso.terminado) return "continuar_full";
  // sin carga inicial, cambio de año (la ventana vigente es otra) o full con
  // más de un mes: recarga completa (higiene del corpus activo)
  // un `last_full` ILEGIBLE es «no sé» igual que su ausencia: `NaN > FULL_HIGIENE_MS`
  // es false y la higiene del corpus se saltaba muda (hermano del censo de este lote)
  const ultimaFull = Date.parse(m.last_full);
  if (!m.last_full || !Number.isFinite(ultimaFull) || m.ano !== anoVigente(ahora) || ahora - ultimaFull > FULL_HIGIENE_MS) return "full";
  /* UNA FECHA QUE NO SE PUEDE LEER ES «NO SÉ», Y ANTE «NO SÉ» SE SINCRONIZA
     (12-sep-2026). Esto decía `Date.parse(m.last_sync || 0)`: sin `last_sync`,
     `Date.parse(0)` es el 1-ene-2000 —una ausencia convertida en fecha
     inventada, que por casualidad daba «delta»—; y con un `last_sync` ilegible
     la resta es NaN, `NaN > FRESCO_MS` es false y la respuesta era «al_dia»:
     el corpus se congelaba y nadie lo veía. La guarda es la MISMA que
     `listar.js` ya aplicaba al publicar `sincronizado_fresco`. */
  const corte = Date.parse(m.last_sync);
  if (!Number.isFinite(corte) || ahora - corte > FRESCO_MS) return "delta";
  return "al_dia";
}

/* ─────────── el rastro de un fallo, tal como se guarda en meta (pura, con prueba) ───────────
   EL FALLO SE GUARDA (6-sep-2026, M-INF-04). Antes vivía solo en el 502 de la
   respuesta —que el cron de las 08:30 recibe y nadie lee— y en un registro que
   dura una hora: producción estuvo 14 h sin sincronizar y se supo por un
   cliente. Se escribe en `meta` para que op=salud (público, SIN token) lo
   publique y la barra de la lista diga «hoy no se pudo actualizar». Texto de
   terceros: pasa por tacharClave (CENSO de secretos del entorno, lib/apu_ocr) y
   DESPUÉS se corta a 200 —al revés, un secreto partido por el corte
   sobreviviría a medias—. Suelta y exportada (remate V-B3a-02): la cerradura
   del handler solo veía un error de 41 caracteres sin secretos, con el que
   tachar, cortar o borrar un nombre del censo daban lo mismo. */
function registroDeFallo(error, modo, ahora = Date.now()) {
  const { tacharClave } = require("../../apu_ocr.js");
  const texto = String((error && error.message) || error);
  return { ts: new Date(ahora).toISOString(), modo, texto: tacharClave(texto).slice(0, 200) };
}
...
module.exports = async function handler(req, res) {
  ...
  const t0 = Date.now();
  /* el cliente de Socrata sabe cuánto le queda a ESTA invocación (remate V-B3a-01):
     ningún intento ni retroceso dura más que el presupuesto; antes, con la fuente
     colgada, una sola página costaba 124 s dentro de un presupuesto de 45 */
  const socrata = crearCliente({ plazoDe: () => presupuestoMs - (Date.now() - t0) });
  ...
  try {
    if (modo === "full") {
      ...
      const meta = (await leerJSON(redis, CLAVES.meta)) || {};
      const decision = decidirAuto({ meta, progreso });
      if (decision === "continuar_full") {
    ...
    /* El rastro del fallo lo arma `registroDeFallo` (ver arriba). Best-effort: si
       lo que falla es Redis, tampoco se podrá escribir y op=salud responderá
       502, que es la señal que el monitor necesita. */
    try {
      const metaAhora = (await leerJSON(redis, CLAVES.meta)) || {};
      await escribirJSON(redis, CLAVES.meta, { ...metaAhora, ultimo_error: registroDeFallo(e, modo) });
  ...
  /* ¿ESTA CORRIDA TRAJO DATOS? Una sola copia del predicado (12-sep-2026): el
     precálculo de la portada ya lo usaba (`r.done && !r.alDia`) y el índice de
     baja usaba otro (`r.done === true`), veinte líneas más arriba y en el mismo
     handler, así que cada «al día» —cada visita, cada cron— reconstruía el
     índice de baja entero: 89 comandos de Redis para no hacer nada. */
  const trajoDatos = !!(r && r.done === true && !r.alDia);
  /* Y CUÁNTO ESCRIBIÓ, según lo que el propio delta ya publica: `historicas` son
     las filas cerradas que entraron al histórico (de las que sale el índice de
     baja) y `guardadas` las que entraron al corpus activo (del que sale la
     portada). `null` = no fue un delta (una full reescribe el corpus entero) o
     la cifra no es legible: ante «no sé» se reconstruye, que es el lado barato
     —`Number(null)` es 0 y un 0 inventado apagaría la reconstrucción—. */
  const cifraDelDelta = (campo) => {
    if (!r || !r.delta || typeof r.delta[campo] !== "number" || !Number.isFinite(r.delta[campo])) return null;
    return r.delta[campo];
  };

  let baja = null;
  // sin filas cerradas nuevas el índice de baja no PUEDE haber cambiado: releer
  // todos los chunks del histórico para llegar al mismo resultado es cuota tirada
  if (trajoDatos && cifraDelDelta("historicas") !== 0 && q.baja !== "0") {
    try {
  ...
  /* …y solo si el delta ESCRIBIÓ algo en el corpus activo (12-sep-2026): agregar
     la portada escala con el CORPUS, no con lo que trajo el delta (778 ms
     medidos, hasta 7.029 ms con datos.gov.co lento), así que un delta de cero
     filas pagaba la agregación entera para producir la misma portada. NO lleva
     reloj de tanda a propósito: agregar no es reanudable y media portada sería
     peor que una portada de hace diez minutos. */
  let portada = null;
  if (trajoDatos && cifraDelDelta("guardadas") !== 0) {
    try {
...
/* la decisión del modo auto, para que el listado publique el MISMO hecho que
   este handler ejecuta (remate B3b-H1), y el rastro del fallo, para probarlo
   con un error largo lleno de secretos (remate V-B3a-02) */
module.exports.decidirAuto = decidirAuto;
module.exports.registroDeFallo = registroDeFallo;
module.exports.PRESUPUESTO_DEFAULT_MS = PRESUPUESTO_DEFAULT_MS;
module.exports.PRESUPUESTO_MAX_MS = PRESUPUESTO_MAX_MS;
/* la suite EJECUTA la carga con un Socrata cuyo count es ilegible (remate H-01,
   6-sep-2026): la guarda por regex sobre el fuente dejaba revertir el -1 en verde */
module.exports.extraerFull = extraerFull;
