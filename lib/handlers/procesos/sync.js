...
function decidirAuto({ meta, progreso, ahora = Date.now() } = {}) {
  ...
  if (!m.last_full || m.ano !== anoVigente(ahora) || ahora - Date.parse(m.last_full) > FULL_HIGIENE_MS) return "full";
  if (ahora - Date.parse(m.last_sync || 0) > FRESCO_MS) return "delta";
  return "al_dia";
}
