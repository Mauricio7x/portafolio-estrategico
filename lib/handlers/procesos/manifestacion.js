...
module.exports = async function handler(req, res) {
  ...
  const cabecera = {
    ...
    como_leerlo: {
      abierto: "Procesos de selección abreviada de menor cuantía en los que NO consta que el plazo para avisar haya vencido. La ley fija un MÁXIMO de 3 días hábiles desde la apertura, no un plazo fijo: la entidad pone el suyo en el pliego y suele ser menor, así que lo que se publica es la VENTANA en la que el plazo puede cerrar, no una fecha de vencimiento. Solo se retiran los procesos cuya fecha límite está PUBLICADA y ya pasó; los demás se quedan a la vista y hay que confirmarlos en SECOP II antes de contar con ellos.",
  ...
  for (const f of ventana.sin_vencer || ventana.abiertos || []) {
    const { estado: est, accion } = estadoDeVentana({ desde: f.puedeCerrarDesdeISO, hasta: f.venceMaximoISO, confirmada: f.fechaLimiteISO, horaLimite: f.horaLimite || null }, hoy, habiles.ahoraColombia(Date.now()));
