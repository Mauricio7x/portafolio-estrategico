...
const { construirIndice, fraseQuienGana } = require("../../indice_competencia.js");
  ...
  return {
    ...
    indice: metaIndice
      ? { construido: metaIndice.construido, entidades: metaIndice.entidades, clasificadas: metaIndice.clasificadas, descartados: metaIndice.descartados,
        ...
        /* «Quién gana aquí» y el perfil del competidor (23-sep-2026): si la última
           construcción los publicó, de cuándo son, o POR QUÉ no — sin arrancar nada.
           Antes solo lo decía la respuesta de la última invocación de la cadena, que
           nadie lee. null = esa construcción no lo dejó escrito (una anterior a este
           cambio), no «no se publicó». */
        ganadores: metaIndice.ganadores || null }
      : null,
    indice_a_medias: indiceParcial ? true : false,
    /* LO MISMO, EN UNA FRASE (24-sep-2026): es lo que busca quien pega esta URL
       después de recalcular la competencia. La redacción es la del botón de Mi
       empresa (`fraseQuienGana`, una sola); con una construcción a medias no se
       afirma nada de la anterior. */
    quien_gana_aqui: indiceParcial
      ? "La competencia se está recalculando (o quedó a medias): cuando termine, esta línea dirá si «Quién gana aquí» quedó publicado."
      : metaIndice ? fraseQuienGana(metaIndice.ganadores) : "Sin dato: la competencia no se ha calculado todavía.",
...
async function extraerHistorico(redis, socrata, { presupuestoMs, desde, hasta, reiniciar }) {
  ...
  /* «rendirse solo después de avanzar» (7-sep-2026, lib/presupuesto): sin esto el
     bucle preguntaba por el presupuesto ANTES de traer la primera página y una
     invocación cuya preparación ya lo hubiera consumido devolvía cero páginas —
     la cadena reanudable repetía el cuadro y no convergía. */
  ...
  while (p.mesIdx < p.meses.length) {
    ...
    if (p.chunkIdx == null) {
      ...
      // cortado por el presupuesto (remate V-B3a-01), se guarda el cursor y se vuelve a pedir en la siguiente invocación
      try { p.esperadosMes = (await socrata.contarMes(mes)) ?? -1; }
      catch (e) {
        if (e && e.presupuesto_agotado) { await escribirJSON(redis, CLAVES.progresoHistorico, p); return { done: false, progreso: resumen(p) }; }
        p.esperadosMes = -1;
      }
    ...
    while (!finDeMes) {
      ...
      try {
        ...
        // el presupuesto cortó la página por dentro: mismo corte que entre páginas
        if (e && e.presupuesto_agotado) { await escribirJSON(redis, CLAVES.progresoHistorico, p); return { done: false, progreso: resumen(p) }; }
        throw e;
      }
      if (filas.length) {
      ...
      tanda.avanzo();   // la página ya está guardada: a partir de aquí sí se puede rendir el presupuesto
    }
  ...
  /* EL MARCADOR DE «HECHO» SE ESCRIBE DESPUÉS DEL HECHO (12-sep-2026). Antes,
     `terminado: true` iba PRIMERO y el sello (`sync:historico:meta`) después: si
     Redis fallaba en medio —o la función moría entre los dos comandos— quedaba un
     progreso que se declaraba terminado sin sello, y ese estado NO SE CURA SOLO:
     la siguiente llamada responde `{done:true, yaEstaba:true}` sin re-extraer nada
     y `decidirRefrescoHistorico` (sync.js) devuelve null para siempre —no hay `ts`
     que comparar con REFRESCO_HISTORICO_MS—, así que el refresco mensual no vuelve
     a dispararse. Con el orden invertido, un fallo al sellar deja `terminado: false`
     con el bucle de meses ya agotado: la siguiente invocación entra, no baja NADA
     (mesIdx == meses.length) y escribe el sello que faltaba. Se reanuda sola. */
...
module.exports = async function handler(req, res) {
  ...
  const t0 = Date.now();
  // lo que le queda a esta invocación: lo miran el cliente de Socrata (remate V-B3a-01)
  // y los constructores de índices; nunca baja de 1 ms (cada uno comprueba y reanuda)
  const restante = () => Math.max(presupuestoMs - (Date.now() - t0), 1);
  const socrata = crearCliente({ plazoDe: restante });
  ...
  try {
    ...
    /* LO QUE ESTA CADENA YA TERMINÓ NO SE VUELVE A CONSTRUIR (7-sep-2026). Cada
       constructor, al acabar, borra su progreso; sin esta memoria la invocación
       siguiente lo empezaba OTRA VEZ desde el primer mes, y como los cuatro se
       reparten el mismo presupuesto, en una máquina lenta el índice y el de baja
       se pisaban sin fin: el índice terminaba, la baja avanzaba un mes con el
       milisegundo que quedaba, y la invocación siguiente reconstruía el índice.
       Medido con el mock de Redis a 4 ms por comando (E2E_REDIS_LENTO_MS): las
       400 invocaciones del tope, con `pendientes` del índice oscilando entre 24
       y 6 para siempre. La memoria se borra en cuanto una invocación BAJA datos
       nuevos —lo construido sobre el corpus viejo deja de valer ahí mismo— y una
       reconstrucción PEDIDA A MANO (?reconstruir_indice=true y compañía) tampoco la mira: la
       memoria solo ahorra trabajo en la cadena automática, jamás desobedece una orden. */
    let hechos = {};
    if (listo) {
      try { hechos = (await leerJSON(redis, CLAVES.derivadosHechos)) || {}; } catch { hechos = {}; }
      if (extraccion && extraccion.done && !extraccion.yaEstaba) hechos = {};
    }
    let hechosCambio = false;
    /* La memoria guarda el RESULTADO del constructor, no un simple «sí»: la respuesta de la
       última invocación de la cadena es la que el dueño lee (y la que la suite comprueba), y
       tenía que seguir diciendo cuántas entidades, cuántos grupos y cuántos pares salieron.
       Un «ya estaba» sin cifras habría cambiado el contrato de la respuesta por un detalle
       interno de cómo se reparte el presupuesto. */
    const construir = async (nombre, pedido, fn, pedidoAMano) => {
      if (!pedido) return null;
      if (hechos[nombre] && !pedidoAMano) return { ...hechos[nombre], done: true, yaEstaba: true };
      const r = await fn();
      if (r && r.done && !hechos[nombre]) { hechos[nombre] = r; hechosCambio = true; }
      return r;
    };
    if (listo) {
      indice = await construir("indice", !soloDerivados || pideIndice,
        () => construirIndice(redis, { presupuestoMs: restante(), reiniciar: pideIndice && reiniciar }), pideIndice);
    }
    if (listo && (!indice || indice.done)) {
      baja = await construir("baja", !soloDerivados || pideBaja,
        () => construirIndiceBaja(redis, { presupuestoMs: restante(), reiniciar: pideBaja && reiniciar }), pideBaja);
    }
    if (listo && (!indice || indice.done) && (!baja || baja.done)) {
      equivalencias = await construir("equivalencias", !soloDerivados || pideEquivalencias,
        () => construirEquivalencias(redis, { presupuestoMs: restante(), reiniciar: pideEquivalencias && reiniciar }), pideEquivalencias);
    }
    if (listo && (!indice || indice.done) && (!baja || baja.done) && (!equivalencias || equivalencias.done)) {
      vocabulario = await construir("vocabulario", !soloDerivados || pideVocabulario,
        () => construirVocabulario(redis, { presupuestoMs: restante(), reiniciar: pideVocabulario && reiniciar }), pideVocabulario);
    }
    if (listo && (hechosCambio || !Object.keys(hechos).length)) {
      try { await escribirJSON(redis, CLAVES.derivadosHechos, hechos); } catch { /* la cadena sigue: peor, pero nunca menos */ }
    }
  } catch (e) {
  ...
  return res.status(200).json({
    ...
    /* con la construcción del índice terminada en ESTA invocación, si «Quién gana
       aquí» quedó publicado y, si no, por qué — la frase que enseña el botón */
    ...(indice && indice.done && !indice.vacio && !indice.yaEstaba ? { quien_gana_aqui: fraseQuienGana(indice.ganadores) } : {}),
  });
};
