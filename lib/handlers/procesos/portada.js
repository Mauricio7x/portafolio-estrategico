...
const { leerPortada, reconstruirPortada, leerHistoria } = require("../../portada.js");
...
module.exports = async function handler(req, res) {
  ...
  let portada, historia = [];
  try { portada = await leerPortada(redis); } catch (e) {
    return res.status(502).json({ ok: false, error: `Redis: ${e.message}` });
  }
  /* la historia del mercado (M-DGF-20): un punto por día; sin historia, lista
     vacía —ningún punto no es «cero procesos»— y una lectura fallida no
     tumba la portada */
  try { historia = await leerHistoria(redis); } catch { historia = []; }
  if (!portada) {
  ...
  return res.status(200).json({
    ok: true, disponible: true, ...portada, historia,
