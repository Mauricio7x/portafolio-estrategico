...
/* La LECTURA COMPLETA desde la pantalla (26-sep-2026): el botón despierta la rutina del
   dictamen por HTTP —el mismo disparo que «Buscar» en Precios, lib/rutina.js— y una sesión
   de Claude Code escribe el dictamen con la suscripción del dueño (motor «sesion»). Sin las
   dos variables el botón no se enseña. */
const hayRutinaDictamen = () => Boolean(process.env.RUTINA_DICTAMEN_URL && process.env.RUTINA_DICTAMEN_TOKEN);
const horaColombia = (iso) => {
  const d = new Date(iso || Date.now());
  try { return d.toLocaleTimeString("es-CO", { timeZone: "America/Bogota", hour: "2-digit", minute: "2-digit", hourCycle: "h23" }); } catch { return d.toISOString().slice(11, 16); }
};
...
module.exports = async function handler(req, res) {
  ...
  const motorExplicito = MOTORES.includes(String((metodo === "POST" ? datos.motor : q.motor) || "").toLowerCase());
  ...
  try {
    ...
    const meta = {
      ...
      lectura_completa_disponible: hayRutinaDictamen(),
      pedido_sesion: null,
    };
    /* lo que cambia cuando lo que se sirve es de OTRO motor que el pedido (la sesión, sin motor pedido) */
    const metaDe = (m) => ({ ...meta, motor: m, modelo_pedido: m === "sesion" ? "sesion" : meta.modelo_pedido, origen_legible: ORIGEN_LEGIBLE[m](), advertencia: m === "reglas" ? D.MENSAJES.ADVERTENCIA_REGLAS : D.MENSAJES.ADVERTENCIA });
    const clavePedido = D.clavePedido(id, perfilId);
    const pedido = await leerJSON(redis, clavePedido);
    if (pedido && pedido.version_texto === t.version) meta.pedido_sesion = pedido;
    const claveSesion = D.claveDictamen(id, perfilId, D.claveCache({ hashTexto: t.hash, modelo: "sesion", esfuerzo, perfilEntrada: entrada.perfil, fila }));
    ...
    const guardarYResponder = async (crudo, origenModelo, uso) => {
      ...
      /* el dictamen de la sesión llegó: la lectura pedida desde la pantalla queda atendida */
      if (motor === "sesion") { await redis.del(clavePedido); meta.pedido_sesion = null; }
      return responder(res, 200, { ...resultado, ...meta, cache: false, duracionMs: Date.now() - t0 });
    };

    /* PEDIR LA LECTURA COMPLETA: despierta la rutina una vez. La marca es también el
       candado (SET NX, 30 min): un segundo clic no abre otra sesión. Un disparo que falla
       seguro se borra para poder reintentar; uno indeterminado (no respondió a tiempo) se
       conserva, porque la sesión pudo arrancar. */
    if (metodo === "POST" && datos.pedir_sesion === true) {
      if (!hayRutinaDictamen()) return responder(res, 503, { ok: false, ...meta, ...conQueHacer("LECTURA_SIN_RUTINA") });
      if (!refrescar) {
        const ya = await leerJSONComprimido(redis, claveSesion);
        if (ya) return responder(res, 200, { ...ya, ...metaDe("sesion"), cache: true, duracionMs: Date.now() - t0 });
      }
      const nuevo = { el: new Date().toISOString(), version_texto: t.version, ok: null, motivo: null, detalle: null, indeterminada: false, sesion_url: null };
      const tomado = await redis.set(clavePedido, JSON.stringify(nuevo), { nx: true, ex: D.PEDIDO_TTL_SEG });
      if (!tomado) {
        const previo = await leerJSON(redis, clavePedido);
        /* una marca de OTRA versión del pliego no bloquea pedir la versión nueva: se reemplaza */
        if (previo && previo.version_texto === t.version) {
          return responder(res, 200, { ...meta, pedido_sesion: previo, hay_dictamen: false, repetida: true, cache: false, ...conQueHacer("LECTURA_YA_PEDIDA", { hora: horaColombia(previo.el) }) });
        }
        await redis.set(clavePedido, JSON.stringify(nuevo), { ex: D.PEDIDO_TTL_SEG });
      }
      const d = await Rutina.despertar({
        url: process.env.RUTINA_DICTAMEN_URL, token: process.env.RUTINA_DICTAMEN_TOKEN,
        /* el texto lleva SOLO el id y el perfil, ya validados: la sesión lee lo demás del expediente */
        texto: `Solicitud de dictamen de Detekta: id_proceso=${id} perfil=${perfilId}. Ejecute /dictamen ${id} ${perfilId}.`,
        que: "la lectura automática", quePlural: "lecturas automáticas", varUrl: "RUTINA_DICTAMEN_URL", varToken: "RUTINA_DICTAMEN_TOKEN",
      });
      const registrado = { ...nuevo, ok: d.ok, motivo: d.motivo, detalle: d.detalle, indeterminada: !!d.indeterminada, sesion_url: d.sesion_url || null };
      if (!d.ok && !d.indeterminada) {
        await redis.del(clavePedido);
        return responder(res, 200, { ...meta, pedido_sesion: registrado, hay_dictamen: false, cache: false, ...conQueHacer("LECTURA_NO_ARRANCO", { motivo: d.motivo }) });
      }
      await redis.set(clavePedido, JSON.stringify(registrado), { ex: D.PEDIDO_TTL_SEG });
      return responder(res, 200, { ...meta, pedido_sesion: registrado, hay_dictamen: false, cache: false, ...conQueHacer(d.indeterminada ? "LECTURA_INDETERMINADA" : "LECTURA_PEDIDA", { motivo: d.motivo }) });
    }
    ...
    /* SIN MOTOR PEDIDO, el dictamen que escribió una SESIÓN manda sobre la lectura por reglas
       (26-sep-2026). Antes la pantalla, que no pide motor, recibía siempre las reglas: el
       dictamen de /dictamen quedaba guardado bajo su clave y solo lo leía quien pidiera
       `&motor=sesion`, que ninguna pantalla pedía. */
    if (metodo === "GET" && !motorExplicito && motor === "reglas") {
      const deSesion = await leerJSONComprimido(redis, claveSesion);
      if (deSesion) return responder(res, 200, { ...deSesion, ...metaDe("sesion"), cache: true, duracionMs: Date.now() - t0 });
    }
