...
module.exports = async function handler(req, res) {
  ...
  /* Y LA FECHA DEL SORTEO (8-sep-2026), por la MISMA vía y con la misma poda.
     Es la ÚNICA fuente de esa fecha: la norma dice que la entidad PUEDE sortear
     con más de diez manifestaciones, no cuándo. Con ella, Mis procesos deja de
     decir «puede haber sorteo» y dice el día que falta — que es de lo que depende
     seguir en carrera en una menor cuantía. */
  let sorteoGuardado = null;
  const hitoSorteo = id && redis ? (delPliego.hitos || []).find((h) => h.id === "sorteo") : null;
  if (hitoSorteo && hitoSorteo.fecha) {
    const r = await Manif.guardarFechaCronograma(redis, id, hitoSorteo.fecha, { hoy, clave: Manif.CLAVE_CRONOGRAMA_SORTEO });
    if (r.guardada) sorteoGuardado = hitoSorteo.fecha;
  }
  ...
  return res.status(200).json({
    ...
    // la fecha del sorteo de oferentes, si el pliego la publica: no hay otra fuente
    sorteo_fecha_guardada: sorteoGuardado,
