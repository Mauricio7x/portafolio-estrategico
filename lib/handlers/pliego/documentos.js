/* lib/handlers/pliego/documentos.js · /api/pliego?op=documentos (3-sep-2026)
   ...
   EL id_del_portafolio SALE DEL CORPUS (M-DGF-05, 6-sep-2026): `lib/proyeccion`
   lo conserva, así que un proceso ingerido desde entonces no cuesta ninguna
   consulta previa a p6dx. Un registro ANTERIOR a esa full no lo trae, y ahí se
   sigue pidiendo a p6dx por id_del_proceso: el respaldo no se retira, porque
   desplegar no puede exigir reconstruir el corpus. El índice guardado declara de
   dónde salió la llave (`id_del_portafolio_desde`). */
"use strict";
...
/* Los hechos guardados con una versión anterior de las reglas se rehacen desde el
   texto guardado (pliego:{id}:doc:{idDoc}); si ese texto ya caducó, el documento sale
   de «leídos» y el navegador lo vuelve a bajar. Devuelve cuántos cambió. */
async function actualizarHechos(redis, docs) {
  let cambios = 0;
  for (const [idDoc, x] of Object.entries(docs.leidos || {})) {
    if (x && x.hechos && x.hechos.version === Docs.hechosVersion()) continue;
    let guardado = null;
    try { guardado = await leerJSONComprimido(redis, Docs.claveDoc(docs.id_proceso, idDoc)); } catch { guardado = null; }
    if (guardado && typeof guardado.texto_normalizado === "string" && guardado.texto_normalizado.length) {
      docs.leidos[idDoc] = { ...x, hechos: Docs.hechosDeTexto(guardado.texto_normalizado, { tipo: (x && x.tipo) || guardado.tipo || "otro" }), hechos_rehechos_el: new Date().toISOString() };
    } else delete docs.leidos[idDoc];
    cambios++;
  }
  return cambios;
}
...
async function consultarIndice(redis, id, { log = () => {} } = {}) {
  ...
  /* LA LLAVE, DEL CORPUS PRIMERO (M-DGF-05, 6-sep-2026). La fila guardada ya se
     leía aquí abajo para saber la fecha de cierre; ahora se lee ANTES, porque si
     trae `id_del_portafolio` sobra la consulta a p6dx entera. Un registro sin la
     llave —los anteriores a la full— cae al respaldo de siempre; un corpus viejo
     tiene que seguir sirviendo igual, y hay prueba de las dos vías. */
  const fila = await filaDe(redis, id).catch(() => null);
  const delCorpus = (fila && String(fila.id_del_portafolio || "").trim()) || "";
  let filaP = null;
  if (!delCorpus) {
    try {
      const r = await conTiempo(cli(BASES.procesos()).pedir({ "$select": "id_del_proceso,id_del_portafolio,fecha_de_recepcion_de,fecha_de_apertura_de_respuesta", "$where": `id_del_proceso='${escSoQL(id)}'`, "$limit": "1" }, "id_del_portafolio", { plazoMs: TIEMPO_MAX_MS }), TIEMPO_MAX_MS);
  ...
  const portafolio = delCorpus || (filaP && String(filaP.id_del_portafolio || "").trim()) || "";
  const desde = portafolio ? (delCorpus ? "corpus" : "p6dx") : null;   // null = no hay llave, no «corpus» por omisión
  if (!portafolio) return { ok: true, indice: { version: Docs.VERSION, consultado_el: ahora, fuente: "dmgg-8hin", id_del_portafolio: null, id_del_portafolio_desde: null, archivos: [], plan: [], resumen: null, motivo: "SECOP II no publica este proceso en datos.gov.co con su id de portafolio (procesos de SECOP I, tienda virtual o anteriores a 2025 no tienen índice de archivos)." } };
  let filas;
  try {
    filas = await conTiempo(cli(BASES.archivos()).pedir({ "$select": "id_documento,nombre_archivo,tamanno_archivo,extensi_n,fecha_carga,url_descarga_documento", "$where": `proceso='${escSoQL(portafolio)}'`, "$order": "fecha_carga", "$limit": String(MAX_FILAS_INDICE) }, "archivos del proceso", { plazoMs: TIEMPO_MAX_MS }), TIEMPO_MAX_MS);
  } catch (e) { return { ok: false, fuente: "dmgg-8hin", motivo: `no se pudo consultar el índice de archivos: ${String((e && e.message) || e)}` }; }
  const cierre = cierreDe(fila, filaP);
  const plan = Docs.planDeLectura(Array.isArray(filas) ? filas : [], { cierre });
  return { ok: true, indice: { version: Docs.VERSION, consultado_el: ahora, fuente: "dmgg-8hin", id_del_portafolio: portafolio, id_del_portafolio_desde: desde, cierre_usado: cierre, archivos: plan.archivos, plan: plan.plan, resumen: plan.resumen,
...
module.exports = async function handler(req, res) {
  ...
  if (metodo === "GET") {
    ...
    if (viejo) {
      ...
      else if (!docs.indice) { aviso = r.motivo; docs.indice = { version: Docs.VERSION, consultado_el: new Date().toISOString(), fuente: "dmgg-8hin", id_del_portafolio: null, id_del_portafolio_desde: null, archivos: [], plan: [], resumen: null, motivo: r.motivo, transitorio: true }; }
      else aviso = `se usa el índice guardado el ${String(docs.indice.consultado_el).slice(0, 10)}: ${r.motivo}`;
    }
    let rehechos = 0;
    try { rehechos = await actualizarHechos(redis, docs); if (rehechos) await escribirDocs(redis, id, docs); } catch { rehechos = 0; }
    return res.status(200).json(respuesta(docs, { cache: !viejo, aviso, hechos_rehechos: rehechos }));
  }
...
module.exports.escribirDocs = escribirDocs;
module.exports.actualizarHechos = actualizarHechos;
