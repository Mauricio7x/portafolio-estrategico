...
// la mediana es la de `lib/estadistica`, no una copia (misma regla para los cinco sitios)
...
async function ejecucionDeEntidad(entidad, { fetchImpl, log = () => {}, tiempoMs = TIEMPO_MAX_MS, ahora = Date.now() } = {}) {
  ...
  try {
    ...
    }, "contratos de la entidad", { plazoMs: tiempoMs }), tiempoMs);
    ...
    return { ...base, motivo: `no se pudo consultar el historial de contratos de la entidad: ${String((e && e.message) || e)}` };
  }
}
