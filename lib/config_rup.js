...
function validarIndicadores(acc, base, ind) {
  ...
  numero(acc, c("liquidez"), ind.liquidez, { min: 0, max: MAX_LIQUIDEZ, exclusivoMin: true });
  numero(acc, c("endeudamiento"), ind.endeudamiento, { min: 0, max: 1, exclusivoMax: true });
  numero(acc, c("cobertura_intereses"), ind.cobertura_intereses, { min: 0 });
  const patOk = numero(acc, c("patrimonio"), ind.patrimonio, { min: 0, entero: true, exclusivoMin: true });
  const utOk = numero(acc, c("utilidad_operacional"), ind.utilidad_operacional, { min: 0, entero: true, exclusivoMin: true });
...
/* ---------- un perfil ---------- */
function validarPerfil(acc, clave, p) {
  ...
  validarIndicadores(acc, base, p.indicadores);
...
function validarPerfilDinamico(id, perfil) {
  const acc = crearAcumulador();
  const limpio = validarPerfil(acc, String(id || "rup"), perfil);
