...
const CLAVES_PERFIL = ["helder", "genesis", "prodiac", "pics", "consorcio", "juntos"];
...
/* Los tamaños de empresa que publica un certificado de RUP, DE MENOR A MAYOR:
   el orden es parte del dato, porque el tamaño que ata a un proponente plural
   es el de su integrante más grande (lib/perfiles.tamanoQueAta). Esta es la
   ÚNICA lista del árbol: la lee el esquema, la lee el combinador plural y la
   lee el lector de certificados (lib/rup_pdf). Una segunda copia se separaría
   de esta a la primera corrección, y un tamaño que el lector sabe leer pero el
   esquema rechaza deja al dueño sin la advertencia de convocatoria limitada
   sin decirle por qué. */
const TAMANOS_EMPRESA = ["microempresa", "pequena", "mediana", "gran_empresa"];
...
/* ---------- componentes del balance (opcional, 25-sep-2026) ----------
   El Documento Tipo calcula los indicadores de un proponente plural SUMANDO los
   componentes de sus integrantes (lib/perfiles.derivarPlural), así que viajan en
   el archivo junto a los índices. Opcional —un archivo viejo no lo trae y el
   consorcio lo dice como «sin dato»—, pero si viene va ENTERO: un balance a
   medias daría un indicador del consorcio calculado con un hueco. La utilidad
   operacional puede ser negativa (el certificado dice «UTILIDAD/PERDIDA») y el
   patrimonio también; los activos, pasivos e intereses no. */
const CAMPOS_BALANCE = Object.freeze({
  activo_corriente: { min: 0 }, activo_total: { min: 0 }, pasivo_corriente: { min: 0 }, pasivo_total: { min: 0 },
  patrimonio: {}, utilidad_operacional: {}, gastos_intereses: { min: 0 },
});
function validarBalance(acc, base, b) {
  if (b === undefined || b === null) return;
  if (typeof b !== "object" || Array.isArray(b)) { acc.error(`${base}.balance`, `debe ser un objeto con ${Object.keys(CAMPOS_BALANCE).join(", ")}`, b); return; }
  for (const [k, reglas] of Object.entries(CAMPOS_BALANCE)) numero(acc, `${base}.balance.${k}`, b[k], reglas);
  for (const k of ["corte", "paginas", "firmeza"]) {
    if (b[k] != null && typeof b[k] !== "string") acc.error(`${base}.balance.${k}`, "debe ser texto o null", b[k]);
  }
}
...
function validarPerfil(acc, clave, p, opciones = {}) {
  ...
  /* El tamaño es OPCIONAL (un archivo viejo no lo trae y conserva el del
     respaldo) pero si viene tiene que ser uno de los publicados: un
     «grande» escrito a mano se guardaría tal cual, no casaría con ninguno y la
     advertencia de convocatoria limitada a Mipyme no saltaría NUNCA, en
     silencio. Error visible, con la lista, en vez de una degradación muda. */
  if (p.tamano_empresa != null && !TAMANOS_EMPRESA.includes(p.tamano_empresa)) {
    acc.error(`${base}.tamano_empresa`, `debe ser uno de: ${TAMANOS_EMPRESA.join(" | ")} (o null si no consta en el certificado)`, p.tamano_empresa);
  }
  ...
  validarBalance(acc, base, p.balance);
  ...
  /* `tope_smmlv` admite `null` = SIN TOPE DECLARADO, y el motor ya lo trataba
     así (`lib/rup`: `topeCop == null ⇒ dentro_de_tope: true`). El esquema era
     más estricto que el motor y rechazaba lo que la propia app sabe servir: un
     socio cuyo apetito estratégico no nos consta —PRODIAC, desde el
     11-sep-2026— no puede llevar un tope inventado, porque un tope inventado
     RECORTA la lista por una cifra que nadie declaró. Sin dato es sin techo,
     que además es la dirección que no esconde procesos. */
  const topeOk = p.tope_smmlv == null ? false
    : numero(acc, `${base}.tope_smmlv`, p.tope_smmlv, { min: 0, exclusivoMin: true });
  /* Del ciclo cerrado «descargar → editar → volver a subir» (lib/perfiles.
     perfilComoConfig): OPCIONALES —un archivo que no los traiga es válido y
     conserva lo del respaldo— pero si vienen tienen que ser números ≥ 0. */
  if (p.capital_trabajo != null) numero(acc, `${base}.capital_trabajo`, p.capital_trabajo, { min: 0 });
  if (p.contratos_rup != null) numero(acc, `${base}.contratos_rup`, p.contratos_rup, { min: 0, entero: true });
  // el factor de experiencia de la capacidad residual (Guía CCE-EICP-GI-22, num. 9.2)
  if (p.experiencia_segmento72_smmlv != null) numero(acc, `${base}.experiencia_segmento72_smmlv`, p.experiencia_segmento72_smmlv, { min: 0, max: MAX_EXPERIENCIA_SMMLV * 100 });
  // los mayores contratos del segmento 72 (lib/reparto, regla de experiencia del plural): hasta 7, cada uno ≥ 0
  if (p.experiencia_segmento72_mayores_smmlv != null) {
    const m = p.experiencia_segmento72_mayores_smmlv;
    // vacía no: «ningún contrato» se escribe omitiendo la lista, y una lista vacía se leería como un cero
    if (!Array.isArray(m) || m.length < 1 || m.length > 7) acc.error(`${base}.experiencia_segmento72_mayores_smmlv`, "debe ser una lista de 1 a 7 valores en salarios mínimos (omítala si no hay contratos)", m);
    else m.forEach((v, i) => numero(acc, `${base}.experiencia_segmento72_mayores_smmlv[${i}]`, v, { min: 0, max: MAX_EXPERIENCIA_SMMLV }));
  }
  const ind = p.indicadores || {};
  for (const k of ["rentabilidad_patrimonio", "rentabilidad_activo"]) {
    if (ind[k] != null) numero(acc, `${base}.indicadores.${k}`, ind[k], {});
  }
...
function validarConfig(body) {
  ...
  if (!perfiles || typeof perfiles !== "object" || Array.isArray(perfiles)) {
    acc.error("perfiles", `es obligatorio y debe ser un objeto con alguna de estas claves: {${CLAVES_PERFIL.join(", ")}}`, perfiles);
    return { ok: false, errores: acc.errores, advertencias: acc.advertencias, config: null, perfiles_cargados: [] };
  }
  ...
  if (plural && esNumero(plural.tope_smmlv)) {
    /* HERMANO VIVO (11-sep-2026): el bucle recorría helder y genesis a mano y
       se dejaba fuera a prodiac, así que a una socia con tope cargado no le
       saltaría el aviso. Se recorre el censo de perfiles individuales, no una
       lista escrita al lado. */
    for (const clave of CLAVES_PERFIL.filter((k) => k !== "consorcio" && k !== "juntos")) {
...
module.exports = {
  CLAVES_PERFIL, TIPOS, TIPOS_PLURAL, CODIGO_RE, NIT_RE, TAMANOS_EMPRESA,
