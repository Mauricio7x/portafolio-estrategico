...
/* El mensaje enumera lo que el campo `perfiles` de la MISMA respuesta lista:
   decía tres cuando ya se servían cuatro, y un error que se contradice con el
   dato de al lado manda a buscar el fallo donde no está. Sale de la fuente
   única para que no puedan volver a separarse. */
const ERROR_PERFIL = `falta ?perfil=${require("./perfiles.js").IDS.join(" | ")} (alias: consorcio) — o el id de un RUP subido (rup_…)`;
