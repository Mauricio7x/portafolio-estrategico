  ...
  return (l) => {
    ...
    c = {
      ...
      /* Manifestación de interés (menor cuantía): null si la modalidad no la
         exige; si la exige, la fecha límite CALCULADA y si ya venció. */
      manifestacion: manif,
      texto: null, // perezoso
    };
    memo.set(l, c);
...
function cumple(l, c, estado) {
  ...
  if (estado.manif) {
    ...
    // «abierta»: se puede manifestar todavía. Sin fecha legible no se afirma
    // ni lo uno ni lo otro: queda fuera de «abierta», dentro de «todas».
    if (estado.manif === "abierta" && c.manifestacion.vencida !== false) return false;
  }
  if (estado.q) {
...
function facetas(filas, clasificar) {
  ...
  const f = {
    ...
    /* Manifestación de interés: cuántos procesos de menor cuantía hay y en
       cuántos todavía se puede manifestar (con la fecha CALCULADA); `urgentes`
       = vence hoy o mañana hábil (≤ 2 días hábiles contando hoy). */
  ...
  for (const l of filas) {
    ...
    if (c.manifestacion) {
      ...
      fm.total++;
      if (m.vencida === true) fm.vencidas++;
      else if (m.vencida === false) { fm.abiertas++; if (m.quedan_habiles != null && m.quedan_habiles <= 2) fm.urgentes++; }
      else fm.sin_fecha++;
    }
