...
const { norm, nucleoEstado, TIPO_CONSULTORIA, FASE_ANTES_MANIFESTACION_RE } = require("./semantica.js");
const habiles = require("./habiles.js");
const { manifestacionDeFila, noConstaVencida, esUrgente, porAbrir, aperturaPublicadaDe } = require("./manifestacion.js");
...
/* La consultoría la decide `TIPO_CONSULTORIA` de lib/semantica —la MISMA regla
   que usa el veredicto (`pertinencia.tipo`)—. Aquí vivía una copia reescrita a
   la que le faltaba «supervisión técnica»: `tipoTrabajoDe(l)` decía «obra» y
   `tipoTrabajoDe(l, rup)` decía «consultoría» del MISMO proceso, y la baja
   máxima cobraba la contribución del 5 % que la ganancia no cobraba (1-sep-2026). */
...
function tipoTrabajoDe(l, rup) {
  ...
  if (contrato === "consultoria" || TIPO_CONSULTORIA.test(t) || (pert && pert.tipo === "consultoria")) return "consultoria";
...
/* ¿Este proceso ADMITE OFERTAS hoy? Medido en el diagnóstico de producción: de
   1.138 procesos servidos, `estado_del_procedimiento` vale «Publicado» en 1.133
   y **«Borrador» en 5**. Entran porque `filtros.coincide` compara por prefijo en
   los DOS sentidos y `ESTADOS_ABIERTOS` trae «borrador de pliegos»: el literal
   suelto casa por accidente, no por decisión.

   A un borrador no se le puede presentar oferta —sea el proyecto de pliego, que
   sigue en observaciones, o un proceso que aún no se publicó— y la tarjeta le
   pintaba cierre, probabilidad y «cumple los requisitos para presentarse». Aquí
   el falso positivo es el caro: el recurso escaso es el tiempo del equipo.

   NO se excluye del corpus a propósito. Si es el proyecto de pliego, es la
   ventana más valiosa del manual (mandamiento 7: observar con la redacción
   alternativa lista para pegar), y esconderla sería el falso negativo que cuesta
   más. Se DICE lo que es — y la distinción no exige resolver la ambigüedad:
   en las dos lecturas «todavía no admite ofertas» es cierto.

   Solo el literal EXACTO responde `false`. Un estado ausente, vacío o
   desconocido devuelve `true`: no saber no es poder afirmar que no admite
   ofertas, que es la regla de «sin dato ≠ cero» aplicada a un booleano. Y
   «Borrador de pliegos» es un estado publicado de la lista canónica: otra cosa. */
function admiteOfertas(l) {
  if (norm(String((l && l.estado_del_procedimiento) || "").trim()) === "borrador") return false;
  /* Y LA FASE ANTERIOR A LA MANIFESTACIÓN (22-sep-2026, medición del dueño): en
     «Presentación de observaciones» —«Publicado» o «Evaluación»— el pliego es el
     proyecto y no se recibe ninguna oferta todavía. Se declara, no se excluye:
     es la ventana de observar. Una fase rezagada solo daría un ámbar de más. */
  const fase = nucleoEstado(norm(String((l && l.fase) || "")));
  /* y la misma guarda que `senalSecop` (22-sep-2026, noche): con la manifestación ya publicada
     (`fecha_de_publicacion`) la fase «observaciones» es vieja y no afirma nada */
  return !FASE_ANTES_MANIFESTACION_RE.test(fase) || !!aperturaPublicadaDe(l);
}

function crearClasificador({ rupDe = () => null, ahora = Date.now(), fechasCronograma = {} } = {}) {
  const memo = new Map();
  ...
  return (l) => {
    ...
    /* `ahora` viaja además de `hoy`: cuando el pliego publicó la HORA del plazo
       («14/09/2026 6:00:00 PM»), el día del vencimiento se decide con el reloj y
       no con el calendario. Sin hora publicada no cambia nada. */
    const manif = manifestacionDeFila(l, hoy, { fechaCronograma: fechasCronograma[l.id_del_proceso] || null, ahora });
    ...
    c = {
      ...
      manifestacion: manif,
      /* false SOLO con el literal «Borrador»: no admite ofertas todavía. Ver
         `admiteOfertas` — no se excluye del corpus, se declara. */
      admite_ofertas: admiteOfertas(l),
      texto: null, // perezoso
    };
    memo.set(l, c);
    return c;
...
function cumple(l, c, estado) {
  ...
  if (estado.manif) {
    ...
    /* La casilla deja pasar TODO AQUELLO EN LO QUE NO CONSTE QUE EL PLAZO
       VENCIÓ (encargo del dueño, 15-sep-2026). Antes exigía poder AFIRMAR que
       seguía abierto, y eso escondía dos grupos enteros: los que pasaron el
       techo CALCULADO —una deducción, no un hecho— y los que no se pudieron
       situar. Quien marca esta casilla pregunta «¿a cuáles puedo presentarme
       todavía?», y la respuesta honesta incluye «no consta que no». Lo único
       que se va es lo que consta vencido, que desde hoy exige fecha publicada. */
    if (estado.manif === "abierta" && !noConstaVencida(c.manifestacion)) return false;
  }
  if (estado.q) {
...
function facetas(filas, clasificar) {
  ...
  const f = {
    ...
    /* Manifestación de interés: cuántos procesos de menor cuantía hay.
       `sin_vencer` = en los que NO CONSTA que el plazo haya vencido, que es lo
       que devuelve la casilla y por tanto lo único que puede ir a su lado en
       pantalla (una cifra que no case con la lista que abre es un defecto).
       `abiertas` ⊂ `sin_vencer` = los que se puede AFIRMAR que siguen vivos.
       `urgentes` ⊂ `abiertas` = hay que entrar a SECOP II HOY. `pudo_vencer` =
       pasó el techo calculado y nadie publicó la fecha. `sin_fecha` = no se
       pudo situar la ventana. `vencidas` = consta —por fecha publicada o por
       la fase publicada— que el plazo pasó: los únicos que la casilla esconde.
       `por_abrir` (22-sep-2026) = la fase publicada es anterior: el plazo no
       ha abierto; se ve, pero no cuenta como «abierta» (nadie puede avisar
       todavía). */
    manifestacion: { total: 0, sin_vencer: 0, abiertas: 0, urgentes: 0, pudo_vencer: 0, vencidas: 0, sin_fecha: 0, por_abrir: 0 },
  };
  for (const l of filas) {
    ...
    if (c.manifestacion) {
      ...
      fm.total++;
      if (noConstaVencida(m)) fm.sin_vencer++;
      if (m.estado === "vencida") fm.vencidas++;
      else if (m.estado === "sin_fecha") fm.sin_fecha++;
      else if (m.estado === "pudo_vencer") fm.pudo_vencer++;
      else if (porAbrir(m)) fm.por_abrir++;
...
module.exports = {
  ...
  crearClasificador, admiteOfertas, cumple, aplicar, facetas, sugerirAflojar, buscarEntidades,
