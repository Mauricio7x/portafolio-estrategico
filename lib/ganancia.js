...
/* La ARITMÉTICA de la cuenta vive en `public/ganancia.js` (UMD) y aquí solo se
   USA: el navegador la necesita para recalcular en vivo en el detalle, y dos
   implementaciones de la misma resta acabarían enseñando dos cifras del mismo
   contrato. Este módulo conserva lo que el navegador NO puede hacer: leer el
   precio de mercado y τ de `pisoTecho`, resolver el costo directo y redactar. */
const GAN = require("../public/ganancia.js");
...
/* El despeje vive en `public/ganancia.js` con la aritmética: aquí solo se
   DELEGA. Dos despejes del mismo punto de equilibrio divergirían, y este es
   el que la tarjeta cita como «necesitaría declarar al menos N %». */
const utilidadMinimaParaNoPerder = GAN.utilidadMinimaParaNoPerder;

/* LA FRASE DE LA BAJA PARA LA TARJETA (23-sep-2026): la del SERVIDOR, no una
   cuarta redactada en la pantalla. Es el `mensaje` del mismo registro de
   `bajaDeMercado` que fijó el precio —el que la tarjeta ya enseña en «Más
   detalles» (`baja_mercado.mensaje`, escrito por `mensajeDe`)—, así que la
   celda y el chip dicen lo mismo con las mismas palabras. Solo se usa si el
   registro y la cuenta coinciden en si hay cifra (con una mediana que el panel
   no usó, la frase diría una baja que el precio no lleva); si no, o si el
   registro no trae frase (uno armado a mano), la del panel Piso/Techo sobre
   la MISMA baja (`frases.baja`: `fraseBaja` o «Sin referencia…»). Nunca dice
   «sin bajar» sobre una baja ausente: esa rama exige una mediana ≤ 0 medida. */
function fraseDeLaBaja(baja, pt) {
  const usada = numero(pt && pt.cifras && pt.cifras.baja_esperada_pct) != null;
  const delIndice = baja && typeof baja.mensaje === "string" && baja.mensaje.trim() ? baja.mensaje : null;
  const indiceConCifra = numero(baja && baja.baja_mediana) != null;
  if (delIndice && indiceConCifra === usada) return delIndice;
  return (pt && pt.frases && pt.frases.baja) || null;
}
...
function gananciaDeProceso(entrada = {}) {
  const {
    ...
    /* ¿el usuario RESPONDIÓ la pregunta de si su administración ya paga los
       impuestos del contrato? No es lo mismo «dijo que no» que «nunca se lo
       preguntamos»: son 5 puntos del contrato —más que la ganancia entera— y
       mientras no conste, la cifra no puede AFIRMAR una pérdida que
       desaparece bajo la otra lectura. Es la distinción «sin dato vs cero»
       de todo el repositorio, aplicada a un booleano. */
    contribucion_declarada = false,
    p_ganar = null,
  } = entrada;
  ...
  /* Declarada también cuando la contribución no viene al caso (interventoría,
     consultoría): ahí no hay ninguna pregunta pendiente que pueda mover el signo. */
  const contribucionDeclarada = contribucion_declarada === true
    || contribucion_en_administracion === true || !aplicaContribucion(tipo_trabajo);
  ...
  const sinCifra = (motivo, frase) => ({
    ...
    precio_esperado: null, origen_precio: null, baja_granularidad: null, baja_donde: null, costo_directo: null,
    /* los campos de la baja viajan también aquí (null = sin cifra, no «sin bajar»):
       antes faltaban en esta rama y la tarjeta leía `undefined` */
    baja_aplicada_pct: null, baja_procesos: null, baja_frase: null,
    ...
    /* Los campos nuevos viajan también en la rama sin cifra: un consumidor que
       lea `veredicto` no puede recibir `undefined` según la rama. */
    desglose: null, veredicto: null, peor: null, mejor: null,
    sin_gastar_imprevisto: null, alivio_contribucion: null,
  ...
  /* LA BAJA QUE NO SE PUDO LEER NO ES «SIN HISTORIAL» (23-sep-2026). Con el
     índice de baja sin leer (`motivo: "no_se_leyo"`, lib/indice_baja.
     SIN_LECTURA_BAJA) la cuenta caía al presupuesto oficial como precio y
     afirmaba «no hay historial suficiente de esta entidad»: sobre una entidad
     que baja un 5 % medido en doce contratos, eso es una ganancia inflada en
     presupuesto × baja. En precios el error caro es el falso POSITIVO: sin la
     baja no se presupone ningún precio de mercado y no hay cifra. La frase es
     la del registro (una sola redacción del hecho) más lo que falta por eso. */
  if (baja && baja.motivo === "no_se_leyo") {
    return sinCifra("no_se_leyo_baja",
      `${baja.mensaje || "No se pudo consultar cuánto se suele bajar en esta entidad."} Sin ese dato no se calcula a qué precio se adjudicaría ni cuánto deja.`);
  }
  ...
  const techo = numero(sonda.cifras && sonda.cifras.techo_competitivo);   // ≤ po: `pisoTecho` acota la mediana negativa (22-sep-2026)
  const V = techo != null && techo > 0 ? techo : po;
  const origenPrecio = techo != null && techo > 0 ? "mercado" : "presupuesto_oficial";

  /* La derivación vive en el módulo UMD para que el detalle del navegador
     rehaga el MISMO costo implícito al mover la estructura de precio. */
  const CD = conApu ? cdMedido : GAN.costoDirectoImplicito(V, A, I, U, modo);
  ...
  const d = GAN.desglose({
    precio: V, costo_directo: CD,
    administracion_pct: A, imprevistos_pct: I, utilidad_pct: U, modo,
    descuentos_pct: tauPct, contribucion_pct: contribucionPct,
    contribucion_declarada: contribucionDeclarada,
  });
  /* La identidad que ata esta tarjeta al panel Piso/Techo: el costo sin
     ganancia del desglose TIENE que ser el `costo_sin_utilidad` que publica
     `pisoTecho`, redondeado una sola vez. Si divergieran, la tarjeta diría que
     deja y el panel que no. Hay prueba, y aquí queda la aserción viva. */
  const costoSinGanancia = numero(pt.cifras.costo_sin_utilidad);
  const descuentos = d.descuentos;
  const valor = d.valor;
  const utilidadDeclarada = d.utilidad_declarada;
  ...
  let frase;
  if (d.veredicto === "deja") {
    frase = `Si gana este contrato a ${cop(V)} le quedan ${cifra(valor)} después de pagar la obra`
      + (tauPct > 0 ? ` y los ${cop(descuentos)} que le descuentan de las actas.` : ".")
      + (d.imprevistos > 0 ? ` Y hasta ${cifra(d.sin_gastar_imprevisto)} si no gasta la reserva para imprevistos.` : "");
  } else if (d.veredicto === "depende") {
    /* NI «deja» NI «pierde»: el signo lo deciden dos cosas que todavía no
       constan, y decir cualquiera de las dos sería inventar. Se dicen LAS DOS
       con su nombre en castellano y qué las cierra. */
    const cierran = [];
    if (d.imprevistos > 0) cierran.push(`si gasta o no la reserva para imprevistos (${cifra(d.imprevistos)})`);
    if (d.alivio_contribucion > 0) cierran.push(`si su administración ya paga o no los impuestos del contrato (${cifra(d.alivio_contribucion)})`);
    frase = `A ${cop(V)} este contrato puede dejarle hasta ${cifra(d.mejor)} o costarle ${cifra(-d.peor)}`
      + (cierran.length ? `: depende de ${cierran.join(" y de ")}.` : ".")
      /* CON APU y peor caso en rojo, la advertencia del panel Piso/Techo SIGUE
         viajando: lo que usted costeó, con la reserva gastada, no cabe en el
         precio al que aquí se adjudica. Suavizar el veredicto no puede
         significar perder la instrucción — es el mismo proceso que el panel
         marca «no se presente», y la tarjeta tiene que decirlo. */
      + (conApu && d.peor < 0
        ? ` Ojo: lo que usted costeó (${cop(costoSinGanancia)} de obra, administración e imprevistos) no cabe en ese precio si gasta la reserva.`
        : (conApu ? "" : ` Calcúlelo en Precios con su costo real para cerrar la cifra.`))
      /* LA INSTRUCCIÓN NO SE PIERDE AL SUAVIZAR EL VEREDICTO. Que el rango
         cruce el cero no quita que, en la cuenta prudente, una ganancia
         declarada del 5 % no cubra una contribución del 5 %: es la lección
         central del manual («el olvido más caro del país») y lo accionable es
         subir la ganancia declarada. Se dice en «depende» igual que en
         «pierde», porque el usuario la necesita en los dos casos. */
      + (!conApu && contribucionAplica && d.peor < 0 && uMinima
        ? ` Con una ganancia declarada del ${pct(U)} la cuenta prudente no cubre la contribución del ${pct(CONTRIBUCION_PCT)}. Necesitaría declarar al menos ${pct(uMinima)}.`
        : "");
  } else if (conApu) {
    frase = `A ${cop(V)} este contrato deja una pérdida de ${cifra(-d.mejor)} aun en el mejor de los casos: lo que usted costeó `
    ...
    frase = `A ${cop(V)} este contrato deja una pérdida de ${cifra(-d.mejor)} aun en el mejor de los casos: con una ganancia declarada del ${pct(U)} `
      + `no alcanza para pagar la contribución del ${pct(CONTRIBUCION_PCT)} de obra pública. Necesitaría declarar al menos ${pct(uMinima)}.`;
  } else {
    frase = `A ${cop(V)} este contrato deja una pérdida de ${cifra(-d.mejor)} aun en el mejor de los casos: `
  ...
  return {
    ...
    motivo: null,
    frase,
    /* EL DESGLOSE ENTERO, para que el detalle no tenga que recalcular nada y
       para que la resta se pueda auditar línea a línea: precio − descuentos −
       obra − administración − imprevistos = lo que queda, cuadrado al peso. */
    desglose: d,
    veredicto: d.veredicto,
    peor: d.peor,
    mejor: d.mejor,
    sin_gastar_imprevisto: d.sin_gastar_imprevisto,
    alivio_contribucion: d.alivio_contribucion,
    contribucion_declarada: contribucionDeclarada,
    ...
    /* DE DÓNDE SALIÓ LA BASE (22-sep-2026): la cascada de `bajaDeMercado` baja de
       la entidad al departamento+familia sin que nadie lo pida, y la tarjeta
       decía «es lo que suele pagar esta entidad» sobre ocho contratos de toda
       Antioquia. `pisoTecho` ya lo publicaba; aquí se perdía en la copia. */
    baja_granularidad: pt.cifras.baja_granularidad || null,
    baja_donde: require("./indice_baja.js").dondeSeMidio(pt.cifras.baja_granularidad),
    baja_frase: fraseDeLaBaja(baja, pt),
    ...
    fuentes: {
      ...
      descuentos: `Contribución especial de obra pública, ${pct(CONTRIBUCION_PCT)} del valor del contrato (Ley 418 de 1997, art. 120; permanente por Ley 1738 de 2014)`
        + (ded != null ? `, más el ${pct(ded)} de estampillas y ReteICA que usted cargó.` : ". Las estampillas del pliego no están cargadas.")
        /* LO QUE SE PIERDE Y LO QUE VUELVE SON DOS COSAS, y aquí solo se resta
           lo primero. La retención en la fuente es un anticipo del impuesto de
           renta —que grava la ganancia, no lo facturado— y la de garantía se
           devuelve al liquidar: restarlas del margen cobraría dos veces una
           plata que el contratista recupera. Sobre un contrato de $3.216 M la
           retefuente sola son $64 M. Aprietan la CAJA, y eso lo calcula el
           análisis de precios, no esta cuenta. */
        + " No se descuentan aquí la retención en la fuente ni la de garantía: se las devuelven o se las cruzan en su declaración.",
    },
  };
}
