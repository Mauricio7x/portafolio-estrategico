...
function desglosarProbabilidad(proceso, indiceCompetencia, indiceBaja, contexto = {}) {
  ...
  const explicacion = [];
  {
    ...
    if (t.fuente === "entidad" && t.encogido && t.peso_datos != null && t.peso_datos < 0.8) {
      ...
      explicacion.push({
        ...
        texto: `Esta entidad publica el número de ofertas en menos de ${MIN_PROCESOS} procesos adjudicados, así que `
          ...
          + `Su oferta sería una entre ${dec(rv + 1, 1)}.`,
      });
    } else {
      explicacion.push({
        tipo: "base",
        texto: (noLeida
          ? "Esta vez no se pudo consultar cuántas empresas compiten en esta entidad (no significa que no tenga datos), así que mientras tanto se asume el escenario prudente: "
          : "Ni esta entidad ni su departamento publican cuántas empresas ofertaron en suficientes procesos, así que se asume el escenario prudente: ")
    ...
    if (pr && pr.factor < 1) {
      explicacion.push({ tipo: "baja", texto: `Aquí el que gana suele bajar el precio ~${pr.baja_mediana_pct} % y usted solo puede bajar hasta ${pr.baja_maxima_pct} % sin perder plata: ofertar por encima de lo habitual le resta.` });
    } else if (pr && pr.origen_b_max === "declarada") {
      explicacion.push({ tipo: "neutro", texto: `Puede bajar hasta ${pr.baja_maxima_pct} % sin perder plata y aquí el que gana baja ~${pr.baja_mediana_pct} %: puede ofertar en el centro del mercado, el precio no le resta.` });
    }
    ...
    /* Sin base medida de ESTA entidad el resultado no se enuncia como un hecho
       (23-sep-2026): con el promedio de otras entidades o con el supuesto, la
       misma cifra solo ordena la lista, y la tarjeta ya no la pinta. */
    if (deCada && t.fuente === "entidad") {
      explicacion.push({ tipo: "cierre", texto: `Resultado: de cada ${deCada} procesos como este, se gana 1 aproximadamente (${dec(finalPP, 2)} %).` });
    } else if (deCada) {
      explicacion.push({ tipo: "cierre", texto: `${t.fuente === "departamento" ? "Con el promedio de su departamento" : "Con ese supuesto"}, `
        + `de cada ${deCada} procesos como este se ganaría 1 (${dec(finalPP, 2)} %): sirve para ordenar la lista, no es una medición de esta entidad.` });
    }
  }
...
function generarResumenEjecutivo(desglose, costo_preparacion_estimado) {
  ...
  const ORIGEN = {
    ...
    departamento: `estimados con el promedio de ${d.departamento || "su departamento"}: la entidad no publica el número de ofertas en suficientes procesos`,
    conservador: noLeida
      ? "por supuesto conservador: esta vez no se pudo consultar cuántos compiten en la entidad, y eso no significa que no tenga datos"
      : "por supuesto conservador: ni la entidad ni su departamento publican el número de ofertas en suficientes procesos",
  };
...
function justificacionTexto(desglose, resumen) {
  ...
  L.push(`Cuantía: ${d.cuantia_cop != null ? cop(d.cuantia_cop) : "no publicada"} · Valor esperado: ${d.valor_esperado_cop != null ? cop(d.valor_esperado_cop) : "no calculable sin cuantía"}`);
  L.push("");
...
async function desgloseDeProceso(redis, idCrudo, {
  ...
  let errorLecturaIndice = null, errorLecturaBaja = null;
  const anotarFallo = (e) => { errorLecturaIndice = textoDeFallo(e); };
  const anotarFalloBaja = (e) => { errorLecturaBaja = textoDeFallo(e); };
  const metaComp = await leerIndiceMeta(redis).catch((e) => { anotarFallo(e); return null; });
  const metaBaja = await leerIndiceBajaMeta(redis).catch((e) => { anotarFalloBaja(e); return null; });
  ...
  if (errorLecturaIndice) indice = INDICE_NO_LEIDO;
  else {
    try { indice = await leerIndice(redis); } catch (e) { anotarFallo(e); indice = INDICE_NO_LEIDO; }
  }
  if (errorLecturaBaja) indiceBaja = INDICE_NO_LEIDO;
  else {
    // sin construir sigue siendo opcional (null); lo que no se pudo leer viaja como centinela
    try { indiceBaja = await leerIndiceBaja(redis); } catch (e) { anotarFalloBaja(e); indiceBaja = INDICE_NO_LEIDO; }
  }
