...
async function desgloseDeProceso(redis, idCrudo, {
  ...
  if (usarCache && !errorLecturaIndice) {
  ...
  const cuerpo = {
    ...
    /* null cuando el índice se leyó (o no está construido); si no, por qué el
       paso 1 cae al supuesto: la entidad puede tener datos */
    aviso_competencia: errorLecturaIndice
      ? `No se pudo consultar cuántos compiten en esta entidad (${errorLecturaIndice}): el paso 1 usa el supuesto `
        + "conservador, pero eso no significa que la entidad no tenga datos. Vuelva a abrir el desglose en unos minutos."
      : null,
  };
  if (!errorLecturaIndice) await guardarCache(redis, id, { sello, cuerpo });
