/* ============================================================================
   ...
   ── SEIS PASOS, SIEMPRE LOS SEIS ──────────────────────────────────────────
     ...
     4. Baja de mercado de la entidad                  ×0,85 · ×1,00 · ×1,10
     5. Colisión de cierres                            ×1,15
   ...
   · El encargo lista cuatro factores; el código aplica SEIS: faltan los dos de
     baja de mercado (×0,85 si la entidad descuenta más del 5 %, ×1,10 si menos
     del 2 %). Son los que convierten la respuesta en «P(ganar a un precio que
     valga la pena)», así que omitirlos habría dejado fuera del desglose un
     ajuste que sí mueve la cifra que se enseña.
   ========================================================================== */
"use strict";
...
const { leerIndiceBaja, leerIndiceBajaMeta, bajaDeMercado } = require("./indice_baja.js");
...
function desglosarProbabilidad(proceso, indiceCompetencia, indiceBaja, contexto = {}) {
  ...
  /* ─────────── PASO 1 · probabilidad base ─────────── */
  {
    ...
    push({
      ...
      formula: "P_base = 1 / (1 + promedio_oferentes_entidad)",
      datos_entrada: datos,
  ...
  /* ─────────── PASO 3 · prórroga del cierre ─────────── */
  {
    ...
    push({
      ...
      formula: `P = P × ${FACTOR_CIERRE_PRORROGADO} si el cierre se movió por adenda`,
      datos_entrada: {
  ...
  /* ─────────── PASO 4 · baja de mercado de la entidad ─────────── */
  {
    const a = aplicado("baja_mercado");
    const conBase = baja.nivel && baja.nivel !== "sin_dato" && baja.baja_mediana != null;
    const pPrevia = a ? a.p_antes : pAntesDe(t, "baja");
    const generado = metaBaja && metaBaja.generado ? String(metaBaja.generado).slice(0, 10) : null;
    push({
      paso: 4,
      nombre: "Ajuste por baja de mercado de la entidad",
      formula: `P = P × f(baja)  ·  f = ${FACTOR_BAJA_BAJA} hasta ${BAJA_BAJA_HASTA} %  ·  ${FACTOR_BAJA_ALTA} desde ${BAJA_ALTA_DESDE} %`
      ...
      fundamento: "No es una señal de cuántos rivales hay sino de a qué precio se adjudica: convierte la cifra en "
        + "«probabilidad de ganar a un precio que valga la pena». Una entidad que adjudica sistemáticamente por debajo "
        + "del presupuesto obliga a descontar, y el descuento sale del margen. Hasta ago 2026 eran dos ESCALONES y "
        + "cruzar el corte del 5 % costaba un 15 % de probabilidad de golpe, sobre una mediana que el índice publica "
        + "con resolución de un punto porcentual: 4,9 % y 5,1 % eran la misma entidad medida dos veces. Ahora interpola. "
        + "Aviso de lectura: la rampa suaviza la FUNCIÓN, pero el dato sigue cuantizado —la mediana es siempre entera—, "
        + "así que lo que se ve es una escalera de cuatro peldaños cuyo salto máximo bajó del 15,0 % al 8,9 %. Y los "
        + "dos extremos, 1,10 y 0,85, siguen siendo supuestos puestos a mano: suavizar no es calibrar.",
      aporte_pp: a ? pp(a.p_despues - a.p_antes) : 0,
    });
  }
  ...
  /* ─────────── PASO 5 · colisión de cierres ─────────── */
  {
    ...
    push({
      ...
      formula: `P = P × ${FACTOR_COLISION_CIERRES} si la entidad cierra 2 o más procesos el MISMO DÍA`,
      datos_entrada: {
    ...
    if (t.fuente === "entidad") {
      explicacion.push({
        ...
        texto: `A los procesos de esta entidad se han presentado, en promedio, ${dec(rv, 1)} empresas — está contado `
          + `sobre ${competencia.total_procesos} procesos que ya se adjudicaron. Si se presentan ${dec(rv, 1)}, tu `
      ...
      explicacion.push({
        ...
        texto: `Esta entidad tiene poco historial (menos de ${MIN_PROCESOS} procesos adjudicados con dato), así que `
          ...
          + `Tu oferta es una entre ${dec(rv + 1, 1)}.`,
      });
    } else {
      explicacion.push({
        tipo: "base",
        texto: `De esta entidad no hay historial en los datos, así que se asume el escenario prudente: `
          + `${PROMEDIO_CONSERVADOR} rivales — tu oferta sería una entre ${PROMEDIO_CONSERVADOR + 1}. Es un supuesto `
          + `declarado, no una medición.`,
      });
    }
    if (aplicado("cierre_prorrogado")) {
      explicacion.push({ tipo: "sube", texto: "La entidad aplazó la fecha de cierre. Eso casi siempre pasa cuando llegaron pocas ofertas, así que tu opción sube un poco." });
    }
    const bj = aplicado("baja_mercado");
    if (bj && bj.factor > 1) {
      explicacion.push({ tipo: "sube", texto: "Aquí los contratos se suelen adjudicar casi por el presupuesto oficial: hay poca guerra de precio, y eso te favorece un poco." });
    } else if (bj && bj.factor < 1) {
      explicacion.push({ tipo: "baja", texto: "Aquí el que gana suele ofertar bastante por debajo del presupuesto: la pelea de precio está dura, y eso te resta un poco." });
    }
    if (aplicado("colision_cierres")) {
      explicacion.push({ tipo: "sube", texto: "La entidad cierra varios procesos el mismo día: los competidores no alcanzan a presentarse a todos, y eso te suma un poco." });
    }
    if (t.limite) {
...
const ORDEN_CADENA = ["competencia", "cierre_prorrogado", "baja", "colision_cierres"];
