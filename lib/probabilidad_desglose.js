...
function desglosarProbabilidad(proceso, indiceCompetencia, indiceBaja, contexto = {}) {
          ...
          : t.encogido
            ...
            : t.fuente === "departamento"
              ? `El promedio de oferentes NO es de esta entidad: sale de los procesos adjudicados de ${depto || "su departamento"}, `
                + `porque no hay datos suficientes de cuántos compiten en esta entidad. Es un respaldo para ordenar la lista, `
    ...
    push({
      ...
      datos_entrada: {
        ...
        fuente: conBase
          ...
          : baja.mensaje || "Sin datos de cuánto se baja en esta entidad",
      },
      calculo: a
        ? `${dec(a.p_antes, 4)} × ${a.factor} = ${dec(a.p_despues, 4)}`
        : "sin dato: no se aplica ajuste",
  ...
  const conBase = conBaseDeEntidad(t.fuente, competencia);
  {
    ...
    if (mezcla) {
      ...
      explicacion.push({
        tipo: "base",
        texto: "No hay datos suficientes de cuántos compiten en esta entidad, así que "
      ...
      explicacion.push({
        ...
        texto: (noLeida
          ...
          : "No hay datos suficientes de cuántos compiten en esta entidad ni en su departamento, así que ")
...
function generarResumenEjecutivo(desglose, costo_preparacion_estimado) {
  ...
  if (!conBaseDeEntidad(d.fuente_del_promedio, d.competencia_entidad)) {
    const FALTA = {
      ...
      departamento: `no hay datos suficientes de cuántos compiten en esta entidad, y el cálculo usa el promedio de `
