...
function desglosarProbabilidad(proceso, indiceCompetencia, indiceBaja, contexto = {}) {
  ...
  {
    const a = aplicado("competencia_baja") || aplicado("competencia_alta");
    ...
    const datos = {
      ...
      cortes_tertiles: cortes && !cortes.degenerado
        ...
        : null,
      factor_aplicado: a ? a.factor : (clasificada ? 1 : null),
      fuente: clasificada
    ...
    push({
      paso: 2,
      nombre: "Ajuste por nivel de competencia de la entidad",
      formula: `P = P × f(nivel)  ·  f(baja) = ${FACTOR_COMPETENCIA_BAJA}  ·  f(media) = 1  ·  f(alta) = ${FACTOR_COMPETENCIA_ALTA}`,
      datos_entrada: datos,
      calculo: a
        ? `${dec(a.p_antes, 4)} × ${a.factor} = ${dec(a.p_despues, 4)}`
        : clasificada
          ? `${dec(t.base, 4)} × 1 = ${dec(t.base, 4)} (nivel «${competencia.nivel}»: sin ajuste)`
          : "sin nivel clasificado: no se aplica ajuste",
      resultado: a ? pct(a.p_despues) : pct(t.base),
      confianza: !clasificada ? SIN_DATO
        : competencia.total_procesos >= PROCESOS_CONFIANZA_ALTA ? ALTA : MEDIA,
      fundamento: "El promedio es un estimador puntual; el tertil sitúa a la entidad frente al resto del mercado. "
        + "Los tres factores son supuestos declarados, no coeficientes calibrados: el corpus no dice qué se ganó y qué se perdió.",
  ...
  {
    const a = aplicado("baja_alta") || aplicado("baja_baja");
    ...
    push({
      ...
      formula: `P = P × ${FACTOR_BAJA_ALTA} si baja_mediana > ${BAJA_ALTA_DESDE} %  ·  × ${FACTOR_BAJA_BAJA} si baja_mediana < ${BAJA_BAJA_HASTA} %  ·  × 1 entre ambos`,
      datos_entrada: {
      ...
      calculo: a
        ? `${dec(a.p_antes, 4)} × ${a.factor} = ${dec(a.p_despues, 4)}`
        : conBase
          ? `${dec(pPrevia, 4)} × 1 = ${dec(pPrevia, 4)} (baja mediana ${baja.baja_mediana} %: entre los dos cortes)`
          : "sin dato: no se aplica ajuste",
      resultado: pct(a ? a.p_despues : pPrevia),
      ...
      fundamento: "No es una señal de cuántos rivales hay sino de a qué precio se adjudica: convierte la cifra en "
        ...
        + "del presupuesto obliga a descontar, y el descuento sale del margen.",
      aporte_pp: a ? pp(a.p_despues - a.p_antes) : 0,
    });
  }
...
const GRUPO_DE = {
  competencia_baja: "competencia", competencia_alta: "competencia",
  cierre_prorrogado: "cierre_prorrogado",
  baja_alta: "baja", baja_baja: "baja",
...
const claveCache = (id) => `${CLAVES.desgloseProbabilidad}${id}`;
