...
function desglosarProbabilidad(proceso, indiceCompetencia, indiceBaja, contexto = {}) {
    ...
    push({
      ...
      fundamento: "Reparto uniforme: con N rivales habilitados, cada oferta gana 1 de cada N+1 veces. "
        + (t.fuente === "conservador"
          ...
          : t.encogido
            ? "Los rivales esperados combinan el histórico de la entidad con el promedio global del índice, pesando "
              + "los datos propios por cuántos son (encogimiento gamma-Poisson): con pocos procesos manda el promedio "
              + "global, con muchos manda la entidad, y no hay ningún salto al cruzar los 5 procesos — antes UN proceso "
  ...
  const explicacion = [];
  {
    ...
    if (t.fuente === "entidad" && t.encogido && t.peso_datos != null && t.peso_datos < 0.8) {
      ...
      explicacion.push({
        ...
        texto: `Esta entidad tiene ${competencia.total_procesos} proceso${competencia.total_procesos === 1 ? "" : "s"} adjudicado${competencia.total_procesos === 1 ? "" : "s"} con dato: poco para fiarse solo de eso, `
          + `así que su historial pesa ${Math.round(t.peso_datos * 100)} % y el resto lo pone el promedio general del mercado`
          + `${mu != null ? ` (${dec(mu, 1)} empresas)` : ""}. Salen ${dec(rv, 1)} rivales esperados: tu oferta sería una entre `
    ...
    if (pr && pr.factor < 1) {
      explicacion.push({ tipo: "baja", texto: `Aquí el que gana suele bajar el precio ~${pr.baja_mediana_pct} % y vos solo podés bajar hasta ${pr.baja_maxima_pct} % sin perder plata: ofertar por encima de lo habitual te resta.` });
    } else if (pr && pr.origen_b_max === "declarada") {
      explicacion.push({ tipo: "neutro", texto: `Podés bajar hasta ${pr.baja_maxima_pct} % sin perder plata y aquí el que gana baja ~${pr.baja_mediana_pct} %: podés ofertar en el centro del mercado, el precio no te resta.` });
    }
