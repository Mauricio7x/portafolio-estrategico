...
const { leerCuerpo, TOPE_PDF_BASE64 } = require("./cuerpo.js");
const { MARCA } = require("./glosario.js");

/* El tope del PDF de origen es TOPE_PDF_BASE64 (3 MB), de lib/cuerpo.js: el PDF vuelve
   en base64 dentro de una respuesta que Vercel corta en 4,5 MB. Los 12 MB que este
   módulo prometía hasta el 6-sep-2026 eran un fallo mudo (HTTP 200 con el JSON
   truncado) para todo lo que pesara entre 3,4 y 12 MB. */
const MB = 1024 * 1024;
const TOPE_PDF_MB = TOPE_PDF_BASE64 / MB;
// con un decimal y coma: «declara 3 MB … hasta 3 MB» era una contradicción para 3,1 MB
const mbLegible = (bytes) => `${(bytes / MB).toFixed(1).replace(".", ",")} MB`;
/* …y cuando el decimal tampoco separa el peso del tope (3 MB + 1 byte … 3,05 MB
   redondean a «3,0 MB»), se dice «algo más de 3 MB»: «declara 3,0 MB … hasta 3 MB»
   era la misma contradicción con una coma (remate H-02, 6-sep-2026). */
const hechoDelPeso = (bytes) => (mbLegible(bytes) === mbLegible(TOPE_PDF_BASE64)
  ? `El archivo pesa algo más de ${TOPE_PDF_MB} MB.`
  : `El archivo pesa ${mbLegible(bytes)}.`);
/* Tope del CUERPO de la petición, que es {"url": "…"} y nada más. No
   confundir con TOPE_PDF_BASE64, que es el tope del PDF que se descarga. */
const MAX_BYTES_CUERPO = 8 * 1024;
/* Lo que hay que HACER cuando el PDF no cabe: el mismo camino que ya existe en la
   pantalla de Precios, nombrado como se ve allí. Una respuesta que no hizo nada dice
   qué hacer. */
const errorPorTope = (hecho) => ({
  ok: false,
  error: `${hecho} Por URL la aplicación solo puede traer hasta ${TOPE_PDF_MB} MB: descárguelo en su computador `
    + "y súbalo con el selector «Archivo PDF», junto al campo de la URL.",
  max_mb: TOPE_PDF_MB,
});
...
async function leerConTope(respuesta) {
  ...
  if (Number.isFinite(declarado) && declarado > TOPE_PDF_BASE64) {
    return errorPorTope(hechoDelPeso(declarado));
  }
  if (!respuesta.body || typeof respuesta.body.getReader !== "function") {
    const buf = Buffer.from(await respuesta.arrayBuffer());
    if (buf.length > TOPE_PDF_BASE64) return errorPorTope(`El archivo pesa más de ${TOPE_PDF_MB} MB.`);
    return { ok: true, buf };
  }
  ...
  for (;;) {
    ...
    if (total > TOPE_PDF_BASE64) {
      try { await lector.cancel(); } catch { /* ya da igual: se aborta la descarga */ }
      return errorPorTope(`El archivo pesa más de ${TOPE_PDF_MB} MB.`);
    }
...
module.exports = async function handler(req, res) {
  ...
  if (metodo !== "POST") {
    ...
    return res.status(405).json({
      ...
      limites: { max_mb: TOPE_PDF_MB, solo_https: true, redirecciones: MAX_REDIRECCIONES },
    });
  }

  /* EL LECTOR DE CUERPOS ES EL CONSOLIDADO, no una copia privada (ago 2026).
     Esta era la CUARTA copia y la consolidación de `lib/cuerpo.js` no la vio:
     traía enteros los tres defectos que aquel módulo existe para no repetir, y
     se reprodujeron los tres. (1) `buf += c` decodificaba cada trozo por
     separado, así que un carácter multibyte partido en la frontera salía como
     dos U+FFFD: una URL con `ñ` —las hay en los portales colombianos— llegaba
     corrupta y la descarga fallaba contra una dirección que nadie envió.
     (2) `buf.slice(0, 8192)` TRUNCABA en silencio, así que un cuerpo de 9 KB
     acababa en un JSON roto, de ahí en `{}` y de ahí en «URL inválida»: el
     diagnóstico contrario al real. (3) un JSON malformado devolvía `{}`, o sea
     indistinguible de un cuerpo vacío. El consolidado nunca lanza y responde el
     status correcto (413 con su tamaño, 400 con el motivo). */
  const cuerpo = await leerCuerpo(req, { maxBytes: MAX_BYTES_CUERPO, que: "url" });
  ...
  if (!leido.ok) return res.status(413).json({ ok: false, error: leido.error, max_mb: leido.max_mb });
