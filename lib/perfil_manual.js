...
function configDesdeManual(entrada) {
  ...
  const advertencias = [
    "Perfil aproximado: se armó con tres datos, no con el certificado RUP. Sirve para saber a cuántas licitaciones podría presentarse hoy; para verificar la capacidad de contratación hace falta el RUP.",
    ...
    `Se asume que usted hace obra en «${act.etiqueta}»: ${clases.length} clases de obra de ese tipo. Con el RUP real la lista es la suya, no la del tipo de obra.`,
    "Tope por defecto: 2 × su contrato más grande. Por encima de esa cuantía no se le muestran procesos.",
  ];
