...
const {
  ...
  escribirValorGrande, leerValoresGrandes,
} = require("./almacen.js");
...
const DEFAULTS = {
  ...
  // 500 KB por chunk: MUY por debajo del límite de 1 MB por request del tier
  // gratuito de Upstash (el cuerpo REST lleva además clave y envoltura JSON;
  // a 800 KB quedaba demasiado justo y producía "Upstash 400" en producción)
  CHUNK_MAX_B64: 500000,
  CHUNK_TOPE_DURO: 900000,    // cinturón: NUNCA escribir un valor mayor que esto
...
function crearExtractor(opts = {}) {
  ...
  function empaquetar(registros) {
    ...
    // tope efectivo: el configurado, y NUNCA por encima del cinturón de
    // 900 KB — aunque alguien suba CHUNK_MAX_B64 por error, ningún valor
    // escrito puede acercarse al límite de 1 MB por request de Upstash
    const tope = Math.min(cfg.CHUNK_MAX_B64, cfg.CHUNK_TOPE_DURO || 900000);
    ...
    if (registros.length <= 1) {
      ...
      if (b2.length <= Math.min(cfg.CHUNK_MAX_B64, cfg.CHUNK_TOPE_DURO || 900000)) return [b2];
  ...
  async function escribirChunks(mes, desdeIndice, registros) {
    ...
    if (paquetes.length > 1) log.info(`[almacen] ${mes}: página dividida en ${paquetes.length} chunks (${paquetes.map((p) => (p.length / 1e3) | 0).join("/")} KB)`);
    let i = desdeIndice, bytes = 0;
    for (const p of paquetes) {
      // escribirValorGrande es el cinturón final: si un paquete superara el
      // límite por request (no debería: empaquetar corta antes), se guarda
      // en partes clave:parte:{j} y la lectura lo re-ensambla sola
      await escribirValorGrande(store, K.chunk(i), p);
      i++; bytes += p.length;
    }
    return { i, bytes }; // siguiente índice libre + bytes escritos (b64)
  }
  ...
  async function extraerTodo({ presupuestoMs = Infinity } = {}) {
    ...
    while (progreso.mesIdx < progreso.meses.length) {
      ...
      while (!fin) {
        ...
        if (filas.length) {
          ...
          const w = await escribirChunks(mes, progreso.chunkIdx, proy);
          progreso.chunkIdx = w.i;
          progreso.bytesMes = (progreso.bytesMes || 0) + w.bytes;
      ...
      progreso.porMes[mes] = { bajados: progreso.filasMes, bytes: progreso.bytesMes || 0, esperados: progreso.esperadosMes >= 0 ? progreso.esperadosMes : null };
      ...
      progreso.filasMes = 0; progreso.bytesMes = 0; progreso.esperadosMes = null;
      await escribirJSON(store, K0.progreso, progreso);
    }
    ...
    meta.bytes = Object.values(progreso.porMes).reduce((a, m) => a + (m.bytes || 0), 0);
  ...
  async function extraerDelta({ presupuestoMs = Infinity } = {}) {
    ...
    for (const [mes, regs] of porMes) {
      const K = claves(mes);
      let manifest = (await leerJSON(store, K.manifest)) || { ini: 0, chunks: 0, count: 0, bytes: 0, esperados: null };
      const w = await escribirChunks(mes, manifest.chunks, regs);
      manifest = { ...manifest, chunks: w.i, count: (manifest.count || 0) + regs.length, bytes: (manifest.bytes || 0) + w.bytes, updatedAt: new Date(now()).toISOString() };
      ...
      meta.porMes[mes] = { bajados: manFinal.count, bytes: manFinal.bytes || 0, esperados: manFinal.esperados ?? null };
    }
    if (meta.porMes) {
      meta.total = Object.values(meta.porMes).reduce((a, m) => a + (m.bajados || 0), 0);
      meta.bytes = Object.values(meta.porMes).reduce((a, m) => a + (m.bytes || 0), 0);
    }
  ...
  async function compactarMes(mes) {
    ...
    const w = await escribirChunks(mes, ini, registros);
    await escribirJSON(store, K.manifest, {
      ...manifest, ini, chunks: w.i, count: registros.length, bytes: w.bytes,
      updatedAt: new Date(now()).toISOString(),
    });
    for (let i = manifest.ini || 0; i < ini; i++) await store.del(K.chunk(i));
    log.info(`${mes}: compactado a ${registros.length} registros (${w.i - ini} chunks, ${(w.bytes / 1e3) | 0} KB)`);
  }
  ...
  async function leerMes(mes) {
    ...
    const brutos = await leerValoresGrandes(store, ks); // re-ensambla chunks multi-parte
    const mapa = new Map();
  ...
  async function auditar() {
    ...
    if ((meta.bytes || 0) > 200e6) {
      log.warn(`almacenamiento en Redis ${(meta.bytes / 1e6).toFixed(0)} MB > 200 MB: recortar CAMPOS_PROYECCION o subir de plan (tier gratuito: 256 MB)`);
      await registrarIncidencia(store, { nivel: "warn", msg: "almacenamiento cerca del límite del tier gratuito", ctx: { bytes: meta.bytes } });
    }
  ...
  /* ================= MANTENIMIENTO: censo y purga de la base =============
     Recorre TODAS las claves con SCAN y las clasifica:
       validas   → esquema vigente (meta/progreso/incidencias/lock,
                   detecta:seen del cron, manifests de meses en rango,
                   chunks dentro del rango [ini, chunks) de su manifest y
                   los del mes que una carga activa está escribiendo)
       huerfanas → nuestras pero muertas (meses fuera de rango, chunks
                   fuera del rango del manifest, restos detecta:*)
       ajenas    → cualquier otra clave (p.ej. esquemas viejos tipo
                   oportunidad:*) — la base debe ser EXCLUSIVA de la app.
     Con purgar=true borra huérfanas+ajenas en lotes (DEL variádico: pocas
     decenas de comandos aunque haya cientos de miles de claves basura).
     Se niega a purgar con una carga completa en curso (sus chunks
     parciales aún no tienen manifest) salvo force. SCAN puede repetir
     claves entre páginas: los conteos son aproximados por exceso y el
     DEL es idempotente. */
  async function diagnosticar({ purgar = false, force = false } = {}) {
    if (!store.scan) throw new Error("el almacén no soporta SCAN");
    const K0 = claves("");
    const progreso = (await leerJSON(store, K0.progreso)) || {};
    const fullEnCurso = progreso.tipo === "full" && !progreso.terminado;
    if (purgar && fullEnCurso && !force) {
      return { ok: false, error: "carga completa en curso: espera a que termine (o pasa force=1) antes de purgar" };
    }
    const mesLimite = rangoDesde().slice(0, 7);
    const meses = mesesDelRango(rangoDesde(), now());
    const rangos = {};
    for (const mes of meses) {
      const man = await leerJSON(store, claves(mes).manifest);
      if (man) rangos[mes] = { ini: man.ini || 0, fin: man.chunks };
    }
    const mesEnCurso = fullEnCurso ? progreso.meses[progreso.mesIdx] : null;
    const fijas = new Set([K0.meta, K0.progreso, K0.incidencias, K0.lock, "detecta:seen"]);

    const clasifica = (k) => {
      if (fijas.has(k)) return "validas";
      let m = k.match(/^detecta:x:mes:(\d{4}-\d{2}):manifest$/);
      if (m) return m[1] >= mesLimite ? "validas" : "huerfanas";
      m = k.match(/^detecta:x:mes:(\d{4}-\d{2}):chunk:(\d+)(?::parte:\d+)?$/);
      if (m) {
        const mes = m[1], i = parseInt(m[2], 10);
        if (mes < mesLimite) return "huerfanas";
        if (mes === mesEnCurso && i < (progreso.chunkIdx || 0)) return "validas"; // carga activa
        const r = rangos[mes];
        return r && i >= r.ini && i < r.fin ? "validas" : "huerfanas";
      }
      if (k.startsWith("detecta:")) return "huerfanas"; // prefijo nuestro, esquema muerto
      return "ajenas";
    };

    const resumen = { validas: 0, huerfanas: 0, ajenas: 0, borradas: 0, ejemplos: { huerfanas: [], ajenas: [] } };
    let cursor = "0", paginas = 0, porBorrar = [];
    do {
      const [sig, ks] = await store.scan(cursor, "*", 1000);
      cursor = String(sig); paginas++;
      for (const k of ks) {
        const c = clasifica(k);
        resumen[c]++;
        if (c !== "validas") {
          if (resumen.ejemplos[c].length < 8) resumen.ejemplos[c].push(k);
          if (purgar) porBorrar.push(k);
        }
        if (purgar && porBorrar.length >= 400) { await store.del(...porBorrar); resumen.borradas += porBorrar.length; porBorrar = []; }
      }
    } while (cursor !== "0" && paginas < 500);
    if (purgar && porBorrar.length) { await store.del(...porBorrar); resumen.borradas += porBorrar.length; }
    resumen.totalClaves = resumen.validas + resumen.huerfanas + resumen.ajenas;
    resumen.paginasScan = paginas;
    try { resumen.dbsize = await store.dbsize(); } catch (e) { /* opcional */ }
    if (purgar) log.info(`purga: ${resumen.borradas} claves borradas (${resumen.huerfanas} huérfanas + ${resumen.ajenas} ajenas)`);
    else log.info(`diagnóstico: ${resumen.totalClaves} claves → ${resumen.validas} válidas, ${resumen.huerfanas} huérfanas, ${resumen.ajenas} ajenas`);
    return { ok: true, purgado: !!purgar, ...resumen };
  }
  ...
  return { extraerTodo, extraerDelta, auditar, leerMes, compactarMes, diagnosticar, estado, _interno: { mesesDelRango, limitesMes, empaquetar, proyectar, inicioAnoVigente } };
}
