...
function crearExtractor(opts = {}) {
  ...
  async function extraerTodo({ presupuestoMs = Infinity } = {}) {
    ...
    // purga de meses que salieron de la ventana vigente (p.ej. cambio de año):
    // sin esto, KV acumula particiones muertas para siempre
    for (const mesViejo of Object.keys(meta.porMes || {})) {
      if (mesViejo in progreso.porMes) continue;
      const Kv = claves(mesViejo);
      const manViejo = await leerJSON(store, Kv.manifest);
      if (manViejo) for (let i = 0; i < manViejo.chunks; i++) await store.del(Kv.chunk(i));
      await store.del(Kv.manifest);
      log.info(`${mesViejo}: fuera de la ventana vigente, purgado`);
    }
