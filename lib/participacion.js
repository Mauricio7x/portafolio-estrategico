...
function oraciones(texto) {
  // las páginas las recorre lib/paginas (la regla de los marcadores `\f<n>` vive allí, no aquí)
  const { lineasConPagina } = require("./paginas.js");
  let chorro = "";
  const inicios = []; // [desde, página]
  for (const { linea, pagina } of lineasConPagina(texto)) {
    const cuerpo = String(linea).normalize("NFKC").replace(/\s+/g, " ").trim();
    if (!cuerpo) continue;
    if (!inicios.length || inicios[inicios.length - 1][1] !== pagina) inicios.push([chorro.length, pagina]);
    chorro += cuerpo + " ";
  }
...
const MINIMO_RE = /minim[oa]|no (?:podra|podran|puede|pueden|debe|deberan?|sera|sea) (?:ser |tener )?[^.;]{0,30}?(?:inferior|menor|menos)|(?:inferior|menor) (?:al?|del?)\b|menos del?\b|igual o superior|superior o igual|mayor o igual|igual o mayor|de por lo menos|de al menos|no menor|superior al?\b|mayor al?\b|que supere/;
/* «superior al 50 %» es ESTRICTO: 50 no lo cumple (revisión adversaria) */
const ESTRICTO_RE = /^(?:superior al?|mayor al?|que supere)\b/;
...
/* dos clases de trampa (revisión adversaria, 26-sep-2026):
   · de CONTEXTO: la oración entera habla de otra cosa (Mipyme, desempates,
     puntaje, la experiencia de un contrato pasado): se descarta la oración;
   · LOCALES: solo la exigencia que la contiene (el tope del 10 % a quien no
     aporta, dicho con o sin «ser»): se descarta esa «participación», no las
     demás de la oración («…y el que la aporte, mínima del 60 %»). */
const TRAMPA_RE = /mipyme|mujer(?:es)?|discapacidad|puntaje|\bpuntos\b|desempate|empate|emprendimiento|factor de calidad|pfmt|incentivo|preferir|preferira|reincorporacion|reintegracion|madre cabeza|jovenes|criterios? diferencial|2\.2\.1\.2\.4\.2|ostentar dicha calidad|(?:anexo|formato) lo podra|composicion accionaria|hay(?:a|an) (?:tenido|sido ejecutad)|hubier(?:e|en) tenido|haber tenido|\btuvo\b|en dicho contrato|en el respectivo contrato|hay(?:a|an) contado|contratos donde|participacion como (?!minim)|residentes en colombia|industria nacional|origen nacional|aportar (?:como )?(?:minimo )?el (?:veinticinco|25\b)/;
const TRAMPA_LOCAL_RE = /no (?:podra|puede|podran|pueden) [^.;]{0,40}?(?:superar|superior|mayor|exceder)/;
...
const VEINTI = { veintiun: 21, veintiuno: 21, veintidos: 22, veintitres: 23, veinticuatro: 24, veinticinco: 25, veintiseis: 26, veintisiete: 27, veintiocho: 28, veintinueve: 29 };
function porcentajeDe(s) {
  const t = plegar(s);
  // hasta tres decimales: «33,333 %» no puede leerse 33 (una cifra más baja que la real)
  const m = /(\d{1,2}(?:[.,]\d{1,3})?)\s*\)?\s*(?:%|por\s*ciento)/.exec(t);
  const w = new RegExp(`\\b(${[...Object.keys(PALABRAS), ...Object.keys(VEINTI)].join("|")})\\b(?: y (${Object.keys(UNIDADES).join("|")})\\b)?\\s*(?:\\(|por\\s*ciento|%)`).exec(t);
  // el que aparezca PRIMERO («treinta por ciento (30 %)» es uno solo)
  if (m && (!w || m.index <= w.index)) return Number(m[1].replace(",", "."));
  return w ? (PALABRAS[w[1]] || VEINTI[w[1]]) + (w[2] ? UNIDADES[w[2]] : 0) : null;
}
...
const FORMAS = [
  ...
  ["aporta_experiencia", /(?:aport\w*|acredit\w*)[^.;]{0,60}?experiencia|experiencia[^.;]{0,40}?(?:aport\w*|acredit\w*)|(?:el|quien|los|las) que la(?:s)? aport\w*/g],
  ["lider_mayoria", /responsabilidades|\blider\b/g],
  ["cada_integrante", /ningun[oa]?\b|cada (?:uno|una) de (?:los |las |sus )?(?:demas )?(?:integrantes|miembros)|cada (?:integrante|miembro)|todos (?:los|sus) (?:integrantes|miembros)|los demas integrantes|los que no la aporten|el porcentaje de participacion no podra|(?:los|sus) (?:miembros|integrantes) (?:del|de la) (?:consorcio|union|proponente|estructura)[^.;]{0,40}deberan tener/g],
...
function leerParticipacion(texto) {
  ...
  for (let i = 0; i < os.length; i++) {
    ...
    const anafora = ANAFORA_RE.test(s) || /\b(?:este|esta|dicho|dicha) (?:integrante|miembro|socio)\b/.test(s.slice(0, Math.max(120, s.indexOf("participacion")))) || esCola(os[i].texto.trim());
    ...
    const sinSujeto = !/integrante|miembro|ningun|socio|quien|\buno\b/.test(s.slice(0, s.indexOf("participacion"))) || /bastara con/.test(s);
    /* el desempate está en la oración ANTERIOR solo cuando esta no tiene sujeto
       propio o remite a aquella («Este integrante…», aunque un encabezado de
       página lo separe): la Nota 3 de Casanare, con sujeto propio, se perdía por
       el «criterio diferencial» de la Nota 2 (revisión adversaria) */
    if ((anafora || sinSujeto || !/experiencia/.test(s)) && DESEMPATE_RE.test(prev)) continue;
    if ((anafora || sinSujeto) && i > 1 && DESEMPATE_RE.test(plegar(os[i - 2].texto))) continue;
    ...
    for (let k = 0; k < ocurrencias.length; k++) {
      ...
      // el corte de la exigencia: desde el último «, y», «;» o «y ninguno» (o la «participación» anterior)
      ...
      const desdeSujeto = Math.max(corte > 0 ? corte : 0, k > 0 ? ocurrencias[k - 1] : 0);
      /* LA TRAMPA SE MIRA EN LA EXIGENCIA, no en la oración entera: «…los
         contratos ejecutados…, el porcentaje de participación no podrá ser
         inferior al 33,33 %» (Antioquia) se perdía entero (revisión adversaria) */
      if (TRAMPA_LOCAL_RE.test(s.slice(desdeSujeto, fin))) continue;
      let pct = null, estricto = false;
      if (MAYORITARIA_RE.test(tramo)) pct = null;
      else {
        const mm = MINIMO_RE.exec(despues) || MINIMO_RE.exec(antes);
        if (!mm) continue;
        estricto = ESTRICTO_RE.test(mm[0]);
        ...
        const trasCifra = desdeMin.slice(desdeMin.search(/%|por ciento/)).replace(/^(?:%|por ciento)/, "");
        if (/^\s*(?:\(\s*\d+(?:[.,]\d+)?\s*%?\s*\)\s*%?)?\s*de (?:la|su) experiencia/.test(trasCifra)) pct = -1;
      }
      // el sujeto: de la última exigencia nueva hasta un poco después de la participación
      const sigCorte = [", y ", "; ", " y ningun"].map((x) => s.indexOf(x, p0)).filter((x) => x > p0);
      const sujeto = `${anafora ? prev + " " : ""}${s.slice(desdeSujeto, Math.min(p0 + 60, ...sigCorte))}`;
      let f = forma(sujeto);
      ...
      // dos «otro» distintos no se funden: cada uno solo protege si su cita se muestra
      const clave = `${f}|${pct}|${mayoritaria}|${estricto}${f === "otro" ? `|${plegar(os[i].texto).slice(0, 160)}` : ""}`;
      ...
      out.push({ forma: f, porcentaje: pct, mayoritaria, ...(estricto && pct != null ? { estricto: true } : {}), cita: citaDe(os[i].texto, enTexto), pagina: os.paginaEn(os[i].desde + enTexto) });
    }
  }
  return out;
}
...
const METODO_RE = {
  suma_componentes: /componente 1 del indicador(?![^.]{0,160}particip)|sumatoria de (?:los )?componentes(?![^.]{0,160}particip)|sumando (?:los|cada uno de los) componentes(?![^.]{0,160}particip)/,
  componentes_ponderados: /componentes? (?:de los indicadores |del indicador )?(?:segun|de acuerdo (?:con|a)|multiplicad[oa]s? por) (?:el|su) (?:porcentaje de )?participacion|c1m1 ?\* ?%|componente \d del indicador[^.]{0,40}(?:x|\*|por) ?%? ?(?:de )?participacion|componentes[^.]{0,120}proporcional a su (?:porcentaje de )?participacion|componentes?[^.]{0,60}multiplicad[oa]s? por (?:el|su) (?:porcentaje de )?participacion/,
  indices_ponderados: /(?:indicadores?|valores individuales|indices?)[^.]{0,120}(?:multiplicad[oa]s? por|se (?:le )?aplica) (?:el|su) porcentaje de participacion|por separado,? luego se aplica el porcentaje de participacion|(?:sumatoria|suma) del indicador[^.]{0,80}(?:afectandolo|multiplicandolo|ponderandolo) por/,
};
...
function leerMetodoPlural(texto) {
  ...
  /* dos pasadas: primero cada oración SOLA (así la cita y la página son las de
     la fórmula), después la oración con la siguiente, para la fórmula partida,
     citada desde la oración que la completa (revisión adversaria: 175 de 177
     citas quedaban en la oración anterior) */
  const mirar = (s, o) => {
    if (!/(?:plural|consorci|union temporal)/.test(s) || !/(?:indicador|componente|capacidad financiera|liquidez|capital de trabajo)/.test(s)) return;
    if (SIN_PONDERAR_RE.test(s) && /particip/.test(s)) anotar("suma_componentes", o);
    else for (const [metodo, re] of Object.entries(METODO_RE)) if (re.test(s)) anotar(metodo, o);
  };
  for (let i = 0; i < os.length; i++) mirar(plegar(os[i].texto), os[i]);
  for (let i = 0; i + 1 < os.length; i++) {
    const s = plegar(`${os[i].texto} ${os[i + 1].texto}`);
    if (Object.keys(METODO_RE).some((m) => METODO_RE[m].test(s) && !METODO_RE[m].test(plegar(os[i].texto)) && !METODO_RE[m].test(plegar(os[i + 1].texto)))) mirar(s, { texto: `${os[i].texto} ${os[i + 1].texto}`, pagina: os[i + 1].pagina });
  }
...
function fraseClausula(c) {
  const cuanto = c.mayoritaria ? "más de la mitad" : `${c.estricto ? "más del" : "al menos el"} ${String(c.porcentaje).replace(".", ",")} %`;
  switch (c.forma) {
    ...
    case "lider_mayoria": return `Uno de los integrantes, el que asume las responsabilidades, con ${c.porcentaje != null && !c.mayoritaria ? cuanto : "más de la mitad"} de la participación.`;
