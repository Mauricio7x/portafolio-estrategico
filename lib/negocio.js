/* ============================================================================
   ...
   Añade a cada licitación cruda de Socrata (dataset p6dx-8zbt) los campos:
     ...
     proceso_abierto   boolean  estado/fase NO terminal (adjudicado, desierto…)
...
/* Estados/fases terminales: el proceso ya no se puede presentar. Regex
   heredado (probado) del código anterior; raíces sin sufijo a propósito. */
const ESTADO_TERMINAL_RE = /adjudicad|celebrad|termin|liquidad|cerrad|desiert|revocad|descartad|suspendid|anulad|cancelad|evaluaci|en\s+ejecuci|ejecutad/i;
...
const SIN_ANTICIPO_RE = /no\s+(?:se\s+)?(?:contempla|habr[aá]|otorga|aplica|pacta|pagar[aá]|paga|entrega|reconoce)n?\s+anticipo|sin\s+anticipo|anticipo[:\s]+(?:0\b|cero|no\s+aplica|n\/a)/i;
...
function fechaCierre(lic) {
  for (const c of CIERRE_CANDIDATOS) {
    const v = lic[c];
    if (v && !isNaN(Date.parse(v))) return String(v);
  }
...
function enriquecer(lic) {
  ...
  const cuantia_rango = cuantiaRango(cuantia_cop);
  ...
  const estadoFase = `${lic.estado_del_procedimiento || ""} ${lic.fase || ""}`;
  ...
  const out = {
    ...
    proceso_abierto: !ESTADO_TERMINAL_RE.test(estadoFase),
...
module.exports = {
  ...
  ESTADO_TERMINAL_RE, CIERRE_CANDIDATOS,
