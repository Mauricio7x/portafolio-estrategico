/* ============================================================================
   ...
   Añade a cada licitación cruda de Socrata (dataset p6dx-8zbt) los campos:
     ...
     anticipo_declarado boolean ¿lo declaró el objeto (incluido un "no se paga"
                                explícito), o es que el dataset no lo trae?
     cuantia_cop       number   valor en COP usado para el rango (auxiliar)
     cuantia_rango     string   "bajo" (<100M) | "medio" (100–500M) | "alto" (>500M)
     nivel_competencia string?  "baja" (≤5 ofertas) | "media" (6–15) | "alta" (>15) | null sin dato
...
/* SIN DATO DE OFERTAS EL PUNTAJE TOMA EL VALOR CENTRAL (6-sep-2026). `ofertas ?? 0`
   convertía «no hay columna de ofertas» en «baja» (≤ 5) y en el 100 del puntaje: un
   cero creíble. El nivel de la fila queda en null y el término de competencia del
   puntaje usa el del nivel «media» —ni premia ni castiga—, no un 0 (que sería peor
   que «alta») ni el 100 de «baja» (que era el defecto). */
const NIVEL_COMPETENCIA_NEUTRO = "media";
...
/* ---------- anticipo ---------- */
/* El texto sobre el que se busca el anticipo, en UN solo sitio: `enriquecer` y
   quien pregunte por una fila suelta tienen que mirar lo mismo. */
const textoDe = (lic) => `${(lic && lic.nombre_del_procedimiento) || ""} ${(lic && lic.descripci_n_del_procedimiento) || ""}`;
/* Columnas que puede traer el PUBLICADOR. `anticipo_pct` va aparte porque es
   además el nombre del campo que escribe `enriquecer`: sobre una fila ya
   enriquecida los dos son indistinguibles, y de eso depende `anticipoDeclarado`. */
const ANTICIPO_CAMPOS_PUBLICADOR = ["porcentaje_de_anticipo", "anticipo_porcentaje", "porcentaje_anticipo", "pct_anticipo", "anticipo"];
const ANTICIPO_CAMPOS = ["anticipo_pct", ...ANTICIPO_CAMPOS_PUBLICADOR];
...
const SIN_ANTICIPO_RE = /no\s+(?:se\s+)?(?:ha\s+previsto|contempla|prev[eé]|habr[aá]|hay|otorga|aplica|pacta|paga|entrega|reconoce|da)(?:r|r[aá]|r[aá]n|n)?(?:\s+\S+){0,5}?\s+(?:anticipo|pago\s+anticipado)|sin\s+anticipo|anticipo[:\s]+(?:0\b|cero|no\s+aplica|n\/a)/i;
...
/* UNA AUSENCIA Y UN CERO DECLARADO NO PUEDEN COMPARTIR NOMBRE (12-sep-2026).
   El detector SIEMPRE supo la diferencia por dentro —«NO SE PAGARÁ ANTICIPO»
   es un 0 REAL declarado en el objeto; un objeto que no lo menciona es SIN
   DATO, porque p6dx-8zbt no trae la columna— pero las dos salían como
   `anticipo_pct = 0` y aguas abajo nadie podía separarlas. P3 (la caja) las
   trataba como el mismo 0 medido y CERRABA la puerta por lo que no se sabe.
   `anticipoDe` devuelve las dos cosas a la vez desde la MISMA cascada: el
   porcentaje y si alguien lo declaró. `anticipoPct` conserva su contrato
   exacto (0 = «no declarado o cero»; media app depende de que ese 0 no
   excluya) y `anticipoDeclarado` publica la distinción. */
function anticipoDe(lic, texto) {
  const t = texto == null ? textoDe(lic) : texto;
  const declarado = primerNumero(lic, ANTICIPO_CAMPOS);
  // la entidad publicó la columna: su 0 es un dato suyo, no la ausencia del dataset
  if (declarado != null && declarado >= 0 && declarado <= 100) return { pct: declarado, declarado: true };
  if (SIN_ANTICIPO_RE.test(t)) return { pct: 0, declarado: true };
  for (const re of ANTICIPO_TEXTO_RE) {
    const m = t.match(re);
    if (m) {
      const n = num(m[1]);
      if (n != null && n > 0 && n <= 100) return { pct: n, declarado: true };
    }
  }
  return { pct: 0, declarado: false }; // sin dato: el requerimiento manda 0 en el PCT
}

function anticipoPct(lic, texto) {
  return anticipoDe(lic, texto).pct; // sin dato: el requerimiento manda 0
}

/* ¿Lo declaró alguien, o es que el dataset no lo trae? La usa lib/puertas para
   no cerrar la caja por ignorancia, y tiene que responder también sobre una
   fila YA ENRIQUECIDA —la caché de Redis guarda el resultado de `enriquecer`,
   y las filas anteriores a este campo no lo llevan—. Ahí `anticipo_pct` existe
   SIEMPRE y vale 0 cuando no se sabe, así que preguntar por él diría «sí, lo
   declararon» sobre cada fila del corpus. Por eso, sobre lo derivado:
     · un porcentaje MAYOR QUE CERO solo puede venir de una declaración;
     · un 0 está declarado solo si lo dice el texto o una columna del
       PUBLICADOR (que no es el campo que escribe `enriquecer`).
   Ante la duda devuelve `false` = «no se sabe», que es el lado que DEJA PASAR. */
function anticipoDeclarado(lic, texto) {
  if (typeof lic.anticipo_declarado === "boolean") return lic.anticipo_declarado;
  const t = texto == null ? textoDe(lic) : texto;
  if (anticipoDe(lic, t).pct > 0) return true;
  if (SIN_ANTICIPO_RE.test(t)) return true;
  return primerNumero(lic, ANTICIPO_CAMPOS_PUBLICADOR) != null;
}
...
function fechaCierre(lic) {
  ...
  /* Red de seguridad: cualquier columna fecha* de recepción/cierre/límite/plazo.
     `l[ií_]mit` y no `l[ií]mit` (15-sep-2026): Socrata trunca y transcribe la
     tilde como guión bajo —`fecha_l_mite_de_recepci` está en la lista de
     candidatas de arriba precisamente por eso—, así que la alternativa con
     tilde no casaba `fecha_l_mite` y una fila cuyo único cierre viviera ahí
     entraba al corpus con `fecha_cierre = null`: sin «Cierra en N días» y sin
     el recorte del techo de la ventana de manifestación.

     ⚠️ Y LO QUE DIGA «MANIFESTACIÓN» NO ES EL CIERRE DE OFERTAS. Es la cicatriz
     de Motavita esperando otra columna: el plazo para manifestar interés y el
     de presentar la oferta son dos fechas distintas —medido en `docs/datos.md`
     §7: `fecha_de_recepcion_de` cae 6–14 días después de la publicación, y la
     manifestación son ≤ 3 hábiles—, y confundirlas cerraría el proceso por el
     reloj al vencer la manifestación y pintaría un «Cierra el …» falso. Si la
     fuente publica algún día esa columna, entra por `lib/manifestacion`, nunca
     por aquí. */
  for (const k in lic) {
    if (/manifest/i.test(k)) continue;
    if (/fecha/i.test(k) && /(recep|cierre|l[ií_]mit|plazo)/i.test(k)) {
      const v = lic[k];
      if (legible(v)) return String(v);
    }
  }
...
function enriquecer(lic) {
  ...
  const { pct: anticipo_pct, declarado: anticipo_declarado } = anticipoDe(lic, texto);
  ...
  const ofertas = primerNumero(lic, COMPETENCIA_CAMPOS);
  const nivel_competencia = ofertas == null ? null : nivelCompetencia(ofertas);
  ...
  const puntaje = PESO_ANTICIPO * anticipoScore(anticipo_pct)
    ...
    + PESO_COMPETENCIA * SCORE_COMPETENCIA[nivel_competencia ?? NIVEL_COMPETENCIA_NEUTRO];

  const out = {
    ...lic,
    anticipo_pct,
    /* SI ALGUIEN LO DECLARÓ, O SI ES QUE NO HAY DATO. `anticipo_pct = 0` no
       puede seguir significando las dos cosas: lib/puertas.p3Caja cerraba la
       caja —y con `solo_viables` encendido, que es el default, borraba la fila
       de la lista— por un dato que SECOP II no publica. El PCT no cambia. */
    anticipo_declarado,
    cuantia_cop,
    cuantia_rango,
    nivel_competencia,
...
module.exports = {
  ...
  anticipoPct,
  /* lib/puertas pregunta por esto para no cerrar P3 con una ausencia. Va como
     función además de campo: las filas guardadas ANTES de que `enriquecer`
     publicara `anticipo_declarado` siguen en la caché de Redis sin él, y una
     full no es requisito para dejar de esconder procesos viables. */
  anticipoDeclarado,
};
