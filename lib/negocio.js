...
const SIN_ANTICIPO_RE = /no\s+(?:se\s+)?(?:ha\s+previsto|contempla|prev[eé]|habr[aá]|hay|otorga|aplica|pacta|paga|entrega|reconoce|da)(?:r[aá]|r[aá]n|n)?(?:\s+lugar\s+a)?\s+(?:ning[uú]n\s+|el\s+pago\s+de\s+|pago\s+de\s+)?anticipo|sin\s+anticipo|anticipo[:\s]+(?:0\b|cero|no\s+aplica|n\/a)/i;
