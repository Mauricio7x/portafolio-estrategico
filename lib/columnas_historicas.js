...
const {
  ...
  desenlaceDe, plazoAdjudicacionDe,
...
/* Percentil sobre una lista YA ordenada; null si no hay muestra. Es RANGO MÁS
   CERCANO (`Math.round((n−1)·p)`), que en conjuntos pares devuelve el elemento
   SUPERIOR. Sirve para p25 y p75, pero NO es la mediana: usarlo con p = 0,5
   publicaba una baja del 10 % donde la mediana de [0,80; 0,90] dice 15 %. La
   mediana la da `lib/estadistica`. */
...
async function censarColumnasHistoricas(redis, { muestras = MUESTRAS } = {}) {
  ...
  /* M-DGF-08: la PAREJA de fechas (cierre Y adjudicación en la misma fila) y los
     desenlaces, con la MISMA regla del índice (`desenlaceDe`,
     `plazoAdjudicacionDe`): el censo dice para cuántos procesos
     puede existir un plazo, no cuántos traen cada fecha por separado. */
  const plazo = { adjudicados: 0, desiertos: 0, con_ambas_fechas: 0, sin_fecha_cierre: 0, sin_fecha_adjudicacion: 0, sin_ninguna_fecha: 0, no_posterior_al_cierre: 0, desierto_con_adjudicacion: 0 };
  ...
  for (const lic of filas) {
    ...
    /* El desenlace lo decide `desenlaceDe` del índice, no un segundo orden aquí:
       el censo existe para DIAGNOSTICAR el índice y con dos reglas distintas
       diagnosticaría otra cosa (remate B9b-H3, 6-sep-2026). */
    const desenlace = desenlaceDe(lic);
    if (desenlace === "adjudicado") {
      plazo.adjudicados++;
      const pl = plazoAdjudicacionDe(lic);
      if (pl.dias != null) plazo.con_ambas_fechas++; else plazo[pl.motivo]++;
    } else if (desenlace === "desierto") {
      plazo.desiertos++;
    } else if (desenlace === "desierto_con_adjudicacion") {
      plazo.desierto_con_adjudicacion++;
    }
  }
  ...
  /* ---------- baja de mercado ---------- */
  ratios.sort((a, b) => a - b);
  const mediana = medianaDe(ratios);
  const baja = {
  ...
  if (total === 0) {
    ...
    siguientePaso = "Resolver el nombre real contra la fuente: "
      + "https://www.datos.gov.co/api/views/p6dx-8zbt.json (datos.gov.co, el portal colombiano) devuelve el "
      + "listado de columnas del dataset sin adivinar. Añadir el nombre a CAMPOS_VALOR_ADJUDICADO y llamar "
  ...
  return {
    ...
    como_leerlo: "`grupos.*.candidatas` sondea TODOS los nombres posibles y dice cuántos respondieron: "
      ...
      + "`desierto_con_adjudicacion` son los que dicen las dos cosas y no entran en ninguna de las dos bases.",
  };
}
