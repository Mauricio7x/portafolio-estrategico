/* ============================================================================
   ...
   encadenado de la full a la pestaña Mi empresa (#/admin) de index.html.
   ========================================================================== */
"use strict";
...
module.exports = async function handler(req, res) {
  ...
  if (texto.trim().length < MIN_TEXTO && imagenes.length) {
    ...
    /* ── SOLO RECONOCER, sin parsear ──
       ...
       cliente ENCADENE tandas —el mismo patrón con que Mi empresa (#/admin) encadena la
  ...
  if (parseado.confianza.color === "rojo") {
    /* «Atención:» y no un círculo de color: los avisos se pintan tal cual en la pantalla
       y un emoji lo dibuja el sistema operativo (6-sep-2026). */
    avisos.push("Atención: menos del 90 % de las filas con precio cuadran aritméticamente: el parseo se considera "
    ...
    avisos.push(parseado.confianza.motivo === "sin_precios_unitarios"
      ? "Atención: el documento trae cantidades sin precios unitarios, así que no hay aritmética que validar. "
        + "La lista sigue siendo útil (ítem + unidad + cantidad ya permite valorar con precios propios), "
        + "pero exige confirmación humana."
      : "Atención: la lista no está completamente validada: exige confirmación humana antes de valorar.");
  }
