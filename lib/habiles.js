...
function hoyColombia(ahoraMs) {
  ...
  return aISO(ms - OFFSET_COLOMBIA_MS);
}

/* EL INSTANTE civil de Colombia, «YYYY-MM-DDTHH:MM» (15-sep-2026). Este módulo
   decía en su cabecera «sin husos ni horas: un plazo de días hábiles no tiene
   hora», y es cierto PARA LOS DÍAS HÁBILES. Pero el plazo de manifestación de
   interés sí tiene hora, y la publica SECOP II en la propia pantalla del
   proceso: «Plazo para manifestación de Interés · 17 horas de tiempo
   transcurrido (14/09/2026 6:00:00 PM(UTC-05:00) Bogotá, Lima, Quito)». Sin la
   hora, el día del vencimiento es indistinguible a las 11 de la mañana —con el
   plazo abierto— y a las 11 de la noche —con siete horas transcurridas—, que es
   justo la diferencia entre presentarse y no poder. Se compara como CADENA,
   igual que las fechas: el orden lexicográfico de «YYYY-MM-DDTHH:MM» es el
   cronológico, y así no entra ningún objeto Date con su huso. */
function ahoraColombia(ahoraMs) {
  const ms = Number.isFinite(ahoraMs) ? ahoraMs : Date.now();
  return new Date(ms - OFFSET_COLOMBIA_MS).toISOString().slice(0, 16);
}

/* «6:00 p. m.» a partir de «18:00». null si la hora no es legible: una hora
   ausente jamás se rellena con las 00:00, que afirmaría «a la medianoche». */
...
module.exports = { pascua, festivos, esFestivo, esHabil, sumarHabiles, habilesEntre, hoyColombia, ahoraColombia, festivosConCache, fechaLegible, horaLegible, sumarDias, fechaOperable, OFFSET_COLOMBIA_MS, NOMBRES, ANIO_MIN, ANIO_MAX };
