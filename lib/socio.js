/* ============================================================================
   lib/socio · Verificá a tu socio antes de firmar (due diligence de 20 minutos)
   ----------------------------------------------------------------------------
...
const FUENTES_MANUAL = [
  {
    ...
    automatica: true,
    que_mirar: "Sanciones disciplinarias e inhabilidades vigentes de la persona natural o del representante legal. La app consulta el dataset abierto de la Procuraduría; el pliego pide el CERTIFICADO: descargalo igual (es gratis y sale al instante).",
  },
  {
    clave: "contraloria",
  ...
  },
  {
    ...
    automatica: true,
    que_mirar: "Multas y sanciones registradas, contratos cancelados, cedidos o suspendidos, y cuántos contratos ha ganado. La app trae SECOP I (multas) y SECOP II (contratos); lo que no aparezca acá, confirmalo en la ficha del proveedor en SECOP II.",
  },
];
...
async function consultarSiri(ids, { fetchImpl, log, tiempoMs }) {
  ...
  try {
    ...
    return { ...base, ok: true, coincidencias, n: filas.length,
      motivo: filas.length ? null : "sin coincidencias en el dataset abierto de SIRI para esta(s) cédula(s) — pedí el certificado igual" };
  } catch (e) {
    log(`socio/siri: ${e && e.message}`);
    return { ...base, motivo: `no se pudo consultar ${DATASETS.siri}: ${String((e && e.message) || e)}` };
  }
}
  ...
  return {
    ...
    inhabilidad_reiterada: {
      ...
      lectura: senal === "posible_inhabilidad"
        ? `${nMax} multas en la vigencia ${anioMax}: con 5 o más en un mismo año fiscal la ley prevé 3 años de inhabilidad. Verificalo en el RUP (la inhabilidad se inscribe ahí) antes de firmar.`
        : senal === "verificar_incumplimientos"
          ? `${nMax} multas en la vigencia ${anioMax}: 2 multas + 1 declaratoria de incumplimiento en el mismo año ya inhabilitan. Confirmá en SECOP II si además hubo incumplimientos declarados.`
...
async function consultarMultas(id, { fetchImpl, log, tiempoMs, ahora }) {
  const base = { ok: false, fuente: DATASETS.multas, consultado: id, motivo: null, filas_prueba_descartadas: 0,
    nota: "Multas y sanciones publicadas por las entidades en SECOP I. SECOP II NO publica un dataset equivalente: lo que no aparezca acá, confirmalo en la ficha del proveedor en SECOP II." };
  ...
  try {
    ...
    return { ...base, motivo: `no se pudo consultar ${DATASETS.multas}: ${String((e && e.message) || e)}` };
  }
}
...
async function consultarContratos(id, { fetchImpl, log, tiempoMs }) {
  ...
  try {
    ...
    }, "contratos del proveedor"), tiempoMs);
    const lista = filas || [];
    ...
    return { ...base, motivo: `no se pudo consultar ${DATASETS.contratos}: ${String((e && e.message) || e)}` };
  }
}
...
async function consultarAdjudicaciones(id, { fetchImpl, log, tiempoMs }) {
  ...
  try {
    ...
    }, "adjudicaciones del proveedor"), tiempoMs);
    const porAnio = [];
    ...
    return { ...base, motivo: `no se pudo consultar ${DATASETS.adjudicaciones}: ${String((e && e.message) || e)}` };
  }
}

/* ---------- Semáforo ----------
   Tres estados y ninguno es «limpio»: lo máximo que la app puede decir es «sin
...
function semaforo({ siri, multas, contratos }) {
  ...
  if (multas.ok && multas.multas > 0) {
    ...
    hallazgos.push({ nivel: vigente ? "rojo" : "ambar", fuente: "multas",
      texto: `${multas.multas} multa(s) en SECOP I${vigente ? " — posible inhabilidad vigente (art. 90 Ley 1474)" : ir.senal === "posible_inhabilidad" ? ` — ${ir.max_multas_en_una_vigencia} en ${ir.vigencia}: inhabilidad de entonces ya vencida, pero mirá cómo ejecuta` : ""}` });
  }
  ...
  const nivel = hallazgos.some((h) => h.nivel === "rojo") ? "rojo" : hallazgos.length ? "ambar" : "sin_hallazgos";
  return {
    nivel,
    hallazgos,
    fuentes_caidas: fuentesCaidas,
    texto: nivel === "rojo" ? "Hay hallazgos que pueden inhabilitar: no firmes sin verificarlos en la fuente oficial."
      : nivel === "ambar" ? "Hay señales que conviene revisar antes de firmar (no son inhabilidades por sí solas)."
        : fuentesCaidas.length ? `Sin hallazgos en lo consultado, pero ${fuentesCaidas.length} fuente(s) no respondieron: repetí la consulta.`
...
function checklist({ siri, multas, contratos, adjudicaciones }) {
  return FUENTES_MANUAL.map((f) => {
    ...
    if (f.clave === "siri") {
      estado = siri.ok ? (siri.n ? "hallazgos" : "sin_hallazgos") : "no_consultada";
      resumen = siri.ok ? (siri.n ? `${siri.n} sanción(es) sobre ${siri.consultados.join(", ")}` : `sin coincidencias para ${siri.consultados.join(", ") || "—"}`) : siri.motivo;
    }
    if (f.clave === "secop") {
      const partes = [];
