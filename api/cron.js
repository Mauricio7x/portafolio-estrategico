/* ============================================================================
   ...
   Variables de entorno (Vercel → Settings → Environment Variables):
     ...
     ANTHROPIC_API_KEY     (opcional) para el resumen IA dentro de la alerta
   ========================================================================== */
...
/* ---------- Resumen IA opcional (formato Leadcitaciones) ---------- */
async function resumenIA(p, best) {
  if (!process.env.ANTHROPIC_API_KEY) return "";
  try {
    const prompt = `Resume en 2 frases, tono ejecutivo y directo, este proceso de contratación pública colombiano para un contratista de obra civil. Frase 1: qué es y monto. Frase 2: por qué encaja (perfil ${best.perfil}, encaje ${best.score}/100) o qué lo limita. Sin preámbulos.\n\nObjeto: ${p.nombre}\nEntidad: ${p.entidad}\nValor: ${p.valor}\nUbicación: ${p.ciudad}, ${p.depto}\nDescripción: ${(p.descripcion || "").slice(0, 600)}`;
    const r = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 220, messages: [{ role: "user", content: prompt }] }),
    });
    const j = await r.json();
...
export default async function handler(req, res) {
  ...
  for (const { p, best } of nuevos.slice(0, 15)) {
    ...
    const resumen = await resumenIA(p, best);
