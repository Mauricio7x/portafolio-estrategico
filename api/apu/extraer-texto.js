/* ============================================================================
   ...
   Cuerpo del POST:
     {
       texto_extraido:  string     el texto del formulario de cantidades. Lo
                                   produce pdf.js EN EL NAVEGADOR (public/apu.js),
   ...
   que sacarlo del request path (docs/APU_Y_RENTABILIDAD.md §1.G.3); el OCR «no
...
const { parsearPliego, UNIDADES_CANONICAS, CORTE_FILAS_VERDE, CORTE_FILAS_ROJO, TOPE_ANTICIPO_SUMA } = require("../../lib/apu_pliego.js");
const { mapearTabla, UMBRAL_ACEPTAR, UMBRAL_FIRME } = require("../../lib/apu_mapeo.js");
const { TIPOLOGIAS, ITEMS, META_CATALOGO } = require("../../lib/apu_catalogo.js");
const { ocrPaginas, hayClaveOcr, MENSAJE_SIN_CLAVE, MAX_BYTES_IMAGEN, MAX_PAGINAS_POR_LOTE } = require("../../lib/apu_ocr.js");
...
/* Mismo lector de cuerpo que /api/admin/rup: Vercel deja `req.body` parseado
   cuando el Content-Type es JSON, pero no siempre (ni en las pruebas), así que
   se cubren los tres casos —objeto, cadena y stream— y el tope se aplica
...
async function leerCuerpo(req) {
  ...
  if (crudo === null) {
    ...
    if (crudo === null) return { ok: false, status: 413, error: "Body demasiado grande", max_mb: 4 };
  }
  if (Buffer.byteLength(String(crudo || ""), "utf8") > MAX_BYTES) {
    return { ok: false, status: 413, error: "Body demasiado grande", max_mb: 4 };
  }
  if (!String(crudo || "").trim()) {
    return { ok: false, status: 400, error: "Body vacío: se esperaba un JSON con «texto_extraido» o «imagenes_base64»." };
  }
  try {
