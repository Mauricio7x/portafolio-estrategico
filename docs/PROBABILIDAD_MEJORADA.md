...
> | **A7** | Medir `f_colisión` sobre el histórico (§9.3) | ✅ **hecho** — `medirColision` en el índice (por entidad, día de cierre; cociente pooled + multiplicador implícito sobre 1/(1+r)); `lib/probabilidad.factorColisionDe` APLICA el medido (≥30 entidades, acotado [0,8; 1,5]) y cae al 1,15 declarado sin medición. **Medido en producción (16-ago-2026, 1 465 entidades, 28 747 en colisión vs 65 501 control): cociente de promedios 0,99, multiplicador implícito 1,06, mediana por entidad 1,07** — el 1,15 no estaba respaldado; hoy se aplica 1,06 y el desglose dice `origen: "medido"` |
> | **B2** | Antigüedad por celda | ✅ **medido y cerrado sin segmentar** (16-ago-2026): el acumulador guarda `por_anio` y los días de cierre; el detalle publica `reparto_por_anio` y la meta `periodos` (por año + dentro/fuera de la ventana de la ley de garantías 2026, estratificado por entidad). En producción: por año 4,35/4,08/4,11; en la ventana **menos** oferentes (cociente 0,95, mediana 0,88, 2 170 entidades) — más procesos diluyen a los oferentes; sesgo agregado ~1 %. **Segmentar no está justificado**; se re-mide en cada reconstrucción |
>
...
| **B3** 🟡 | **Calibrar `f_prórroga`** — **sembrado el 16-ago-2026**: el delta estampa `cierre_prorrogado`/`versiones_vistas`/`fecha_cierre_inicial` al histórico, el índice publica `meta.prorroga` y `factorProrrogaDe` aplica el medido con ≥30 entidades (hasta entonces 1,20 declarado, con origen) | nada más que esperar: la señal se acumula desde el 16-ago-2026 (el backfill no la trae) | Es la única señal ex-ante que existe, y su magnitud (1,20) es un supuesto puro hasta que haya base | ~6 meses de acumulación |
