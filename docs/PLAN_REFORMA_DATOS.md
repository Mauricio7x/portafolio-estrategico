...
- `CLAUDE.md` (entero: es el único que se auto-carga) y `docs/PROMPT_INICIAL.md § «3. El ciclo ECC (di en qué paso estás)»`, `§ «8. Verificación · qué cuenta como «hecho» (los matices que CLAUDE.md no lleva)»`, `§ «9. Esfuerzo proporcional a lo que está en juego»`, `§ «10. Reglas de respuesta (obligatorias)»`, `§ «11. Mantenimiento de este documento»`, `§ «Apéndice A · El prompt corto para pegar»`.
